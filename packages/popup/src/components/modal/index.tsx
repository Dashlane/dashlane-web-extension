...
const Modal: React.FunctionComponent<Props> = ({ visible, children, onClose, className, }) => {
    useEscapeHandler(onClose);
    const modalRoot = document.getElementById('modal');
    if (!modalRoot || !visible) {
        return null;
    }
    return createPortal(<aside className={classNames(styles.modal, className)}>{children}</aside>, modalRoot);
};
