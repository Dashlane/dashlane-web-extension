...
function savePerformanceMeasurements(measurements: PerformanceMeasurement[]): void {
    const performanceMeasurementsString = window.localStorage.getItem(PERFORMANCE_MEASUREMENTS_STORAGE_KEY);
    const performanceMeasurementsArray: PerformanceMeasurement[][] = performanceMeasurementsString
        ? JSON.parse(performanceMeasurementsString)
        : [];
    performanceMeasurementsArray.push(measurements);
    window.localStorage.setItem(PERFORMANCE_MEASUREMENTS_STORAGE_KEY, JSON.stringify(performanceMeasurementsArray));
}
