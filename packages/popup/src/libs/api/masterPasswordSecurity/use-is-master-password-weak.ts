...
export function useIsMasterPasswordWeak(): boolean {
  const { data: isLeaked, status: isLeakedStatus } = useModuleQuery(
    masterPasswordSecurityApi,
    "isMasterPasswordWeak"
  );
  const { data: isDismissed, status: isDismissedStatus } = useModuleQuery(
    masterPasswordSecurityApi,
    "isMasterPasswordNotificationDismissed"
  );
  if (
    isLeakedStatus !== DataStatus.Success ||
