...
import { Button, colors, CrossCircleIcon, FlexChild, FlexContainer, jsx, LoadingIcon, Paragraph, } from '@dashlane/ui-components';
...
const DashlaneAuthenticatorStepComponent = ({ errorKey, switchToOtpTokenStep, sendDashlaneAuthenticatorPushNotification, }: DashlaneAuthenticatorStepProps) => {
    ...
    return (<FlexContainer flexDirection="column" sx={{
      ...
      <Paragraph sx={{ fontSize: '21px', fontWeight: 500 }} color={colors.white}>
        {translate(I18N_KEYS.DASHLANE_AUTHENTICATOR_TITLE)}
      </Paragraph>
      {!errorKey ? (<LoadingIcon data-testid="dashlane-authenticator-loading-icon" size="80px" sx={{ margin: '24px 0px' }}/>) : (<>
          <CrossCircleIcon size={80} color={colors.white} sx={{ margin: '24px 0px' }} data-testid="dashlane-authenticator-error-icon"/>
      ...
      <FlexChild sx={{ width: '100%', marginTop: 'auto' }}>
        {errorKey ? (<Button type="button" nature="primary" size="large" theme="dark" onClick={() => {
                sendDashlaneAuthenticatorPushNotification();
            }} sx={{ marginBottom: '8px', width: '100%' }}>
            {translate(I18N_KEYS.BUTTON_SEND_PUSH_NOTIFICATION)}
          </Button>) : null}
        <Button onClick={() => switchToOtpTokenStep()} type="button" nature="secondary" theme="dark" size="large" sx={{ width: '100%' }}>
          {translate(I18N_KEYS.SEND_CODE_BY_EMAIL)}
        </Button>
      </FlexChild>
    </FlexContainer>);
};
export const DashlaneAuthenticatorStep = memo(DashlaneAuthenticatorStepComponent);
