...
import MasterPasswordInput from 'src/components/inputs/common/master-password-input/master-password-input';
import { Checkbox } from 'src/components/checkbox/checkbox';
import { FORGOT_PASSWORD_URL, openExternalUrl } from 'libs/externalUrls';
...
import { DashlaneUpdateNeeded } from 'app/login/errors/dashlaneUpdateNeeded';
import { checkDoesLocalRecoveryKeyExist } from 'src/libs/api';
...
import styles from 'app/login/PasswordStep/styles.css';
const I18N_KEYS = {
...
const PasswordStep: React.FC<PasswordStepProps> = (props: PasswordStepProps) => {
    ...
    const [showStartRequestDialog, setShowStartRequestDialog] = React.useState(false);
    const [showPendingRequestDialog, setShowPendingRequestDialog] = React.useState(false);
    const [showApprovedRequestDialog, setShowApprovedRequestDialog] = React.useState(false);
    const [showRefusedRequestDialog, setShowRefusedRequestDialog] = React.useState(false);
    ...
    return (<FormWrapper title={{ text: translate(I18N_KEYS.TITLE), labelId: 'password_label' }} description={props.descriptionKey ? translate(props.descriptionKey) : undefined} handleSubmit={(event) => {
      ...
      <div>
        <MasterPasswordInput error={props.error} isDisabled={props.isLoading || showDashlaneUpdateInfoBox(props.error)} password={password} showPassword={showPassword} handlePasswordInputChange={handlePasswordInputChange} handleShowPassword={handleShowPassword} theme={ThemeEnum.Dark} spellCheck={false}/>
        {showDashlaneUpdateInfoBox(props.error) ? (<DashlaneUpdateNeeded />) : null}
        {props.displayRememberMe && !showDashlaneUpdateInfoBox(props.error) && (<div className={styles.rememberMe}>
            <Checkbox checked={rememberMeIsChecked} label={translate(I18N_KEYS.REMEMBER_ME)} onChange={onRememberMeCheckBoxChanged} theme={ThemeEnum.Dark}/>
            {rememberMeIsChecked && (<div className={styles.warningMessage}>
                {translate(I18N_KEYS.REMEMBER_ME_WARNING)}
              </div>)}
          </div>)}
      </div>
      <div id="dialog" className={styles.dialog}>
        <AccountRecoveryRequestStartDialog isVisible={showStartRequestDialog} onDismiss={() => {
            setShowStartRequestDialog(false);
        }} onStartAccountRecovery={onSendNewRequest}/>
        <AccountRecoveryRequestPendingDialog isVisible={showPendingRequestDialog} action={shouldSendNewRequest ? 'sendNewRequest' : 'cancel'} onCancelOrSendNew={shouldSendNewRequest ? onSendNewRequest : onCancelRequest} onDismiss={() => {
            setShowPendingRequestDialog(false);
            setShouldSendNewRequest(false);
        }}/>
        <AccountRecoveryRequestApprovedDialog onRecoverUserAccount={() => {
            void onRecoverUserAccount();
        }} isVisible={showApprovedRequestDialog}/>
        <AccountRecoveryRequestRefusedDialog isVisible={showRefusedRequestDialog} onSendNewRequest={onSendNewRequest} onDismiss={() => {
            setShowRefusedRequestDialog(false);
        }}/>
      </div>
    </FormWrapper>);
