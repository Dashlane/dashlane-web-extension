...
import { WebAuthnStep } from "./WebAuthnStep/WebAuthnStep";
import { WebAuthnErrorStep } from "./WebAuthnErrorStep/WebAuthnErrorStep";
import OTPWrapper from "./OTPWrapper";
import EmailStep from "./EmailStep";
import WelcomeStep from "./WelcomeStep";
import ActivateSSOStep from "./ActivateSSOStep";
import SSOStep from "./SSOStep";
import TokenWrapper from "./TokenWrapper";
import { DashlaneAuthenticatorStep } from "./DashlaneAuthenticatorStep";
import { DeviceLimitFlow } from "./DeviceLimitFlow";
import {
  EmailError,
  isInEmailError,
  resendToken,
...
const Login: React.FC<LoginProps> = ({
  ...
  localAccountsLogin,
  openSignupOrLogin,
  ssoMigrationServiceProviderUrl,
  ...
  const [isWaitingCarbonResponse, setIsWaitingCarbonResponse] =
    React.useState(false);
  const [displayRememberMeWithAutoLogin, setDisplayRememberMeWithAutoLogin] =
    React.useState(true);
  const [animationRunning, setAnimationRunning] = React.useState(false);
  const [
    isDashlaneAuthenticatorAvailable,
    setIsDashlaneAuthenticatorAvailable,
  ] = React.useState(false);
  const [twoFactorAuthenticationMode, setTwoFactorAuthenticationMode] =
    React.useState<OtpType>(OtpType.NO_OTP);
  const { loginUserWithEnclaveSSO } = useModuleCommands(confidentialSSOApi);
  const { initiateLoginWithSSO, logout } = useModuleCommands(
  ...
  React.useEffect(() => {
    ...
    if (login === lastUsedAccount?.login) {
      if (
        browserSupportsWebAuthnAuthentication() &&
  ...
  React.useEffect(() => {
    if (step && step in loginStepToPageView) {
      logPageView(
        loginStepToPageView[step as keyof typeof loginStepToPageView]
      );
    }
  }, [step]);
  React.useEffect(() => {
  ...
  const switchToAlternateMethodStep = (
    target: OTPLoginStep | BackupCodeLoginStep
  ): void => {
    setStep(target);
    resetErrorState();
  };
  ...
  const switchToOtpTokenStepFromDashlaneAuthenticatorStep =
    async (): Promise<void> => {
      setIsWaitingCarbonResponse(false);
  ...
  const onSSOClickFromWelcomeStep = async (
    rememberMeForSSOPreference: boolean
  ) => {
    localStorage.setItem(
      REMEMBER_ME_FOR_SSO_PREFERENCE,
      rememberMeForSSOPreference ? "true" : "false"
    );
  ...
  const passwordStepDescription =
    login === lastUsedAccount?.login &&
    lastUsedAccount.rememberMeType === "webauthn" &&
    lastUsedAccount.shouldAskMasterPassword
      ? "login/webauthn_password_session_expired_description"
      : undefined;
  const isInitialLocalLogin =
    localAccountsLogin.length === 0 || !localAccountsLogin.includes(login);
  const getPasswordLessStepComponent = () => (
    <PasswordLessStep switchToEmailStep={switchToEmailStep} />
  );
  const getPasswordStepComponent = () => (
    <PasswordStep
      login={login}
      isLoading={isWaitingCarbonResponse}
      sendPassword={sendPassword}
      displayRememberMe={
        twoFactorAuthenticationMode !== OtpType.OTP_LOGIN &&
        displayRememberMeWithAutoLogin &&
        !isInitialLocalLogin
      }
      setWaitingForResponseState={setWaitingForResponseState}
      error={error}
      resetErrorState={resetErrorState}
      descriptionKey={passwordStepDescription}
    />
  );
  const getWebAuthnStepComponent = () => (
    <WebAuthnStep
      login={login}
      onUseMasterPassword={switchToPasswordStep}
      getExtensionId={kernel.browser.getExtensionId}
      showError={switchToWebAuthnErrorStep}
    />
  );
  const getWebAuthnErrorStepComponent = () => (
    <WebAuthnErrorStep
      onUseMasterPassword={switchToPasswordStep}
      onRetryWebAuthn={switchToWebAuthnStep}
    />
  );
  const getTwoFactorAuthStepComponent = () => {
    if (step) {
      const { handleSubmit, alternateMethod, component } =
        TWO_FACTOR_AUTH_STEP_MAPPER[step];
      return React.createElement(component, {
        ...
        error,
        handleSubmit,
        onSwitchAuthMethodClick: () =>
          switchToAlternateMethodStep(alternateMethod),
        resetErrorState,
      });
    }
    return null;
  };
  const getTokenStepComponent = () => (
    <TokenWrapper
      login={login}
      isLoading={isWaitingCarbonResponse}
      setWaitingForResponseState={setWaitingForResponseState}
      ...
      resetErrorState={resetErrorState}
      isDashlaneAuthenticatorAvailable={isDashlaneAuthenticatorAvailable}
      switchToDashlaneAuthenticatorStep={switchToDashlaneAuthenticatorStep}
    />
  );
  const getDashlaneAuthenticatorStepComponent = () => (
  ...
  const getEmailStep = () => (
    <EmailStep
      onCreateAnAccountClick={() => openSignupOrLogin("/signup")}
  ...
  const getWelcomeStep = () => (
    <WelcomeStep
      onCreateAccountClick={onNewAccountClickFromWelcomeStep}
      onLoginClick={switchToEmailStepFromWelcomeStep}
    />
  );
  const getSSOStep = () => (
    <SSOStep onSSOSignInClicked={onSSOClickFromWelcomeStep} />
  );
  const getActivateSSOStep = () => (
    <ActivateSSOStep
      onActivateSSOClicked={() => {
        void onActivateSSOClick();
      }}
    />
  );
  const getDeviceLimitFlow = () => (
    <DeviceLimitFlow
  ...
  return (
    <div className={styles.loginPanel}>
      {showHeader ? (
        <Header
          login={login}
          localAccountsLogin={localAccountsLogin}
          onOtherAccountClick={switchToEmailStepFromDropdown}
          onNewAccountClick={onNewAccountClickFromDropdown}
          onSelectLogin={updateLogin}
          ...
          showLogoutDropdown={step === LoginStep.ActivateSSO}
        />
      ) : null}
