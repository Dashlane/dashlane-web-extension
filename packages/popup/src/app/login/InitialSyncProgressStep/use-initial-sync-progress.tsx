...
import { DeviceLimitDoneStageView, LoginDeviceLimitFlowStage, UnlinkingAndOpeningSessionStageView, } from '@dashlane/communication';
import { DeviceLimitFlowAnimationPhase, UseDeviceLimitFlowAnimationParams, } from '../DeviceLimitFlow/use-device-limit-flow-animation';
...
export const useInitialSyncProgress = ({ stage, setIsInitialSyncAnimationPending, }: UseInitialSyncProgressProps): {
    ...
    const notifyPendingState = (isPending: boolean) => () => setIsInitialSyncAnimationPending(isPending);
    ...
    const onIntroComplete = React.useCallback((startPhase: (next: DeviceLimitFlowAnimationPhase) => void) => startPhase(DeviceLimitFlowAnimationPhase.INNER_LOOP), []);
