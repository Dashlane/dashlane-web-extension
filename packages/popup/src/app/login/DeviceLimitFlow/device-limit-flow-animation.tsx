...
import { assertUnreachable } from 'src/libs/assert-unreachable';
...
export const DeviceLimitFlowAnimation = ({ containerClassName, ...rest }: DeviceLimitFlowAnimationProps) => {
    ...
    switch (phase) {
        case DeviceLimitFlowAnimationPhase.INTRO:
            return (<Animation animationParams={ANIMATION_DATA[DeviceLimitFlowAnimationPhase.INTRO]} containerClassName={containerClassName} eventListeners={introEventListeners} loadingComp={<Placeholder className={containerClassName}/>}/>);
        case DeviceLimitFlowAnimationPhase.INNER_LOOP:
            return (<Animation animationParams={ANIMATION_DATA[DeviceLimitFlowAnimationPhase.INNER_LOOP]} containerClassName={containerClassName} eventListeners={innerLoopEventListeners} loadingComp={<Placeholder className={containerClassName}/>}/>);
        case DeviceLimitFlowAnimationPhase.OUTRO:
            return (<Animation animationParams={ANIMATION_DATA[DeviceLimitFlowAnimationPhase.OUTRO]} containerClassName={containerClassName} eventListeners={outroEventListeners} loadingComp={<Placeholder className={containerClassName}/>}/>);
        default:
