...
import InputWithMessage, { MessageType, } from 'components/inputs/login/InputWithMessageWrapper';
...
const EmailStep: React.FunctionComponent<EmailStepProps> = (props: EmailStepProps) => {
    ...
    const getErrorDesc = (error: string): string | undefined => {
        ...
        return translate(error in I18N_ERROR_KEYS
    ...
    const labelId = 'email';
    return (<FormWrapper title={{ text: translate(I18N_KEYS.ENTER_YOUR_EMAIL), labelId }} handleSubmit={onSubmit} formActionsProps={formActionsProps}>
      <InputWithMessage type={MessageType.Error} message={props.error && getErrorDesc(props.error)} theme={ThemeEnum.Dark}>
        <FormInput value={emailValue} hasError={!!props.error} placeholder={translate(I18N_KEYS.EMAIL_PLACEHOLDER)} inputType="email" handleChange={handleInputChange} ariaLabelledBy={labelId} theme={ThemeEnum.Dark}/>
      </InputWithMessage>
