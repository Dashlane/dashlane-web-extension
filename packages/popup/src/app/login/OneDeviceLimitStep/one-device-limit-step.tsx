...
import { BackIcon, FlexChild, FlexContainer, Heading, jsx, Link, Paragraph, } from '@dashlane/ui-components';
...
const OneDeviceLimitStepComponent = (props: OneDeviceLimitStepProps) => {
    ...
    const [showUnlinkPreviousDeviceStep, setShowUnlinkPreviousDeviceStep] = React.useState(false);
    ...
    return showUnlinkPreviousDeviceStep ? (<UnlinkPreviousDeviceStep previousDeviceInfo={props.previousDevice} onCancel={onStartOver} onUnlinkPreviousDevice={onUnlinkPreviousDevice}/>) : (<FlexContainer flexDirection="column" justifyContent="space-between" alignItems="stretch" sx={{
      ...
      <Button fullsize onClick={onUpgradeToPremium2} mood="brand" size="large" sx={{
            marginBottom: '16px',
        }}>
        {translate(I18N_KEYS.SEE_PREMIUM_PLAN)}
      </Button>
      <Button fullsize onClick={onShowUnlinkPreviousDevice} mood="neutral" intensity="quiet" size="large">
