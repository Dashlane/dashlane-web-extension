import { ChangeEvent, FormEvent, Fragment, FunctionComponent, memo, useState, } from 'react';
import { Checkbox, colors, InfoBox, jsx, Link, Paragraph, } from '@dashlane/ui-components';
...
import styles from 'app/login/SSOStep/styles.css';
...
const SSOStep: FunctionComponent<SSOStepProps> = (props: SSOStepProps) => {
    const rememberMeForSSOStatus = localStorage.getItem(REMEMBER_ME_FOR_SSO_PREFERENCE) !== 'false';
    const { translate } = useTranslate();
    const [loading, setLoading] = useState(false);
    const [rememberMeForSSOPreference, setRememberMeForSSOPreference] = useState(rememberMeForSSOStatus);
    const isAutoSsoLoginDisabled = useIsAutoSsoLoginDisabled();
    const onRememberMeWithSSOCheckBoxChanged = (e: ChangeEvent<HTMLInputElement>): void => {
    ...
    return (<FormWrapper formActionsProps={formActionProps} handleSubmit={handleFormSubmit}>
      ...
      {isAutoSsoLoginDisabled.status === DataStatus.Success &&
          ...
          <Checkbox sx={{ marginTop: '24px', marginBottom: '24px' }} checked={rememberMeForSSOPreference} onChange={onRememberMeWithSSOCheckBoxChanged} label={<Paragraph size="x-small" color={colors.white}>
                {translate(I18N_KEYS.AUTOMATIC_SSO)}
              </Paragraph>}/>
          <InfoBox size="small" severity="subtle" title={<div sx={{ display: 'flex', flexDirection: 'column' }}>
