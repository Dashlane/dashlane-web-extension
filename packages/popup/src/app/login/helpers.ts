...
export const resendToken = (params: {
    login: string;
}): void => {
    const { login } = params;
    carbonConnector.updateLoginStepInfo({ login });
...
export const isInTokenError = (errorCode: string): errorCode is TokenError => !!TokenError[errorCode];
export const isInEmailError = (errorCode: string): errorCode is EmailError => !!EmailError[errorCode];
