    ...
    return (<FormWrapper title={{
      ...
      <div className={styles.inputContainer}>
        ...
        <InputWithMessageWrapper type={MessageType.Error} message={props.error} theme={ThemeEnum.Dark}>
          <PinInput value={props.codeValue} maxLength={OTP_MAX_LENGTH} handleInputChange={handleInputChange} hasError={!!props.error} id={codeLabelId} labelId={codeLabelId} a11yLabelId={a11yLabelId} onFocus={props.onInputFocus}/>
        </InputWithMessageWrapper>
        {props.formDetails ? props.formDetails : null}
      </div>
      {showSyncDevicesHelp ? (<InfoBox sx={{ marginTop: '16px' }} severity="subtle" size="small" title={translate(I18N_KEYS.SYNC_DEVICES_TIME_INFOBOX)}/>) : null}
    </FormWrapper>);
};
