...
const isDefaultActions = (
  actions: CustomActionsProps | DefaultActionsProps
): actions is DefaultActionsProps => {
  return (actions as DefaultActionsProps).formActionsProps !== undefined;
};
const FormWrapper: React.FunctionComponent<React.PropsWithChildren<Props>> = (
  props: React.PropsWithChildren<Props>
) => (
  <form className={styles.form} onSubmit={props.handleSubmit} noValidate>
    {props.title && (
      <h2
    ...
    {props.description ? (
      <Paragraph
        color={colors.dashGreen05}
        size="small"
        sx={{ mb: "32px", lineHeight: "20px" }}
      >
        {props.description}
      </Paragraph>
    ) : null}
    <div className={styles.inputContainer}>{props.children}</div>
    {isDefaultActions(props) ? (
