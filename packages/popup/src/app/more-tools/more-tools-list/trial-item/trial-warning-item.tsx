...
export const BANNER_UTM_SOURCE_PARAM = 'button:buy_dashlane+click_origin:more_tab_extension+origin_page:settings/more+origin_component:extension_popup';
export const TrialWarningItem = () => {
    ...
    const { subscriptionCode, billingInfo, trialStatus, isTeam, isBusiness, isTeamSoftDiscontinued, } = trialInfo;
    ...
    const formattedEndDate = shortDate(fromUnixTime(billingInfo.nextBillingDetails.dateUnix), LocaleFormat.LL);
    ...
    return (<MoreToolsListItemClickable icon={<Icon name="PremiumOutlined" color="ds.text.neutral.catchy"/>} onClick={handleBuyDashlane} title={isBusiness
            ? translate(I18N_KEYS.TITLE_BUSINESS)
            : translate(I18N_KEYS.TITLE_TEAM)} isWarning={daysLeftInTrial ? daysLeftInTrial <= 7 : false} explanation={isBusiness
