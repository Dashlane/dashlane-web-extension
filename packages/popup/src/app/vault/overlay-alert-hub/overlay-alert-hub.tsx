...
import { DarkWebBreachAlert } from 'src/app/vault/overlay-alert-hub/alerts/dark-web-breach-alert';
export const OverlayAlertHub = () => {
    const [isDismissed, setIsDismissed] = React.useState(false);
    const breachesQueryResult = useBreaches(BreachStatus.PENDING);
    const pendingBreaches = breachesQueryResult.status === DataStatus.Success
    ...
    if (shouldDisplayDarkWebBreach) {
        return (<DarkWebBreachAlert pendingBreaches={pendingBreaches} onClick={onClick} onDismiss={onDismiss}/>);
    }
    return null;
};
