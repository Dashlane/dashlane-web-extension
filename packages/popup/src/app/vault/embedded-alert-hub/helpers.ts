...
const hasTrialExpiredUserMessageDismissed = (
  messages: UserMessage[]
): boolean => {
  return messages.some(
    (message) =>
      message.type === UserMessageTypes.TRIAL_EXPIRED && message.dismissedAt
  );
};
interface ShouldDisplayOnboardingAlertParams {
  webOnboardingMode: WebOnboardingModeEvent | null;
}
const shouldDisplayOnboardingAlert = ({
  webOnboardingMode,
}: ShouldDisplayOnboardingAlertParams): boolean => {
...
export function getAlert({
  ...
  if (
    paymentFailureNotification === PaymentFailureNotificationStatus.Churning
  ) {
  ...
  if (
    isPremiumTrialExpired({ premiumStatus }) &&
