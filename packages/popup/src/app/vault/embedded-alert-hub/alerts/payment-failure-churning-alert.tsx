...
import { dismissNotification, usePremiumStatusData } from 'libs/api';
import { getSubscriptionUrl } from 'src/app/vault/embedded-alert-hub/helpers';
...
const PaymentFailureChurningComponent: React.FC<PaymentFailureChurningAlertProps> = ({ onClick, onDismiss }) => {
    ...
    if (premiumStatusQuery.status === DataStatus.Success) {
        ...
        return (<InfoBox size="descriptive" severity="alert" title={translate(I18N_KEYS.TITLE)} className={styles.alert} actions={[
                <Button type="button" nature="secondary" key="secondary" onClick={dismiss}>
            {translate(I18N_KEYS.DISMISS_LABEL)}
          </Button>,
                <Button type="button" nature="primary" key="primary" onClick={handleClick}>
...
export const PaymentFailureChurningAlert = React.memo(PaymentFailureChurningComponent);
