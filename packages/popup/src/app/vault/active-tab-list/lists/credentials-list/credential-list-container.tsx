...
const PAGE_SIZE = 20;
const CredentialListContainer = ({ order, onOrderChangeHandler, website, }: CredentialListContainerProps) => {
    ...
    return (<BaseListContainer hasItems={Boolean(matchCount)} vaultTabType={VaultTabType.Credentials}>
      <CredentialsList credentials={items} credentialsCount={matchCount} hasMore={hasMore} isNextPageLoading={items.length !== amountToFetch && hasMore} onLoadMore={() => setPageNumber((prevPageNumber) => prevPageNumber + 1)} onOrderChange={(newOrder) => {
