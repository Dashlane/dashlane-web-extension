...
const PasswordDropdownElement = ({
  credential,
}: PasswordDropdownElementProps) => {
  ...
  if (hasSharedLimitedPermission || !credential.password) {
    return (
      <DisabledDropdownElement
        dropdownLabel={translate(I18N_KEYS.COPY_PASSWORD)}
        tooltipTitle={translate(
          hasSharedLimitedPermission
  ...
  return isProtected ? (
    <ProtectedCopyDropdownElement
      {...defaultCopyDropdownProps}
      showNeverAskOption
      confirmLabelMode={ConfirmLabelMode.CopyPassword}
      credentialUrl={credential.URL}
    />
  ) : (
    <CopyDropdownElement {...defaultCopyDropdownProps} />
  );
};
