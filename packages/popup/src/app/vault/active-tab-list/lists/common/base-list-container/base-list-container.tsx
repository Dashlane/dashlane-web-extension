...
const BaseListContainer = ({ vaultTabType, hasItems, children, }: IBaseListContainerProps) => {
    const { searchValue } = useSearchContext();
    const isSearching = searchValue !== '';
    if (!hasItems) {
        return isSearching ? (<NoResultsVaultItemsList vaultTabType={vaultTabType} searchValue={searchValue}/>) : (<EmptyVaultItemsList vaultTabType={vaultTabType}/>);
    }
    return children;
};
