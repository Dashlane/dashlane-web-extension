...
const SectionListHeaderWithSort = ({
  sortingOrder,
  onOrderChange,
  credentialsCount,
  headerRef,
  ...
  const getListLabel = () => {
    const item = sortingOptions.find(
      (propItem) => propItem.value === sortingOrder
    );
    return item ? item.label : "sort-criteria-listbox-label";
  };
  return (
    <SectionListHeader headerRef={headerRef} itemsCount={credentialsCount}>
      <Select
        items={sortingOptions.map((item) => ({
          ...item,
          active: item.value === sortingOrder,
        }))}
        onChange={onOrderChange}
        optionsTitle={translate(I18N_KEYS.SORT_BY_TITLE)}
