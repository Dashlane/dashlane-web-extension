...
export const ActionsDropdown = ({ isOpen, setIsOpen, onKeyDown, ...rest }: DropdownProps) => {
    ...
    const handleContentKeyboardNavigation = (e: React.KeyboardEvent) => {
        ...
        const dropdownElements = contentRef.current?.querySelectorAll('button:not([disabled])');
        if (!dropdownElements?.length) {
            return;
        }
        const currentFocusedElementIndex = Array.from(dropdownElements).findIndex((dropdownElement) => dropdownElement.isSameNode(document.activeElement));
        ...
        const nextElementIndex = currentFocusedElementIndex + (e.key === 'ArrowDown' ? 1 : -1);
        if (nextElementIndex < dropdownElements.length) {
            (dropdownElements[nextElementIndex] as HTMLElement).focus();
        }
    };
    return (<div ref={dropdownMenuRef} onBlur={handleDropdownMenuBlur} onKeyDown={handleContentKeyboardNavigation}>
      <Dropdown ref={contentRef} isOpen={isOpen} setIsOpen={setIsOpen} onKeyDown={(event) => {
            if (event.key === 'Enter') {
