...
import { BaseListContainer, ISearchOrderProps, withSearchOrder, } from '../common';
import { SecureNotesList } from './secure-notes-list';
const PAGE_SIZE = 20;
const SecureNotesListContainer = ({ order, onOrderChangeHandler, }: ISearchOrderProps) => {
    ...
    const { secureNotesResult: { items, matchCount }, } = data;
    SearchEventLogger.totalCount = matchCount;
    const hasMore = items.length < matchCount;
    return (<BaseListContainer hasItems={!!items.length} vaultTabType={VaultTabType.SecureNotes}>
      <SecureNotesList notes={items} notesCount={matchCount} hasMore={hasMore} isNextPageLoading={items.length !== amountToFetch && hasMore} onLoadMore={() => setPageNumber((prevPageNumber) => prevPageNumber + 1)} onOrderChange={(newOrder) => {
