...
import { ProtectedTextArea } from 'src/components/inputs/common/protected-text-area/protected-text-area';
...
export const I18N_KEYS = {
    ...
    CARD_NUMBER_COPY: 'tab/all_items/paymentCard/actions/card_number_copy_to_clipboard',
    CARD_NUMBER_COPIED: 'tab/all_items/paymentCard/actions/card_number_copied_to_clipboard',
    ...
    SECURITY_CODE_COPY: 'tab/all_items/paymentCard/actions/security_code_copy_to_clipboard',
    SECURITY_CODE_COPIED: 'tab/all_items/paymentCard/actions/security_code_copied_to_clipboard',
...
const PaymentCardFormComponent = ({ card }: PaymentCardDetailFormProps) => {
    ...
    const { cardNumber, expireMonth, expireYear, id, ownerName, note, securityCode, } = card;
    ...
    return (<FormContainer>
      {ownerName && (<Input id="accountHolder" inputType="text" label={translate(I18N_KEYS.ACCOUNT_HOLDER_LABEL)} value={ownerName} readonly/>)}
      {cardNumber && (<ProtectedValueField protectedItemId={id} valueFieldId="cardNumber" valueFieldLabel={translate(I18N_KEYS.CARD_NUMBER_LABEL)} value={cardNumber.replace(/(.{4})/g, '$1 ')} fieldType={ItemType.CreditCard} revealValueLabel={translate(I18N_KEYS.CARD_NUMBER_SHOW)} hideValueLabel={translate(I18N_KEYS.CARD_NUMBER_HIDE)} onCopyClick={() => {
                void cardNumberCopyAction();
            }} copyValueLabel={translate(I18N_KEYS.CARD_NUMBER_COPY)}/>)}
      {securityCode && (<ProtectedValueField protectedItemId={id} valueFieldId="securityCode" valueFieldLabel={translate(I18N_KEYS.SECURITY_CODE_LABEL)} value={securityCode} fieldType={ItemType.CreditCard} revealValueLabel={translate(I18N_KEYS.SECURITY_CODE_SHOW)} hideValueLabel={translate(I18N_KEYS.SECURITY_CODE_HIDE)} onCopyClick={() => {
                void securityCodeCopyAction();
            }} copyValueLabel={translate(I18N_KEYS.SECURITY_CODE_COPY)}/>)}
      {expireMonth && expireYear ? (<Input id="expiryDate" inputStyle={paymentCardExpiredStyle(expireMonth, expireYear)} label={translate(I18N_KEYS.EXPIRATION_DATE_LABEL)} value={`${expireMonth}/${expireYear}`} readonly inputType={'text'}/>) : null}
      {note && (<ProtectedTextArea protectedItemId={note} valueFieldId="notes" valueFieldLabel={translate(I18N_KEYS.NOTE_LABEL)} value={note} revealValueLabel={translate(I18N_KEYS.NOTE_SHOW)} hideValueLabel={translate(I18N_KEYS.NOTE_HIDE)} onCopyClick={() => {
                void noteCopyAction();
