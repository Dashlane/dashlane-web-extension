...
import { OpenWebsiteIconButton } from "../form-fields/open-website-icon-button";
import { openCredentialWebsite } from "../../helpers";
import styles from "./linked-websites-view.css";
export const I18N_KEYS = {
  PRIMARY: "tab/all_items/credential/linked_websites_view/label/primary",
  ADDED_BY_YOU:
    "tab/all_items/credential/linked_websites_view/label/added_by_user",
  ADDED_BY_DASHLANE:
    "tab/all_items/credential/linked_websites_view/label/added_by_dashlane",
...
const LinkedWebsitesViewFormComponent = ({
  credential,
  dashlaneDefinedLinkedWebsites,
}: LinkedWebsitesViewFormProps) => {
  const { translate } = useTranslate();
  const rootDomainOfPrimaryWebsite = new ParsedURL(
    credential.URL
  ).getRootDomain();
  ...
  const labelAddedByDashlane = translate(
    I18N_KEYS.ADDED_BY_DASHLANE
  ).toLocaleUpperCase();
  return (
    <div className={styles.viewContainer} data-testid="linked_websites_view">
      <div className={styles.container}>
        <label className={styles.inputContainer}>
          <span className={styles.label}>{labelPrimary}</span>
          <input
            aria-label={translate(I18N_KEYS.PRIMARY)}
        ...
        <div className={styles.button}>
          <OpenWebsiteIconButton
            text={translate(I18N_KEYS.WEBSITE_OPEN)}
            onClick={() => {
      ...
      {credential.linkedURLs.length > 0 ? (
        <div className={styles.container}>
          <label className={styles.inputContainer}>
            ...
            {credential.linkedURLs.map((website: string, index: number) => {
              ...
              return (
                <div key={index} className={styles.website}>
                  <input
                    aria-label={translate(I18N_KEYS.ADDED_BY_YOU)}
                    className={classNames([styles.text, styles.editableItem])}
                    value={website}
                    readOnly={true}
                  />
                  <div
                    ...
                    <OpenWebsiteIconButton
                      ...
                      onClick={() => {
                        void openExternalUrl(
                          parsedUrl.getUrlWithFallbackHttpsProtocol()
                        );
                      }}
                      ariaLabel={translate(I18N_KEYS.WEBSITE_OPEN_A11Y, {
                        value: website,
      ...
      {dashlaneDefinedLinkedWebsites.length > 1 ? (
        <div className={styles.container}>
          <label className={styles.inputContainer}>
            ...
            {dashlaneDefinedLinkedWebsites.map(
              (website: string, index: number) => {
                const parsedUrl = new ParsedURL(website);
                return (
                  <div key={index} className={styles.website}>
                    <input
                      aria-label={translate(I18N_KEYS.ADDED_BY_DASHLANE)}
                      className={classNames([styles.text, styles.lockedItem])}
                      value={website}
                      readOnly={true}
                    />
                    <div className={styles.button}>
