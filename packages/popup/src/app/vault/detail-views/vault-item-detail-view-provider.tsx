...
export const VaultItemDetailViewProvider = ({
  children,
  onCloseModal,
}: VaultItemDetailViewProviderProps) => {
  const [activeItem, setActiveItem] =
    usePopupPersistedState<ActiveItemDetail | null>(
      ActiveItemDetail.storageKey,
      null
    );
  ...
  const getActiveDetailView = (item: ActiveItemDetail) => {
    const DetailViewComponent =
      ItemTypeToDetailViewDictionary[
        activeItem!.type as keyof typeof ItemTypeToDetailViewDictionary
      ];
    return <DetailViewComponent itemId={item.id} onClose={closeDetailView} />;
  };
  return (
    <VaultItemDetailViewContext.Provider value={{ openDetailView }}>
      {children}
      {activeItem && (
        <DetailViewModal
          onClose={closeDetailView}
          skipAnimation={shouldSkipAnimationRef.current}
        >
