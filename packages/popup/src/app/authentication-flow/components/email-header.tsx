...
import DropdownMenu, { MenuItem, } from 'components/dropdown/dropdown-menu/dropdown';
...
export const EmailHeader: FunctionComponent<EmailHeaderProps> = ({ loginEmail, showAccountsActionsDropdown, showLogoutDropdown, localAccounts = [], }: EmailHeaderProps) => {
    ...
    const menuItems: MenuItem[] = [
        ...localAccounts.map((account) => ({
            label: account.login,
            onClick: () => loginEmail !== account.login && changeLogin({ login: account.login }),
    ...
    return (<div sx={{ padding: '0px 24px 4px', maxWidth: '300px' }}>
      {showLogoutDropdown ? (<DropdownMenu title={loginEmail} items={logoutDropdownMenuItems} theme={ThemeEnum.Light}/>) : menuItems.length > 0 ? (<DropdownMenu title={loginEmail} items={menuItems} theme={ThemeEnum.Light}/>) : null}
    </div>);
};
