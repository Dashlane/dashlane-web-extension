...
export const AccountEmailSelectContainer = (props: Props) => {
  const [showWelcomeStep, setShowWelcomeStep] = useState(
    !props.localAccounts || props.localAccounts.length === 0
  );
  ...
  return showWelcomeStep ? (
    <AccountEmailWelcome
      onLoginClick={handleOnWelcomeLoginClick}
      onCreateAccountClick={handleOnWelcomeCreateAccountClick}
    />
  ) : (
    <AccountEmailSelect {...props} />
  );
};
