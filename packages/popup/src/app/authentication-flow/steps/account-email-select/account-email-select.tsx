...
export const AccountEmailSelect: FunctionComponent<Props> = ({ loginEmail, localAccounts, sendEmail, isLoading, error, clearInputError, }: Props) => {
    ...
    const { handleSubmit, values: { email }, setFieldValue, } = formik;
    ...
    return (<>
      <EmailHeader loginEmail={loginEmail ?? ''} showAccountsActionsDropdown={false} showLogoutDropdown={false} localAccounts={localAccounts}/>
      <form sx={FORM_SX_STYLES} onSubmit={handleSubmit} noValidate>
        <Heading id={EMAIL_STEP_DESCRIPTION} size="x-small" color="text.neutral.catchy" sx={{ marginBottom: '24px' }}>
          {translate(I18N_KEYS.ENTER_YOUR_EMAIL)}
        </Heading>
        ...
        <div sx={{
          ...
          <TextInput name="email" id="extng-account-email-input" value={email} type="email" onChange={handleEmailChange} placeholder={translate(I18N_KEYS.EMAIL_PLACEHOLDER)} aria-describedby={EMAIL_STEP_DESCRIPTION} disabled={isLoading} feedback={error
            ? {
        ...
        <Button type="submit" id="extng-account-email-next-button" isLoading={isLoading} disabled={!email} fullsize size="large" sx={{ marginBottom: '8px' }}>
          {translate(I18N_KEYS.CONFIRM)}
        </Button>
        <Button onClick={openWebAppSignup} mood="neutral" intensity="quiet" fullsize size="large">
