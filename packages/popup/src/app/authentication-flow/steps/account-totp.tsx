...
export const AccountTotp: FunctionComponent<Props> = ({ loginEmail, isLoading, error, changeTwoFactorAuthenticationOtpType, clearInputError, submitTotp, localAccounts, }: Props) => {
    ...
    return (<>
      ...
      <form sx={FORM_SX_STYLES} onSubmit={handleSubmit} noValidate>
        ...
        <div sx={{
          ...
          <SecurityCodeInput title={translate(I18N_KEYS.DESCRIPTION)} securityToken={twoFactorAuthenticationOtpValue} maxLength={SECURITY_TOKEN_MAX_LENGTH} onTokenInputChange={onTokenInputChange} errorMessage={error &&
            translate(error in ERROR_I18N_KEYS
                ? ERROR_I18N_KEYS[error as keyof typeof ERROR_I18N_KEYS]
                : ERROR_I18N_KEYS.UNKNOWN_ERROR)}/>
        </div>
        {showSyncDevicesHelp ? (<Infobox sx={{ marginBottom: '16px' }} title={translate(I18N_KEYS.SYNC_DEVICES_TIME_INFOBOX)} size="small" mood="brand"/>) : null}

        <Button type="submit" aria-label={translate(I18N_KEYS.CONTINUE)} isLoading={isLoading} disabled={twoFactorAuthenticationOtpValue.length < OTP_MAX_LENGTH} fullsize size="large" sx={{ marginBottom: '8px' }}>
