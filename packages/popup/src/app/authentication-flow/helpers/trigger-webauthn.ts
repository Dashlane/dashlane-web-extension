...
import { startAssertion } from "../../login/WebAuthnStep/helpers/credential";
import { carbonConnector } from "../../../carbonConnector";
import { kernel } from "../../../kernel";
export const triggerWebAuthn = async (
  loginEmail: string,
  ...
  try {
    ...
    if (!relyingPartyId) {
      return {
        success: false,
        error: new Error(
          "WebAuthn Authentication init Open Session cannot be started as extension Id is missing"
        ),
      };
    }
    const initOpenSessionResult =
