...
const I18N_KEYS = {
  STATUS_B2C_PREMIUM_TRIAL_N_DAYS:
    "tab/all_items/trial_banner/status_b2c_free_trial_n_days_markup",
  CTA_PURCHASE_FREE_TRIAL: "tab/all_items/trial_banner/cta_purchase_free_trial",
  POST_TRIAL_EXPIRED:
    "tab/all_items/trial_banner/b2c_post_trial_banner_expired",
  POST_TRIAL_BUY_DASHLANE:
    "tab/all_items/trial_banner/b2c_post_trial_banner_buy_dashlane",
};
interface Props {
  isPostTrial?: boolean;
}
    ...
    <>
      {premiumStatus.isTrial && (
        <FooterAlertButton
          ...
          labelText={translateMarkup(
            I18N_KEYS.STATUS_B2C_PREMIUM_TRIAL_N_DAYS,
            { count: daysLeftInTrial }
          )}
          buttonText={translate(I18N_KEYS.CTA_PURCHASE_FREE_TRIAL)}
          handleClick={handleClick}
        />
      )}
      {isPostTrial && daysAfterTrial >= 1 && daysAfterTrial <= 15 && (
        <FooterAlertButton
          colors={{
            backgroundColor: "ds.container.expressive.warning.catchy.idle",
            color: "ds.text.inverse.catchy",
          }}
          labelText={translate(I18N_KEYS.POST_TRIAL_EXPIRED)}
          buttonText={translate(I18N_KEYS.POST_TRIAL_BUY_DASHLANE)}
          handleClick={handleClick}
        />
      )}
    </>
