import { Checkbox, InfoBox, jsx } from '@dashlane/ui-components';
...
import { disableCredentialProtection, unlockProtectedItems, updateProtectPasswordsSetting, } from 'libs/api';
...
const I18N_KEYS = {
    ...
    NEVER_ASK_CHECKBOX_CREDENTIAL_SETTING_ON: 'master_password_dialog/never_ask_checkbox_credential_setting_on',
    NEVER_ASK_CHECKBOX_GLOBAL_SETTING_ON: 'master_password_dialog/never_ask_checkbox_global_setting_on',
    NEVER_ASK_CHECKBOX_GLOBAL_SETTING_ON_WARNING: 'master_password_dialog/never_ask_checkbox_global_setting_on_warning',
};
const MasterPasswordDialog = ({ confirmLabelMode, onCancel, onSuccess, onError, showNeverAskOption = false, credentialId, requirePasswordGlobalSetting = false, }: Props) => {
    ...
    const handlePasswordInputChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
        setNoErrorState();
    ...
    const confirmDialog = useCallback(async (e: FormEvent<HTMLElement>) => {
        e.preventDefault();
            ...
            case UnlockProtectedItemsStatus.SUCCESS:
                if (neverAskCheckbox) {
                    if (requirePasswordGlobalSetting) {
                        await updateProtectPasswordsSetting(!requirePasswordGlobalSetting);
                    }
    ...
    return (<Dialog visible={true} onConfirm={(event: FormEvent<HTMLElement>) => {
            void confirmDialog(event);
        }} onCancel={cancelDialog} confirmLabel={getConfirmLabel()} cancelLabel={translate(I18N_KEYS.CANCEL)} isConfirmDisabled={isPasswordEmpty}>
      <h2 className={dialogStyles.title}>{translate(I18N_KEYS.TITLE)}</h2>
      <form onSubmit={(event: FormEvent<HTMLElement>) => {
            void confirmDialog(event);
        }} noValidate>
        <div className={dialogStyles.inputWrapper}>
          <MasterPasswordInput error={error} password={password} showPassword={showPassword} handlePasswordInputChange={handlePasswordInputChange} handleShowPassword={handleShowPassword} theme={ThemeEnum.Light}/>
          {showNeverAskOption ? (<>
              <Checkbox data-testid="neverAskCheckbox" checked={neverAskCheckbox} onChange={handleCheckboxChange} sx={{ margin: '12px 0px' }} label={requirePasswordGlobalSetting
                ? translate(I18N_KEYS.NEVER_ASK_CHECKBOX_GLOBAL_SETTING_ON)
                : translate(I18N_KEYS.NEVER_ASK_CHECKBOX_CREDENTIAL_SETTING_ON)}/>
              {requirePasswordGlobalSetting && (<InfoBox severity="warning" size="small" title={translate(I18N_KEYS.NEVER_ASK_CHECKBOX_GLOBAL_SETTING_ON_WARNING)}/>)}
            </>) : null}
        </div>
      </form>
    </Dialog>);
