...
type IconParameter = Omit<
  ConfigItem,
  "disabledIcon" | "icon" | "selectedIcon"
> & {
  iconName: IconName;
};
...
const getIcon = ({ iconName, titleKey }: IconParameter): ConfigItem => {
  return {
    titleKey,
    icon: (
      <Icon name={iconName} size={ICON_SIZE} color="ds.text.neutral.standard" />
    ),
    selectedIcon: (
      <Icon name={iconName} size={ICON_SIZE} color="ds.text.brand.standard" />
    ),
    disabledIcon: (
      <Icon name={iconName} size={ICON_SIZE} color="ds.text.oddity.disabled" />
    ),
  };
};
...
export function getTabConfig(isBusinessAdmin: boolean): ConfigItem[] {
  const result = [...ICONS];
  if (isBusinessAdmin) {
