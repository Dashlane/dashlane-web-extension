...
const I18N_KEYS = {
  ...
  AUTOFILL_LOGINS_DISABLED:
    "tab/autofill_settings/preferences/autofill_logins_disabled_description",
  AUTOFILL_FORMS: "tab/autofill_settings/preferences/autofill_forms",
  AUTOFILL_FORMS_DISABLED:
    "tab/autofill_settings/preferences/autofill_forms_disabled_description",
  AUTOLOGIN: "tab/autofill_settings/preferences/autologin_label",
  AUTOLOGIN_ENABLED_DESCRIPTION:
    "tab/autofill_settings/preferences/autologin_on_description",
  AUTOLOGIN_DISABLED_DESCRIPTION:
    "tab/autofill_settings/preferences/autologin_off_description",
...
export const AutofillPreferencesContent = memo(
  function AutofillPreferencesContent() {
    const { translate } = useTranslate();
    const {
      disableAnalysis,
      disableAutofillOnForms,
      disableAutofillOnLogins,
      disableAutologin,
      enableAnalysis,
      enableAutofillOnForms,
      enableAutofillOnLogins,
      enableAutologin,
    ...
    useEffect(() => {
      logPageView(
        PageView.SettingsAutofillPreferences,
        BrowseComponent.ExtensionPopup
      );
    }, []);
    const userFrozenStatus = useIsUserFrozen();
    if (userFrozenStatus.isLoading) {
      return null;
    }
    const isPreferenceReadOnly = userFrozenStatus.isUserFrozen;
    const disabledSourceTypes =
      getAutofillSettingsResult.status === DataStatus.Success
        ? getAutofillSettingsResult.data.disabledSourceTypes
        : [];
    const autofillOnFormsEnabled = FORM_DATA_MODELS.some(
      (sourceType) => !disabledSourceTypes.includes(sourceType)
    );
    const autofillOnLoginsEnabled = CREDENTIAL_DATA_MODELS.some(
      (sourceType) => !disabledSourceTypes.includes(sourceType)
    );
    ...
    const toggleAutofillOnLogins = () => {
      if (autofillOnLoginsEnabled) {
        ...
        logAutofillToggledForSourceTypes(
          [...CREDENTIAL_DATA_MODELS],
          "disabled"
        );
      } else {
    ...
    return (
      <div sx={SX_STYLES.MAIN_CONTAINER}>
        ...
        <div sx={SX_STYLES.TOGGLE_OPTION_CONTAINER}>
          <Toggle
            label={translate(I18N_KEYS.AUTOLOGIN)}
            description={
              autologinEnabled
                ? translate(I18N_KEYS.AUTOLOGIN_ENABLED_DESCRIPTION)
                : translate(I18N_KEYS.AUTOLOGIN_DISABLED_DESCRIPTION)
            }
            onChange={toggleAutologin}
            checked={autologinEnabled}
            sx={{ fontSize: "14px" }}
            readOnly={isPreferenceReadOnly}
          />
        </div>
        <div sx={SX_STYLES.TOGGLE_OPTION_CONTAINER}>
          <Toggle
            label={translate(I18N_KEYS.AUTOFILL_LOGINS)}
            description={
              !autofillOnLoginsEnabled
                ? translate(I18N_KEYS.AUTOFILL_LOGINS_DISABLED)
                : null
            }
            onChange={toggleAutofillOnLogins}
            checked={autofillOnLoginsEnabled}
            sx={{ fontSize: "14px" }}
            readOnly={isPreferenceReadOnly}
          />
        </div>
        <div sx={SX_STYLES.TOGGLE_OPTION_CONTAINER}>
          <Toggle
            label={translate(I18N_KEYS.AUTOFILL_FORMS)}
            description={
              !autofillOnFormsEnabled
                ? translate(I18N_KEYS.AUTOFILL_FORMS_DISABLED)
                : null
            }
            onChange={toggleAutofillOnForms}
            checked={autofillOnFormsEnabled}
            sx={{ fontSize: "14px" }}
            readOnly={isPreferenceReadOnly}
          />
        </div>
        <div sx={SX_STYLES.TOGGLE_BY_SOURCETYPE_CONTAINER}>
          <ToggleSourceType
            label={translate(I18N_KEYS.NAMES_TOGGLE)}
            sourceTypes={["KWIdentity"]}
            disabledSourceTypes={disabledSourceTypes}
            disabled={!autofillOnFormsEnabled}
            readOnly={isPreferenceReadOnly}
          />
          <ToggleSourceType
            label={translate(I18N_KEYS.EMAILS_TOGGLE)}
            sourceTypes={["KWEmail"]}
            disabledSourceTypes={disabledSourceTypes}
            disabled={!autofillOnFormsEnabled}
            readOnly={isPreferenceReadOnly}
          />
          <ToggleSourceType
            label={translate(I18N_KEYS.IDS_TOGGLE)}
            sourceTypes={[
              "KWIDCard",
              "KWDriverLicence",
              "KWPassport",
          ...
          />
          <ToggleSourceType
            label={translate(I18N_KEYS.PHONES_TOGGLE)}
            sourceTypes={["KWPhone"]}
            disabledSourceTypes={disabledSourceTypes}
            disabled={!autofillOnFormsEnabled}
            readOnly={isPreferenceReadOnly}
          />
          <ToggleSourceType
            label={translate(I18N_KEYS.ADDRESS_TOGGLE)}
            sourceTypes={["KWAddress"]}
            disabledSourceTypes={disabledSourceTypes}
            disabled={!autofillOnFormsEnabled}
            readOnly={isPreferenceReadOnly}
          />
          <ToggleSourceType
            label={translate(I18N_KEYS.PAYMENTS_TOGGLE)}
            sourceTypes={[
              "KWBankStatement",
