...
const I18N_KEYS = {
  FOLLOW_UP_NOTIFICATION:
    "tab/autofill_settings/preferences/follow_up_notification",
  FOLLOW_UP_NOTIFICATION_DISABLED:
    "tab/autofill_settings/preferences/follow_up_notification_disabled_information",
};
export const WebcardsSettingsContent = ({ readOnly }: Props) => {
  const { translate } = useTranslate();
  const getAutofillSettings = useModuleQuery(
    autofillSettingsApi,
    "getAutofillSettings"
  );
  const { enableFollowUpNotification, disableFollowUpNotification } =
    useModuleCommands(autofillSettingsApi);
  const isFollowUpNotificationEnabled =
    getAutofillSettings.status === DataStatus.Success &&
  ...
  return (
    <React.Fragment>
      <div sx={SX_STYLES.TOGGLE_OPTION_CONTAINER}>
        <Toggle
          label={translate(I18N_KEYS.FOLLOW_UP_NOTIFICATION)}
          description={
            !isFollowUpNotificationEnabled
              ? translate(I18N_KEYS.FOLLOW_UP_NOTIFICATION_DISABLED)
              : null
          }
          onChange={toggleFollowUpNotification}
          checked={isFollowUpNotificationEnabled}
          sx={{ fontSize: "14px" }}
          readOnly={readOnly}
        />
      </div>
