...
export const SettingsView = ({
  ...
  const userCorrections =
    getAutofillSettingsResult.status === DataStatus.Success
      ...
      : [];
  const disablingRules = userCorrections.filter(
    (correction) =>
      correction.domain === tabFullDomain &&
      isDashlaneDisabledPermanently(correction)
  );
  const handleResetDisabledFields = () => {
    const restoredUserAutofillCorrections = userCorrections.filter(
      (correction) =>
        correction.domain !== tabFullDomain ||
        !isDashlaneDisabledPermanently(correction)
    );
  ...
  return (
    <>
      ...
      <div className={styles.container}>
        <AutofillSettingsContent
          website={website}
          changeActiveView={handleClickGoToPreferences}
        />
      </div>

      <AutofillDisabledOnFieldsStatus
        disabledFields={disablingRules.length}
        resetFieldsHandler={handleResetDisabledFields}
      />
    </>
  );
};
