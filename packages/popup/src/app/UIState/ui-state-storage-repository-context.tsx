...
const UIStateStorageRepositoryProvider = ({ children }: Provider) => {
  const [storageRepository, setStorageRepository] =
    useState<StorageRepository | null>(null);
  ...
  useEffect(() => {
    ...
    StorageRepository.createInstance(username)
      .then((storageRepositoryInstanciated) =>
        setStorageRepository(storageRepositoryInstanciated)
      )
      .catch(() => {});
  }, [username]);
  ...
  return (
    <UIStateStorageRepositoryContext.Provider value={contextValue}>
      {children}
    </UIStateStorageRepositoryContext.Provider>
  );
};
const useUIStateStorageRepositoryContext = () =>
  useContext(UIStateStorageRepositoryContext);
