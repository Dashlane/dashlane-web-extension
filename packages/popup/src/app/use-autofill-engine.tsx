...
export const useAlertAutofillEngine = () => {
  ...
  const alertAutofillEngine = async (
    itemId: string,
    copyValue: string,
    itemType: VaultItemType,
    field: Field
  ) => {
    autofillEngine.dataCopiedToClipboardDetected(
      await hashString(computeHash, copyValue),
      itemId,
      {
        type:
          vaultItemTypeTypeToVaultSourceTypeMap[itemType] ??
