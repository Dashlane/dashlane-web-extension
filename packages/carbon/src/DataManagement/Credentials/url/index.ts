...
const getUserSelectedUrl = (cred: Credential): string | null =>
  cred.UseFixedUrl && cred.UserSelectedUrl ? cred.UserSelectedUrl : null;
...
const getUrlWithoutPath = (url: string): string | null => {
  ...
  if (protocol && /__REDACTED__/.test(protocol)) {
    return formatURL({
      protocol,
      hostname,
      port,
...
const getSchemeUrlWithoutPath = compose<string, string, string, string | null>(
  getUrlWithoutPath,
  addFallbackHttpsScheme,
  trim
);
const isNonNilTrustedUrl = (url: string | null): url is string => !isNil(url);
const filterNonNilTrustedUrls = (urls: (string | null)[]): string[] =>
  urls.filter(isNonNilTrustedUrl);
export const getBaseTrustedUrls = compose<
  any,
  TrustedUrl[],
  string[],
  string[],
  ...
  string[]
>(
  filterNonNilTrustedUrls,
  map(getSchemeUrlWithoutPath),
  map(defaultTo("")),
  map<TrustedUrl, string>(prop("TrustedUrl")),
  defaultTo([])
);
export const getFirstTrustedUrl = (credential: Credential): string => {
  const defaultToEmptyString = defaultTo("");
  return defaultToEmptyString(
    head(getBaseTrustedUrls(credential.TrustedUrlGroup))
  );
};
...
export function cleanUrlForPersonalData(
  url: string,
  options?: ClearUrlForPersonalDataOptions
): string {
  if (!url) {
    return "";
  }
...
export function getUpdatedTrustedUrlList(
  currentTrustedUrlList: TrustedUrl[],
  newUrl: string
): TrustedUrl[] {
  ...
  const currentList = (currentTrustedUrlList || []).filter(
    (trustedUrlGroup) =>
      new ParsedURL(trustedUrlGroup.TrustedUrl).getRootDomain() === newDomain
  );
  const isUrlAlreadySaved = currentList.some(
    (trustedUrlGroup) => trustedUrlGroup.TrustedUrl === urlToSave
  );
  if (isUrlAlreadySaved) {
    return currentList;
  }
  const nbrItemsToRemove =
    currentList.length >= TRUSTED_URL_LIST_MAX_SIZE
