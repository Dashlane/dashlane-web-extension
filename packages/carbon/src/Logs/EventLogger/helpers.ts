import { App, Browser, BuildType, Os, OsType, Platform, } from '@dashlane/hermes';
import { ApplicationBuildType, PlatformInfo, PlatformString, } from '@dashlane/communication';
...
export const platformStringToAppPlatform = (platformString: PlatformString): Platform => {
    switch (platformString) {
        ...
        case 'server_tac':
            return Platform.Web;
        default:
            assertUnreachable(platformString, `Hermes Web SDK: Unsupported platform "${platformString}" detected. This platform is not supported by the Hermes Web SDK or Styx API.`);
    }
};
export const applicationBuildTypeToHermesBuildType = (applicationBuildType?: ApplicationBuildType) => {
    switch (applicationBuildType) {
        case ApplicationBuildType.DEV:
            return BuildType.Dev;
        case ApplicationBuildType.QA:
            return BuildType.Qa;
        case ApplicationBuildType.***:
            return BuildType.***;
        default:
            return BuildType. **  **  * ;
    }
};
const buildVersionField = (appVersion: string, manifestVersion: ManifestVersion | null): string => {
...
export const getAppContext = (storeService: StoreService): App => {
    ...
    return {
        ...
        version: buildVersionField(platformInfo.appVersion, config.MANIFEST_VERSION),
...
export const getOsVersion = (platformInfo: PlatformInfo): string => {
    const formattedOSVersion = platformInfo.os === 'Mac OS'
