...
export function _makeRequest<ExpectedData extends Object, RequestData extends Object>(wsName: string, wsVersion: number, wsMethod: string, requestData: RequestData, options?: {
    ...
    return postDataToUrl<ExpectedData>(url, requestData, postDataOptions).then((res) => {
...
function treatData(webService: string, data: any) {
    ...
    const requestFailed = ("code" in data && data["code"] !== 200) ||
    ...
    if (requestFailed) {
        ...
        throw error.addAdditionalInfo({
            ...
            webServiceSubMessage: data.content?.message || data.content?.error || data.content,
            code: data.code,
        });
    }
    return data;
