import { PremiumStatusBillingInfo, PremiumStatusResponse, } from "@dashlane/communication";
...
export interface WSPremium {
    ...
    setAutoRenewal: (params: SetAutoRenewalParams) => Promise<SetAutoRenewalResponse>;
    updateCardToken: (params: UpdateCardTokenParam) => Promise<UpdateCardTokenResponse>;
    ***: (params: GetABTestingVersionDetailsRequest) => Promise<GetABTestingVersionDetailsResult>;
    getSubscriptionCode: (params: SubscriptionCodeRequest) => Promise<SubscriptionCodeResult>;
}
...
function status(params: PremiumStatusParams) {
    return _makeRequest<PremiumStatusResponse, PremiumStatusWSParams>(WSNAME, WSVERSION, "status", {
        login: params.login,
        uki: params.uki,
        ...
        if (response.hasOwnProperty("billingInformation")) {
            response.billingInformation = reformatBillingInformations(response.billingInformation as any as UnformattedPremiumStatusBillingInfo);
        }
        return response;
    });
}
...
function reformatBillingInformations(rawBillingInfo: UnformattedPremiumStatusBillingInfo): PremiumStatusBillingInfo {
    if (!rawBillingInfo) {
        return undefined;
    }
    return {
...
function setAutoRenewal(params: SetAutoRenewalParams) {
    return _makeRequest<SetAutoRenewalResponse, SetAutoRenewalParams>(WSNAME, WSVERSION, "setAutoRenewal", {
        login: params.login,
        uki: params.uki,
...
function updateCardToken(params: UpdateCardTokenParam) {
    return _makeRequest<UpdateCardTokenResponse, UpdateCardTokenParam>(WSNAME, WSVERSION, "updateCardToken", {
        login: params.login,
        uki: params.uki,
...
function getSubscriptionCode({ login, uki }: SubscriptionCodeRequest) {
    return _makeRequest<SubscriptionCodeResult, SubscriptionCodeRequest>(WSNAME, WSVERSION, "getSubscriptionCode", {
        login,
        uki,
    });
}
