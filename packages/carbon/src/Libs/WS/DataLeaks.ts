...
export interface WSDataLeaks {
  ...
  leaks: (
    params: WSDataLeaksBreachesParams
  ) => Promise<WSDataLeaksBreachesResult>;
}
...
function wsDataLeaksStatus(
  params: WSDataLeaksStatusParams
): Promise<WSDataLeaksStatusResult> {
  return _makeDataLeaksRequest<
    WSDataLeaksStatusResult,
    WSDataLeaksStatusParams
  >(WSNAME, WSVERSION, "status", params);
}
function wsDataLeaksBreaches(
  params: WSDataLeaksBreachesParams
): Promise<WSDataLeaksBreachesResult> {
  return _makeDataLeaksRequest<
    WSDataLeaksBreachesResult,
    WSDataLeaksBreachesParams
  >(WSNAME, WSVERSION, "leaks", params);
}
async function _makeDataLeaksRequest<
  RequestResult extends WSDataLeaksBaseResult,
  RequestData extends {}
>(
  wsName: string,
  wsVersion: number,
  ...
  const { data } = await postDataToUrl<RequestResult>(
    url,
    requestData,
    postDataOptions
  );
...
function treatDataLeaksResponse<T extends WSDataLeaksBaseResult>(
