...
import {
  isSaveCredentialDisabledOnDomain,
  signalSaveCredentialDisabled,
} from "DataManagement/Settings/index";
...
export const makeSessionController = (
  ...
  const sessionController: SessionController = {
    ...
    changeDeviceName: (deviceId: string, updateName: string) =>
      changeDeviceName(
        services.storeService,
        services.sessionService,
        deviceId,
    ...
    generateAndSavePassword: (url: string) =>
      generateAndSavePassword(
        services.storeService,
        services.sessionService,
        url
      ),
    generatePassword: (settings: PasswordGenerationSettings) =>
      generatePassword(services.storeService, settings),
    ...
    getPasswordGenerationSettings: () =>
      getPasswordGenerationSettings(services.storeService),
    getCurrentUserInfo: () => getCurrentUserInfo(services.storeService),
    getDevicesList: () =>
      getDevicesList(services.storeService, services.sessionService),
    getInvoices: () =>
      getInvoices(services.storeService, services.sessionService),
    getPersonalSettings: () => getPersonalSettings(services.storeService),
    getUki: () => getUki(services.storeService),
    isSaveCredentialDisabled: (domainInfo: DomainInfo) =>
      isSaveCredentialDisabledOnDomain(services.storeService, domainInfo),
    signalSaveCredentialDisabled: (domainInfo: DomainInfo) =>
      signalSaveCredentialDisabled(
        services.storeService,
        services.sessionService,
        domainInfo
    ...
    saveGeneratedPassword: (password: string, url: string) =>
    ...
    sessionForceSync: (trigger: Trigger = Trigger.Manual) =>
      sessionForceSync(services.sessionService, trigger),
    updateCardToken: (tokenId: string, stripeAccount: string) =>
      updateCardToken(
        services.storeService,
        services.sessionService,
        services.wsService,
    ...
    validateMasterPassword: (masterPassword: string) =>
...
export async function getAndTriggerRefreshAccountInfo(
  storeService: StoreService,
  sessionService: SessionService,
  wsService: WSService
): Promise<AccountInfo> {
  ...
  try {
    const accountInfoAndNodePremiumStatusSpaceData =
      await getRefreshedAccountInfoAndNodePremiumStatusSpaceData(
        storeService,
        sessionService,
        wsService
      );
...
async function getRefreshedAccountInfoAndNodePremiumStatusSpaceData(
  storeService: StoreService,
  sessionService: SessionService,
  wsService: WSService
): Promise<{
  ...
  const premiumStatusPromise = refreshPremiumStatus(
    storeService,
    wsService,
    storeService.getUserLogin(),
    ukiSelector(storeService.getState())
  ).then(() => {
    return {
...
export async function sessionForceSync(
  sessionService: SessionService,
...
async function cancelPremiumSubscription(
  ...
  sessionService: SessionService,
  wsService: WSService
): Promise<CancelPremiumSubscription> {
  if (!storeService.isAuthenticated()) {
    return { success: false };
  }
  try {
    await sessionService.getInstance().payment.cancelPremiumSubscription();
    getRefreshedAccountInfoAndNodePremiumStatusSpaceData(
      storeService,
      sessionService,
      wsService
    ).catch((_) => {});
...
async function updateCardToken(
  storeService: StoreService,
  sessionService: SessionService,
  wsService: WSService,
  tokenId: string,
...
export function resumeSession(
  ...
  triggerABTestingChanged(storeService.getABTesting());
  SessionCommunication.sendLocationInfo(storeService.getPlatform().location);
  if (storeService.isAuthenticated()) {
    return getAndTriggerRefreshAccountInfo(
      storeService,
      sessionService,
      wsService
    ).then(() => {
...
export function generateAndSavePassword(
  ...
  return GeneratedPassword.generateAndSavePassword(
    storeService,
    sessionService,
    url
  );
}
export function getPasswordGenerationSettings(
  storeService: StoreService
): PasswordGenerationSettings {
  return GeneratedPassword.getDefaultPasswordGenerationSettings(
...
export function saveGeneratedPassword(
  ...
  return GeneratedPassword.saveGeneratedPassword(
    storeService,
    sessionService,
    password,
    url
...
export function askWebsiteInfo(
  storeService: StoreService,
  url: string
): WebsiteOptions {
  return getWebsiteInfo(storeService, url);
}
export const AutofillProtectionStatus = Enum(
  "UNPROTECTED",
  "LOCKED",
  "UNLOCKED"
);
...
export function refeshAccountInfo(
  storeService: StoreService,
  sessionService: SessionService,
  wsService: WSService
) {
