...
import type {
  AutofillEngineApplicationDependencies,
...
export const replicateFFInStorage = (
  ...
  try {
    return browserApi.storage.local
      ...
      .then(() => {
        if (!__REDACTED__) {
          messageLogger(
            `Successfully wrote the ${key} feature flip status to local storage`,
            {
              timestamp: Date.now(),
            }
          );
...
export const isFeatureFFInStorage = (
  browserApi: BrowserApi,
  storageKey: string
): Promise<boolean> => {
  return browserApi.storage.local
...
export const checkFeatureInCarbonFFs = async (
  connectors: AutofillEngineConnectors,
  featureFlips: string[]
): Promise<boolean> => {
  try {
    const result = await Promise.all(
      featureFlips.map((feature) => connectors.carbon.getHasFeature(feature))
    );
...
export const isAnalyisEnabledOnUrl = async (
  connectors: AutofillEngineConnectors,
  grapheneClient: ClientsOf<AutofillEngineApplicationDependencies>,
  url: string
) => {
  const isUserGivingConsent =
    (
      await connectors.carbon
        .getGlobalExtensionSettings()
        .catch(() => ({ personalDataConsent: false }))
    ).personalDataConsent !== false;
  const analysisStatusOnUrlQueryResult = await getQueryValue(
    grapheneClient.autofillSettings.queries.getAnalysisStatusOnUrl({
      url,
    })
  );
  return (
...
export const updateDomainAnalysisStatus = async (
  connectors: AutofillEngineConnectors,
  grapheneClient: ClientsOf<AutofillEngineApplicationDependencies>,
  ...
  const isAnalysisEnabled = await isAnalyisEnabledOnUrl(
    connectors,
    grapheneClient,
    url
  );
...
export const updateUserDetailsAndAnalysisStatus = async (
  connectors: AutofillEngineConnectors,
  grapheneClient: ClientsOf<AutofillEngineApplicationDependencies>,
  actions: AutofillEngineActionsWithOptions,
  target: AutofillEngineActionTarget,
