...
export const connectToAutofillEngine = (browserApi: BrowserApi, actionsHandler: Partial<AutofillEngineActions>, clientType: AutofillEngineClientType, messageLogger?: AutofillEngineMessageLogger): AutofillEngineCommands => {
    let port: chrome.runtime.Port | undefined;
    const usePostMessage = clientType === AutofillEngineClientType.Webcards;
    const autofillEngineCommands = makeAutofillEngineCommandSerializer((command) => {
        if (!*****) {
    ...
    const onMessageListener = (message: unknown, sendResponse?: (response?: unknown) => void) => {
        if (isSerializedAutofillEngineAction(message) &&
            ...
            try {
                const handler = actionsHandler[message.action] as (...args: any) => void;
    ...
    const connectOnMessageListener = (message: unknown) => onMessageListener(message);
    const runtimeOnMessageListener = (message: unknown, _sender: chrome.runtime.MessageSender, sendResponse: (response?: unknown) => void) => onMessageListener(message, sendResponse);
