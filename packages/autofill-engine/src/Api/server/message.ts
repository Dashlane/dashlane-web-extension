import {
  ...
  SerializedAutofillEngineAction,
...
export const makeAutofillEngineActionSender =
  (args: {
  ...
  (
    ...
    if (!__REDACTED__) {
      messageLogger(
        `Sending action to tab ${tabId} / ${target}` +
...
export const makeAutofillEngineActionBroadcaster =
  (
    browserApi: BrowserApi,
    messageLogger: (message: string, details: Record<string, unknown>) => void,
    openPorts: PortStore
  ) =>
  async (
    action: SerializedAutofillEngineAction,
