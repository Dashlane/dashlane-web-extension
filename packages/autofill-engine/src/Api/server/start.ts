...
import { contains, request } from "@dashlane/webextensions-apis";
...
import {
  checkFeatureInCarbonFFs,
  isFeatureFFInStorage,
  replicateFFInStorage,
  updateUserDetailsAndAnalysisStatus,
} from "../../implementation/modules/analysis/utils";
...
import {
  makeAutofillEngineActionBroadcaster,
  makeAutofillEngineActionSender,
} from "./message";
import { subscribeToAutofillSettingsChange } from "./queries/subscribe-to-autofill-settings-change";
import { saveAsZip } from "./save-as-zip";
...
const storageKeyForTab = (tabId: number) =>
  `${STORAGE_KEY_FOR_TAB_STATE_PREFIX}${tabId}`;
...
const checkInjectScriptOnStartFF = async (
  ...
  try {
    ...
    const isFeatureInStorage = await isFeatureFFInStorage(
      browserApi,
...
): Promise<AutofillEngineConnectors> {
...
const buildStateForTab = (
  ...
  return {
    global: {
      ...
      set: (newState) =>
        stateStorage.setState(STORAGE_KEY_FOR_GLOBAL_STATE, newState),
    },
    tab: {
      get: async () => {
      ...
      set: (newState) =>
        stateStorage.setState(storageKeyForTab(tabId), newState),
    },
  };
};
const buildStateForPopup = (
  stateStorage: AutofillEngineStateStorage,
  exceptionLogger: AutofillEngineExceptionLogger
): State => {
  return {
    ...
    tab: {
      get: () => {
        throw new Error(
          `Can't access tab state in a command called from Popup`
        );
      },
      set: () => {
        throw new Error(
          `Can't change tab state in a command called from Popup`
        );
      },
    },
  };
...
const onMessageListener = (args: {
  ...
  messageLogger: AutofillEngineMessageLogger;
  message: unknown;
  launchCapture?: PageCaptureBackground["activate"];
  ...
  const exceptionLogger: AutofillEngineExceptionLogger = (
    exception: unknown,
    log?: {
      funcName: string;
      fileName: string;
  ...
  const handleMessage = async () => {
    ...
    if (messageSender.tab?.id) {
      ...
      if (!__REDACTED__) {
        messageLogger(
          `Received command from tab ${tabId} / frame ${messageSender.frameId}`,
          { content: message }
        );
      }
      context = {
      ...
      actions = makeAutofillEngineActionSerializer(
        makeAutofillEngineActionSender({
          browserApi,
          messageLogger,
          tabId,
    ...
    await AutofillEngineCommandHandlers[message.cmd](
      context,
      actions,
      messageSender,
      ...message.parameters
...
const onGetUserLoginStatus = async (
  browserApi: BrowserApi,
  injectedConnectors: AutofillEngineInjectedConnectors,
  openPorts: PortStore,
  messageLogger: AutofillEngineMessageLogger,
  ...
  const connectors = await setupConnectors(injectedConnectors);
  if (loggedIn) {
    subscribeToAutofillSettingsChange(
      browserApi,
      connectors,
      openPorts,
      messageLogger,
      connectors.grapheneClient
    );
  ...
  if (tab?.url) {
    const actionSerializer = makeAutofillEngineActionSerializer(
      makeAutofillEngineActionSender({
        browserApi,
        messageLogger,
        tabId: tab.id ?? 0,
...
const tabsOnHighlightedListener = async (
  browserApi: BrowserApi,
  injectedConnectors: AutofillEngineInjectedConnectors,
  openPorts: PortStore,
  messageLogger: AutofillEngineMessageLogger,
  ...
  const { loggedIn }: GetLoginStatus =
    await connectors.carbon.getUserLoginStatus();
  const injectableHighlightedTabs = (
    await queryInjectableTabs(browserApi)
  ).filter((tab) => tab.url && tab.id && highlightInfo.tabIds.includes(tab.id));
  await Promise.all(
    injectableHighlightedTabs.map(async (tab) => {
...
export const startAutofillEngine = (
  browserApi: BrowserApi,
  injectedConnectors: AutofillEngineInjectedConnectors,
  stateStorage: AutofillEngineStateStorage,
  messageLogger: AutofillEngineMessageLogger
  ...
  try {
    ...
    const pageCaptureBackground = new PageCaptureBackground({});
    const globalState = buildGlobalState(stateStorage, exceptionLogger);
    injectedConnectors.carbon.liveLoginStatus.on(
      async (loginStatus: GetLoginStatus) => {
        try {
          await onGetUserLoginStatus(
            browserApi,
            injectedConnectors,
            openPorts,
            messageLogger,
    ...
    browserApi.contextMenus.onClicked.addListener((info, tab) => {
      ...
      if (!tab?.id) {
        throw new Error(
          "No tab ID while handling a right-click menu interaction"
        );
      }
      if (info.menuItemId === "capture-page") {
        const capturePage = () =>
          pageCaptureBackground.activate(tab, {
            onSuccess: (url, storage) => {
              saveAsZip(url, storage, messageLogger, exceptionLogger);
              messageLogger("Vortex capture success", {
                timestamp: Date.now(),
              });
            },
            onError: (err) => {
              exceptionLogger(err, {
                message: "Exception in pageCaptureBackground.activate",
                fileName: "start.ts",
                funcName: "startAutofillEngine",
              });
            },
      ...
      if (info.menuItemId === "correct-autofill") {
        makeAutofillEngineActionSerializer(
          makeAutofillEngineActionSender({
            browserApi,
            messageLogger,
        ...
        ).computeContextMenuTargetInfo(
          AutofillEngineActionTarget.AllFrames,
          info.frameId ?? BROWSER_MAIN_FRAME_ID
        );
      }
    });
    ...
    browserApi.tabs.onHighlighted.addListener(async (highlightInfo) => {
      try {
        await tabsOnHighlightedListener(
          browserApi,
          injectedConnectors,
          openPorts,
          messageLogger,
      ...
      port.onMessage.addListener((message: unknown) => {
        try {
          return onMessageListener({
            ...
            port,
            launchCapture: (
              ...args: Parameters<PageCaptureBackground["activate"]>
            ) => {
              pageCaptureBackground.activate(...args);
            },
            sendResponse: () => {},
          });
        } catch (exception) {
