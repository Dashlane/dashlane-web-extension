import { connectToAutofillEngine } from "@dashlane/autofill-engine/client";
import { connectToDispatcher } from "@dashlane/autofill-engine/dispatcher";
...
export const getEnvVariables = () => ({
  __REDACTED__: process.env.NODE_ENV === "__REDACTED__",
  isDevBuild: process.env.REACT_APP_DEV_BUILD === "true",
  eventBusTimeout: parseInt(
    process.env.REACT_APP_TS_EVENT_BUS_TIMEOUT ?? "",
    10
  ),
});
export class UtilsInterface {
  ...
  public getFromUrl() {
    ...
    const features = (
      typeof parsedFeatures === "string"
        ? [parsedFeatures]
        : parsedFeatures || []
    ).reduce(
      (prev: object, feature: string) => ({
        ...prev,
        [feature]: true,
      }),
      {}
  ...
  public loadUrl(webcardType: string, mockData?: string, langCode?: string) {
    ...
    window.history.pushState(
      ...
      webcardType,
      `__REDACTED__${window.location?.host}/?${queryString}`
    );
  }
