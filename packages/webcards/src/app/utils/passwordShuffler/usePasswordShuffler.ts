...
export function usePasswordShuffler({
  shouldShuffle,
  passwordLength,
}: UsePasswordShufflerParams): UsePasswordShuffler {
  const intervalRef = React.useRef<number | null>(null);
  const [shuffledPassword, setShuffledPassword] = React.useState(
  ...
  const [isPasswordShuffling, setIsPasswordShuffling] =
    React.useState(shouldShuffle);
  React.useEffect(() => {
    if (shouldShuffle) {
