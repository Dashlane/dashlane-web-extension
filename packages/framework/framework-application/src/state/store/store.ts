...
export class Store<TMemoryState = any, TPersistedState = any> implements IStore<TMemoryState> {
    ...
    private readonly localState$: BehaviorSubject<DataWithTimestamp<TMemoryState>>;
    private readonly mergedState$: Observable<DataWithTimestamp<TMemoryState>>;
    constructor(moduleName: string, private conf: StoreDefinition<TMemoryState>, factory: StoreInfrastructureFactory, userName: string, private allowedToFail: AllowedToFail, private storage?: IStorage<TPersistedState>, private codec?: Codec<TPersistedState, TMemoryState>) {
        this.infrastructure = new ResilienceStore<TMemoryState>(moduleName, conf, factory, userName);
        ...
        this.state$ = this.mergedState$.pipe(filter((x) => !conf.persist || !!x.timestamp), map((x) => x.data ?? (defaultValue as TMemoryState)));
    }
    ...
    public async load(): Promise<void> {
        ...
        const success = await this.allowedToFail.conditionallyAllowToFailOne(async () => {
            const inStorage: TPersistedState = (await storage.read()) ?? conf.storage.Definition.initialValue;
            const value = await codec.decode(inStorage);
            await this.setWithoutPersist(value);
        }, !!conf.isCache);
        if (!success) {
            await this.setWithoutPersist(await codec.decode(conf.storage.Definition.initialValue));
        }
    }
