...
export class ExceptionLoggingInterceptor implements NestInterceptor {
  ...
  public intercept(
    ...
    console.log(
      `[background/framework] Processing incoming request ${request.module}.${request.name}`,
      `(id: ${request.context.get(FrameworkRequestContextValues.UseCaseId)})`
    );
    return next.handle().pipe(
      tap(() => {
        console.log(
          `[background/framework] Done processing incoming request ${request.module}.${request.name}`,
  ...
  private handleException(request: ApplicationRequest) {
    ...
    return (exception: unknown) => {
      console.error(
        `[background/framework] Error while processing incoming ${request.module}.${request.name} ${request.type}`,
        `(id: ${request.context.get(FrameworkRequestContextValues.UseCaseId)})`,
        exception
      );
