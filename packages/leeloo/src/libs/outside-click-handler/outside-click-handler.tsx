...
import ReactOutsideClickHandler, { Props as OutsideClickHandlerProps, } from 'react-outside-click-handler';
...
export const OutsideClickHandler = (props: Props) => {
    ...
    const cb: Props['onOutsideClick'] = React.useCallback((e) => {
        if (!ignoredClassName) {
            onOutsideClick(e);
            return;
        }
        ...
        while (el) {
            if (ignoredClassNames.find((className) => el?.classList?.contains(className))) {
                return;
            }
            el = el.parentElement;
        }
