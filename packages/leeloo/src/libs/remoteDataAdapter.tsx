...
import { AsRemoteData, RemoteData, RemoteDataTypes } from 'libs/remoteData';
...
export interface RemoteDataConfig<InjectedProps, WrapperProps> {
    errorComponent?: JSX.Element | ((props: WrapperProps, error: string) => JSX.Element);
    loadingComponent?: JSX.Element | ((props: WrapperProps) => JSX.Element) | React.FC<WrapperProps>;
    strategies: RemoteDataStrategies<InjectedProps>;
}
export function remoteDataAdapter<InjectedProps extends Record<string, any>, Props extends InjectedProps>(WrappedComponent: React.ComponentType<Props>, strategy: RemoteDataConfig<InjectedProps, Subtract<Props, InjectedProps> & AsRemoteData<InjectedProps>>): React.ComponentType<Subtract<Props, InjectedProps> & AsRemoteData<InjectedProps>> {
    ...
    return class extends React.Component<WrapperProps, WrapperState> {
        ...
        private getRemoteDataInProps = (key: keyof RemoteInjectedProps): RemoteData<any> => this.props[key];
        private canRender = (): boolean => {
            const propKeys = Object.keys(strategy.strategies);
            return propKeys.every((propKey) => this.props[propKey].type === 'Success');
        };
        ...
        private getWrapperProps = (): WrapperProps => {
            ...
            return Object.keys(this.props)
                ...
                .reduce((acc, key) => ({ ...acc, [key]: this.props[key] }), {} as WrapperProps);
        };
