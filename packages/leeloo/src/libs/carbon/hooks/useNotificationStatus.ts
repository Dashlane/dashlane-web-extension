...
export const useNotificationSeen = (
  ...
  const { current: setAsSeen } = React.useRef(() =>
    carbonConnector.markNotificationAsSeen(name)
  );
...
export const useNotificationInteracted = (
  name: NotificationName
): {
  interacted: boolean;
  ...
  const { current: setAsInteracted } = React.useRef(() =>
    carbonConnector.markNotificationAsInteracted(name)
  );
  const statusData = useCarbonEndpoint(
    {
      queryConfig: {
  ...
  const interacted =
    statusData.status === DataStatus.Loading ||
    (statusData.status === DataStatus.Success &&
