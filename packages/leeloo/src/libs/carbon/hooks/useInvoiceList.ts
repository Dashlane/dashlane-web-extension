...
export const useInvoiceList = (
  tokenQueryParam?: GetInvoiceListRequest
): Invoice[] => {
  ...
  const sortDirection =
    tokenQueryParam?.sortDirection === OrderDir.ascending
      ? "ascend"
      : "descend";
...
export const useInvoicesCount = (hasListLoaded?: boolean): number => {
  const queryResult = useCarbonEndpoint(
    { queryConfig: { query: carbonConnector.getInvoicesCount } },
    [hasListLoaded]
  );
...
export const useInvoicesListYears = (): string[] => {
  const queryResult = useCarbonEndpoint(
    { queryConfig: { query: carbonConnector.getInvoiceListYears } },
    []
  );
  if (queryResult.status !== DataStatus.Success || !queryResult?.data) {
    return [];
