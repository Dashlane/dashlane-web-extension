...
export interface PaginationConfig<FirstTokenParams, D> {
  tokenEndpoint: Slot<FirstTokenParams, string> &
    CarbonApiEvents[keyof CarbonApiEvents];
...
export function connectPagination<
  D,
  FirstTokenParams,
  Props extends InjectedProps<D>
>(
  config: PaginationConfig<FirstTokenParams, D>,
  ...
  function prependPageToState(
    state: State,
    page: RemotePage,
    token: string
  ): State {
    const formattedPrepend: [string, RemotePage] = [token, page];
  ...
  return class WrappedComponentClass extends React.Component<
    WrapperProps,
    State
  > {
    ...
    private loadPrevious = (canUnload: boolean) =>
      this.loadPrevious$.next(canUnload);
    ...
    private loadFirstToken = (
      state$: BehaviorSubject<State>,
      tokenParams: FirstTokenParams
    ): void => {
      const { tokenEndpoint } = config;
      const request = tokenEndpoint(tokenParams);
      const obs = from(request).pipe(
        withLatestFrom(state$),
        map(([token, state]) => appendPageToState(state, getNotAsked(), token))
      );
      obs.subscribe((state) => state$.next(state));
    };
    private subscribeToLiveBatch = (
      state$: Subject<State>,
      token: string
    ): void => {
      const { batchLiveEndpoint } = config;
      const obs = new Observable<D[]>((observer) =>
        batchLiveEndpoint.on(token, (batch) => observer.next(batch))
      );
      const sub = obs
        .pipe(
          withLatestFrom(state$),
          map(([batch, state]) => updateBatchInState(state, batch, token))
        )
    ...
    private unloadFirstPage = (state: State): State => {
      ...
      const withNewFirstPage = prependPageToState(
        withoutFirstPage,
        getNotAsked(),
        prevToken
      );
    ...
    private unloadLastPage = (state: State): State => {
      ...
      const lastLoadedPage = withoutLastPage.pages.get(
      ...
      const withNewLastPage = appendPageToState(
        withoutLastPage,
        getNotAsked(),
        nextToken
      );
    ...
    private requestNextPage = this.requestPage(
      "nextToken",
      appendPageToState,
      this.unloadFirstPage
    );
    private requestPreviousPage = this.requestPage(
      "prevToken",
      prependPageToState,
      this.unloadLastPage
    );
    private subscribeToLoadNext = (state$: BehaviorSubject<State>) =>
      this.loadNext$
        .pipe(
          withLatestFrom(state$),
          filter(([, state]) => this.hasNext(state)),
          map(([canUnload, state]) => ({
            canUnload,
            token: this.getLastToken(state),
          })),
          distinctUntilKeyChanged("token"),
          mergeMap(({ canUnload, token }) =>
            this.requestNextPage(state$, token, canUnload)
          )
        )
        .subscribe((updatedState) => state$.next(updatedState));
    private subscribeToLoadPrevious = (state$: BehaviorSubject<State>) =>
      this.loadPrevious$
        .pipe(
          withLatestFrom(state$),
          filter(([, state]) => this.hasPrevious(state)),
          map(([canUnload, state]) => ({
            canUnload,
          ...
          distinctUntilKeyChanged("token"),
          mergeMap(({ canUnload, token }) =>
            this.requestPreviousPage(state$, token, canUnload)
          )
        )
        .subscribe((updatedState) => state$.next(updatedState));
    private getStateObservableFromConfig = (
      tokenParams: FirstTokenParams
    ): Observable<State> => {
    ...
    private getState$ = (): Observable<State> => {
      return this.tokenParams$.pipe(
        switchMap(this.getStateObservableFromConfig)
      );
    };
