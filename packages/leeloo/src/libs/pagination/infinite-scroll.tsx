...
export const InfiniteScroll = ({ customContainer, throttle = DEFAULT_THROTTLE, threshold = DEFAULT_THRESHOLD, hasNext, hasPrevious, loadNext, loadPrevious, isLoading, children, }: InfiniteScrollProps) => {
    ...
    const checkNext = React.useCallback(() => {
        if (!isLoading && hasNext && reachingBottom()) {
            ...
            if (canUnload === undefined) {
                console.error('[InfiniteScroll][checkNext] - Full page calcluation failed, unable to loadNext');
                return;
            }
            loadNext(canUnload);
        }
    ...
    const checkPrevious = React.useCallback(() => {
        if (!isLoading && hasPrevious && reachingTop()) {
            const canUnload = fullPage();
            if (canUnload === undefined) {
                console.error('[InfiniteScroll][checkPrevious] - Full page calcluation failed, unable to loadNext');
                return;
            }
    ...
    const checkWindowScroll = React.useCallback(() => {
        ...
        if (!bottomSentinelTop) {
            console.error('[InfiniteScroll][checkWindowScroll] - BottomSentinelTop calculation failed, returning.');
            return;
        }
    ...
    return (<ul className={styles.wrapper} ref={containerRef}>
