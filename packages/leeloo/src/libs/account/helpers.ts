...
import { PremiumStatus, PremiumStatusCode, PremiumStatusSpace, PreviousPremiumPlan, SpaceStatus, SpaceTier, SpaceTiers, } from '@dashlane/communication';
...
export function isAccountTeamTrialBusiness(premiumStatus: PremiumStatus): boolean {
...
export const isTeamTier = (premiumStatus: PremiumStatus): boolean => (premiumStatus?.spaces ?? []).some((space) => space.tier === 'team');
export const isBusinessTier = (premiumStatus: PremiumStatus): boolean => (premiumStatus?.spaces ?? []).some((space) => space.tier === 'business');
export const isStarterTier = (premiumStatus: PremiumStatus): boolean => (premiumStatus?.spaces ?? []).some((space) => space.tier === 'starter');
...
export function isPaidAccount(premiumStatus: PremiumStatus | PreviousPremiumPlan): boolean {
    return (premiumStatus?.statusCode === PremiumStatusCode.PREMIUM ||
        premiumStatus?.statusCode === PremiumStatusCode.PREMIUM_CANCELLED);
}
export function isEssentialsPlan(premiumStatus: PremiumStatus | PreviousPremiumPlan): boolean {
    const isPaidAccountEssentials = isPaidAccount(premiumStatus) && premiumStatus?.planFeature === 'essentials';
    if (premiumStatus.planName) {
        return (isPaidAccountEssentials && premiumStatus.planName.includes('essentials'));
    }
    else {
        return isPaidAccountEssentials;
    }
}
export function isAdvancedPlan(premiumStatus: PremiumStatus | PreviousPremiumPlan): boolean {
    if (premiumStatus.planName) {
        return (isPaidAccount(premiumStatus) &&
            premiumStatus.planName.includes('advanced'));
    }
    return false;
}
export function isFreeTrial(premiumStatus: PremiumStatus | PreviousPremiumPlan): boolean {
    return premiumStatus?.statusCode === PremiumStatusCode.NEW_USER;
}
export function isPremiumPlan(premiumStatus: PremiumStatus | PreviousPremiumPlan): boolean {
    return isPaidAccount(premiumStatus) && premiumStatus?.planFeature === 'sync';
}
export function isPremiumPlusPlan(premiumStatus: PremiumStatus | PreviousPremiumPlan): boolean {
    return premiumStatus?.planFeature === 'premiumplus';
}
export function getActiveSpace(premiumStatus: PremiumStatus): PremiumStatusSpace | undefined {
    return premiumStatus.spaces?.find((space) => space.status === SpaceStatus.Accepted);
}
...
export function getPlanNameFromTier(tier: SpaceTier): typeof SpaceTiers.Starter | typeof SpaceTiers.Team | typeof SpaceTiers.Business {
...
export function getLocalizedPlanTier({ tier, translate, }: GetLocalizedPlanTierParameters): string {
...
export function getPremiumDaysRemaining(premiumStatus: PremiumStatus): number | undefined {
    return premiumStatus &&
