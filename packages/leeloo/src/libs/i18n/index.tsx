...
import { DEFAULT_LANGUAGE } from "@dashlane/framework-infra/spi";
...
import LinkComponent from "../dashlane-style/link";
...
const make = function (): TranslatorInterface {
  ...
  const shouldWarn = process.env.NODE_ENV !== "__REDACTED__";
  ...
  translate.setLocale = async (locale: string): Promise<void> => {
    ...
    translate.localeMeta = await fetchRelativeLocaleStrings(
      locale in localeMapping ? localeMapping[locale] : locale
    );
  };
  translate.namespace = (prefix: string) => {
    const newF = ((key: string, params?: {}) =>
      translate(prefix + key, params)) as TranslateFunction;
    newF.markup = (key: string, params?: {}, linkParams?) =>
      translate.markup(prefix + key, params, linkParams);
    newF.localeMeta = translate.localeMeta;
    return newF;
  };
  translate.shortDate = (
    timeAsDate: Date = new Date(),
    format:
      | LocaleFormat
      | DateTimeFormat
      | Intl.DateTimeFormatOptions
      | string = LocaleFormat.L
  ): string => {
    ...
    if (format in LocaleFormat) {
      return timeAsDate.toLocaleString(
        translate.localeMeta?.code,
        LOCALE_FORMAT[LocaleFormat[format]]
      );
    }
  ...
  translate.priceSymbol = (currency: string) =>
    ...
    return (
      <ReactMarkdown
        ...
        renderers={{
          ...
          Link: function Link(props: { href: string; children: string }) {
            return (
              <LinkComponent
                {...linkProps}
                aria-label={rawTranslation}
                href={props.href}
                target={linkTarget}
                onClick={() => {
...
export function getTranslateSingletonForTests(): TranslatorInterface {
  if (!testSingleton) {
    testSingleton = make();
    testSingleton.setLocale(DEFAULT_LANGUAGE);
  }
