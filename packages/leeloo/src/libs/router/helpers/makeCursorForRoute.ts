...
const arePathsEqual = (a: PathType, b: PathType): boolean =>
  isEmpty(
    symmetricDifference(Array.isArray(a) ? a : [a], Array.isArray(b) ? b : [b])
  );
export const isStringChildOf = (
  child: string,
  parentPath: PathType
): boolean => {
  const parentPathAsArray = Array.isArray(parentPath)
    ? parentPath
    : [parentPath];
  return parentPathAsArray.some((parentPathAsString) =>
    child.startsWith(parentPathAsString)
  );
};
...
const isAlreadyRegistered = (
  ...
  return list.some(
    (element) =>
      element.reducer === reducer && arePathsEqual(element.path, path)
  );
};
export const registerPathAndReducer = (
  path: PathType | null,
  reducer: LocalReducer<any>
): void => {
  if (!path) {
...
export const makeCursor = (
  ...
  const cursor = reducersStore
    ...
    .reduce(
      (acc: Cursor<any>, cur: LocalReducer<any>) => acc.child(cur),
      rootCursor
    );
