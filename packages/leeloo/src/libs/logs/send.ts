...
const send = function (dispatch: Function, anonymousUserId: string, sessionId: number, logs: State): void {
...
const throttledSend = debounce(throttle(send, THROTTLE_INTERVAL_MS), DEBOUNCE_DELAY_MS);
const logLength = (logs: State) => {
    return Object.keys(logActions)
        .map((key: string) => logs[key] && logs[key].length)
        .reduce((a, b) => a + b);
};
export default function (dispatch: Function, anonymousUserId: string, sessionId: number, logs: State): void {
    if (logLength(logs)) {
        throttledSend(dispatch, anonymousUserId, sessionId, logs);
    }
}
export const sendImmediately = (dispatch: Function, anonymousUserId: string, sessionId: number, logs: State): void => {
    if (logLength(logs)) {
