...
export type PlanType = 'free_trial' | 'offer' | 'stripe' | 'mac' | 'playstore' | 'amazon' | 'ios' | 'ios_renewable' | 'paypal' | 'paypal_renewable' | 'invoice';
...
export interface TeamPlansStatusResult {
    code: number;
    content: {
        team: Team;
    };
...
export interface PremiumCache {
    status: Promise<PremiumStatusResult> | null;
}
export interface TeamPlansCache {
    ...
    status: {
        data?: TeamPlansStatusResult;
        lastUpdateTimestamp: number;
        promise?: Promise<TeamPlansStatusResult>;
    };
}
export interface ConfirmPaymentPendingResult {
    code: number;
    message: string;
}
