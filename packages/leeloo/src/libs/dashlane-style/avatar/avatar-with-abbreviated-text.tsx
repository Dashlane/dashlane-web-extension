...
const createAbbreviatedText = (
  placeholderTextStyle: AbbreviatedTextType,
  name: string
) => {
...
export const AvatarWithAbbreviatedText = ({
  email,
  placeholderTextType,
  avatarSize,
  placeholderFontSize,
  avatarStyleOptions = {},
}: AvatarWithAbbreviatedTextProps) => {
  const { border = "" } = avatarStyleOptions;
  const gravatarImg = isValidEmail(email) ? (
    <GravatarImage email={email} size={avatarSize} />
  ) : null;
  return (
    <div
      role={gravatarImg !== null ? undefined : "img"}
      aria-label={gravatarImg !== null ? undefined : "blank avatar image"}
      style={{
        width: avatarSize,
        height: avatarSize,
      }}
    >
      <div className={styles.placeholderTextAndImgContainer} style={{ border }}>
        ...
        {isUserCurrentlyOnline() && gravatarImg ? (
          <div className={styles.placeholderImg} style={{ border }}>
            {gravatarImg}
          </div>
        ) : null}
      </div>
