...
import DetailSelect, {
  Option as DetailSelectOption,
} from "../../select-field/detail";
import {
  createDaysOptions,
  createMonthsOptions,
  createYearsOptions,
...
const parseDate = (value: string, dateFormat: string): Date =>
  parse(value, dateFormat, new Date("1900-01-01"));
...
const handleFieldChange =
  (value: string, dateFormat: string, onChange: (newValue: string) => void) =>
  (setFieldHandler: (d: Date, newValue: number) => Date) =>
...
const getCurrentOption = (
  ...
  options.find(({ value: optionValue }) => value === optionValue) ?? options[0];
const I18N_KEYS = {
...
export const DateFieldSelect = ({
  ...
  const monthsOptions: Option[] = createMonthsOptions((timestamp) =>
    monthLabelFormatter(timestamp)
  );
  const handleChangeDateGenerator = handleFieldChange(
    value,
    dateFormat,
    onChange
  );
  ...
  const daysOptions: Option[] = createDaysOptions(
    getDaysInMonth(value === "" ? 31 : parsedDate)
  );
  if (allowEmpty) {
  ...
  const currentYearOption: DetailSelectOption = getCurrentOption(
    yearsOptions,
    parsedDate.getFullYear()
  );
  const currentMonthOption: DetailSelectOption = getCurrentOption(
    monthsOptions,
    parsedDate.getMonth()
  );
  const currentDayOption: DetailSelectOption = getCurrentOption(
    daysOptions,
    parsedDate.getDate()
  );
            ...
            if (!hideDay) {
              return (
                <DetailSelect
                  key={`day-${value}`}
                  label={aIndex ? "" : label}
                  ariaLabel={`${label} :${translate(I18N_KEYS.DAY)}`}
                  placeholder={currentDayOption.label}
                  dataName="dateDay"
                  options={daysOptions}
                  defaultOption={currentDayOption}
          ...
          case "month":
            return (
              <DetailSelect
                key={`month-${value}`}
                label={aIndex ? "" : label}
                ariaLabel={`${label} :${translate(I18N_KEYS.MONTH)}`}
                placeholder={currentMonthOption.label}
                dataName="dateMonth"
                options={monthsOptions}
                defaultOption={currentMonthOption}
          ...
          case "year":
            return (
              <DetailSelect
                key={`year-${value}`}
                label={aIndex ? "" : label}
                ariaLabel={`${label} :${translate(I18N_KEYS.YEAR)}`}
                placeholder={currentYearOption.label}
                dataName="dateYear"
                options={yearsOptions}
                defaultOption={currentYearOption}
