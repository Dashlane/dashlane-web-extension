...
import { Logo as Lockup } from "@dashlane/design-system";
import { Logo } from "@dashlane/ui-components";
  ...
  const getTab = (tab: Tab) => {
    ...
    return (
      <div className={styles.tabContainer} key={tab.id}>
        {tabItem}
        {tab.items ? (
          <div className={styles.subTabs}>{tab.items.map(getTabItem)}</div>
        ) : null}
      </div>
    );
  };
  const selectedLogo = includeLockup ? (
    <Lockup
      height={lockupSize ?? 64}
      name="DashlaneLockup"
      title={translate(I18N_KEYS.DASHLANE_LOGO_TITLE)}
      color="ds.text.inverse.catchy"
    />
  ) : (
    <Logo color={colorVars["--white"]} />
  );
  const LogoComponent = logoComponent ?? <Link to="/" target="self" />;
  return (
    <header
      className={classnames(styles.standardHeader, classes?.standardHeader)}
    >
