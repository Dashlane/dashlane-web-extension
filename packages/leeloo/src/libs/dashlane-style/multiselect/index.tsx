...
export const Multiselect = (props: MultiselectProps) => {
  ...
  const handleClickOnMultiselect = (
  ...
  const updateSuggestionDropdownPosition = () => {
    ...
    const newDropdownPosition = {
      top:
        inputField.current.getOffsetTop() -
        mainContainer.current.scrollTop +
        yOffset,
      left:
        inputField.current.getOffsetLeft() -
        mainContainer.current.scrollLeft +
        xOffset,
    };
    if (
      newDropdownPosition.top !== suggestionDropdownPosition.top ||
  ...
  const getCandidates = () => {
    return props.dataSource.filter(
      (item) => !selectedItems.includes(item.text)
    );
  };
  ...
  const getAvailableCandidates = () => {
    ...
    return [currentUserInput].concat(
      getCandidates()
  ...
  const handleKeyDownOnInputContainer = (
    ...
    if (event.key === "ArrowDown" || event.key === "ArrowUp") {
      ...
      let newCandidateItemIndex =
        (candidateItemIndex + increment) % nbItemsInDropdown;
  ...
  return (
    <div style={{ position: "relative" }}>
      <div
        ...
        ref={mainContainer}
        onMouseDown={handleClickOnMultiselect}
      >
        ...
        <div className={styles.inputContainer}>
          <InputField
            placeholder={selectedItems.length ? undefined : props.placeholder}
            ref={inputField}
            key={inputKey}
            className={classnames({
              [styles.greyInputContainer]: !userInputIsEmpty(),
            })}
            onBlur={handleBlurOnInputContainer}
            onChange={handleInputFieldChange}
            onFocus={handleFocusOnInputContainer}
            onKeyDown={handleKeyDownOnInputContainer}
            onSubmit={handleInputFieldSubmit}
          />
        </div>
      </div>
      <div ref={dropdownContainer}>
        {shouldShowSuggestionDropdown() && (
          <div
            ...
            style={suggestionDropdownPosition}
          >
            {getAvailableCandidates().map((item, index) => (
              <div
                key={item.text}
                onMouseDown={(event) => handleSelectionClick(event, item.text)}
                className={classnames(styles.candidateItem, {
                  [styles.highlightedItem]:
                    index === candidateItemIndex && !hovered,
                })}
              >
                {item.icon}
                {item.text}
