...
import { EvaluatePasswordStrength } from 'libs/validators';
...
export const PasswordStrength = (props: Props) => {
    ...
    const getCheckedStateFromRule = (rule: boolean): string => rule ? styles.checked : styles.unchecked;
    return (<div className={props.className}>
      <h3 className={styles.headline}>{translate(I18N_KEYS.HEADLINE)}</h3>
      <ul className={styles.list}>
        <li className={getCheckedStateFromRule(props.firstPasswordStrength.hasLowerCase)}>
          {translate(I18N_KEYS.LOWER_CASE)}
        </li>
        <li className={getCheckedStateFromRule(props.firstPasswordStrength.hasUpperCase)}>
          {translate(I18N_KEYS.UPPER_CASE)}
        </li>
        <li className={getCheckedStateFromRule(props.firstPasswordStrength.hasNumeric)}>
          {translate(I18N_KEYS.NUMERIC)}
        </li>
        <li className={getCheckedStateFromRule(props.firstPasswordStrength.meetsLength)}>
