...
export const Dropdown = ({ children, renderRoot, position, alignment, withBackdrop, onToggle, anchor, }: Props) => {
    ...
    const handleClick = (e: MouseEvent) => {
        ...
        const forceClose = parents.some((p) => p?.getAttribute('data-close-dropdown'));
        if (forceClose) {
            closeDropdown();
        }
    };
    ...
    const dropdownWrapperClassName = classnames(styles.dropdownWrapper, styles[alignmentClass], styles[positionClass]);
    return (<div onKeyDown={handleKeyDown}>
      <CSSTransition key="CSSTransition" classNames={{ ...backdropAnimation }} timeout={150} in={withBackdrop && isOpen} unmountOnExit>
        <div className={styles.backdrop}/>
      </CSSTransition>
      <div key="dropdown" className={styles.dropdown} ref={dropdownRef}>
        {renderRoot(toggleOpen)}
        {anchor}
        <CSSTransition classNames={{ ...styles }} timeout={150} in={isOpen} unmountOnExit>
