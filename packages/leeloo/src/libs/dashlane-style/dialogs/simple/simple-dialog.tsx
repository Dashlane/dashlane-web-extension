...
import { Dialog, DialogBody } from "@dashlane/ui-components";
...
export const SimpleDialog = (props: PropsWithChildren<Props>) => {
  ...
  return (
    <Dialog
      modalContentClassName={contentClassName}
      ...
      disableOutsideClickClose={props.disableOutsideClickClose}
    >
      {typeof props.title === "string" ? (
        <SimpleDialogHeader>{props.title}</SimpleDialogHeader>
      ) : null}
      {props.title && typeof props.title !== "string" ? props.title : null}
      {props.children ? (
        props.replaceDialogBody ? (
          props.children
        ) : (
