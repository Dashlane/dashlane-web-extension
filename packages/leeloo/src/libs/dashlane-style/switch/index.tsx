...
import { Flex, Toggle } from "@dashlane/design-system";
...
export default class SwitchWithFeedback extends Component<Props, State> {
  ...
  private handleSwitchToggled = async () => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : this.props.genericErrorMessage;
      this.setState({
        errorMessage,
        showSaveFieldFeedbackMessage: false,
      });
  ...
  FEEDBACK_MESSAGE_TYPE = {
    SAVING: {
      ...
      messageIcon: (
        <LoadingIcon
          color={accessibleValidatorGreen}
          size={20}
          sx={{ mr: "2px" }}
        />
      ),
    ...
    SAVED: {
      textColor: accessibleValidatorGreen,
      messageIcon: (
        <CheckCircleIcon size={22} color={accessibleValidatorGreen} />
      ),
    },
    ERROR: {
  ...
  public render() {
    ...
    return (
      <div className={styles.switchContainer} {...nativeProps}>
        {this.props.isLoading && !this.state.isSavingField ? (
          <LoadingIcon size={25} color="ds.border.brand.standard.idle" />
        ) : (
          <Toggle
        ...
        {feedbackMessage ? (
          <Flex
            ...
            sx={{ height: "22px" }}
            className={
              this.props.isDisabled ? styles.errorFeedbackContainer : undefined
            }
          >
            {feedbackMessageType.messageIcon}
            <Paragraph
              size="x-small"
              color={feedbackMessageType.textColor}
              sx={{ ml: "5px", flex: 1, lineHeight: 0 }}
            >
              {feedbackMessage}
            </Paragraph>
          </Flex>
