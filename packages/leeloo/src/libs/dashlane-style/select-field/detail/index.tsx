...
export default class DetailSelect extends React.Component<DetailSelectProps, DetailSelectState> {
    constructor(props: DetailSelectProps) {
        super(props);
        let optionSelected: Option | null = null;
        if (this.props.options.length) {
            optionSelected = getOptionByValue(this.props.options, this.props.defaultOption === null
    ...
    public render() {
        ...
        const selectContainerClass = classnames(styles.selectContainer, sharedStyles[marginSide], {
        ...
        {this.props.label && (<label className={sharedStyles.label} color="ds.text.neutral.catchy" htmlFor={this.state.fieldHtmlId} title={this.props.label}>
            {this.props.label}
          </label>)}
        <div className={selectContainerClass}>
          <div className={selectorClass}>
            {this.state.optionSelected && this.state.optionSelected.icon && (<span className={styles.icon}>
                {this.state.optionSelected.icon}
              </span>)}
            <span className={styles.text} sx={{ color: 'ds.text.neutral.catchy' }}>
              {selectedOptionText}
            </span>
          </div>
          <select key={this.state.fieldHtmlId} className={sharedStyles.select} ref={this.select} id={this.state.fieldHtmlId} value={valueSelected} name={this.props.name} disabled={this.props.disabled} data-name={this.props.dataName} onKeyDown={this.handleKeyDown} onChange={this.handleChange} onFocus={this.handleFocus} onBlur={this.handleBlur} aria-label={this.props.ariaLabel ?? this.props.label}>
            {this.props.options.map((option) => (<option key={option.value} disabled={option.disabled} value={String(option.value)}>
                {option.label}
              </option>))}
          </select>
        </div>
