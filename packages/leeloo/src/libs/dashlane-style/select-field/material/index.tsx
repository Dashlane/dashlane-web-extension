...
export const OptionField = (props: SelectOptionProps) => {
  ...
  return (
    <option className={styles.option} {...attributes}>
      {props.primaryText}
    </option>
  );
};
...
export class SelectField extends React.Component<SelectFieldProps, State> {
  ...
  public render() {
    ...
    const label = this.props.floatingLabelText && (
      <label className={styles.label} style={this.props.floatingLabelStyle}>
    ...
    const hint = ((!this.props.value && this.props.hintText) ||
      hasSuccess ||
      hasError) && (
      <span className={styles.hint} style={this.props.hintStyle}>
