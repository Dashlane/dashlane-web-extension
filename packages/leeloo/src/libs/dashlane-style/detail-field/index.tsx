...
export default class EditableField extends React.Component<Props, State> {
    ...
    public componentDidUpdate() {
        if (this.state.selectionRangeStart !== null &&
    ...
    private getTextarea() {
        return (<Textarea inputRef={(el: HTMLTextAreaElement) => {
                this.field = el;
            }} id={this.state.fieldHtmlId} disabled={this.props.disabled} value={this.props.value ?? ''} name={this.props.name} data-name={this.props.dataName} sx={{
                borderColor: this.props.error && 'ds.border.danger.standard.idle',
            }} className={styles.textarea} placeholder={this.props.placeholder} onBlur={this.props.handleFieldBlur} onKeyDown={this.props.onFieldKeyDown} onChange={this.handleChange}/>);
    }
    private getInput() {
        ...
        return this.props.mask ? (<ReactInputMask {...maskManagedProps} {...this.props.maskProps} mask={this.props.mask} onCopy={() => overrideDefaultCopy(maskManagedProps.value)} onCut={() => overrideDefaultCopy(maskManagedProps.value)}>
        {(otherInputProps: MaskManagedProps | {}) => (<input {...inputProps} {...otherInputProps}/>)}
      </ReactInputMask>) : (<div className={styles.inputRow}>
        <input {...inputProps} {...maskManagedProps}/>
        <div hidden={!this.props.error || typeof this.props.error !== 'string'} sx={{
                color: 'ds.text.danger.quiet',
    ...
    public render() {
        const tooltipText = this.props.type !== 'password'
            ? this.props.value || this.props.placeholder
            : '';
        return (<div className={classnames(styles.container, this.props.multiLine ? styles.containerMultiLine : null)}>
        {this.props.label && (<label className={styles.label} sx={{ color: 'ds.text.neutral.catchy' }} htmlFor={this.state.fieldHtmlId}>
            <span className={styles.text} title={this.props.label}>
              {this.props.label}
            </span>
          </label>)}
        <IntelligentTooltipOnOverflow tooltipText={tooltipText} className={classnames(styles.value, {
