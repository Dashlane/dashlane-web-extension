...
import { CallToAction, FormName, PageView, PossibleFormAnswers as ReasonToExtendTrialAnswer, UserCallToActionEvent, UserSubmitInProductFormAnswerEvent, } from '@dashlane/hermes';
...
const I18N_KEYS = {
    ...
    OPTION_TIME_FOR_DECISION_MAKER: 'team_trial_extension_survey_option_time_for_decision_maker',
...
export const ExtendTrialSurvey = ({ isOpen, handleClose }: Props) => {
    ...
    const [reasons, setReasons] = useState<Partial<Record<ReasonToExtendTrialAnswer, boolean>>>({
    ...
    const handleSendSurvey = () => {
        ...
        logEvent(new UserSubmitInProductFormAnswerEvent({
            ...
            chosenAnswerList: Object.keys(reasons).filter((item) => reasons[item]) as ReasonToExtendTrialAnswer[],
        }));
        handleClose();
    };
    ...
    return (<Dialog isOpen={isOpen} isMandatory onClose={handleCloseSurvey} title={translate(I18N_KEYS.SURVEY_TITLE)} actions={{
            primary: {
      ...
      <div sx={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <Checkbox label={translate(I18N_KEYS.OPTION_TIME_FOR_TEAM)} checked={reasons.need_more_time_team_usage} onChange={() => handleChangeCheckbox(ReasonToExtendTrialAnswer.NeedMoreTimeTeamUsage)}/>
        <Checkbox label={translate(I18N_KEYS.OPTION_TIME_FOR_DECISION_MAKER)} checked={reasons.need_more_time_convince} onChange={() => handleChangeCheckbox(ReasonToExtendTrialAnswer.NeedMoreTimeConvince)}/>
        <Checkbox label={translate(I18N_KEYS.OPTION_UNAWARE)} checked={reasons.not_aware_trial_ending} onChange={() => handleChangeCheckbox(ReasonToExtendTrialAnswer.NotAwareTrialEnding)}/>
        <Checkbox label={translate(I18N_KEYS.OPTION_HELP_SSO)} checked={reasons.need_help_sso} onChange={() => handleChangeCheckbox(ReasonToExtendTrialAnswer.NeedHelpSso)}/>
        <Checkbox label={translate(I18N_KEYS.OPTION_HELP_SCIM)} checked={reasons.need_help_scim} onChange={() => handleChangeCheckbox(ReasonToExtendTrialAnswer.NeedHelpScim)}/>
        <Checkbox label={translate(I18N_KEYS.OPTION_SALES_REP)} checked={reasons.need_sales_rep} onChange={() => handleChangeCheckbox(ReasonToExtendTrialAnswer.NeedSalesRep)}/>
        <Checkbox sx={{ marginBottom: '8px' }} label={translate(I18N_KEYS.OPTION_OTHER)} checked={reasons.other} onChange={() => handleChangeCheckbox(ReasonToExtendTrialAnswer.Other)}/>
      </div>
    </Dialog>);
};
