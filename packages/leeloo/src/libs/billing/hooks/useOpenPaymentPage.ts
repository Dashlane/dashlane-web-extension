...
import {
  ...
  DASHLANE_UPDATE_PAYMENT_DETAILS_DOMAIN,
} from "../../../app/routes/constants";
import { openDashlaneUrl } from "../../external-urls";
import { useRequestPaymentUpdateAuthenticationToken } from "../../carbon/hooks/useRequestPaymentUpdateAuthenticationToken";
...
export function useOpenPaymentPage(
  b2c: boolean,
  ...
  const paymentUpdateAuthenticationToken =
    useRequestPaymentUpdateAuthenticationToken();
  ...
  React.useEffect(() => {
    if (APP_PACKAGED_IN_EXTENSION) {
      ...
      paymentUpdateAuthenticationToken.status === DataStatus.Success &&
      ...
      const paymentData = {
        ...
        creationDateUnix:
          paymentUpdateAuthenticationToken.data.creationDateUnix,
        expirationDateUnix:
          paymentUpdateAuthenticationToken.data.expirationDateUnix,
        secretKey: paymentUpdateAuthenticationToken.data.secretKey,
        livemode: paymentUpdateAuthenticationToken.data.livemode,
        b2bPaymentTokens: {
          customerId:
            paymentUpdateAuthenticationToken.data.b2bPaymentTokens?.customerId,
          tokenId:
            paymentUpdateAuthenticationToken.data.b2bPaymentTokens?.tokenId,
        },
        b2cPaymentTokens: {
          customerId:
            paymentUpdateAuthenticationToken.data.b2cPaymentTokens?.customerId,
          tokenId:
            paymentUpdateAuthenticationToken.data.b2cPaymentTokens?.tokenId,
        },
      };
      windowRef.current = window.open(
        `${getPaymentsUrl(
          b2c,
          mode
        )}&utm_medium=subscriptionManagement&utm_source=webapp&utm_term=goToPayments`
      );
