...
export const getNamedRoutes = (
  routingScheme: RoutingSchemeOptions
): NamedRoutes => {
  const hostedUrlSegment =
    routingScheme === RoutingSchemeOptions.CLIENT
      ? TAC_URL_SEGMENT
      : CLIENT_TAC_URL_SEGMENT;
  const tacUrlSegment = APP_PACKAGED_IN_EXTENSION
    ? TEAM_CONSOLE_URL_SEGMENT
    : hostedUrlSegment;
  const teamRoutesBasePath =
    routingScheme === RoutingSchemeOptions.TEAM_ADMIN_CONSOLE
      ? ""
      : tacUrlSegment;
  const adminAssistedRecoveryRoutesBasePath =
    ADMIN_ASSISTED_RECOVERY_URL_SEGMENT;
  const clientRoutesBasePath =
    routingScheme === RoutingSchemeOptions.CLIENT ? "" : CLIENT_URL_SEGMENT;
  return {
    ...
    teamAccountChangePlanRoutePath: join(
      teamRoutesBasePath,
      "account/changeplan"
    ),
    teamAccountCheckoutRoutePath: join(teamRoutesBasePath, "account/checkout"),
    ...
    teamGroupRoutePath: (groupUuid) =>
      join(teamRoutesBasePath, `groups/${groupUuid}`),
    teamIntegrationsRoutePath: join(teamRoutesBasePath, "integrations"),
    teamSettingsSso: join(teamRoutesBasePath, "integrations/sso"),
    teamSettingsSsoConfidential: join(
      teamRoutesBasePath,
      "integrations/sso/confidential-sso"
    ),
    teamSettingsSsoSelfHosted: join(
      teamRoutesBasePath,
      "integrations/sso/self-hosted-sso"
    ),
    loggedOutMonitoring: join(teamRoutesBasePath, "logged-out-monitoring"),
    familyDashboard: join(clientRoutesBasePath, "family-dashboard"),
    clientRoutesBasePath: clientRoutesBasePath,
    userDeviceRegistration: join(
      adminAssistedRecoveryRoutesBasePath,
      "device-registration"
    ),
    userCreateMasterPassword: join(
      adminAssistedRecoveryRoutesBasePath,
      "create-master-password"
    ),
    userSendRequest: join(adminAssistedRecoveryRoutesBasePath, "send-request"),
    userPendingRequest: join(
      adminAssistedRecoveryRoutesBasePath,
      "pending-request"
    ),
    userAddApplicationCredential: join(
      clientRoutesBasePath,
      "credentials/add/application"
    ),
    userAddDatabaseCredential: join(
      clientRoutesBasePath,
      "credentials/add/database"
    ),
    changeLoginEmail: join(clientRoutesBasePath, "change-login-email"),
    ...
    userAddWebsiteCredentialWithPrefilledParameters: (
      ...
      const pathname = join(
        clientRoutesBasePath,
        "password-history/credentials/add"
      );
      return {
        pathname,
        state: routeState,
    ...
    userCredential: (credentialUuid) =>
      join(
        clientRoutesBasePath,
        `credentials/${getUrlCompatibleUuid(credentialUuid)}`
      ),
    userPasswordHistory: join(clientRoutesBasePath, "password-history"),
    userPasswordHistoryFilteredByCredential: (credentialUuid) =>
      join(
        clientRoutesBasePath,
        `password-history/filter/${getUrlCompatibleUuid(credentialUuid)}`
      ),
    ...
    userProfileAdmin: join(clientRoutesBasePath, "profile-admin"),
    ...
    userSecureNote: (secureNoteUuid) =>
      join(
        clientRoutesBasePath,
        `secure-notes/${getUrlCompatibleUuid(secureNoteUuid)}`
      ),
    userSecret: (secretUuid) =>
      join(clientRoutesBasePath, `secrets/${getUrlCompatibleUuid(secretUuid)}`),
    userAddPersonalInfoCompany: join(
      clientRoutesBasePath,
      "personal-info/companies/add"
    ),
    userAddPersonalInfoEmail: join(
      clientRoutesBasePath,
      "personal-info/emails/add"
    ),
    userAddPersonalInfoAddress: join(
      clientRoutesBasePath,
      "personal-info/addresses/add"
    ),
    userAddPersonalInfoIdentity: join(
      clientRoutesBasePath,
      "personal-info/identities/add"
    ),
    userAddPersonalInfoPhone: join(
      clientRoutesBasePath,
      "personal-info/phones/add"
    ),
    userAddPersonalInfoWebsite: join(
      clientRoutesBasePath,
      "personal-info/websites/add"
    ),
    userPersonalInfo: join(clientRoutesBasePath, "personal-info"),
    userPersonalInfoAddress: (uuid) =>
      join(
        clientRoutesBasePath,
        `personal-info/addresses/${getUrlCompatibleUuid(uuid)}`
      ),
    userPersonalInfoCompany: (uuid) =>
      join(
        clientRoutesBasePath,
        `personal-info/companies/${getUrlCompatibleUuid(uuid)}`
      ),
    userPersonalInfoEmail: (uuid) =>
      join(
        clientRoutesBasePath,
        `personal-info/emails/${getUrlCompatibleUuid(uuid)}`
      ),
    userPersonalInfoIdentity: (uuid) =>
      join(
        clientRoutesBasePath,
        `personal-info/identities/${getUrlCompatibleUuid(uuid)}`
      ),
    userPersonalInfoPhone: (uuid) =>
      join(
        clientRoutesBasePath,
        `personal-info/phones/${getUrlCompatibleUuid(uuid)}`
      ),
    userPersonalInfoWebsite: (uuid) =>
      join(
        clientRoutesBasePath,
        `personal-info/websites/${getUrlCompatibleUuid(uuid)}`
      ),
    userPayments: join(clientRoutesBasePath, "payments"),
    userAddPaymentCard: join(clientRoutesBasePath, "payments/card/add"),
    userPaymentCard: (paymentUuid) =>
      join(
        clientRoutesBasePath,
        `payments/card/${getUrlCompatibleUuid(paymentUuid)}`
      ),
    userAddBankAccount: join(clientRoutesBasePath, "payments/bank-account/add"),
    userBankAccount: (paymentUuid) =>
      join(
        clientRoutesBasePath,
        `payments/bank-account/${getUrlCompatibleUuid(paymentUuid)}`
      ),
    userIdsDocuments: join(clientRoutesBasePath, "ids"),
    userAddIdDocument: (type: IdVaultItemType) =>
      join(
        clientRoutesBasePath,
        `ids${getVaultItemRouteTypeDictionary[type]}/add`
      ),
    userEditIdDocument: (type: IdVaultItemType, id: string) =>
      join(
        clientRoutesBasePath,
        `ids${getVaultItemRouteTypeDictionary[type]}/${getUrlCompatibleUuid(
          id
        )}`
      ),
    userEmergency: join(clientRoutesBasePath, "emergency"),
    userSettings: join(clientRoutesBasePath, "settings"),
    userSettingsTab: (settingTab) =>
      join(clientRoutesBasePath, `settings/${settingTab}`),
    ...
    userGoAdvanced: (subCode?: string, pricing?: PricingMode) =>
      getUrlGivenDefinedQuery(GET_ADVANCED_URL, { subCode, pricing }),
    userGoEssentials: (subCode?: string, pricing?: PricingMode) =>
      getUrlGivenDefinedQuery(GET_ESSENTIALS_URL, { subCode, pricing }),
    userGoFamily: (subCode?: string, pricing?: PricingMode) =>
      getUrlGivenDefinedQuery(GET_PREMIUM_FAMILY_URL, { subCode, pricing }),
    userGoPremium: (subCode?: string, pricing?: PricingMode) =>
      getUrlGivenDefinedQuery(GET_PREMIUM_URL, { subCode, pricing }),
    userSharingGroupInfo: (groupId: string) =>
      join(
        clientRoutesBasePath,
        `sharing-center/group/${getUrlCompatibleUuid(groupId)}`
      ),
    userSharingUserInfo: (userId: string) =>
      join(clientRoutesBasePath, `sharing-center/user/${userId.toLowerCase()}`),
    ...
    twoFactorAuthenticationEnforce: join(
      clientRoutesBasePath,
      "two-factor-authentication/enforce"
    ),
    dashlaneLabs: join(clientRoutesBasePath, "dashlane-labs"),
    recover2faCodes: (login?: string) =>
      login ? `recover-2fa-codes?account=${login}` : "recover-2fa-codes",
    reactivatePinAndArk: join(
      clientRoutesBasePath,
      "account-settings?view=recovery-key-reactivate"
    ),
    changeLoginEmailAccountSettings: join(
      clientRoutesBasePath,
      "account-settings?view=change-login-email"
    ),
  };
};
