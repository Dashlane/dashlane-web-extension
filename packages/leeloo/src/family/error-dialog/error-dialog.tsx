...
export const ErrorDialog = ({ errorCode, resetErrorCode, }: ErrorDialogProps) => {
    const { translate } = useTranslate();
    const isUserAlreadyInThisFamily = errorCode === 'ALREADY_JOINED_THIS_FAMILY';
    const isUserAlreadyPremiumPlus = errorCode ===
        'USER_MUST_CANCEL_PREMIUM_PLUS_RENEWAL_TO_DOWNGRADE_TO_FAMILY';
    const isUserAlreadyInFamilyPlan = alreadyInPlanErrors.has(errorCode);
    if (errorCode !== 'ALREADY_JOINED_THIS_FAMILY' &&
    ...
    const renderTitle = () => isUserAlreadyInThisFamily
        ? translate('family_invitee_page_failed_heading_already_in_plan')
        : translate('family_invitee_page_failed_heading_in_another_plan');
    const renderSubtitle = () => isUserAlreadyInThisFamily
        ? translate('family_invitee_page_failed_heading_already_in_plan_description')
        : isUserAlreadyPremiumPlus
            ? translate.markup('family_invitee_page_failed_heading_already_premium_plus_description_markup')
            : translate('family_invitee_page_failed_heading_in_another_plan_description');
    return (<SimpleDialog isOpen showCloseIcon onRequestClose={resetErrorCode} footer={<DialogFooter primaryButtonTitle={translate('family_invitee_page_failed_button_title_another_plan')} primaryButtonOnClick={goToAccountSummary} intent="primary"/>} title={renderTitle()}>
