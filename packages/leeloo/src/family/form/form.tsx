...
const SetLoginForm = ({
  handleJoinFamily,
  displayLoginErrorCode,
}: SetLoginFormProps) => {
  const { translate } = useTranslate();
  const [login, setLogin] = React.useState("");
  const [showLoginError, setShowLoginError] = React.useState(false);
  const handleLoginChanged = React.useCallback(
    ...
    >
      <div className={styles.loginTextFieldWrapper}>
        <TextInput
          type="email"
          value={login}
          placeholder={translate("family_invitee_page_form_login_placeholder")}
          fullWidth
          feedbackText={
            showLoginError || displayLoginErrorCode
              ? translate("family_invitee_page_form_invalid_email")
              : undefined
          }
          feedbackTextId="badLoginInputError"
          feedbackType={
            showLoginError || displayLoginErrorCode ? "error" : undefined
          }
      ...
      <div className={sharedStyles.joinFamilyRow}>
        <div className={sharedStyles.joinFamilyColumn}>
          ...
          <SetLoginForm
            ...
            displayLoginErrorCode={displayLoginErrorCode}
          />
        ...
        {errorCode ? (
          <ErrorDialog
            errorCode={errorCode}
            resetErrorCode={handleResetErrorCode}
          />
        ) : null}
      </div>
    </JoinFamily>
