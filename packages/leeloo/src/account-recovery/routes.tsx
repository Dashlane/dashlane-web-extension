...
import { SessionStates } from "@dashlane/session-contracts";
...
export default function routes({ path }: RoutesProps): JSX.Element {
  ...
  const getAccountRecoveryKeyComponent = () => {
    return authenticationFlowStatus.data?.step === "MasterPasswordStep" ||
      ...
      sessionState === SessionStates.Open
      ? AccountRecoveryKeyContainer
      : undefined;
  };
  return (
    <WrappingRoute
      path={[`${path}/admin-assisted-recovery`, `${path}/account-recovery-key`]}
    >
      <AdminAssistedRecoveryRoutes path={`${path}/admin-assisted-recovery`} />
      <CustomRoute
        exact
        path={`${path}/account-recovery-key`}
        component={getAccountRecoveryKeyComponent()}
      />
    </WrappingRoute>
  );
}
