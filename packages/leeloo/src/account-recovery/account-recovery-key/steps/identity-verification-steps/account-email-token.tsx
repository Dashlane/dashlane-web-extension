...
import { FlowStep, UserResendTokenEvent, UserUseAccountRecoveryKeyEvent, } from '@dashlane/hermes';
...
interface Props extends Pick<IdentityVerificationFlowContracts.IdentityVerificationFlowEmailTokenView, 'error' | 'isLoading' | 'isDashlaneAuthenticatorAvailable'> {
...
export const AccountEmailToken = ({ error, isLoading, isDashlaneAuthenticatorAvailable, resendEmailToken, switchToDashlaneAuthenticator, submitEmailToken, cancelAccountRecoveryKey, clearInputError, }: Props) => {
    ...
    return (<form onSubmit={handleSubmit}>
      ...
      <FlexContainer flexDirection="row" sx={{ margin: '24px 0px 16px' }}>
        ...
        <Link onClick={handleTokenResent} color="ds.text.brand.quiet" sx={{ marginRight: '4px' }}>
          {translate(I18N_KEYS.RESEND_TOKEN)}
        </Link>
      </FlexContainer>

      {showConfirmTokenResent ? (<FlexContainer style={{
                marginTop: '24px',
                marginBottom: '24px',
            }}>
      ...
      {isDashlaneAuthenticatorAvailable ? (<FlexContainer flexDirection="row">
          <Paragraph color="ds.text.neutral.quiet" sx={{
                ...paragraphStyle,
                marginRight: '4px',
            }}>
          ...
          <Link onClick={handleSwicthToDashlaneAuthenticator} color="ds.text.brand.quiet">
      ...
      <div sx={{
        ...
        <Button type="submit" id="extng-submit-email-token-button" data-testid="login-button" isLoading={isLoading} disabled={emailToken.length < EMAIL_TOKEN_MINIMAL_LENGTH} onClick={() => handleSubmit()}>
