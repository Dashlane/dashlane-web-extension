...
import { Button, colors, FlexContainer, Heading, Link, LoadingIcon, Paragraph, TokenInput, } from '@dashlane/ui-components';
import { EmailTokenResult } from '@dashlane/communication';
import { useUserLogin } from 'account-recovery/admin-assisted-recovery/verify-account-panel/useGetUserLogin';
...
const I18N_KEYS = {
    ...
    SECURITY_CODE_DESCRIPTION: 'webapp_account_recovery_security_code_description',
    ...
    TOKEN_RESENT_CONFIRMATION: 'webapp_login_form_password_fieldset_security_code_resent',
...
export const VerifyAccountPanel = ({ dispatchGlobal, setShowGenericRecoveryError, }: Props) => {
    ...
    const [tokenResentResult, setTokenResentResult] = React.useState<EmailTokenResult | undefined>();
    const [otpType, setOtpType] = React.useState('');
    const [tokenVericationError, setTokenVerificationError] = React.useState<ErrorTypes.CODE_ERROR | null>(null);
    ...
    const verifyCode = async (event: React.FormEvent<HTMLFormElement> | React.MouseEvent<HTMLElement>) => {
    ...
    return (<div className={styles.verifyAccountPanelContainer}>
      <div className={styles.content}>
        <LogOutContainer dispatchGlobal={dispatchGlobal}/>
        <FlexContainer className={styles.inner}>
          ...
          <Paragraph className={styles.setEmail} bold size="medium" color={colors.midGreen00}>
            {login}
          </Paragraph>
          <form className={styles.form} onSubmit={verifyCode}>
            <TokenInput autoFocus value={token} maxLength={MAX_LENGTH_TOKEN} inputMode="numeric" feedbackType={tokenVericationError ? 'error' : undefined} feedbackText={tokenVericationError === ErrorTypes.CODE_ERROR
            ? translate(I18N_ERROR_KEYS.REGISTER_DEVICE_FAILED)
            : ''} onChange={onCodeChange}/>
            <Button type="submit" size="large" onClick={verifyCode} className={styles.verifyButton} disabled={isLoadingOrDisabled()}>
          ...
          {showResendCode ? (<div>
              ...
              <FlexContainer>
                <Link onClick={onResendCode} color="ds.text.brand.quiet" sx={{ marginRight: '4px' }}>
                  {translate(I18N_KEYS.RESEND_TOKEN)}
                </Link>
                {tokenResentResult ? (<FlexContainer style={{
                    marginTop: '24px',
                    marginBottom: '24px',
                    flexWrap: 'nowrap',
                }}>
                    <Animation height={18.5} width={18.5} animationParams={{
                    renderer: 'svg',
                    ...
                    <Paragraph color={tokenResentResult.success
                    ? 'ds.text.positive.quiet'
                    : 'ds.text.danger.quiet'} sx={{ marginLeft: '4px' }}>
                      {translate(tokenResentResult.success
