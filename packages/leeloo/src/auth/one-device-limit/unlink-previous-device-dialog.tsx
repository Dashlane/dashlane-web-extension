...
export const UnlinkPreviousDeviceDialog = ({
  handleOnCancel,
  handleOnConfirm,
  handleOnGetUnlimitedAccess,
  isOpen,
  previousDeviceInfo,
}: UnlinkPreviousDeviceDialogProps) => {
  ...
  return (
    ...
    >
      <>
        <DialogTitle
          title={translate(I18N_KEYS.CONFIRM_TITLE, {
            deviceName: previousDeviceInfo?.name,
          })}
        />
        <DialogBody sx={{ color: "ds.text.neutral.standard" }}>
          {previousDeviceInfo ? (
            <PreviousDeviceInfo previousDeviceInfo={previousDeviceInfo} />
          ) : null}
          <Paragraph
            sx={{
              marginTop: "20px",
        ...
        >
          <Button
            intensity="quiet"
            mood="brand"
            onClick={handleOnGetUnlimitedAccess}
            sx={{ marginRight: "16px" }}
          >
