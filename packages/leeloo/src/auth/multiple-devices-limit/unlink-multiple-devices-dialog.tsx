...
import { DeviceToDeactivateInfo } from 'auth/multiple-devices-limit/device-to-deactivate-info';
import { dismissUnlinkDeviceLog, unlinkPreiousDeviceLog, upgradePremiumLog, } from 'auth/device-limit-flow/logs';
...
export const UnlinkMultipleDevicesDialog = ({ isOpen, devicesToDeactivate, handleOnConfirm, handleOnCancel, onHandleGoToPremiumPlan, }: UnlinkMultipleDevicesDialogProps) => {
    const [chosenDevicesToBeUnlinked, setChosenDevicesToBeUnlinked] = React.useState<string[]>([]);
    ...
    const handleOnSelection = (device: DeviceToDeactivateInfoView) => {
        if (chosenDevicesToBeUnlinked.includes(device.deviceId)) {
            setChosenDevicesToBeUnlinked(chosenDevicesToBeUnlinked.filter((id) => id !== device.deviceId));
        }
        else {
    ...
    const formatParams = () => {
        return chosenDevicesToBeUnlinked.reduce((memo, id) => {
            const deviceInfo = devicesToDeactivate?.find(({ deviceId }) => deviceId === id);
    ...
    return (<Dialog closeIconName={translate(I18N_KEYS.CLOSE)} isOpen={isOpen} onClose={onClose}>
      ...
      <DialogBody>
        {devicesToDeactivate
            ? devicesToDeactivate.map((deviceToDeactivate) => (<DeviceToDeactivateInfo key={deviceToDeactivate.deviceId} deviceToDeactivate={deviceToDeactivate} handleOnSelection={handleOnSelection} checked={chosenDevicesToBeUnlinked.includes(deviceToDeactivate.deviceId)}/>))
            : null}
      </DialogBody>
      <DialogFooter primaryButtonTitle={translate(I18N_KEYS.UNLINK_BUTTON)} primaryButtonOnClick={onPrimaryButtonClick} primaryButtonProps={{
            type: 'button',
            disabled: !enoughDevicesSelected,
        }} secondaryButtonTitle={translate(I18N_KEYS.UPGRADE_BUTTON)} secondaryButtonOnClick={onSecondaryButtonClick} secondaryButtonProps={{
            type: 'button',
        }}/>
    </Dialog>);
};
