...
import { UnlinkMultipleDevicesRequest, UnlinkMultipleDevicesResult, } from '@dashlane/communication';
export interface MultipleDevicesLimitReachedStage {
    logOut: () => void;
    unlinkMultipleDevices: (devices: UnlinkMultipleDevicesRequest) => Promise<UnlinkMultipleDevicesResult>;
...
export function useMultipleDevicesLimitReachedStage(): MultipleDevicesLimitReachedStage {
    ...
    const onHandleGoToPremiumPlanFactory = (subscriptionCode: string) => {
        if (premiumStatus.status === DataStatus.Success) {
            const goPremiumUrl = routes.userGoPremium(subscriptionCode, getPlanRenewalPeriodicity(premiumStatus.data.autoRenewInfo));
    ...
    return {
        ...
        onHandleGoToPremiumPlan: (subscriptionCode) => onHandleGoToPremiumPlanFactory(subscriptionCode),
    };
}
