...
import { MultipleDevicesLimit } from "../multiple-devices-limit/multiple-devices-limit";
import {
...
const DeviceLimitFlow = ({ stage }: DeviceLimitFlowProps) => {
  ...
  const isInitialSyncAnimationIntroPhasePending = useSelector(
    isInitialSyncAnimationIntroPhasePendingSelector
  );
  if (
    stage.name === LoginDeviceLimitFlowStage.OneDeviceLimitReached &&
  ...
  switch (stage.name) {
    ...
    case LoginDeviceLimitFlowStage.MultipleDevicesLimitReached:
      return (
        <MultipleDevicesLimit
          subscriptionCode={stage.subscriptionCode}
          devicesToDeactivate={stage.devicesToDeactivate}
          wasRefreshed={refreshedState}
        />
      );
    default:
