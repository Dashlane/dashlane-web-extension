...
export function useDeviceLimitInfrastructureProvider(flow: CarbonQueryResult<LoginDeviceLimitFlowView | null>) {
    const [initialFlowState, setInitialFlowState] = useState<typeof flow | undefined>(undefined);
    useEffect(() => {
...
export function useLoginDeviceLimitFlow(): LoginDeviceLimitFlowView | null | undefined {
    ...
    const determinedIfInitialState = useDeviceLimitInfrastructureProvider(endpointResult);
