...
const I18N_KEYS = {
  ...
  STEPS: {
    ...
    LOGIN_WITH_RECOVERY_CODE:
      "webapp_recover_otp_codes_steps_login_with_recovery_code",
  ...
  FORM: {
    ...
    GET_RECOVERY_CODE_CTA:
      "webapp_recover_otp_codes_form_get_recovery_code_cta",
  },
  MORE_INFO: {
...
export const Recover2faCodesRequestComponent = ({
  onSuccess,
}: Recover2faCodesRequestComponentProps) => {
  ...
  const [emailErrorLabel, setEmailErrorLabel] =
    useState<Recover2FAErrors | null>(null);
  ...
  return (
    <div>
      <Flex flexDirection="column" gap="16px">
        ...
        <Infobox
          mood="brand"
          title={translate(I18N_KEYS.BUSINESS_INFOBOX_HELP.TITLE)}
          description={translate(I18N_KEYS.BUSINESS_INFOBOX_HELP.DESCRIPTION)}
        />
        ...
        >
          ...
          >
            ...
            >
              <EmailField
                label={translate(I18N_KEYS.FORM.EMAIL_LABEL)}
                value={email}
            ...
            <Button
              size="large"
              type="submit"
              disabled={!canSend || email.length === 0}
              sx={{ marginTop: "4px" }}
            >
              {state === State.Sending ? (
                <IndeterminateLoader mood="brand" />
              ) : (
                translate(I18N_KEYS.FORM.GET_RECOVERY_CODE_CTA)
              )}
            </Button>
          </Flex>
        </form>
