...
import { EmailHeader, Header, SecurityCodeInput, WebappLoginLayout, } from 'auth/login-panel/authentication-flow/components';
...
const I18N_KEYS = {
    ...
    SECURITY_CODE_DESCRIPTION: 'webapp_login_form_password_fieldset_security_code_otp_description',
...
export const AccountTotp = ({ loginEmail, error, isLoading, changeTwoFactorAuthenticationOtpType, clearInputError, submitTotp, prefilledToken, onClearPrefilledToken, }: Props) => {
    ...
    const onTokenInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        clearInputError();
        void setFieldValue('twoFactorAuthenticationOtpValue', formatAuthenticatorCode(value));
    ...
    return (<WebappLoginLayout>
      ...
      <form onSubmit={handleSubmit}>
        <SecurityCodeInput title={translate(I18N_KEYS.SECURITY_CODE_DESCRIPTION)} securityToken={twoFactorAuthenticationOtpValue} maxLength={SECURITY_TOKEN_MAX_LENGTH} onTokenInputChange={onTokenInputChange} errorMessage={error
            ? translate(I18N_ERROR_KEYS[error] ?? I18N_ERROR_KEYS.UNKNOWN_ERROR)
            : undefined}/>
        {showSyncDevicesHelp ? (<Infobox sx={{ marginTop: '16px' }} title={translate.markup(I18N_KEYS.SYNC_DEVICES_INFOBOX, {
                supportArticleLink: HELPCENTER_CANNOT_LOGIN_SECURITY_CODE_URL,
            }, { linkTarget: '_blank' })} mood="brand" size="small"/>) : null}
        ...
        })} color="ds.text.brand.quiet" target="_self">
        ...
        <Button type="submit" data-testid="submitTotp" isLoading={isLoading} disabled={twoFactorAuthenticationOtpValue.length < OTP_MAX_LENGTH} size="large" fullsize sx={{ marginTop: '16px' }}>
