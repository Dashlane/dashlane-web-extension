...
export const Sso = (props: Props) => {
    ...
    const { loginViaSSO } = useModuleCommands(AuthenticationFlowContracts.authenticationFlowApi);
    const ssoUserSettings = useModuleQuery(AuthenticationFlowContracts.authenticationFlowApi, 'getSsoUserSettings');
    const { login, ssoToken, key: ssoServiceProviderKey, currentAuths, expectedAuths, inStore, } = extractSsoInfoFromUrl(props);
    ...
    const shouldRememberMeForSSO = isAutoSsoLoginDisabled.status === DataStatus.Success &&
    ...
    React.useEffect(() => {
        if (ssoUserSettings.status === DataStatus.Loading ||
