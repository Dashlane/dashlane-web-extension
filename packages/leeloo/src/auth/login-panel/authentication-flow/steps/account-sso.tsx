...
import { Button, Dialog, EmailField, Heading, Icon, jsx, Paragraph, } from '@dashlane/design-system';
...
import { I18N_VALUES } from '../../login-form/login-errors/nitro-sso-web-extension-needed';
import { ClientBypass, ClientBypassType } from '../types/manual-bypass';
import { Header, WebappLoginLayout } from '../components';
interface AccountSSOProps extends Omit<AuthenticationFlowContracts.AuthenticationFlowMachineSSORedirectionToIdpView, 'step'> {
    sendUsageLog?: boolean;
    changeLogin?: (params: AuthenticationFlowContracts.ChangeAccountEmailCommandRequest) => void;
...
export const AccountSSO = ({ serviceProviderRedirectUrl, isNitroProvider, loginEmail, sendUsageLog, changeLogin, setUseClientBypass, }: AccountSSOProps) => {
    ...
    useEffect(() => {
        if (serviceProviderRedirectUrl) {
            ...
            const newSsoServiceProviderUrl = augmentUrlWithProperSsoQueryParameters(serviceProviderRedirectUrl);
            if (isNitroProvider) {
                nitroLoginCommand();
            }
            else {
    ...
    return (<>
      <Dialog title="" isOpen={showNitroSSOExtensionNeededError} actions={{
            primary: {
                children: I18N_VALUES.DIALOG_CTA,
                onClick: (e: SyntheticEvent<HTMLButtonElement>) => {
                    e.preventDefault();
                    redirectToUrl(DASHLANE_DOWNLOAD_EXTENSION_URL);
                },
            },
        }} onClose={() => setShowNitroSSOExtensionNeededError(false)} closeActionLabel={translate(I18N_KEYS.CLOSE)}>
        <div sx={{ marginBottom: '25px' }}>
          <Icon sx={{ size: '60px' }} name="FeedbackFailOutlined" color="ds.container.expressive.danger.catchy.idle" aria-hidden="true"/>
        </div>
        <Heading title={I18N_VALUES.DIALOG_TITLE} as={'h1'}/>
        <Paragraph sx={{ marginBottom: '24px' }}>
          {I18N_VALUES.DIALOG_BODY}
        </Paragraph>
      </Dialog>
      ...
      <WebappLoginLayout>
        ...
        <div sx={{ gap: '25px', display: 'flex', flexDirection: 'column' }}>
          <EmailField name="email" id="extng-account-email-input" value={loginEmail} type="email" label={''}/>
          <Button type="submit" id="extng-account-email-next-button" aria-label={translate(I18N_KEYS.NEXT)} data-testid="login-button" fullsize size="large" disabled={true} isLoading={true}>
