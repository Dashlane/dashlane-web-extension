...
const I18N_KEYS = {
  ...
  TOOLTIPS_TITLE:
    "webapp_auth_panel_standalone_account_creation_zxcvbn_tooltips_title",
};
interface Props {
  showTooltip: boolean;
...
const getContentKeys = (
  ...
  return [
    ...suggestions.map(
      (suggestion: string) => `${I18N_KEYS.BASE}${suggestion}`
    ),
    ...(warning ? [`${I18N_KEYS.BASE}${warning}`] : []),
  ];
};
export const PasswordStrengthTooltip = ({
  id,
  passwordStrength,
  children,
  showTooltip,
  ...
  return (
    <Tooltip
      ...
      visible={visible}
      placement="top"
      overlayClassName={tooltipStyles.tooltipContainer}
      overlay={
