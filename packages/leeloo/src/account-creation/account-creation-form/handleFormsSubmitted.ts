...
import { ConfirmSubmitOptions } from "./confirm/masterpassword-form";
import { getUserId } from "../../user";
import { getDefaultDeviceName } from "../../helpers";
import { translate } from "../../libs/i18n";
export const handleStep2Submit = async (
  lee: Lee,
  confirm: ConfirmSubmitOptions,
  step1Response: CreateAccountResult,
  isTACFlow: boolean
): Promise<void> => {
  if (!step1Response.valid) {
    const error = new Error(
      "accountCreationStep1.valid was not defined on confirm submit"
    );
...
export const handleAccountCreationSubmit = async (
  lee: Lee,
  ...
  const settings = {
    ...
    deviceName: getDefaultDeviceName(
      translate("webapp_login_form_regsiter_fallback_browser_name")
    ),
  };
