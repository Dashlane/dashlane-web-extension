...
import { LoginUserWithEnclaveSSOCommandParam } from "@dashlane/sso-scim-contracts";
import { Lee } from "../../../lee";
...
export interface HandleInfoFormSubmittedParams {
  ...
  loginUserWithEnclaveSSO?: (
    command: LoginUserWithEnclaveSSOCommandParam
  ) => Promise<unknown>;
  isInviteLinkFlow: boolean;
}
export const isFormValid = (fields: InfoFormFields) =>
  isValidEmail(fields.email);
export const handleInfoFormSubmitted = ({
  lee,
  onSubmit,
  fields,
  setIsLoading,
  ...
  loginUserWithEnclaveSSO,
  isInviteLinkFlow,
}: HandleInfoFormSubmittedParams) => {
  setLoginErrorType(
    isValidEmail(fields.email) ? null : LoginErrorTypes.INVALID_EMAIL
  );
  ...
  carbonConnector
    ...
    .then(
      ({
        ...
        let isLoading = false;
        const inviteLinkUserNotProposed =
          isInviteLinkFlow &&
          isUserProposed === false &&
          isUserAccepted === false;
        const teamAcceptanceNeeded =
          isInviteLinkFlow &&
          isUserProposed === true &&
          isUserAccepted === false;
        if (!info.isValid) {
        ...
        } else if (inviteLinkUserNotProposed && !info.isNitroSsoUser) {
        ...
        if (
          ...
          onSubmit({
            login: fields.email,
            isProposedExpired: isProposedExpired,
            isUserProposed,
            isUserAccepted,
          });
        }
