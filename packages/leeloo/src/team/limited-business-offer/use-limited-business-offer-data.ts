...
import { getMonthlySeatPrice } from 'team/change-plan/utils';
...
export const useLimitedBusinessOfferData = (): LimitedBusinessOfferData => {
    ...
    const hasLimitedOffer = canUpgradeToBusiness && monthlySeatPrice !== monthlySeatEquivalentPrice;
    const translatedPrice = translate.price(businessOffer.currency, monthlySeatPrice / 100, { notation: 'compact' });
    const translatedEquivalentPrice = translate.price(businessOffer.currency, monthlySeatEquivalentPrice / 100, { notation: 'compact' });
    return {
        hasLimitedOffer,
        translatedPrice,
