...
import { getInviteDialogErrorText } from "./error-text";
import { InviteErrors, REFUSED_MEMBER_KEYS } from "./types";
import { useTeamSettings } from "../settings/hooks/useTeamSettings";
import { DASHLANE_SUPPORT } from "../urls";
...
export type InvitePartialSuccessState = Pick<
  InviteResultDialogProps,
  "invitedMembers" | "refusedMembers"
> & {
  show: boolean;
};
...
export const useInviteResultPartialSuccess = () => {
  const [invitePartialSuccessState, setInvitePartialSuccessState] =
    React.useState<InvitePartialSuccessState>(emptyInvitePartialSuccessState);
...
const I18N_KEYS = {
  ...
  REFUSED_MEMBER_LIST_TITLE:
    "team_invite_result_dialog_refused_member_list_title",
...
export const InviteResultDialog = ({
  teamId,
  isOpen,
  invitedMembers,
  refusedMembers,
  ...
  const [refusedMembersByType, setRefusedMembersByType] = React.useState<
    MemberErrorInformation[]
  >([]);
  const [refusedMembersByCopy, setRefusedMembersByCopy] = React.useState<
    MemberErrorInformation[]
  >([]);
  ...
  React.useEffect(() => {
    ...
    Object.keys(refusedMembers).forEach((memberEmail: string) => {
      ...
      if (
        inviteErrorType === InviteErrors.SSO_ALREADY_ACTIVATED &&
        !ssoEnabled
      ) {
        ...
        inviteErrorType === InviteErrors.USER_HAS_MASTER_PASSWORD &&
        ssoEnabled
      ) {
        refusedMembersIndexedByCopy.push({
          email: memberEmail,
    ...
    >
      ...
      <DialogBody>
        {teamSettingsLoading ? (
          ...
          <>
            {invitedMembersCount ? (
              <div className={styles.successfulInvitesRatio}>
            ...
            <div className={styles.refusedMemberListTitle}>
              ...
              <span
                id="refusedList"
                className={styles.inviteResultDialogHeader}
              >
            ...
            <ul className={styles.refusedMemberList}>
              {refusedMembersByType.map((error) => (
                <li className={styles.refusedMember} key={error.email}>
                  {error.email}
                  <span className={styles.reason}>
                    ...
                    {REFUSED_MEMBER_KEYS[error.errorType.toUpperCase()]
                      ...
                      : translate.markup(
                          REFUSED_MEMBER_KEYS.GENERIC_ERROR,
              ...
              {refusedMembersByCopy.map((error) => (
                <li className={styles.refusedMember} key={error.email}>
                  {error.email}
                  <span className={styles.reason}>
      ...
      <DialogFooter
        primaryButtonTitle={translate(I18N_KEYS.OK_GOT_IT)}
        primaryButtonOnClick={onClose}
        secondaryButtonTitle={translate(I18N_KEYS.HELP_CENTER)}
        secondaryButtonOnClick={() => openUrl(DASHLANE_SUPPORT)}
      />
    </Dialog>
  );
};
