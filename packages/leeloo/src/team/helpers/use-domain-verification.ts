...
export const useDomainVerification = () => {
    const [verifiedOrPendingDomain, setVerifiedOrPendingDomain] = useState<{
        domain: null;
        status: DomainVerificationStatus.Loading | DomainVerificationStatus.Start;
    } | {
        domain: Domain;
        status: Omit<DomainVerificationStatus, DomainVerificationStatus.Loading | DomainVerificationStatus.Start>;
    ...
    const updateDomainState = useCallback(async () => {
        try {
            ...
            if (result.success && result.domains.length) {
                const pendingDomains = result.domains.filter((domainInfo) => domainInfo.status === DomainStatus.pending);
                const validatedDomains = result.domains.filter((domainInfo) => domainInfo.status === DomainStatus.valid);
                if (validatedDomains.length) {
                    ...
                    const mostRecentPendingDomain = pendingDomains[pendingDomains.length - 1];
                    setVerifiedOrPendingDomain({
