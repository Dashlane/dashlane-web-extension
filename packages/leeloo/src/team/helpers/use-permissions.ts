...
} from "@dashlane/access-rights-contracts";
...
const dataIsArray = (
  data: UserPermissionsQueryResult | undefined
): data is Permission[] => Array.isArray(data);
export const usePermissions = (): UsePermissions => {
  ...
  const hasPermission = (permission: Permission): boolean => {
    return (
      dataReceived(status) && dataIsArray(data) && data.includes(permission)
    );
  };
