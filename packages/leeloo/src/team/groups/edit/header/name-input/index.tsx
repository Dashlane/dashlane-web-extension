...
export const NameInput = ({ onNameChanged, defaultValue, editable, saving, }: Props) => {
    ...
    return (<Heading as="h1" color="ds.text.neutral.standard" textStyle="ds.title.section.large">
      {!editing && (<div className={classnames(styles.nameContainer, {
                [styles.hoverableContainer]: editable,
            })} onClick={handleNameContainerClicked}>
          {groupName}
          {saving ? (<LoadingSpinner size={20} containerClassName={styles.nameEditIcon}/>) : (<img className={styles.nameEditIcon} src={pencilIcon}/>)}
        </div>)}

      {editing && (<div className={styles.editNameContainer}>
          <input ref={input} type="text" className={classnames(styles.nameInput, {
                [styles.error]: showErrorMessage,
            })} defaultValue={groupName} onBlur={saveName} onKeyUp={handleKeyUpOnNameInput}/>
        </div>)}

      {editing && showErrorMessage ? (<div className={styles.errorMessage}>{errorMessage}</div>) : null}
    </Heading>);
};
