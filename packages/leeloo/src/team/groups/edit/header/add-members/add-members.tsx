...
import { AlertSeverity, jsx } from '@dashlane/ui-components';
...
import { Multiselect, MultiselectError } from 'libs/dashlane-style/multiselect';
import { getCurrentTeamId } from 'libs/carbon/spaces';
import { isValidEmail } from 'libs/validators';
import useTranslate from 'libs/i18n/useTranslate';
import { useAlert } from 'libs/alert-notifications/use-alert';
import InviteResultDialog, { useInviteResultPartialSuccess, } from 'team/invite-result-dialog';
...
import { AddAdminHelpTip } from './add-admin-help-tip';
...
const I18N_KEYS = {
    ...
    MISSING_PUB_KEY_ERROR: 'team_groups_edit_invite_member_to_group_missing_public_key_error_message_markup',
    INVITE_GENERIC_ERROR: 'team_groups_edit_invite_member_to_group_error_message_markup',
    ...
    ADD_MEMBER_NOT_ENOUGH_SLOTS_ERROR: 'team_group_edit_add_member_not_enough_slots_error',
};
export const AddMembers = ({ lee, userGroup }: AddMembersProps) => {
    ...
    const getActiveUserGroupMembersAliases = () => userGroup.users
    ...
    const getAutocompleteData = (): {
        ...
        const userGroupMembersAliasSet = new Set(getActiveUserGroupMembersAliases());
        return teamMembers
            .filter(({ login, status }) => status !== 'removed' && !userGroupMembersAliasSet.has(login))
            .map(({ login }) => ({
            text: login,
    ...
    const handleAddMemberConfirmed = async () => {
        ...
        if (inviteMembersResult.error) {
            if (inviteMembersResult.error.message === 'no_free_slot' ||
                inviteMembersResult.error.message === 'not_enough_free_slots') {
                alert.showAlert(translate(I18N_KEYS.ADD_MEMBER_NOT_ENOUGH_SLOTS_ERROR, {
                    count: users.length,
                    groupName,
                }), AlertSeverity.ERROR);
                return;
                ...
                alert.showAlert(translate(I18N_KEYS.INVITE_GENERIC_ERROR), AlertSeverity.ERROR);
                return;
            }
        }
    ...
    return (<div>
      <ButtonWithPopup ref={newGroupButtonWithPopupRef} buttonLabel={translate(I18N_KEYS.ADD_MEMBERS_BUTTON)} confirmButtonLabel={translate(I18N_KEYS.ADD_MEMBERS_CONFIRM_BUTTON)} cancelButtonLabel={translate(I18N_KEYS.ADD_MEMBERS_CANCEL_BUTTON)} confirmButtonDisabled={!isInputValid()} onCancelButtonClick={handleAddMemberCancelled} onConfirmButtonClick={handleAddMemberConfirmed} popupClassName={styles.addMemberPopup}>
        <AddAdminHelpTip tooltipDismissed={tooltipDismissed} setTooltipDismissed={setTooltipDismissed}>
          <span className={styles.addMemberLabel}>
            {translate(I18N_KEYS.ADD_MEMBER_HEADER)}
          </span>
          {error && <div className={styles.error}>{error}</div>}

          <Multiselect autoFocus={true} className={styles.addMemberMultiselect} dataSource={getAutocompleteData()} getCurrentSelectionAvatar={getCurrentSelectionAvatar} defaultValue={addMemberEmails} isItemValid={(item: string) => isValidEmail(item)} onChange={handleAddMemberEmailsChanged} onError={(err: MultiselectError) => setError(err ? translate(`team_groups_edit_error_${err.message}`) : null)} placeholder={translate(I18N_KEYS.ADD_MEMBER_INPUT_PLACEHOLDER)}/>
        </AddAdminHelpTip>
      </ButtonWithPopup>

      <InviteResultDialog teamId={getCurrentTeamId(lee.globalState)} isOpen={invitePartialSuccessState.show} onClose={handleInvitationResultClosed} invitedMembers={invitePartialSuccessState.invitedMembers} refusedMembers={invitePartialSuccessState.refusedMembers}/>
    </div>);
};
