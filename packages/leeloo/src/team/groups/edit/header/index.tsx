...
import { GROUP_NAME_ALREADY_IN_USE } from "../types";
...
const I18N_KEYS = {
  ...
  DELETE_GROUP_CANCEL_BUTTON:
    "team_groups_edit_header_delete_group_cancel_button",
  DELETE_GROUP_CONFIRM_BUTTON:
    "team_groups_edit_header_delete_group_confirm_button",
  DELETE_GROUP_CONFIRM_TITLE_MARKUP:
    "team_groups_edit_header_delete_group_confirm_title_markup",
  DELETE_GROUP_CONFIRM_SUBTITLE:
    "team_groups_edit_header_delete_group_confirm_subtitle",
...
export const EditHeader = ({
  ...
  const [confirmDeleteGroupName, setConfirmDeleteGroupName] = useState<
    Maybe<string>
  >(Maybe.nothing());
  ...
  const handleConfirmDeleteGroupNameChange = (
    event: ChangeEvent<HTMLInputElement>
  ) => {
    const trimmedValue = event.target["value"]?.trim();
    setConfirmDeleteGroupName(Maybe.maybe<string>(trimmedValue));
  };
  const isConfirmDeleteGroupNameValid = () =>
    confirmDeleteGroupName.caseOf({
    ...
    <div sx={ROOT_GROUP_STYLES.HEADER}>
      ...
      >
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          <ButtonWithPopup
            buttonLabel={translate(I18N_KEYS.DELETE_GROUP)}
            cancelButtonLabel={translate(I18N_KEYS.DELETE_GROUP_CANCEL_BUTTON)}
            confirmButtonLabel={translate(
              I18N_KEYS.DELETE_GROUP_CONFIRM_BUTTON
            )}
            onCancelButtonClick={handleDeleteGroupCancelled}
            onConfirmButtonClick={handleConfirmButtonClicked}
            confirmButtonDisabled={!isConfirmDeleteGroupNameValid()}
            popupClassName={styles.confirmPopup}
            buttonMood="neutral"
          ...
          >
            ...
            >
              ...
              <TextInput
                fullWidth
                onChange={handleConfirmDeleteGroupNameChange}
                type="text"
                defaultValue={confirmDeleteGroupName.caseOf({
                  just: (name) => name,
                  nothing: () => undefined,
                })}
                autoFocus
