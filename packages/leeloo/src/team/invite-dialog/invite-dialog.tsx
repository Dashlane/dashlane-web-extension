...
import { Alert, AlertSeverity, colors, Dialog, DialogBody, DialogFooter, DialogTitle, jsx, LoadingIcon, MailIcon, } from '@dashlane/ui-components';
...
import { ADDITIONAL_SEAT_CAP, isSeatCountAboveCap, SALES_EMAIL, } from 'team/account/upgrade/helpers';
import { useAdditionalSeatCountPricing } from 'team/account/upgrade/useAdditionalSeatCountPricing';
import { useAlertQueue } from 'team/alerts/use-alert-queue';
import { useBillingCountry } from 'team/helpers/useBillingCountry';
import { InvitePartialSuccessState } from 'team/invite-result-dialog';
import { hasRefusedMembers } from 'team/members';
import findEmails from './find-emails';
import styles from 'team/invite-dialog/styles.css';
...
const I18N_KEYS = {
    ...
    INVITE_SUCCESS_MESSAGE_COUNT: 'team_members_invite_success_message_with_count',
    INVITE_SUCCESS_MESSAGE_ADDED_SEATS: 'team_members_invite_success_message_with_added_seats',
    INVITE_SUCCESS_MESSAGE_ADDED_SEATS_ONE_INVITE: 'team_members_invite_success_message_with_added_seats_one_invite',
...
export const InviteDialog = ({ lee, isFreeTrial, isOpen, slotsTaken, slotsTotal, exclude, preFilledEmails, closeInviteDialog, handleInviteCompleteSuccess, handleInvitePartialSuccess, setShowActivationDialog, }: Props) => {
    ...
    const { billingDetails, amountToPay, amountToTax, onSeatCountChange } = useAdditionalSeatCountPricing(null);
    const [isSending, setIsSending] = useState<boolean>(false);
    const [isOverlayVisible, setIsOverlayVisible] = useState<boolean>(false);
    const [emails, setEmails] = useState<Set<string>>(preFilledEmails ?? new Set([]));
    ...
    const sendEmails = async (emailsToSend: string[]): Promise<TeamPlansProposeMembersResultContent> => {
        ...
        try {
            ...
            const errorLocaleKey = error.code && error.subCode
                ? `invite_error_${error.code}_${error.subCode}`
                :
                    I18N_KEYS.INVITE_ERROR_MSG;
            alert.showAlert(translate(errorLocaleKey), AlertSeverity.ERROR);
    ...
    const handleEmailsSent = async (emailsToSend: string[], offset?: number): Promise<void> => {
        offset = offset ?? 0;
        ...
        const serverData: TeamPlansProposeMembersResultContent = {} as TeamPlansProposeMembersResultContent;
        try {
            ...
            if (hasRefusedMembers(serverData)) {
                handleInvitePartialSuccess(serverData.proposedMembers, serverData.refusedMembers);
                const invitedMemberCount = Object.keys(serverData.proposedMembers).length;
                ...
                if (remainingSlots < 0) {
                    if (emails.size === 1) {
                        alert.showAlert(translate(I18N_KEYS.INVITE_SUCCESS_MESSAGE_ADDED_SEATS_ONE_INVITE, {
                            cost: translate.price(billingDetails.currency, (amountToPay + amountToTax) / 100),
                        }), AlertSeverity.SUCCESS);
                    }
                    else {
                        alert.showAlert(translate(I18N_KEYS.INVITE_SUCCESS_MESSAGE_ADDED_SEATS, {
                            total: emails.size,
                    ...
                    alert.showAlert(translate(I18N_KEYS.INVITE_SUCCESS_MESSAGE_COUNT, {
                        count: emails.size,
                    }), AlertSeverity.SUCCESS);
                }
                logEvent(new UserSendManualInviteEvent({
    ...
    const handleSendClicked = async (): Promise<void> => {
        try {
            ...
            alert.showAlert(translate(I18N_KEYS.COMMON_GENERIC_ERROR), AlertSeverity.ERROR);
        }
        setEmails(new Set([]));
    };
    ...
    const invalidCount = [...emails].filter((email) => !validateEmail(email)).length;
    let sendDisabled = emails.size === 0 ||
        invalidCount > 0 ||
        (remainingSlots < 0 && isSeatCountAboveCap(-remainingSlots)) ||
        showStarterWarning;
    let counterElement: JSX.Element | null = null;
    useEffect(() => {
        if (slotsTotal) {
    ...
    if (slotsTotal) {
        const standardCounter = remainingSlots < 0
        ...
        const counterClassName = classnames(styles.counter, {
            [styles.alert]: remainingSlots < 1,
            [styles.active]: !isSending,
        });
        counterElement = <div className={counterClassName}>{counter}</div>;
    }
    return (<Dialog isOpen={isOpen} closeIconName={!isSending ? translate(I18N_KEYS.DIALOG_CLOSE_BUTTON) : undefined} onClose={onDialogRequestClose} ariaDescribedby="dialogContent">
      <DialogTitle title={translate(I18N_KEYS.DIALOG_TITLE)}/>
      <DialogBody>
        <div className={styles.dialog}>
          <div id="dialogContent" className={styles.instruction}>
            {isSending
            ? translate(I18N_KEYS.INPUT_LABEL_SENDING)
            : translate(I18N_KEYS.INPUT_LABEL)}
          </div>
          <FileDropZone accept={ACCEPT} passClicksThrough className={styles.fileDropZone} onChange={(arrayBuffer) => {
            mergeEmails(formatEmails(arrayBuffer.join('\0')));
        }} onError={handleError}>
            <TagsField key="tags" setIsOverlayVisible={setIsOverlayVisible} placeholder={translate(I18N_KEYS.INPUT_PLACEHOLDER)} tags={[...emails]} isOverlayVisible={isOverlayVisible} setTags={setEmailsList} formatToTags={formatEmails} validate={validateEmail}/>
          </FileDropZone>
          {!showStarterWarning && counterElement}
          {showStarterWarning ? (<div className={styles.alertOverride}>
              <Alert severity={AlertSeverity.SUBTLE}>
          ...
          <div className={classnames(styles.import, {
            [styles.active]: !isSending,
        })}>
            <FileDropZone accept={ACCEPT} onChange={(arrayBuffer) => {
            mergeEmails(formatEmails(arrayBuffer.join('\0')));
        }} onError={handleError}>
              <span className={styles.importTextAction}>
                {translate(I18N_KEYS.INPUT_DESCRIPTION)}
              </span>
            </FileDropZone>
          </div>
          <div className={classnames(styles.importHint, {
            [styles.active]: !isSending,
        })}>
      ...
      <DialogFooter primaryButtonTitle={<>
            ...
            {translate(I18N_KEYS.BUTTON_SEND)}
          </>} primaryButtonOnClick={handleSendClicked} primaryButtonProps={{
