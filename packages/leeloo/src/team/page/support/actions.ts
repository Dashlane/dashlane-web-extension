...
import { BUSINESS_BUY, CONTACT_SUPPORT_WITH_TRACKING, DASHLANE_ENTERPRISE_CONTACT_FORM, DASHLANE_REGISTER_FOR_WEBINAR, REQUEST_PHONE_CALL, } from 'team/urls';
...
const CONTACT_SUPPORT_UTM_CODE = 'button:upgrade_business_tier+origin_page:tac/modal_contact_support+origin_component:main_app';
export const handleClickPhoneSupportAction = (hasPhoneSupportCapability: boolean, routes: NamedRoutes, isFreeTrial?: boolean, isGracePeriod?: boolean, subscriptionCode?: string) => {
    ...
    if (!hasPhoneSupportCapability && (isFreeTrial || isGracePeriod)) {
        ...
        openUrl(`${BUSINESS_BUY}?plan=business&subCode=${subscriptionCode ?? ''}&utm_source=${CONTACT_SUPPORT_UTM_CODE}`);
        return;
    }
    callToActionList.push(CallToAction.RequestUpgrade);
    logEvent(new UserCallToActionEvent({
...
export const handleClickSupportOption = (contactOption: SupportOption, hasPhoneSupportCapability: boolean) => {
    const callToActionList: CallToAction[] = [
        CallToAction.SendEmail,
        CallToAction.RequestDemo,
        CallToAction.RegisterWebinar,
    ...
    callToActionList.push(hasPhoneSupportCapability
...
export const handleDismissDialog = (onDismiss: () => void, hasPhoneSupportCapability: boolean | null) => {
    const callToActionList: CallToAction[] = [
        CallToAction.SendEmail,
        CallToAction.RequestDemo,
        CallToAction.RegisterWebinar,
