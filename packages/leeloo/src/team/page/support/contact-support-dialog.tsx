...
import { ContactSupportItem } from "./contact-support-item";
import { HorizontalDivider } from "./horizontal-divider";
import {
  handleClickPhoneSupportAction,
  handleClickSupportOption,
  handleDismissDialog,
  SupportOption,
} from "./actions";
interface Props {
...
const I18N_KEYS = {
  ...
  HELP_CUSTOMER_SUPPORT_TITLE:
    "team_contact_support_help_customer_support_title",
  HELP_CUSTOMER_SUPPORT_SUBTITLE:
    "team_contact_support_help_customer_support_subtitle",
...
export const ContactSupportDialog = ({ onDismiss }: Props) => {
  ...
  return (
    <Dialog
      isOpen
      onClose={() => handleDismissDialog(onDismiss, hasPhoneSupportCapability)}
      closeActionLabel={translate(I18N_KEYS.CLOSE)}
      title={translate(I18N_KEYS.TITLE)}
    >
      {isLoading ? (
        <Paragraph sx={{ textAlign: "center" }}>
          <IndeterminateLoader size={44} />
        </Paragraph>
      ) : (
        <>
          ...
          <div sx={{ gap: "8px" }}>
            <ContactSupportItem
              title={translate(I18N_KEYS.PHONE_SUPPORT_TITLE)}
              description={translate(I18N_KEYS.PHONE_SUPPORT_SUBTITLE)}
            ...
            <HorizontalDivider />
            <ContactSupportItem
              title={translate(I18N_KEYS.SECURITY_GUIDANCE_TITLE)}
              description={translate(I18N_KEYS.SECURITY_GUIDANCE_SUBTITLE)}
              iconName="ProtectionOutlined"
              button={{
                content: translate(I18N_KEYS.SECURITY_GUIDANCE_REQUEST_DEMO),
                onClick: () =>
                  handleClickSupportOption(
                    SupportOption.SECURITY_GUIDANCE,
                    hasPhoneSupportCapability
                  ),
                isRoleLink: true,
              }}
            ...
            <HorizontalDivider />
            <ContactSupportItem
              title={translate(I18N_KEYS.TRAINING_TITLE)}
              description={translate(I18N_KEYS.TRAINING_SUBTITLE)}
              iconName="AccountSettingsOutlined"
              button={{
                content: translate(I18N_KEYS.REGISTER_FOR_WEBINAR),
                onClick: () =>
                  handleClickSupportOption(
                    SupportOption.TRAINING,
                    hasPhoneSupportCapability
                  ),
                isRoleLink: true,
              }}
            ...
            <HorizontalDivider />
            <ContactSupportItem
              title={translate(I18N_KEYS.HELP_CUSTOMER_SUPPORT_TITLE)}
              description={translate(I18N_KEYS.HELP_CUSTOMER_SUPPORT_SUBTITLE)}
              iconName="ItemEmailOutlined"
              button={{
                content: translate(I18N_KEYS.SEND_MESSAGE_TO_SUPPORT),
                onClick: () =>
                  handleClickSupportOption(
                    SupportOption.SEND_MESSAGE,
                    hasPhoneSupportCapability
                  ),
                isRoleLink: true,
              }}
