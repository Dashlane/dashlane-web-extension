...
import { Accordion, AccordionDetails, AccordionSection, AccordionSummary, Button, Card, colors, FlexChild, FlexContainer, GridContainer, Heading, jsx, Paragraph, RefreshIcon, TextInput, } from '@dashlane/ui-components';
...
const I18N_KEYS = {
    ...
    VERIFY_DNS_TEXT_DESCRIPTION: 'team_breach_report_domain_verify_dns_description',
    VERIFY_DNS_TEXT_SECONDARY_DESCRIPTION: 'team_breach_report_domain_verify_dns_secondary_description',
...
const RetrySection = ({ lastVerificationAttemptDateUnix, onRetry, }: {
    ...
    return (<FlexContainer sx={{
      ...
      <Paragraph size="x-small" bold>
        ...
        {lastVerificationAttemptDateUnix ? (<LocalizedTimeAgo date={fromUnixTime(lastVerificationAttemptDateUnix)}/>) : null}
      </Paragraph>
      <Button type="button" nature="ghost" sx={{
            width: 'fit-content',
            margin: 0,
            padding: 0,
            minWidth: 0,
...
export const VerifyDomainPending = ({ domain, isDeactivating, handleDeactivateDomain, handleRetryValidateDomain, }: VerifyDomainDarkWebInsightsProps) => {
    const { translate } = useTranslate();
    const { dnsToken: { computedToken, challengeDomain } = {
        computedToken: '',
        challengeDomain: '',
    }, lastVerificationAttemptDateUnix, } = domain;
    return (<>
      <Stepper />
      <div>
        <Card sx={{ padding: '24px', margin: '24px 0' }}>
          ...
          <RetrySection lastVerificationAttemptDateUnix={lastVerificationAttemptDateUnix} onRetry={handleRetryValidateDomain}/>
          <FlexContainer gap="16px">
            <FlexChild sx={{ width: '70%' }}>
              <TextInput fullWidth defaultValue={domain.name} disabled/>
            </FlexChild>
            <FlexChild>
              <Button type="button" disabled={isDeactivating} nature="secondary" onClick={handleDeactivateDomain}>
        ...
        <Accordion size="medium">
          <AccordionSection open>
            ...
            <AccordionDetails>
              <GridContainer gap="24px" gridTemplateColumns="1fr" gridAutoRows="auto">
                <Paragraph size="small" color={colors.grey00}>
                  {translate(I18N_KEYS.VERIFY_DNS_TEXT_DESCRIPTION)}
                </Paragraph>
                <InputWithCopyButton inputValue={challengeDomain} textInputProps={{
            fullWidth: true,
            readOnly: true,
            label: translate(I18N_KEYS.VERIFY_HOST),
        }}/>
                <InputWithCopyButton inputValue={computedToken} textInputProps={{
            fullWidth: true,
            readOnly: true,
            label: translate(I18N_KEYS.VERIFY_TXT),
        }}/>
                <Paragraph size="small" sx={{ marginBottom: '20px' }} color={colors.grey00}>
