...
import { DomainVerificationStep, HelpCenterArticleCta, UserOpenHelpCenterEvent, UserVerifyDomainEvent, } from '@dashlane/hermes';
import { logEvent } from 'libs/logs/logEvent';
import { Button, Card, CheckCircleIcon, colors, FlexChild, FlexContainer, Heading, jsx, Paragraph, TextInput, } from '@dashlane/ui-components';
import { VerifyDomainDialog } from 'team/settings/sso-with-encryption-service/domain/VerifyDomainDialog';
import { Stepper } from 'team/dark-web-insights/verify-domain/stepper/stepper';
...
const I18N_KEYS = {
    ...
    SECONDARY_DESCRIPTION: 'breach_report_verify_domain_dialog_secondary_description',
...
export const VerifyDomainStart = ({ onSuccess, onError, }: VerifyDomainDarkWebInsightsProps) => {
    ...
    const registerDomain = async (url: string) => {
        logEvent(new UserVerifyDomainEvent({
            domainVerificationStep: DomainVerificationStep.TapContinueCtaWithDomainUrl,
        }));
        ...
        if (!registerResult.success) {
            const { error: { code }, } = registerResult;
            const errorI18nKey = ERROR_I18N_KEYS[code];
            setRegistrationError(translate(errorI18nKey ?? I18N_KEYS.REGISTER_INVALID));
        }
    };
    return (<>
      <Stepper />
      <div>
        <Card sx={{ padding: '24px', margin: '24px 0' }}>
          <Heading size="x-small">
            {translate(I18N_KEYS.VERIFY_DOMAIN_TITLE)}
          </Heading>
          <Paragraph sx={{ marginBottom: '20px', marginTop: '10px' }} color={colors.grey00}>
            {translate(I18N_KEYS.VERIFY_DOMAIN_DESCRIPTION)}
          </Paragraph>
          <FlexContainer gap="16px">
            <FlexChild sx={{ width: '75%' }}>
              <TextInput onChange={onChange} fullWidth value={domainName} autoFocus endAdornment={registrationError ||
            domainName === '' ||
            !validator.isFQDN(domainName) ? undefined : (<CheckCircleIcon color={colors.accessibleValidatorGreen}/>)} feedbackText={registrationError ? registrationError : undefined} feedbackType={registrationError ? 'error' : undefined}/>
            </FlexChild>
            <FlexChild sx={{ width: '20%' }}>
              <Button type="button" disabled={domainName === '' || isLoading} onClick={() => {
        ...
        {showVerifyDomainDialog ? (<VerifyDomainDialog description={translate.markup(I18N_KEYS.DESCRIPTION)} secondaryDescription={translate(I18N_KEYS.SECONDARY_DESCRIPTION)} linkLabel={translate(I18N_KEYS.VIEW_DOMAIN_VERIFICATION_LABEL)} href={VIEW_DOMAIN_VERIFICATION_GUIDE_LINK} onClick={() => {
                logEvent(new UserOpenHelpCenterEvent({
                    helpCenterArticleCta: HelpCenterArticleCta.ViewDomainVerificationGuide,
                }));
            }} domainName={domainName} onSuccess={() => {
                onSuccess(domainName);
            }} isDarkWebInsights={true} onError={onError} onDismiss={onError}/>) : null}
      </div>
    </>);
};
