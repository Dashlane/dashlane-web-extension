...
export const DownloadDWIButton = ({ domainName }: DownloadDWIButtonProps) => {
  ...
  const downloadCSV = async () => {
    ...
    const remainingRequests = Math.ceil(
      response.emailsImpactedCount / REQUEST_SIZE
    );
    allReports.push(...response.emails);
    const batchRequests = async (currentRequestCount = 0) => {
      const requests = [];
      while (
        currentRequestCount < remainingRequests ||
        (remainingRequests - currentRequestCount) % BATCH_SIZE !== 0
      ) {
        requests.push(
          fetchDarkWebInsightsResults(
            domainName,
            REQUEST_SIZE,
            currentRequestCount * REQUEST_SIZE
          )
        );
