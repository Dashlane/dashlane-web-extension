...
const createCSVHeader = () => {
  return (
    ["email", "domain", "data_affected", "incident_date", "view_status"].join(
      ","
    ) + "\n"
  );
};
const convertReportIntoCSV = (
...
export const downloadDWIReportCSV = (
  reports: EmailIncidentInfo[],
  dateFormatter: DateFormatter
) => {
  downloadFile(
    convertReportIntoCSV(reports, dateFormatter),
    "DWI-report.csv",
