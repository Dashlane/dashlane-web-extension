...
import { Button, Heading, Paragraph, TextInput } from "@dashlane/design-system";
import {
  DataStatus,
  useModuleCommands,
  useModuleQuery,
...
const VatNumberForm = ({
  ...
  const [cancelButtonState, setCancelButtonState] =
    useState<CancelButtonState>("cancel");
  const [showSuccessMessage, setShowSuccessMessage] = useState(false);
  const [addUpdateStatus, setAddUpdateStatus] = useState<AddUpdateState>(
    AddUpdateState.ADD
  );
  ...
  const getVatNumberQuery = useModuleQuery(teamVatApi, "getTeamVat");
  useEffect(() => {
    const getVatNumber = () => {
      if (
        getVatNumberQuery.status === DataStatus.Success &&
        getVatNumberQuery.data !== ""
      ) {
        setVatNumber(getVatNumberQuery.data);
    ...
    getVatNumber();
  }, [getVatNumberQuery.status, getVatNumberQuery.data]);
  const handleEdit = () => {
  ...
  const handleDelete = async () => {
    ...
    if (result.tag === "success") {
      ...
      alert.showAlert(
        translate(I18N_KEYS.DELETE_VAT_NUMBER_SUCCESS),
  ...
  const successMessageCopy =
    addUpdateStatus === AddUpdateState.ADD
    ...
    <>
      {vatNumber === "" &&
        (isInAccountSummary ? (
          ...
          <UICompParagraph color="ds.text.neutral.quiet" size="medium">
            {translate(I18N_KEYS.ADD_VAT_DESCRIPTION)}
          </UICompParagraph>
        ))}
      {formView === FormMode.INITIAL_VIEW ? (
          ...
          >
            {({
              ...
              <Form>
                <GridContainer
                  gap="16px"
                  gridTemplateColumns={"2fr 0.25fr 1fr"}
                >
                  <GridChild gridColumn={"1 / 2"}>
                    <Field name="vatNumber" validate={validateVatNumber}>
                      {({ field: { onChange, ...restField } }: FieldProps) => (
                        <TextInput
                          label={translate(I18N_KEYS.VAT_NUMBER)}
                          disabled={formView === FormMode.EDIT}
                          onChange={(v) => {
                            onChange(v);
                            handleOnChange();
                          }}
                  ...
                  <GridChild gridColumn={"3 / 4"} sx={{ marginTop: "16px" }}>
                    {cancelButtonState === "cancel" ? (
                      <Button
                        mood="neutral"
                        intensity="quiet"
                        layout="labelOnly"
                        disabled={formView === FormMode.SAVING}
                        onClick={() => setFormView(FormMode.INITIAL_VIEW)}
                      >
      ...
      {showSuccessMessage && (
        <UICompParagraph color="ds.text.positive.quiet" size="x-small">
  ...
  return (
    <div>
      {isInAccountSummary ? (
        ...
        <ChangePlanCard title={translate(I18N_KEYS.VAT_NUMBER)}>
          <div sx={{ marginTop: "16px" }}>
            <VatNumberForm isInAccountSummary={isInAccountSummary} />
          </div>
        </ChangePlanCard>
