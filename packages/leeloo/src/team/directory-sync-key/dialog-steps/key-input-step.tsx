...
import { jsx, Paragraph, TextArea } from '@dashlane/design-system';
...
export const KeyInputStep = ({ onSubmitKeySuccess, onSubmitKeyError, checkDirectorySyncKeyRequest, handleDismiss, handleError, }: Props) => {
    ...
    const handleKeyInputChange: React.ChangeEventHandler<HTMLTextAreaElement> = ({ target: { value: inputKey }, }) => {
        setInputKeyState({
            inputKey,
            isInputKeyInvalid: false,
        });
    };
    return (<StepDialogWithTitle onRequestClose={handleDismiss} footer={<DialogFooter primaryButtonTitle={translate(I18N_KEYS.VERIFY)} primaryButtonOnClick={handleSubmitKey} secondaryButtonTitle={translate(I18N_KEYS.POSTPONE)} secondaryButtonOnClick={handleDismiss} intent="primary"/>}>
      <Paragraph sx={{ marginBottom: '2em' }} color="ds.text.neutral.quiet">
        {translate.markup(I18N_KEYS.MESSAGE_MARKUP)}
      </Paragraph>
      <TextArea placeholder={translate(I18N_KEYS.PLACEHOLDER)} onChange={handleKeyInputChange} value={inputKeyState.inputKey} feedback={inputKeyState.isInputKeyInvalid
            ? { text: translate(I18N_KEYS.INVALID) }
            : undefined} error={inputKeyState.isInputKeyInvalid}/>
    </StepDialogWithTitle>);
};
