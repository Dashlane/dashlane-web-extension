...
export const getBillingDetailsFromPricingResults = (pricing: ComputePlanPricingResults) => {
...
export const getTaxBillingDetailsFromPricingResults = (pricing: ComputePlanPrice) => {
    const billingDetails: BillingDetails = {
        renewal: {
            seatsCount: pricing.renewal.seatsCount,
            value: pricing.renewal.price.value,
...
export const useAdditionalSeatCountPricing = (getLatestRequestedSeatCount: (() => number) | null) => {
    ...
    const computePlanPricingTax = async (seats: number) => {
        try {
            ...
            const billingDetailsResult = getTaxBillingDetailsFromPricingResults(planPrice.data);
            if (getLatestRequestedSeatCount !== null &&
            ...
            const computePlanPricingTaxError = new Error(transformUnknownErrorToErrorMessage(error));
