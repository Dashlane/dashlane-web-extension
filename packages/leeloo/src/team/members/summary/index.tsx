...
import { teamPasswordHealthApi } from '@dashlane/team-admin-contracts';
...
const I18N_KEYS = {
    ...
    REINVITE_USERS_NO_NEW_INVITES_LABEL: 'team_members_summary_reinvite_users_no_new_invites_label',
    REINVITE_USERS_NO_NEW_INVITES_CONTENT: 'team_members_summary_reinvite_users_no_new_invites_content',
    REINVITE_USERS_NO_MEMBERS_CONTENT: 'team_members_summary_reinvite_users_no_members_content',
    SECURITY_SCORE_CONTENT: 'team_members_summary_security_score_content',
    SECURITY_SCORE_CONTENT_NO_DATA: 'team_members_summary_security_score_content_no_data',
    SECURITY_SCORE_CTA: 'team_members_summary_security_score_cta',
};
const Summary = ({ onReinviteAllClick, }: {
    ...
    useEffect(() => {
        if (status === 'success' && shouldShowTrialDiscontinuedDialog !== null) {
    ...
    }, [data, status, shouldShowTrialDiscontinuedDialog]);
    return (<div sx={{
            display: 'flex',
      ...
      <div sx={{
        ...
        <SummaryCard isLoading={isLoading} title={`${remainingSeats}`} titleColor={usedSeatsRatio <= 0.5
            ? 'ds.text.positive.quiet'
            : usedSeatsRatio <= 0.9
                ? 'ds.text.warning.quiet'
                : 'ds.text.danger.quiet'} subtitle={translate(I18N_KEYS.SEAT_COUNT_CONTENT, { totalSeats })} action={{
        ...
        <div sx={{ width: '1px', backgroundColor: 'ds.border.neutral.quiet.idle' }}/>
        <SummaryCard isLoading={isLoading} title={`${numberOfInviteNotAccepted ||
            (hasNoMembersYet
                ? '-'
                : translate(I18N_KEYS.REINVITE_USERS_NO_NEW_INVITES_LABEL))}`} titleColor={hasNoMembersYet
            ? 'ds.text.neutral.quiet'
            : numberOfInviteNotAccepted === 0
                ? 'ds.text.positive.quiet'
                : 'ds.text.warning.quiet'} subtitle={numberOfInviteNotAccepted === 0
            ? hasNoMembersYet
        ...
        <div sx={{ width: '1px', backgroundColor: 'ds.border.neutral.quiet.idle' }}/>
        <SummaryCard isLoading={isLoading} title={`${teamSecurityScore || '-'}`} titleColor={teamSecurityScore >= 80
            ? 'ds.text.positive.quiet'
            : teamSecurityScore >= 40
                ? 'ds.text.warning.quiet'
                : teamSecurityScore > 0
                    ? 'ds.text.danger.quiet'
                    : 'ds.text.neutral.quiet'} subtitle={!teamSecurityScore || isLoading
            ? translate(I18N_KEYS.SECURITY_SCORE_CONTENT_NO_DATA)
            : translate(I18N_KEYS.SECURITY_SCORE_CONTENT)} action={{
