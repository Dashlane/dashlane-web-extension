...
export const MembersMissingPublicKeyErrorPopup = ({ membersWithoutPublicKey, onClose, }: {
    membersWithoutPublicKey: TeamMemberInfo[];
    onClose: () => void;
}) => {
    const { translate } = useTranslate();
    return (<SimpleDialog isOpen={Boolean(membersWithoutPublicKey.length)} footer={<DialogFooter intent="secondary" secondaryButtonTitle={translate(I18N_KEYS.ERROR_POPUP_PK_OK)} secondaryButtonOnClick={onClose}/>} onRequestClose={onClose} title={translate(I18N_KEYS.ERROR_POPUP_PK_TITLE)}>
      <div>
        ...
        <ul sx={{
          ...
          {membersWithoutPublicKey.map(({ login }) => (<li key={login}>{login}</li>))}
        </ul>
      </div>
    </SimpleDialog>);
};
