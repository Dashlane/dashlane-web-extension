...
const I18N_KEYS = {
  ...
  SINGLE: {
    ...
    REVOKE_DIALOG_MSG_B2B_ONLY:
      "team_members_revoke_single_dialog_message_b2b_only",
  ...
  MULTIPLE: {
    ...
    REVOKE_DIALOG_MSG_B2B_ONLY:
      "team_members_revoke_multiple_dialog_message_b2b_only",
...
export const RevokeDialog = ({
  ...
  const revokeMembers = async () => {
    ...
    try {
      ...
      const {
        content: { removedMembers, unproposedMembers, refusedMembers },
      } = await new TeamPlans().revokeMembers({
        auth,
        memberLogins,
      });
      ...
      refusedLogins.forEach((refusedLogin) => {
        ...
        if (!member) {
          ...
          alert.showAlert(
            translate(I18N_KEYS.REVOKE_ERROR_MSG),
            AlertSeverity.ERROR
          );
        }
      ...
      if (acceptedMembers.length) {
        alert.showAlert(
          translate(I18N_KEYS.REVOKE_SUCCESS_MSG, {
