...
import { addTeamAdmin, removeTeamAdmin, teamUpdated, } from 'libs/carbon/triggers';
...
export const RoleAssignmentDialog = ({ selectedMembers, newRole, lee, closeDialog, }: {
    ...
    const { addGroupManager, removeGroupManager } = useModuleCommands(permissionsApi);
    const updateUsersRights = async (memberUpdates: MemberUpdate[]): Promise<RoleAssignmentErrorsAndSuccesses | undefined> => {
    ...
    if (!selectedMembers.length) {
        return (<SimpleDialog isOpen footer={<DialogFooter intent="secondary" secondaryButtonTitle={translate(I18N_KEYS.RIGHTS_CHANGE_NOBODY_CANCEL)} secondaryButtonOnClick={closeDialog}/>} onRequestClose={closeDialog} title={translate(I18N_KEYS.RIGHTS_CHANGE_NOBODY_TITLE)}>
        ...
        return (<SingleRoleAssignmentDialog member={selectedMembers[0]} newRole={newRole} updateUsersRights={updateUsersRights} closeDialog={closeDialog}/>);
    }
    return (<MultipleRoleAssignmentDialog selectedMembers={selectedMembers} newRole={newRole ?? Role.Member} updateUsersRights={updateUsersRights} closeDialog={closeDialog}/>);
};
