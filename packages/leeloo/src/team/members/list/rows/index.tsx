...
const I18N_KEYS = {
  SECURITY_SCORE_BREAKDOWN_LT_FIVE:
    "team_members_row_security_score_breakdown_less_than_five",
  SECURITY_SCORE_BREAKDOWN_UNAVAILABLE:
    "team_members_row_security_score_unavailable",
  ...
  ROW_ARIA_LABEL_USERS_ACTIONS: "team_members_row_aria_label_users_actions",
};
...
export const Rows = ({
  ...
  const [quickActionMenuOpenFor, setQuickActionMenuOpenFor] = useState<
    string | null
  >(null);
  const toggleActionMenuFor = (memberLogin: string | null): void => {
    setQuickActionMenuOpenFor(
      memberLogin === quickActionMenuOpenFor ? null : memberLogin
    );
  };
  const handleMemberActionSelect = (
  ...
  const UnavailableOrLessThanFive = () => (
    <Flex gap="4px">
      <Paragraph
      ...
      <Tooltip
        wrapTrigger
        location="bottom"
        content={translate(I18N_KEYS.SECURITY_SCORE_BREAKDOWN_LT_FIVE)}
      >
        <div
          sx={{
            display: "flex",
  ...
  const getSecurityScore = (member: MemberData): JSX.Element => {
    const securityIndex = Maybe.maybe(
      member.nbrPasswords >= DISPLAY_HEALTH_SCORE_MIN_PASSWORDS ||
        (!member.nbrPasswords &&
    ...
    return securityIndex.caseOf({
      just: (value) =>
        value === 0 ? (
          <UnavailableOrLessThanFive />
        ) : (
          <SecurityScoreBadge score={securityIndex} size="small">
    ...
    switch (true) {
      ...
      case Boolean(member.lastUpdateDateUnix || member.revokedDateUnix):
        return (
          <LocalizedTimeAgo
            date={fromUnixTime(
              (member.lastUpdateDateUnix || member.revokedDateUnix) as number
            )}
          />
        );
  ...
  const getQuickActionMenu = (member: MemberData): JSX.Element => {
    return (
      <QuickActionMenu
        member={member}
        onDismiss={() => toggleActionMenuFor(member.login)}
        onMemberActionSelect={handleMemberActionSelect}
      />
    );
  };
  const getRows = (): JSX.Element[] => {
    const getPasswordLabel = (status: string, value?: number | null) =>
    ...
    return members.map((member: MemberData) => (
      <tr
        id={member.status === "removed" ? "revoked" : "active"}
        key={member.login}
        sx={mergeSx([
          SX_STYLES.ROW,
      ...
      >
          ...
          {member.login !== currentLoggedLogin &&
            member.status !== "removed" && (
              <Checkbox
                ...
                onChange={() => onMemberToggleCheck(member)}
              />
            )}
        </td>
        <td className={styles.loginCell}>
          ...
          <div
            sx={SX_STYLES.LOGIN_TEXT}
            className={"automation-tests-member-login"}
          >
        ...
        <td>
          {member.isBillingAdmin ? (
            <Tooltip
              location="top"
              content={translate(I18N_KEYS.BILLING_ADMIN_TOOLTIP)}
              sx={{ textAlign: "start" }}
            >
        ...
        <td className={styles.actionsCell} style={{ position: "relative" }}>
          ...
          {member.login !== currentLoggedLogin ? (
            <Button
              ...
              aria-label={translate(I18N_KEYS.ROW_ARIA_LABEL_USERS_ACTIONS)}
              onClick={() => toggleActionMenuFor(member.login)}
              layout="iconOnly"
              intensity="supershy"
              icon={
                <Icon
