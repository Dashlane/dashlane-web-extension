...
import { State as MembersState } from "..";
...
const I18N_KEYS = {
  FILTER_CONTROL_SEARCH_PLACEHOLDER:
    "team_members_filter_control_search_placeholder",
  ...
  FILTER_CONTROL_INCLUDE_PENDING_CHANGE_EMAIL:
    "team_members_filter_control_include_pending_email_change",
  ...
  HEADING_STANDARD_RIGHTS_TOOLTIP:
    "team_members_heading_entrylevel_tooltip_markup",
  HEADING_ACTIONS: "team_members_heading_actions",
};
const icons = {
  search: searchIcon,
};
...
export const List = ({
  ...
  const [mappedMembers, setMappedMembers] = useState<MemberData[]>(
    mapMember(members)
  );
  const [checkedMembers, setCheckedMembers] = useState<Set<MemberData>>(
    new Set()
  );
  ...
  const [filterPendingChangeEmail, setFilterPendingChangeEmail] =
    useState(false);
  const filteredResult = [...statusFilters, filterPendingChangeEmail];
  ...
  const rightsToolTip = isStandardPlan
    ? I18N_KEYS.HEADING_STANDARD_RIGHTS_TOOLTIP
    : I18N_KEYS.HEADING_RIGHTS_TOOLTIP;
  useEffect(() => {
  ...
  const getFilteredAndSortedMembers = (): MemberData[] => {
    ...
    return sortWith([comparator(prop(sortState.sortBy))])(
      mappedMembers
        .filter(
          (member: MemberData) =>
            member.filterableStatus &&
            filteredResult.includes(member.filterableStatus)
        )
        .filter((member: MemberData) =>
          member.changeLoginEmailPending && filterPendingChangeEmail
            ? filteredResult.includes(member.changeLoginEmailPending)
            : mappedMembers
        )
        .filter((member: MemberData) =>
          searchQuery
  ...
  const sortBy = (property: SortableMemberProps): void => {
    const sortOrder =
      property === sortState.sortBy && sortState.sortOrder === "asc"
        ? "desc"
        : "asc";
  ...
  const toggleStatusFilter = (status: MemberStatusFilter): void => {
    ...
    setStatusFilters((currFilters) =>
      currFilters.includes(status)
  ...
  const onToggleStatusPendingChangeEmail = () =>
    setFilterPendingChangeEmail(
      (filterPendingChangeEmail) => !filterPendingChangeEmail
    );
  ...
  const sortByCompromisedPasswords = () =>
    sortBy("sortableCompromisedPasswords");
    ...
    switch (selectedRole) {
      case "member":
        members = members.filter(
          (member: MemberData) => member.isTeamCaptain || member.isGroupManager
        );
        break;
      case "teamCaptain":
        members = members.filter(
          (member: MemberData) =>
            !member.isTeamCaptain && member.status === "accepted"
        );
        break;
      case "groupManager":
        members = members.filter(
          (member: MemberData) =>
            !member.isGroupManager && member.status === "accepted"
        );
    }
  ...
  const toggleCheckFor = (member: MemberData): void => {
    setCheckedMembers((prevCheckedMembers) => {
      const isCheckedMember = Array.from(prevCheckedMembers).find(
        (checkedMember) => checkedMember.login === member.login
      );
  ...
  const checkAll = () => {
    ...
    Array.from(checkedMembers).forEach((checkedMember) =>
      newCheckedMembers.add({ ...checkedMember })
    );
    setCheckedMembers(newCheckedMembers);
  };
  const headerCheckbox = (membersToDisplay: Array<MemberData>) => {
    const numberOfCheckedMembers = membersToDisplay.filter((member) =>
      Array.from(checkedMembers).find(
        (checkedMember) => checkedMember.login === member.login
      )
    ).length;
    const isLoggedInUserDisplayed = membersToDisplay.find(
      (member) => member.login === currentLoggedLogin
    );
  ...
  const membersToDisplay = filteredAndSortedMembers.slice(
    startIndex,
  ...
  const columns = [
    ...
    },
    {
      ...
      sortOrder:
        sortState.sortBy !== "sortableSecurityScore"
          ? undefined
          : sortState.sortOrder,
    ...
    },
    {
      ...
      sortOrder:
        sortState.sortBy !== "sortablePasswords"
          ? undefined
          : sortState.sortOrder,
    ...
    },
    {
      ...
      sortOrder:
        sortState.sortBy !== "sortableSafePasswords"
          ? undefined
          : sortState.sortOrder,
    ...
    },
    {
      ...
      sortOrder:
        sortState.sortBy !== "sortableWeakPasswords"
          ? undefined
          : sortState.sortOrder,
    ...
    },
    {
      ...
      sortOrder:
        sortState.sortBy !== "sortableReused" ? undefined : sortState.sortOrder,
    ...
    },
    {
      ...
      sortOrder:
        sortState.sortBy !== "sortableCompromisedPasswords"
          ? undefined
          : sortState.sortOrder,
    ...
    },
    {
      ...
      sortOrder:
        sortState.sortBy !== "sortableLastActivity"
          ? undefined
          : sortState.sortOrder,
    ...
    },
    {
      ...
      tooltipLabel: (
        ...
        >
          {translate.markup(rightsToolTip)}
        </div>
      ),
      onClick: () => sortByRights(),
      sortOrder:
        sortState.sortBy !== "sortableRights" ? undefined : sortState.sortOrder,
      headerKey: "permissions",
    },
    {
      headerLabel: translate(I18N_KEYS.HEADING_ACTIONS),
      headerKey: "actions",
    },
  ];
  return (
    <div className={styles.container}>
      <div sx={SX_STYLES.TOP}>
        <div className={styles.tableHeaderContainer}>
          <TextField
            ...
            onChange={searchFor}
            placeholder={translate(I18N_KEYS.FILTER_CONTROL_SEARCH_PLACEHOLDER)}
          ...
          <div className={styles.topContainer}>
            <div className={styles.topLeft}>
              <div className={styles.statusFilters}>
                <Checkbox
                  label={translate(I18N_KEYS.FILTER_CONTROL_INCLUDE_ACTIVE)}
                  onChange={onToggleStatusAccepted}
                  checked={statusFilters.includes("accepted")}
                />
                <Checkbox
                  label={translate(I18N_KEYS.FILTER_CONTROL_INCLUDE_INVITED)}
                  onChange={onToggleStatusPending}
                  checked={statusFilters.includes("pending")}
                />
                <Checkbox
                  label={translate(I18N_KEYS.FILTER_CONTROL_INCLUDE_REVOKED)}
                  onChange={onToggleStatusRemoved}
                  checked={statusFilters.includes("removed")}
                />
                {hasChangeLoginEmailFeatureFlip ? (
                  <Checkbox
                    label={translate(
                      I18N_KEYS.FILTER_CONTROL_INCLUDE_PENDING_CHANGE_EMAIL
                    )}
                    onChange={onToggleStatusPendingChangeEmail}
                    checked={filterPendingChangeEmail}
                  />
                ) : null}
              </div>
            </div>
            {checkedMembers.size > 0 && [
              <MultipleAssignMenu
                key="dropdown"
                onMultipleReassignActionSelect={onMultipleReassignActionSelect}
              />,
              <SecondaryButton
                key="button"
                classNames={[styles.revokeButton]}
                label={translate(I18N_KEYS.REVOKE_MULTIPLE_LABEL)}
                disabled={!hasRevokableMemberChecked()}
                onClick={onRevokeSelect}
              />,
            ]}
            <div className={styles.topRight}>
              <div className={styles.topPagination}>
                ...
                <Pagination
                  ...
                  isShort={true}
                  itemsLength={filteredAndSortedMembers.length}
              ...
              {members.length > 1 && (
                <GlobalActionMenu
                  members={members}
                  onMembersActionSelect={onMembersActionSelect}
                />
              )}
            </div>
          </div>
      ...
      <table className={styles.table}>
        ...
        <Rows
          ...
          currentLoggedLogin={currentLoggedLogin}
          members={membersToDisplay}
          onMemberActionSelect={onMemberActionSelect}
          onMemberToggleCheck={toggleCheckFor}
        />
      </table>
