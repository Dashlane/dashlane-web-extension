...
import { Heading, IndeterminateLoader, jsx, Paragraph, Toggle, } from '@dashlane/design-system';
...
export const JustInTimeProvisioning = () => {
    ...
    useEffect(() => {
        if (ssoProvisioningState.status === DataStatus.Success) {
            const inferredSsoState = ssoProvisioningState.data?.global.inferredSsoState ?? '';
            const isSelfHosted = [
                'SelfHostedComplete',
                'SelfHostedIncomplete',
            ].includes(inferredSsoState);
            if (isSelfHosted) {
                carbonConnector.getTeamDomains().then((result) => {
                    if (result.success) {
                        setHasValidDomain(result.domains.some((domain) => domain.status === 'valid'));
                    }
                });
            }
            else {
                setHasValidDomain((ssoProvisioningState.data?.domainSetup ?? []).some((setup) => setup.verificationStatus === DomainVerificationStatus.enum.valid));
            }
        }
    ...
    return (<div sx={{
      ...
      <div sx={{ display: 'flex', flexDirection: 'row', gap: '16px' }}>
        ...
        <div>
          {status === DataStatus.Loading ? (<IndeterminateLoader mood="brand"/>) : (<Toggle disabled={!hasValidDomain} onChange={handleToggleChange} defaultChecked={!!data?.teamPolicies.ssoProvisioning}/>)}
        </div>
      </div>
    </div>);
};
