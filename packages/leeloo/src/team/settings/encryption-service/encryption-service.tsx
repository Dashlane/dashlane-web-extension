...
import {
  ...
  EncryptionServiceRestartStatusCodes,
...
const I18N_KEYS = {
  ...
  GO_TO_ES_LABEL_HELPER_AWS:
    "team_settings_encryption_service_go_to_label_helper_aws_markup",
  GO_TO_ES_LABEL_HELPER_AZURE:
    "team_settings_encryption_service_go_to_label_helper_azure_markup",
  ...
  ENTER_ENDPOINT_LABEL_HELPER_DEFAULT:
    "team_settings_encryption_service_enter_endpoint_label_helper_no_selection",
  ENTER_ENDPOINT_LABEL_HELPER_AWS:
    "team_settings_encryption_service_enter_endpoint_helper_aws_linux",
  ENTER_ENDPOINT_LABEL_HELPER_AZURE:
    "team_settings_encryption_service_enter_endpoint_helper_azure",
  SELECT_HOST: "team_settings_encryption_service_select_host",
  AWS_PLACEHOLDER:
    "team_settings_encryption_service_enter_endpoint_placeholder_aws_linux",
  AZURE_PLACEHOLDER:
  ...
  MIGRATE_SSO_DESCRIPTION:
    "team_settings_encryption_service_migrate_sso_description_markup",
  CURRENT_ENDPOINT_LABEL:
    "team_settings_encryption_service_current_endpoint_label",
  SSO_CONNECTOR_KEY_PLACEHOLDER:
    "team_settings_encryption_service_sso_connector_key_placeholder",
};
const I18N_ERRORS = {
  ...
  ERROR_INVALID_ENCRYPTION_SERVICE_ENDPOINT:
    "team_settings_encryption_service_error_invalid_endpoint",
  ERROR_INVALID_SSO_CONNECTOR_KEY:
    "team_settings_encryption_service_error_invalid_ssoConnectorKey",
};
const ENDPOINT_PROTOCOL_PREFIX = "__REDACTED__";
const serviceHostUrls: {
  [K in EncryptionServiceDeploymentLocation]: string;
} = {
  AWS: "__REDACTED__",
  "Microsoft Azure": "__REDACTED__",
};
...
export const inferESEndpoint = (
  ...
  if (deploymentLocation === "Microsoft Azure") {
    ...
    return (
      domain.slice(0, AZURE_MAX_LENGTH - AZURE_APPEND.length) + AZURE_APPEND
    );
  }
...
export const EncryptionService = ({
  migratingFromSSOConnector = false,
  esConfig,
  onSave,
  teamSettings,
  updateTeamSettings,
  isSettingupScim,
}: EncryptionServiceProps) => {
  ...
  const getGotoServiceHostHelperText = (
    serviceHost: DeploymentLocationOptions
  ) => {
    if (!serviceHost) {
      serviceHost = "AWS";
    }
    return translate.markup(
      goToServiceHostHelperMap[serviceHost],
      {
  ...
  const initialFormValues: FormFields = {
    encryptionServiceEndpoint: (esEndpoint ?? "").replace(
  ...
  const validateDeploymentLocation = (
    value: FormFields["deploymentLocation"]
  ) => (value ? null : translate(I18N_ERRORS.ERROR_SELECT_SERVICE));
  ...
  const validateEncryptionServiceEndpoint = (
    value: FormFields["encryptionServiceEndpoint"]
  ): null | string => {
    const errorMessage = translate(
      I18N_ERRORS.ERROR_INVALID_ENCRYPTION_SERVICE_ENDPOINT
    );
    if (!value) {
      return errorMessage;
    }
  ...
  const handleOnSaveAndGenerateConfig = async (
    fieldValues: FormFields,
    { setFieldValue, setFieldTouched }: FormikHelpers<FormFields>
  ) => {
    setFormError("");
    setIsSaving(true);
    setRestartEsFailed(false);
    const { deploymentLocation, encryptionServiceEndpoint, ssoConnectorKey } =
      fieldValues;
    ...
    try {
      const createConfigRequest: GenerateAndSaveEncryptionServiceConfigRequest =
        {
          ...
          encryptionServiceEndpoint:
            ENDPOINT_PROTOCOL_PREFIX + formatUrl(encryptionServiceEndpoint),
      ...
      const createConfigResponse =
        await carbonConnector.generateAndSaveEncryptionServiceConfig(
          createConfigRequest
        );
      ...
      if (
        fieldValues[FIELDS.ENCRYPTION_SERVICE_ENDPOINT] !==
        initialFormValues.encryptionServiceEndpoint
      ) {
        let newSettings: TeamSettings = {
      ...
      setIsSaving(false);
      if (
        createConfigResponse.data.encryptionServiceReloadingStatus !==
  ...
  return (
    <Formik
      onSubmit={handleOnSaveAndGenerateConfig}
      initialValues={initialFormValues}
      onReset={reRouteBackToRoot}
    >
      {(formikProps: FormikProps<FormFields>) => {
        return (
          <Form autoComplete="off">
            <CatchUnsavedChanges
              dirty={formikProps.dirty && !isSaving && !restartEsFailed}
            />
            <Card>
              ...
              <CardContent>
                  ...
                  <>
                    ...
                    >
                      ...
                      <GridChild gridArea="input">
                        <Field
                          name={FIELDS.SSO_CONNECTOR_KEY}
                          validate={validateSSOConnectorKey}
                        >
                          {({
                            field: { onChange, ...restField },
                            meta: { error, touched },
                          }: FormFieldProps<FormFields["ssoConnectorKey"]>) => {
                            return (
                              <TextInput
                                onChange={(v) => {
                                  onChange(v);
                                ...
                                placeholder={translate(
                                  I18N_KEYS.SSO_CONNECTOR_KEY_PLACEHOLDER
                                )}
                                fullWidth
                                feedbackText={
                ...
                <GridContainer
                  gridTemplateAreas="'label select' 'desc select'"
                  gap="8px"
                >
                  <GridChild
                    gridArea="label"
                  ...
                  <GridChild gridArea="desc" as={Paragraph}>
                    {translate.markup(
                      I18N_KEYS.DEPLOY_ES_LABEL_HELPER,
                      {
                        customerSupport: "__REDACTED__",
                      },
                      {
                        linkTarget: "_blank",
                      }
                  ...
                  <GridChild gridArea="select">
                    <Field
                      name={FIELDS.DEPLOYMENT_LOCATION}
                      validate={validateDeploymentLocation}
                    >
                      {({
                        field: { onChange, ...restField },
                        meta: { error, touched },
                      }: FormFieldProps<FormFields["deploymentLocation"]>) => (
                        <Select
                          onChange={(v) => {
                            formikProps.setFieldValue(
                              FIELDS.ENCRYPTION_SERVICE_ENDPOINT,
                              inferESEndpoint(associatedEmail, v.target.value)
                            );
                            setConfigRegenRequired(
                              initialFormValues.deploymentLocation !==
                                v.target.value
                            );
                            logSelfHostedSSOSetupStep({
                              ssoSetupStep:
                                SsoSetupStep.SelectEncryptionServicePlatform,
                              encryptionServicePlatformSelected:
                                v.target.value === "AWS"
                          ...
                          options={Object.keys(endpointLabelMap)}
                          placeholder={translate(I18N_KEYS.SELECT_HOST)}
                          intent={touched && error ? "error" : undefined}
                          feedbackText={touched && error ? error : undefined}
                          readOnly={formikProps.isSubmitting}
                          {...restField}
                        />
                      )}
                ...
                <GridContainer gridTemplateColumns="auto" gap="8px">
                  ...
                  >
                    {translate(
                      endpointLabelMap[
                        formikProps.values[FIELDS.DEPLOYMENT_LOCATION]
                      ] ?? I18N_KEYS.ENTER_ENDPOINT_LABEL_HELPER_DEFAULT
                    )}
                  </GridChild>
                  {migratingFromSSOConnector ? (
                    <Paragraph>
                  ...
                  <Field
                    name={FIELDS.ENCRYPTION_SERVICE_ENDPOINT}
                    validate={validateEncryptionServiceEndpoint}
                  >
                    {({
                      field: { onChange, ...restField },
                      meta: { error, touched },
                      form: {
                        values: { deploymentLocation },
                      },
                    }: FormFieldProps<
                      FormFields["encryptionServiceEndpoint"]
                    >) => {
                      return (
                        <TextInput
                          sx={{ pl: 0 }}
                          ...
                          id="endpointInput"
                          placeholder={translate(
                            endpointPlaceholderMap[deploymentLocation] ??
                              I18N_KEYS.AWS_PLACEHOLDER
                          )}
                          fullWidth
                          feedbackText={touched && error ? error : undefined}
                          feedbackType={touched && error ? "error" : undefined}
                          aria-invalid={touched && !!error}
                          readOnly={formikProps.isSubmitting}
                          {...restField}
                        />
                ...
                <Field name={FIELDS.CONFIG}>
                  {({
                    ...
                    meta,
                    form: {
                      values: { deploymentLocation, ssoConnectorKey },
                    },
                  }: FormFieldProps<FormFields["config"]>) => (
                    <BasicConfigField
                      configRegenRequired={configRegenRequired}
                      disabled={
                        !deploymentLocation ||
                        (migratingFromSSOConnector && !ssoConnectorKey)
                      }
                      lastGeneratedTimeStamp={esConfig?.lastGeneratedTimeStamp}
                      value={field.value}
                      error={
                        meta.error ??
                      ...
                      isLoading={isSaving || formikProps.isSubmitting}
                    />
                  )}
                </Field>
                <hr />
                ...
                >
                  ...
                  >
                    {getGotoServiceHostHelperText(
                      formikProps.values.deploymentLocation
                    )}
                  </GridChild>
                  <GridChild
                    gridArea="button"
                    ...
                    type="button"
                    mood={
                      formikProps.values.config && !configRegenRequired
                        ? "brand"
                        : "neutral"
                    }
                    disabled={
                    ...
                    onClick={() => {
                      logSelfHostedSSOSetupStep({
                        ssoSetupStep:
                          SsoSetupStep.AddConfigurationToServiceHost,
                      });
                      openUrl(
                        serviceHostUrls[
                          formikProps.values[FIELDS.DEPLOYMENT_LOCATION]
