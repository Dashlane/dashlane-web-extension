...
const ENCRYPTION_SERVICE_HELP_LINK = '*****';
...
export const EncryptionServiceSetup = ({ esConfig, loading, parentPath, disableSetupButton, }: EncryptionServiceSetupProps) => {
    const { translate } = useTranslate();
    const history = useHistory();
    const isPrimaryCta = !esConfig && !loading;
    return (<Card>
      <GridContainer as={CardContent} gridTemplateAreas="'header button' 'description button' 'status .'" gridTemplateColumns="1fr auto" gap="8px" sx={{ bg: 'white', border: 'none' }}>
        <GridChild gridArea="header">
          {!esConfig ? (<Heading as="h3" color="ds.text.neutral.quiet" textStyle="ds.title.supporting.small">
        ...
        <GridChild gridArea="description">
          ...
          <br />
          <Paragraph as="a" textStyle="ds.title.block.small" href={ENCRYPTION_SERVICE_HELP_LINK} color="ds.text.brand.standard" rel="noopener noreferrer" target="_blank">
            {translate(I18N_KEYS.ES_DESCRIPTION_LINK)}
          </Paragraph>
        </GridChild>
        <GridChild id="esSetupButton" as={Button} disabled={loading || disableSetupButton} mood={isPrimaryCta ? 'brand' : 'neutral'} intensity={isPrimaryCta ? 'catchy' : 'quiet'} gridArea="button" type="button" alignSelf="center" size="large" onClick={() => {
        ...
        {esConfig ? (<GridChild gridArea="status" sx={{ mt: '20px' }}>
