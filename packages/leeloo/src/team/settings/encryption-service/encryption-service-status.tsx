...
export const EncryptionServiceStatus = ({
  config,
}: EncryptionServiceStatusProps) => {
  const { translate } = useTranslate();
  const { teamDevice, refreshTeamDevice, teamDeviceLoading } = useTeamDevice(
    config.deviceAccessKey
  );
  const ssoTooltipInfoText = translate.markup(
    I18N_KEYS.TOOLTIP_INFO,
    {
      updateServiceUrl: "__REDACTED__",
    },
    { linkTarget: "_blank" },
    {
      ...
      {teamDeviceLoading ? (
        ...
        <Flex flexDirection="column" gap="5px">
          ...
          <Flex gap="8px">
            ...
            {teamDevice?.hasLatestVersion === false ? (
              <Tooltip wrapTrigger content={ssoTooltipInfoText}>
                <Badge
                  mood="danger"
                  iconName="FeedbackWarningOutlined"
                  layout="iconLeading"
                  label={translate(I18N_KEYS.ES_AVAILABLE)}
                />
              </Tooltip>
            ) : null}
          </Flex>
