...
interface RestartEncryptionServiceInfoboxProps
  extends Omit<Parameters<typeof Infobox>[0], "title"> {
  context?: RestartESContext;
}
export const RestartEncryptionServiceInfobox = ({
  context = "GENERIC",
  ...infoBoxProps
}: RestartEncryptionServiceInfoboxProps) => {
  const { esConfig, esConfigLoading } = useEncryptionServiceConfig();
  const { teamDevice, teamDeviceLoading } = useTeamDevice(
    esConfig?.deviceAccessKey
  );
  const [shouldRestartES, setShouldRestartES] = useState(false);
  const { translate } = useTranslate();
  const restartESInfoBoxContent = {
    CONTENT: translate.markup(
      "team_settings_encryption_service_restart_failed_info_content_markup",
      {
        helpArticle:
          restartESGuideUrls[esConfig?.deploymentLocation ?? ""] ?? "",
        supportLink: "__REDACTED__",
      },
      {
        linkTarget: "_blank",
      }
  ...
  const RESTART_ES_INFOBOX_COPY: RestartInfoBoxCopy = {
    GENERIC: {
      TITLE: translate(
        "team_settings_encryption_service_restart_failed_info_title"
      ),
      ...restartESInfoBoxContent,
    },
    SCIM: {
      TITLE: translate(
        "team_settings_encryption_service_restart_failed_info_title_scim"
      ),
      ...restartESInfoBoxContent,
    },
    ES: {
      TITLE: translate(
        "team_settings_encryption_service_restart_failed_info_title_es"
      ),
      ...restartESInfoBoxContent,
    },
    SSO: {
      TITLE: translate(
        "team_settings_encryption_service_restart_failed_info_title_sso"
      ),
      ...restartESInfoBoxContent,
    },
  };
  useEffect(() => {
    ...
    if (!isDataLoading && teamDevice && esConfig) {
      const shouldRenderESRestart =
        (!teamDevice.hasLatestConfig && context === "GENERIC") ||
        context !== "GENERIC";
  ...
  return shouldRestartES ? (
    <Infobox
      mood="warning"
      size="large"
      title={RESTART_ES_INFOBOX_COPY[context].TITLE}
      description={RESTART_ES_INFOBOX_COPY[context].CONTENT}
      actions={
