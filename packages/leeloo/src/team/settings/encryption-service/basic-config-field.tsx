import { Button, Flex, Paragraph, TextArea } from "@dashlane/design-system";
...
const I18N_KEYS = {
  GENERATE_CONFIGURATION_BUTTON:
    "team_settings_encryption_service_generate_configuration_button",
  GENERATE_CONFIGURATION_LABEL:
    "team_settings_encryption_service_generate_configuration_label",
  GENERATE_CONFIGURATION_LABEL_HELPER:
    "team_settings_encryption_service_generate_configuration_label_helper",
  COPY_CONFIGURATION_BUTTON: "input_copy_button",
  DOWNLOAD_CONFIGURATION_BUTTON:
    "team_settings_encryption_service_config_download_button",
  GENERATE_CONFIGURATION_BUTTON_RESET:
    "team_settings_encryption_service_generate_configuration_reset_button",
  GENERATE_CONFIGURATION_TIMESTAMP:
    "team_settings_encryption_service_generate_configuration_timestamp",
};
...
export const BasicConfigField = ({
  ...
  const lastGeneratedTimeStampDisplay = lastGeneratedTimeStamp ? (
    <Flex gap="4px" sx={{ mt: "8px" }} justifyContent="center">
      ...
      <Paragraph textStyle="ds.body.reduced.regular">
        {fromUnixTime(lastGeneratedTimeStamp).toLocaleString(
          navigator.language,
          {
            ...LOCALE_FORMAT.lll,
            timeZoneName: "short",
    ...
    >
      ...
      <GridChild gridArea="button">
        <Button
          ...
          mood={value && !configRegenRequired ? "neutral" : "brand"}
          intensity={value && !configRegenRequired ? "quiet" : "catchy"}
          fullsize
          type="submit"
          isLoading={isLoading}
        >
      ...
      {value ? (
        <>
          <GridChild gridArea="config" sx={{ marginTop: "18px" }}>
            <TextArea
              aria-labelledby={ES_CONFIG_LABEL_ID}
              readOnly
              disabled={configRegenRequired}
              value={value}
              feedback={
                error
                  ? {
          ...
          >
            <CopyButton
              disabled={configRegenRequired}
              copyValue={value}
              buttonText={translate(I18N_KEYS.COPY_CONFIGURATION_BUTTON)}
              fullsize
            />
            <DownloadButton
              disabled={configRegenRequired}
              buttonText={translate(I18N_KEYS.DOWNLOAD_CONFIGURATION_BUTTON)}
