...
import { colors, GridChild, GridContainer, jsx, OpenWebsiteIcon, Paragraph, } from '@dashlane/ui-components';
...
export const TestSso = ({ actionsDisabled, ssoServiceProviderUrl, }: SsoTestProps) => {
    ...
    return (<GridContainer gap="8px" gridTemplateAreas="'header buttons' 'description buttons' " gridTemplateColumns="1fr auto">
      ...
      <GridContainer gap="8px" gridTemplateAreas="'test copy'" as={GridChild} gridArea="buttons">
        <GridChild gridArea="test">
          <DisabledButtonWithTooltip onClick={openServiceProviderUrl} size="medium" aria-label={translate(I18N_KEYS.TEST_BUTTON)} disabled={actionsDisabled ?? !serviceProviderUrl} content={translate(I18N_KEYS.SSO_SETUP_INCOMPLETE)}>
            {translate(I18N_KEYS.TEST_BUTTON)}
            <OpenWebsiteIcon sx={{ ml: '4px' }} color="white"/>
          </DisabledButtonWithTooltip>
        </GridChild>
        <GridChild gridArea="copy">
          <CopyButton buttonText={translate(I18N_KEYS.SSO_COPY_URL_BUTTON)} disabled={actionsDisabled ?? !serviceProviderUrl} copyValue={serviceProviderUrl ?? ''}/>
        </GridChild>
      </GridContainer>
    </GridContainer>);
};
