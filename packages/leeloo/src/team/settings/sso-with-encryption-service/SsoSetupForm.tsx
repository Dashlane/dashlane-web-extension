  ...
  const handleSubmit = async (
    ...
    try {
      ...
      formikHelpers.setFieldError(
        FIELDS.METADATA,
        translate(I18N_METADATA_ERROR_KEYS.GENERIC_ERROR)
      );
      setIsSaving(false);
    }
  };
        ...
        return (
          <Form autoComplete="off" noValidate>
            ...
            <Card>
              ...
              <CardContent>
                <DomainFields
                  fieldArrayName={FIELDS.DOMAINS}
                  domainLoadError={domainLoadError}
                  refreshDomains={loadDomains}
                />
                <hr />
                <Infobox
                  title={translate.markup(I18N_KEYS.COPY_INFO_TO_IDP)}
                  sx={{ mb: "32px" }}
                />
                <GridContainer
                ...
                >
                  <IdpInputField
                    id="entity-id-input"
                    inputValue={makeEntityId(ssoServiceProviderUrl)}
                    labelText={translate(I18N_KEYS.ENTITY_ID_LABEL)}
                    disabled={teamSettingsLoading || !ssoServiceProviderUrl}
                    loading={teamSettingsLoading}
                  />
                  <IdpInputField
                    id="acs-url-input"
                    inputValue={makeAcsUrl(ssoServiceProviderUrl)}
                    labelText={translate(I18N_KEYS.ACS_URL_LABEL)}
                    disabled={teamSettingsLoading || !ssoServiceProviderUrl}
                    loading={teamSettingsLoading}
                  />
                </GridContainer>
