...
import { DeactivateDomainDialog } from "../../sso/domain/steps/deactivate-dialog";
...
export const DomainFields = <T extends string>({
  fieldArrayName,
  domainLoadError,
  refreshDomains,
}: {
  ...
  >({});
  const [domainsUpdating, setDomainsUpdating] = useState<
    Record<string, boolean>
  >({});
  const [verifyingDomainName, setVerifyingDomainName] = useState("");
  const [domainRemovalRequested, setDomainRemovalRequested] =
  ...
  const addDomain = async (url: string): Promise<void> => {
    ...
    if (
      values[fieldArrayName].reduce((previousValue, currentValue) => {
    ...
    if (!registerResult.success) {
      const errorI18nKey: Partial<
        Record<RegisterTeamDomainResultFailure["error"]["code"], string>
      > = {
      ...
      throw new Error(
        translate(
          errorI18nKey[registerResult.error.code] ?? I18N_KEYS.REGISTER_INVALID
        )
      );
    }
  };
  ...
  return (
    <>
      <Paragraph
        as="label"
        htmlFor={`${fieldArrayName}.${values[fieldArrayName].length - 1}`}
        bold
        ...
        {(arrayHelpers: FieldArrayRenderProps) => (
          ...
          >
            {values[arrayHelpers.name].map((domain: Domain, index: number) => {
              ...
              const endIcon = isUpdating ? (
                <IndeterminateLoader size="medium" />
              ) : isValid ? (
                <CheckCircleIcon color={accessibleValidatorGreen} />
              ) : isNotVerified ? (
                <CrossCircleIcon
                  color={
                    domain.status === DomainStatus.pending
              ...
              return (
                <Fragment key={`${arrayHelpers.name}.${domain.id ?? index}`}>
                  <DomainField
                    placeholder={translate(I18N_KEYS.DOMAIN_FIELD_PLACEHOLDER)}
                    fieldName={`${arrayHelpers.name}.${index}`}
                    readOnly={!!domain.status ?? isUpdating}
                    autoFocus={index === values[arrayHelpers.name].length - 1}
                    endIcon={endIcon}
                    feedbackType={
                      !domain.status && registrationErrors[domain.name]
                        ? "error"
                        : undefined
                    }
                    feedbackText={
                    ...
                    actions={
                      <>
                        ...
                        {domain.name.length && domain.status !== undefined ? (
                          <IconButton
                            nature="secondary"
                            type="button"
                            disabled={isUpdating}
                            onClick={() => setDomainRemovalRequested(domain)}
                            icon={<TrashIcon />}
                          />
                        ) : null}
                      </>
      ...
      {verifyingDomainName ? (
        <VerifyDomainDialog
          domainName={verifyingDomainName}
          onSuccess={async () => {
            setVerifyingDomainName("");
            await refreshDomains();
          }}
          onDismiss={() => setVerifyingDomainName("")}
        />
      ) : null}
      {domainRemovalRequested ? (
        <DeactivateDomainDialog
          domain={domainRemovalRequested}
          onDismiss={() => setDomainRemovalRequested(null)}
          onConfirm={async () => {
