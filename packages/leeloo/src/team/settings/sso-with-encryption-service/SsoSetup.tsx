...
export const SsoSetup = ({
  ...
  const [ssoSetupStatus, setSsoSetupStatus] = useState<SSOSetupStatus>(
    SSOSetupStatus.UNKNOWN
  );
  ...
  useEffect(() => {
    ...
    carbonConnector.getTeamDomains().then((result) => {
      ...
      const isInNitro =
        ssoState.data?.ssoSolution === SsoSolution.enum.confidentialSaml;
      if (!ssoServiceProviderUrl || isInNitro) {
        setSsoSetupStatus(SSOSetupStatus.UNAVAILABLE);
        return;
      }
      const hasValidDomains = result.domains.some(
        ({ status }) => status === "valid"
      );
  ...
  }, [ssoState.data, ssoIdpMetadata, ssoServiceProviderUrl]);
  const setupDisabled =
    ssoSetupStatus === SSOSetupStatus.UNAVAILABLE ||
    teamSettingsLoading ||
    disableSetupButton;
  ...
  return (
    <Card>
      ...
      >
        ...
        <GridChild gridArea="button" alignSelf="center">
          <DisabledButtonWithTooltip
            id="ssoSetupButton"
            loading={
              teamSettingsLoading || ssoSetupStatus === SSOSetupStatus.LOADING
            }
            ...
            content={translate(I18N_KEYS.CONFIGURE_ES_TOOLTIP)}
            neverShowTooltip={!isSsoCapable}
            mood={ssoSetupStarted ? "neutral" : "brand"}
            intensity={ssoSetupStarted ? "quiet" : "catchy"}
          >
            {ssoSetupStarted
      ...
      {ssoSetupStarted ? (
        <>
          <TestSso
            actionsDisabled={ssoSetupStatus !== SSOSetupStatus.COMPLETE}
