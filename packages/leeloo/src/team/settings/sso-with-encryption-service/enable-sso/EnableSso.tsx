...
import { AlertSeverity, colors, GridChild, GridContainer, jsx, Paragraph, } from '@dashlane/ui-components';
...
import { confidentialSSOApi } from '@dashlane/sso-scim-contracts/src';
...
const I18N_KEYS = {
    ...
    CONFIRM_DIALOG_PRIMARY_ACTION_LABEL: 'team_settings_enable_sso_confirm_dialog_primary_action_label',
};
export const EnableSso = ({ actionsDisabled, ssoEnabled, updateTeamSettings, loading, }: EnableSsoProps) => {
    ...
    const turnOnSso = async () => {
        ...
        try {
            await updateTeamSettings({ ssoEnabled: true });
    ...
    return (<>
      <GridContainer gap="8px" gridTemplateAreas="'header buttons' 'description toggle' " gridTemplateColumns="1fr auto">
        ...
        <GridChild gridArea="toggle">
          <Toggle disabled={actionsDisabled || loading} checked={ssoEnabled} onChange={handleToggleDialogVisibility}/>
        </GridChild>
      </GridContainer>
      {showDisableDialog ? (<DisableSsoDialog closeDialog={closeDisableDialog}/>) : null}
      {showConfirmDialog ? (<ConfirmDialog title={translate(I18N_KEYS.CONFIRM_DIALOG_TITLE)} body={translate(I18N_KEYS.CONFIRM_DIALOG_BODY)} primaryActionLabel={translate(I18N_KEYS.CONFIRM_DIALOG_PRIMARY_ACTION_LABEL)} onCancel={onCancelConfirmationDialog} onConfirm={onConfirmConfirmationDialog}/>) : null}
    </>);
};
