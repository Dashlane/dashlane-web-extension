...
import { BasicConfig, GetEncryptionServiceConfigResult, GetEncryptionServiceConfigsFailure, UnknownTeamAdminError, } from '@dashlane/communication';
import { carbonConnector } from 'libs/carbon/connector';
interface UseEncryptionServiceConfig {
    esConfigLoading: boolean;
    esConfig?: BasicConfig;
    refreshEncryptionServiceConfig: (abortController?: AbortController) => Promise<GetEncryptionServiceConfigResult>;
}
export const useEncryptionServiceConfig = (): UseEncryptionServiceConfig => {
    ...
    const loadEncryptionServiceConfig = useCallback(async () => {
        try {
            const getESConfigResult: GetEncryptionServiceConfigResult = await carbonConnector.getEncryptionServiceConfig();
    ...
    const refreshEncryptionServiceConfig = useCallback(async (abortController: AbortController = new AbortController()) => {
        if (!abortController.signal.aborted) {
            setESConfigLoading(true);
        }
        const getEncryptionServiceConfigResult = await loadEncryptionServiceConfig();
        if (!abortController.signal.aborted) {
