...
import { GetTeamDeviceResult, GetTeamDeviceResultFailure, TeamDevice, UnknownTeamAdminError, } from '@dashlane/communication';
import { carbonConnector } from 'libs/carbon/connector';
interface UseTeamDevice {
    teamDeviceLoading: boolean;
    teamDevice?: TeamDevice;
    refreshTeamDevice: (abortController?: AbortController) => Promise<GetTeamDeviceResult>;
}
export function useTeamDevice(teamDeviceAccessKey?: string): UseTeamDevice {
    const memoizedTeamDeviceAccessKey = useMemo(() => teamDeviceAccessKey, [teamDeviceAccessKey]);
    ...
    const loadTeamDevice = useCallback(async () => {
        ...
        try {
            ...
            const getTeamDeviceResult: GetTeamDeviceResult = await carbonConnector.getTeamDevice({
    ...
    const refreshTeamDevice = useCallback(async (abortController: AbortController = new AbortController()) => {
