...
import { Policies, teamPlanDetailsApi, TeamPolicyUpdates, } from '@dashlane/team-admin-contracts';
...
export const useTeamAdmin = (): UseTeamAdminResponse => {
    const { data: teamIdData, status: teamIdStatus } = useModuleQuery(teamPlanDetailsApi, 'getTeamId');
    const { data: teamPoliciesData, status: teamPoliciesStatus } = useModuleQuery(teamPlanDetailsApi, 'getTeamPolicies');
    const { editTeamPolicies } = useModuleCommands(teamPlanDetailsApi);
    if (teamPoliciesStatus === DataStatus.Error ||
        teamIdStatus === DataStatus.Error) {
        return {
            status: DataStatus.Error,
        };
    }
    if (teamPoliciesStatus === DataStatus.Loading ||
    ...
    return {
        ...
        editTeamPolicies: (updates: TeamPolicyUpdates) => editTeamPolicies({
