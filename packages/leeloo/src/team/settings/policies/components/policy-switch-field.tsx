...
export const PolicySwitchField = ({
  field,
  editSettings,
  checkForAuthenticationError,
  policies,
}: PolicySwitchFieldProps) => {
  ...
  const confirm =
    field.value ?? false ? field.confirmDisable : field.confirmEnable;
  ...
  const handleMasterPasswordFieldChanged = useCallback(
  ...
  return (
    <>
      <SwitchWithFeedback
        ...
        isDisabled={field.constraintsFromOtherFields?.disabledWhen?.some(
          (disabledCondition) =>
            policies && disabledCondition?.condition?.(policies)
        )}
        disabledFeedbackMessage={
          field.constraintsFromOtherFields?.disabledWhen?.find(
            (disabledField) =>
              policies &&
              disabledField?.condition?.(policies) &&
              disabledField?.warningMessage
          )?.warningMessage
        }
        saveValueFunction={saveValueFunction}
        successMessage={translate(I18N_KEYS.SAVE_SUCCESS_LABEL)}
        savingMessage={translate(I18N_KEYS.SAVING_LABEL)}
      />
      {dialogIsVisible && setup ? (
        <ConfirmationDialog
          isOpen
          title={setup.title}
          labelDismiss={setup.dismiss}
          labelConfirm={setup.confirm}
          ...
          ctaIsDisabled={!masterPasswordFieldValue}
