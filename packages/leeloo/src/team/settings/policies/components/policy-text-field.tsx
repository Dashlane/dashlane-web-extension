...
export const PolicyTextField = ({ field, editSettings, checkForAuthenticationError, }: PolicyTextFieldProps) => {
    ...
    const oldValue = field.deserializer
        ...
        : field.value;
    const handleSaveSetting = useCallback(async (value: string) => {
        if (!editSettings) {
            return;
        }
        if (field.validator) {
        ...
        if (['disableAutoLoginDomains', 'teamDomain'].includes(field.feature ?? '')) {
            const oldDomains = field.value as string[];
            if (oldDomains.length) {
                const newDomains = field.serializer?.(value);
                const oldValuesToRemove = oldDomains.filter((oldDomain: string) => !newDomains.some((newDomain: string) => newDomain === oldDomain));
                const oldDomainsToAdd = newDomains.filter((newDomain: string) => !oldDomains.some((oldDomain) => oldDomain === newDomain));
                if (field.feature) {
    ...
    return (<FieldWithButton multiLine={multiLine} defaultValue={oldValue} key={field.label} ariaLabelledBy={field.feature} saveBtnLabel={translate(I18N_KEYS.BUTTON_SAVE_LABEL)} savingBtnLabel={translate(I18N_KEYS.SAVING_LABEL)} editBtnLabel={translate(I18N_KEYS.BUTTON_EDIT_LABEL)} placeholder={field.hintText} inputStyle={field.inputStyle} hintStyle={field.hintStyle} successMessage={translate(I18N_KEYS.SAVE_SUCCESS_LABEL)} onSave={handleSaveSetting} textFieldClassName={styles.textFieldInputColumn} isDisabled={field.isReadOnly}/>);
};
