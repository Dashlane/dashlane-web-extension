...
import { SettingField, SettingFieldProps, SettingRowModel, SettingTextWithButton, } from '../types';
...
export const TextSettingWithButton = ({ textWithButton, }: TextSettingWithButtonProps) => {
    return (<div key={textWithButton.label} sx={{
            display: 'flex',
      ...
      <div>
        <Heading as="h3" textStyle="ds.title.block.medium" color="ds.text.neutral.catchy" sx={SX_STYLES.SETTINGS_LABEL_MARGIN_TOP}>
        ...
        {textWithButton.helperLabel ? (<div id="helper-label-panel" sx={SX_STYLES.HELPER_LABEL_PANEL}>
            {textWithButton.helperLabel}
          </div>) : null}
      </div>
      <div>
        <PrimaryButton label={textWithButton.buttonLabel} onClick={textWithButton?.onClick}/>
      </div>
    </div>);
};
export const PolicySettingRow = ({ settingRow, ...rest }: PolicySettingRowProps) => {
    switch (settingRow.type) {
        ...
        case 'quickDisable':
            return (<QuickDisableOfSmartSpaceManagementSetting settingRow={settingRow}/>);
        default:
