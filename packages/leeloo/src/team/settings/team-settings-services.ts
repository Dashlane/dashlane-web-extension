import { GetTeamInfoError, PlanTier, Seats, TeamCapabilities, TeamInfo, } from '@dashlane/communication';
...
export async function getTeamInfo(): Promise<TeamInfoResponseData> {
    ...
    if (result.success) {
        const { data: { teamInfo, planTier, capabilities, seats }, } = result;
...
export async function getLastADSyncDate(): Promise<LastADSyncDateData> {
    ...
    if (result.success) {
        return {
            ...
            lastSuccessfulSyncRequestForTeamUnix: result.data.lastSuccessfulSyncRequestForTeam,
        };
    }
