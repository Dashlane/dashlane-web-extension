...
import { Accordion, AccordionDetails, AccordionSection, AccordionSummary, InfoBox, } from '@dashlane/ui-components';
...
import { DomainContainer } from 'team/settings/sso/domain/domain-container';
import { EnableSsoStep } from 'team/settings/sso/enable-sso-step/enable-sso-container';
import { ConfigureIDP } from 'team/settings/sso/configure-idp/configure-idp';
import settingStyles from 'team/settings/styles.css';
import { SettingsPage } from 'team/settings';
import Row from 'team/settings/base-page/row';
import SSOConnector from 'team/settings/sso/sso-connector/sso-connector';
import styles from 'team/settings/sso/styles.css';
import { DASHLANE_SUPPORT, SSO_SETUP_GUIDE } from 'team/urls';
import { SettingAccordionSections, SSOSettingProps, SSOSettingStep, } from 'team/settings/sso/types';
...
const I18N_KEYS = {
    ...
    INFOBOX_TEAM_TIER_WARNING: 'team_settings_sso_teamplan_infobox_warning_markup',
    INFOBOX_TEAM_TIER_WARNING_BUTTON: 'team_settings_sso_teamplan_infobox_warning_button',
...
export const SSOSettings = ({ lee }: SSOSettingProps) => {
    ...
    const [stepsCompleted, setStepsCompleted] = useState(new Set<SSOSettingStep>());
    const [openSections, setOpenSections] = useState(new Set<SettingAccordionSections>());
    ...
    useEffect(() => {
        ...
        if (stepsCompleted.has(SSOSettingStep.ClaimDomain) &&
    ...
    const completeStep = useCallback((step: SSOSettingStep) => {
        setStepsCompleted((prevSteps) => {
            const checkDependentStepsCompleted = (dependentSteps: SSOSettingStep[]) => {
    ...
    const { teamSettingsLoading, teamSettings, updateTeamSettings, planTier } = useTeamSettings(teamId);
    ...
    const accordion = (): React.ReactNode => {
        if (teamSettingsLoading) {
            return (<LoadingSpinner containerClassName={styles.loadingContainer} size={40}/>);
        }
        return (<Accordion size="large">
        {accordionSections.map((section) => (<AccordionSection open={section.open} key={section.label} readOnly={section.locked}>
            <AccordionSummary>
              {`${section.index}. ${section.label}`}
            </AccordionSummary>
            <AccordionDetails>
              <fieldset disabled={section.locked}>
                <section.component isTeamSettingsLoading={teamSettingsLoading} teamSettings={teamSettings} setStepComplete={completeStep} updateTeamSettings={updateTeamSettings} teamId={teamId ?? null} planTier={planTier ?? null}/>
              </fieldset>
            </AccordionDetails>
          </AccordionSection>))}
      </Accordion>);
    ...
    const displayInfoBoxWarning = () => {
        ...
        if (planTier === 'team' || planTier === 'legacy') {
            const infoBoxTitle = translate.markup(I18N_KEYS.INFOBOX_TEAM_TIER_WARNING);
            const primaryLabel = translate(I18N_KEYS.INFOBOX_TEAM_TIER_WARNING_BUTTON);
            return (<div className={styles.infoBox}>
          <InfoBox title={infoBoxTitle} size="simple" secondaryLabel={primaryLabel} secondary={() => openUrl(DASHLANE_SUPPORT)}/>
        </div>);
        }
        return null;
    };
    return (<SettingsPage secondaryContent={accordion()} title={translate(I18N_KEYS.PAGE_TITLE)}>
      {displayInfoBoxWarning()}
      <p className={settingStyles.settingsHeader}>
        {translate(I18N_KEYS.TITLE)}
      </p>
      <Row label={translate(I18N_KEYS.SETUP_GUIDE_TITLE)} labelHelper={translate(I18N_KEYS.SETUP_GUIDE_DESCRIPTION)}>
        <PrimaryButton label={translate(I18N_KEYS.SETUP_GUIDE_BUTTON)} onClick={openHelpPage}/>
      </Row>
      <DomainContainer isTeamSettingsLoading={teamSettingsLoading} teamSettings={teamSettings} setStepComplete={completeStep} uncompleteSteps={uncompleteSteps} updateTeamSettings={updateTeamSettings} teamId={teamId} planTier={planTier}/>
    </SettingsPage>);
};
