...
const DEFAULT_HREF = "__REDACTED__";
export const VerifyConfirmDialog = ({
    ...
    try {
      ...
      if (isDarkWebInsights) {
        logEvent(
          new UserVerifyDomainEvent({
            domainVerificationStep:
              DomainVerificationStep.DomainVerificationError,
          })
        );
      }
    } finally {
      ...
      if (isDarkWebInsights) {
        logEvent(
          new UserVerifyDomainEvent({
            domainVerificationStep:
              DomainVerificationStep.TapVerifyDomainCtaWithDnsInformation,
          })
        );
      }
    }
  ...
  const getFieldProps = ({
    ...
    textInputProps: {
      ...
      disabled: loading,
      fullWidth: true,
      startAdornment: loading ? <IndeterminateLoader /> : undefined,
    },
    iconButtonProps: {
      disabled: loading,
    },
    ...
    >
      ...
      <DialogBody>
        <GridContainer gap="32px" gridTemplateColumns="1fr" gridAutoRows="auto">
          ...
          {error ? (
            <Paragraph color={colors.red00}>{error}</Paragraph>
          ) : (
            <>
              <InputWithCopyButton
                {...getFieldProps({
          ...
          {secondaryDescription ? (
            <Paragraph color={colors.grey00}>{secondaryDescription}</Paragraph>
          ) : null}
        </GridContainer>
      </DialogBody>
      <DialogFooter
        primaryButtonTitle={translate(I18N_KEYS.VERIFY_CONFIRM)}
        primaryButtonOnClick={handleVerify}
        primaryButtonProps={{
