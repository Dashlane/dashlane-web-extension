    ...
    <>
      <div className={styles.domainUrlInput}>
        <TextInput
          ...
          fullWidth
          feedbackText={registrationError}
          feedbackType={registrationError ? "error" : undefined}
          onChange={onTextInputChange}
          disabled={!!domainLoadError || disabled}
        />
        {domainLoadError && (
          <p className={styles.domainLoadErrorText}>{domainLoadError}</p>
        )}
      </div>
      <Button
        type="button"
        className={classnames(styles.domainUrlButton, {
          [styles.domainLoadError]: domainLoadError,
        })}
        disabled={disabled}
        nature="secondary"
        onClick={() => registerDomain(input)}
      >
