...
export interface DomainRowProps {
  ...
  domain: Domain;
  isDisabled: boolean;
  setCurrentSteps: React.Dispatch<
    React.SetStateAction<Record<string, DomainSteps>>
  >;
...
export const DomainRow = ({
  ...
  const [showDialogForCurrentStep, setShowDialogForCurrentStep] =
      ...
      case DomainSteps.VerifyFailed:
        return (
          <>
            <VerifyDomain
              domain={domain}
              verifyDomain={() => setShowDialogForCurrentStep(true)}
              domainStep={currentStep}
            />
            <DeactivateDomain
              domain={domain}
              deactivateDomain={() => deactivateDomain(domain)}
              isDisabled={isDisabled}
              disabledTooltipText={translate(I18N_KEYS.DEACTIVATE_ERROR)}
            />
          </>
        );
      ...
      case DomainSteps.Verify:
        return (
          <VerifyConfirmDialog
            ...
            onConfirm={() => verifyDomain(domain)}
            onDismiss={() => setShowDialogForCurrentStep(false)}
          />
        );
      case DomainSteps.VerifyFailed:
        return (
  ...
  return (
    <div className={styles.domainContainer} key={domain.id}>
