...
import { RegisterDomain } from 'team/settings/sso/domain/steps';
import { DomainRow } from 'team/settings/sso/domain/domain-row';
...
export const DomainContainer = ({ isTeamSettingsLoading, teamSettings, setStepComplete, uncompleteSteps, }: SSOSettingSectionProps & Required<Pick<SSOSettingSectionProps, 'uncompleteSteps'>>) => {
    ...
    const [currentSteps, setCurrentSteps] = React.useState<Record<string, DomainSteps>>({});
    const isDomainVerified = (domain: Domain) => domain.status === 'valid';
    const getVerifiedDomains = () => domains.filter((domain) => isDomainVerified(domain));
    ...
    const getDomains = React.useCallback(async (): Promise<Domain[] | undefined> => {
    ...
    const verifyDomain = async (domain: Domain) => {
        ...
        const isSuccessful = verification.success &&
    ...
    return (<>
      <Row label={`1. ${translate(I18N_KEYS.DOMAIN_TITLE)}`} labelHelper={translate(I18N_KEYS.DOMAIN_DESCRIPTION)}>
        {isDomainDataLoading || isTeamSettingsLoading ? (<LoadingSpinner size={20} containerStyle={{ alignItems: 'normal' }}/>) : (<RegisterDomain registerDomain={registerDomain} domainLoadError={domainLoadError} registrationError={registrationError} disabled={domains.length >= MAX_DOMAINS} clearRegistrationErrors={() => setRegistrationError('')}/>)}
      </Row>
      {isDomainDataLoading ||
          ...
          <Row label={translate(I18N_KEYS.ADDED_DOMAINS)}>
            <div className={styles.domainsContainer}>
              {domains.map((domain, index) => index < MAX_DOMAINS ? (<DomainRow key={domain.id} currentStep={currentSteps[domain.id]} deactivateDomain={deactivateDomain} domain={domain} isDisabled={ssoEnabled &&
                    getVerifiedDomains().length === 1 &&
                    isDomainVerified(domain)} setCurrentSteps={setCurrentSteps} verifyDomain={verifyDomain}/>) : null)}
            </div>
          </Row>
        </>)}
    </>);
