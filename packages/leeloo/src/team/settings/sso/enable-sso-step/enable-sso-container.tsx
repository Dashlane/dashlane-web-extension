...
import { Toggle } from "@dashlane/ui-components";
import { carbonConnector } from "../../../../libs/carbon/connector";
import { openUrl } from "../../../../libs/external-urls";
import useTranslate from "../../../../libs/i18n/useTranslate";
import Row from "../../base-page/row";
import { EnableSsoDialog } from "./enable-dialog";
import { DisableSsoDialog } from "./disable-dialog";
import styles from "./styles.css";
import { DialogAction, SSOSettingSectionProps, SSOSettingStep } from "../types";
...
export const EnableSsoStep = ({
  ...
  const [ssoEnabled, setSsoEnabled] = useState<boolean>(
    teamSettings?.ssoEnabled ?? false
  );
  ...
  const openServiceProviderUrl = (): void => {
    openUrl(
      `${teamSettings?.ssoServiceProviderUrl}/saml/login?redirect=test_idp`
    );
  };
  ...
  const toggleOrContactUs = () => {
    if (redirectToBuyBusiness) {
      return (
        <Button
          mood="brand"
          intensity="catchy"
          onClick={() => openUrl(DASHLANE_SUPPORT)}
        >
    ...
    return (
      <div>
        ...
        {toggleWarning ? (
          <div className={styles.toggleWarning}>
  ...
  return (
    <div className={styles.enableSSOContainer}>
      <Row
        label={translate(I18N_KEYS.TEST_TITLE)}
        labelHelper={translate(I18N_KEYS.TEST_DESCRIPTION)}
      >
        <Button
          mood="brand"
          intensity="catchy"
          onClick={openServiceProviderUrl}
          disabled={!(teamSettings && teamSettings.ssoServiceProviderUrl)}
        >
          {translate(I18N_KEYS.TEST_BUTTON)}
        </Button>
      </Row>
      <Row
        label={translate(I18N_KEYS.ACTION_TITLE)}
        labelHelper={translate(I18N_KEYS.ACTION_DESCRIPTION)}
      >
        {toggleOrContactUs()}
      </Row>
      {showDialog ? (
        ssoEnabled ? (
          <DisableSsoDialog closeDialog={closeDialog} />
        ) : (
          <EnableSsoDialog closeDialog={closeDialog} />
        )
      ) : null}
    </div>
  );
