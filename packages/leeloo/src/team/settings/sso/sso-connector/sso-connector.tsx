...
const SSOConnector = ({
  ...
  const saveIdpMetadata = React.useCallback(
  ...
  const saveConnectorURL = React.useCallback(
    async (url: string) => {
      try {
  ...
  return (
    <div className={styles.ssoConnectorContainer}>
      <ConnectorInfo
        isTeamSettingsLoading={isTeamSettingsLoading}
        teamSettings={teamSettings}
        saveConnectorURL={saveConnectorURL}
        saveIdpMetadata={saveIdpMetadata}
      />
      <Installer
        isTeamSettingsLoading={isTeamSettingsLoading}
        isSsoEnabled={teamSettings?.ssoEnabled ?? false}
        idpMetadata={teamSettings?.ssoIdpMetadata ?? ""}
        connectorUrl={teamSettings?.ssoServiceProviderUrl ?? ""}
      />
    </div>
  );
};
