...
import { Button, DownloadIcon, jsx, OpenWebsiteIcon, Tooltip, } from '@dashlane/ui-components';
...
import { SSO_SETUP_GUIDE } from 'team/urls';
import { KeyGeneratorDisplay } from 'team/settings/sso/sso-connector/key-generator-display';
...
const I18N_KEYS = {
    ...
    INSTALLER_DOCUMENTATION_TITLE: 'team_settings_connector_installer_setup_title',
    INSTALLER_DOCUMENTATION_DESCRIPTION: 'team_settings_connector_installer_setup_description',
...
export const Installer = ({ isTeamSettingsLoading, isSsoEnabled, idpMetadata, connectorUrl, }: InstallerProps) => {
    ...
    const hasNeededConfigInfo = Boolean(idpMetadata && connectorUrl && (connectorKey || isSsoEnabled));
    return (<>
      <KeyGeneratorDisplay keyValue={connectorKey} onGenerateClicked={generateKey} isGeneratorDisabled={isSsoEnabled} generatorDisabledText={translate(I18N_KEYS.GENERATOR_HELPER_WARNING)}/>
      <Row label={translate(I18N_KEYS.INSTALLER_TITLE)} labelHelper={translate(I18N_KEYS.INSTALLER_DESCRIPTION)}>
        <div className={isTeamSettingsLoading
            ? styles.loading
            : styles.installerButtonContainer}>
          <Tooltip content={translate(I18N_KEYS.INSTALLER_TOOLTIP)} passThrough={hasNeededConfigInfo} sx={{ maxWidth: '210px' }}>
            <Button type="button" aria-describedby="configError" disabled={!hasNeededConfigInfo} onClick={createAndDownloadConfigScript}>
              <div className={styles.downloadIcon}>
                <DownloadIcon color={colors['--white']}/>
              </div>
              {translate(I18N_KEYS.INSTALLER_CONFIG_BUTTON)}
            </Button>
            {configError ? (<p id="configError" className={styles.errorText}>
                {translate(I18N_KEYS.INSTALLER_ERROR)}
              </p>) : null}
          </Tooltip>
        </div>
      </Row>
      <Row label={translate(I18N_KEYS.INSTALLER_DOCUMENTATION_TITLE)} labelHelper={translate(I18N_KEYS.INSTALLER_DOCUMENTATION_DESCRIPTION)}>
        <div className={isTeamSettingsLoading
            ? styles.loading
            : styles.installerButtonContainer}>
          <Tooltip content={translate(I18N_KEYS.INSTALLER_TOOLTIP)} passThrough={hasNeededConfigInfo} sx={{ maxWidth: '210px' }}>
            <Button type="button" disabled={!hasNeededConfigInfo} onClick={openInstallerScriptDocumentation}>
