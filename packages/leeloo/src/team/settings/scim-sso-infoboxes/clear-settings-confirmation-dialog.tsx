...
export const ClearSettingsConfirmationDialog = ({ onClose, onConfirm, titleText, bodyText, }: ClearSettingsConfirmationDialogProps) => {
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            setConfirmingError(e instanceof Error ? e.message : translate(I18N_KEYS.GENERIC_ERROR));
            setIsConfirming(false);
        }
    };
    const closeText = translate(I18N_KEYS.CANCEL);
    return (<Dialog isOpen disableOutsideClickClose disableScrolling closeIconName={closeText} onClose={onClose}>
      <DialogTitle title={titleText}/>
      <DialogBody>
        <Paragraph color={confirmingError
      ...
      <DialogFooter>
        <Button sx={{ mr: '12px' }} mood="neutral" intensity="quiet" onClick={handleClose} disabled={isConfirming}>
          {closeText}
        </Button>
        <Button mood="brand" intensity="catchy" onClick={handleConfirm} isLoading={isConfirming} disabled={isConfirming}>
