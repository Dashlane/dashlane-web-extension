...
import { Heading, Infobox, Paragraph } from "@dashlane/design-system";
...
const NestedOrderedList = ({ content }: { content: NestedList }) => {
  return (
    <ol sx={{ color: "ds.text.neutral.quiet" }}>
      {content.map(({ li, ol }, index) => (
        <Paragraph key={index} as="li" textStyle="ds.body.standard.regular">
  ...
  return (
    ...
    >
      ...
      <Infobox
        mood="brand"
        size="small"
        title={translate(I18N_KEYS.INFOBOX_MESSAGE)}
      />
      <div
        sx={{
          display: "grid",
      ...
      >
        <InputWithCopyButton
          ...
          label={translate(I18N_KEYS.ENTITY_ID_LABEL)}
          readOnly
        />
        <InputWithCopyButton
          isLoading={isLoading}
          value={formData.acsUrl}
          label={translate(I18N_KEYS.ACS_URL_LABEL)}
          readOnly
        />
        <InputWithCopyButton
          isLoading={isLoading}
          value={formData.signOnUrl}
          label={translate(I18N_KEYS.SIGN_ON_URL_LABEL)}
          readOnly
        />
      </div>
    </div>
