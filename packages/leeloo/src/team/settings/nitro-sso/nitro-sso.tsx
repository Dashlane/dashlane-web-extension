...
import { Card, FlexContainer, PropsOf } from '@dashlane/ui-components';
...
import { TestNitroSSO } from './test-nitro-sso';
...
const I18N_VALUES = {
    ...
    CARD_2_BODY_SECOND: 'They must use the browser extension when using Dashlane on a computer. They can also log in with SSO on mobile.',
...
const NitroSSOForm = () => {
    ...
    const isSsoUnused = ssoStateStatus === DataStatus.Success
        ? !(ssoState as any).enableSSO.ssoEnabled
        : null;
    return (<div>
      
      
      <FlexContainer flexDirection="column" gap="32px" sx={{ flexGrow: 1 }}>
...
const NitroSSOInfo = (params: PropsOf<typeof FlexContainer>) => {
    return (<div sx={{
            border: 'none',
            display: 'flex',
            gap: '24px',
      ...
      <InfoCard>
        ...
        <Paragraph as="ul" textStyle="ds.body.standard.regular" color="ds.text.neutral.quiet" sx={{
            margin: '0px 0px 8px 20px',
            listStyleType: 'disc',
        }}>
          {I18N_VALUES.CARD_1_BODY_POINTS.map((string) => (<li key={string}>{string}</li>))}
        </Paragraph>
        <Heading as="h3" textStyle="ds.title.block.medium">
        ...
        <ExternalLink href={`mailto:${SUPPORT_EMAIL}`} sx={{ fontSize: '14px' }}>
          {SUPPORT_EMAIL}
        </ExternalLink>
      </InfoCard>
      <InfoCard>
        ...
        <Paragraph textStyle="ds.title.block.small" color="ds.text.danger.standard">
          {I18N_VALUES.CARD_2_BODY_THIRD}
        </Paragraph>
      </InfoCard>
    </div>);
};
export const NitroSSO = ({ backRoute }: {
    backRoute: string;
}) => {
    const { data } = useModuleQuery(confidentialSSOApi, 'ssoProvisioning');
    const { translate } = useTranslate();
    const showNewBadge = !data?.enableSSO.ssoEnabled;
    ...
    return (<BackPageLayout title={translate(I18N_KEYS.PAGE_TITLE)} badgeLabel={showNewBadge ? translate(I18N_KEYS.NEW_BADGE) : ''} showBackButton={showBackButton} onBackClicked={onBackClicked}>
      <ResponsiveMainSecondaryLayout mainContent={NitroSSOForm} mainProps={{
