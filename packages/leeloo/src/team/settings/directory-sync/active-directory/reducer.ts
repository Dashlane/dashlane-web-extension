...
export const activeDirectorySettingsLoaded =
  reducer.action<ActiveDirectorySettingsLoadedParams>(
    "active-directory-settings-loaded",
    ({ param }) => ({
      activeDirectoryScript: generateActiveDirectoryScript(
        param.activeDirectoryToken,
        param.teamId
      ),
...
export const toggleScriptCopyFeedback = reducer.action(
  "script-copy-feedback-toggled",
  ({ param }: any) => ({ isCopied: param })
);
export const setActiveDirectorySyncType = reducer.action(
  "active-directory-sync-type-set",
  ({ param }: any) => ({ activeDirectorySyncType: param })
);
