...
export const ScimSetup = ({
  ...
  const { teamDeviceLoading, teamDevice, refreshTeamDevice } = useTeamDevice(
    esConfig?.deviceAccessKey
  );
  const dataLoading = loading || teamDeviceConfigLoading;
  const isSetupDisabled =
    !teamSettings?.ssoServiceProviderUrl ||
    dataLoading ||
    disableSetupButton ||
    !isScimCapable;
      ...
      >
        ...
        <GridChild gridArea="button">
          <DisabledButtonWithTooltip
            ...
            neverShowTooltip={disableSetupButton}
            size="large"
            onClick={() => {
              history.push(`${parentPath}${scimSettingsSubPath}`);
            }}
            content={translate(I18N_KEYS.CONFIGURE_ES_TOOLTIP)}
            mood={isPrimaryCta && !isSetupDisabled ? "brand" : "neutral"}
            intensity={isPrimaryCta && !isSetupDisabled ? "catchy" : "quiet"}
          >
            {isPrimaryCta
        ...
        {teamDeviceConfig?.configProperties?.scimEnabled ? (
          <GridChild gridArea="status" sx={{ mt: "20px" }}>
            <SCIMSyncStatus
              loading={teamDeviceLoading}
              version={teamDevice?.version}
              refreshData={refreshTeamDevice}
              lastSyncUnix={teamDevice?.lastActivityDateUnix}
            />
          </GridChild>
        ) : null}
      </GridContainer>
