...
const I18N_KEYS = {
  ...
  BUTTON_SAVE_CHANGES:
    "team_settings_directory_sync_scim_set_up_button_save_changes",
  ...
  SCIM_TOKEN_GENERATE_BUTTON:
    "team_settings_directory_sync_scim_set_up_generate_buttons",
  SCIM_TOKEN_RESET_BUTTON:
    "team_settings_directory_sync_scim_set_up_reset_buttons",
  SCIM_ENDPOINT_LABEL:
    "team_settings_directory_sync_scim_set_up_endpoint_label",
  SCIM_ENCRYPTION_SERVICE_LABEL:
    "team_settings_directory_sync_scim_set_up_service_label",
  SCIM_ENCRYPTION_SERVICE_DESC:
    "team_settings_directory_sync_scim_set_up_service_description",
  SCIM_ES_INFOBOX: "team_settings_directory_sync_scim_infobox",
  SCIM_HEADER: "team_settings_directory_sync_scim_header",
  SCIM_CONFIRMATION_DIALOG_TITLE:
    "team_settings_directory_sync_scim_confirm_dialog_title",
  SCIM_CONFIRMATION_DIALOG_PRIMARY_ACTION_LABEL:
    "team_settings_directory_sync_scim_confirm_dialog_primary_action_label",
  SCIM_CONFIRMATION_DIALOG_BODY:
    "team_settings_directory_sync_scim_confirm_dialog_body",
};
...
export const ScimSetupForm = ({ config, esEndpoint }: ScimSetupFormProps) => {
  ...
  const updateScimConfig = async ({
    values,
    resetForm,
  }: Pick<FormikHelpers<FormFields>, "resetForm"> & {
    values: FormFields;
  }) => {
    setIsSaving(true);
    ...
    if (!updateResponse.success) {
      ...
      updateResponse.data.encryptionServiceReloadingStatus !==
  ...
  const initialValues = {
    scimToken: obfuscateSCIMToken(
      teamDeviceConfig?.configProperties?.scimAuthToken
    ),
    scimEndpoint: esEndpoint ? appendToUrl(esEndpoint, "scim/2.0") : "",
    encryptionServiceSync:
      teamDeviceConfig?.configProperties?.scimEnabled ?? false,
  };
  const onCancel = () => {
      ...
      {({
        ...
        return (
          <Form autoComplete="off" noValidate>
            <CatchUnsavedChanges
              dirty={dirty && !isSaving && !restartEsFailed}
            />
            <Card>
              ...
              <CardContent>
                <Infobox
                  mood="brand"
                  title={translate(I18N_KEYS.SCIM_ES_INFOBOX)}
                />
                <GridContainer
                  gridTemplateAreas="'header button' 'desc button' 'inputField inputField'"
                  gap="8px"
                  sx={{
                    mt: "32px",
                  }}
                >
                  ...
                  <GridChild gridArea="inputField">
                    <Field name="scimToken">
                      {({ field }: FormFieldProps<FormFields["scimToken"]>) =>
                        field.value ? (
                          <InputWithCopyButton
                            inputValue={field.value}
                            iconButtonProps={{
                              disabled:
                                (Boolean(values.scimToken) && !tokenReset) ||
                                isSubmitting,
                            }}
                            passwordInputProps={{
                              disabled:
                ...
                <GridContainer gridTemplateColumns="auto" gap="8px">
                  ...
                  <Field name="scimEndpoint">
                    {({
                      field,
                    }: FormFieldProps<FormFields["scimEndpoint"]>) => (
                      <InputWithCopyButton
                        inputValue={field.value}
                        textInputProps={{
                          fullWidth: true,
                ...
                <hr />
                <GridContainer
                  gridTemplateAreas="'label toggle' 'desc .'"
                  gap="8px"
                >
                  <GridChild
                    gridArea="label"
                  ...
                  <GridChild gridArea="toggle">
                    {teamDeviceConfigLoading ? (
                      ...
                      <Toggle
                        ...
                        onChange={(e) => onScimEnabledChange(e, setFieldValue)}
                        defaultChecked={
                          teamDeviceConfig?.configProperties?.scimEnabled ??
                          false
                        }
                      />
                    )}
              ...
              <CardFooter>
                ...
                <Button
                  type="submit"
                  disabled={dataLoading || isSubmitting || !dirty}
                >
            ...
            {showConfirmDialog ? (
              <ConfirmDialog
                title={translate(I18N_KEYS.SCIM_CONFIRMATION_DIALOG_TITLE)}
                body={translate(I18N_KEYS.SCIM_CONFIRMATION_DIALOG_BODY)}
                primaryActionLabel={translate(
                  I18N_KEYS.SCIM_CONFIRMATION_DIALOG_PRIMARY_ACTION_LABEL
                )}
                onCancel={onCancel}
