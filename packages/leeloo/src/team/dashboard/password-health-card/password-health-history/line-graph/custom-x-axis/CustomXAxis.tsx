...
import { GraphViewOptions, MINIMUM_MONTH_COUNT_TO_OMIT_DATE, } from 'team/constants';
...
const determineMonthCount = (firstHistoryEntryDateString: string) => {
    ...
    let monthCount = (currentDate.getFullYear() - firstHistoryEntryDate.getFullYear()) * 12;
    monthCount -= firstHistoryEntryDate.getMonth();
    monthCount += currentDate.getMonth();
    return monthCount + 1;
};
const determineTicks = (view: GraphViewOptions, historyToDisplay: PasswordHealthHistory[], monthCount: number) => {
    const ticks: string[] = [];
    switch (view) {
...
const formatDate = (date: string, view: GraphViewOptions, monthCount: number, translate: TranslatorInterface) => {
    const minimumMonthsToOmitDateReached = view === GraphViewOptions.AllTime &&
        monthCount >= MINIMUM_MONTH_COUNT_TO_OMIT_DATE;
    const dateObj = new Date(date);
    const formattedDate = translate.shortDate(dateObj, minimumMonthsToOmitDateReached ? LocaleFormat.MMM_YYYY : LocaleFormat.L_M_D);
...
export const CustomXAxis = ({ historyToDisplay, passwordHealthHistoryEmpty, view, }: CustomXAxisProps) => {
    const { translate } = useTranslate();
    const monthCount = historyToDisplay.length !== 0
        ? determineMonthCount(historyToDisplay[0].date)
        : 0;
    determineTicks(view, historyToDisplay, monthCount);
    return (<XAxis dataKey="date" ticks={passwordHealthHistoryEmpty
            ? []
            : determineTicks(view, historyToDisplay, monthCount)} tickSize={4} tickFormatter={passwordHealthHistoryEmpty
            ? () => ''
            : (date: string) => formatDate(date, view, monthCount, translate)} fontSize="small"/>);
};
