...
import { DarkWebInsightsSummary, GetDarkWebInsightsSummaryResponse, NotificationName, } from '@dashlane/communication';
...
const GetStartedButton = () => {
    ...
    return (<Button key="primary" onClick={() => {
...
const AggressiveTile = ({ data, companyName, }: {
    ...
    return (<DWICard>
      <Badge layout="labelOnly" label={translate(I18N_KEYS.TEAM_NEW_LABEL)} mood="brand"/>
      <Heading textStyle="ds.title.section.medium" color="ds.text.neutral.catchy" as="h3" sx={{
            marginTop: '1em',
            width: '100%',
            overflow: 'hidden',
        }}>
      ...
      <div sx={{
        ...
        <TileDataWithLabel count={domainData.leaksCount} labelAndTooltip={I18N_KEYS.SECURITY_INCIDENTS} primaryColor={colors.dashGreen01}/>
        <TileDataWithLabel count={domainData.emailsImpactedCount} labelAndTooltip={I18N_KEYS.EMAILS_AFFECTED} primaryColor={colors.dashGreen01}/>
      </div>
      <GetStartedButton />
    </DWICard>);
};
const AlternateTile = ({ companyName }: {
    companyName: string;
}) => {
    const { translate } = useTranslate();
    return (<DWICard>
...
export const DWIGetStartedTile = ({ dwiSummaryResponse, }: {
    ...
    return dwiSummaryResponse?.success ? (<AggressiveTile data={dwiSummaryResponse.data} companyName={companyName}/>) : (<AlternateTile companyName={companyName}/>);
};
