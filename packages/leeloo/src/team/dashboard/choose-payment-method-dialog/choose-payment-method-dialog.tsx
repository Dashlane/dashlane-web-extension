...
const contactSupportUrl = "__REDACTED__";
export const ChoosePaymentMethodDialog = ({
  ...
  return (
    <Dialog isOpen={isOpen} ariaDescribedby="dialogContent" onClose={() => {}}>
      <DialogTitle title={translate("team_payment_method_dialog_title")} />
      <DialogBody>
        <Card
          sx={cardStyles}
          onClick={() => {
            const url = `${BUSINESS_BUY}?plan=${plan}&subCode=${
              subscriptionCode ?? ""
            }`;
            openUrl(url);
          }}
        >
          <GridContainer
            gap="12px"
            gridTemplateColumns="40px 480px 60px"
