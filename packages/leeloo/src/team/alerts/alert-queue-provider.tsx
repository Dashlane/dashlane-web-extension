...
export const AlertQueueContext = React.createContext<AlertQueueContextInterface | null>(null);
export const LEE_NETWORK_ERROR = 'NETWORK ERROR';
export const AlertQueueProvider = ({ globalReportError, children, }: AlertQueueProviderProps) => {
    const [alertQueue, setAlertQueue] = React.useState<Alert[]>([]);
    const { translate } = useTranslate();
    const addAlertToQueue = (alert: Alert) => {
        if (!alertQueue.find((existingAlert) => existingAlert.title === alert.title &&
    ...
    return (<AlertQueueContext.Provider value={{
            queue: alertQueue,
            addAlertToQueue,
            popAlertFromQueue,
            reportTACError,
