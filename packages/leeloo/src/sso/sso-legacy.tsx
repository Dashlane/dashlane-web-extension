...
import { ChangeMPFlowPath, LoginViaSsoCode, MigrationSsoToMP, SsoMigrationServerMethod, UserConsent, } from '@dashlane/communication';
...
import { AUTO_SSO_LOGIN_SUCCESS_URL_SEGMENT, LOGIN_URL_SEGMENT, SSO_URL_SEGMENT, } from 'app/routes/constants';
import { getDefaultDeviceName } from 'helpers';
import { SSO_DOWNLOAD, WEBAPP_LOGIN } from 'team/urls';
import { getUserId } from 'user';
import { extractSsoInfoFromUrl, requiredPermissions } from 'sso/utils';
import { Props, SsoSteps } from 'sso/types';
import { ConfirmNewPassword, CreateMasterPassword, } from 'sso/create-master-password/create-master-password';
import { Progress } from 'sso/progress/progress-wrapper';
import { Terms, TermsSubmitOptions } from 'sso/terms/terms';
import { useLiveChangeMasterPasswordStatus } from 'sso/hooks/useLiveChangeMasterPasswordStatus';
import styles from 'sso/sso.css';
...
export const SsoLegacy = (props: Props) => {
    ...
    const { login, ssoToken, key: ssoServiceProviderKey, exists, currentAuths, expectedAuths, inStore, } = extractSsoInfoFromUrl(props);
    const { lee } = props;
    const userExists = exists === 'true';
    const [ssoSteps, setSsoSteps] = useState<SsoSteps>(userExists ? SsoSteps.D_SPINNER : SsoSteps.ACCOUNT_CREATION);
    const { translate } = useTranslate();
    const [error, setError] = useState('');
    const [progressValue, setProgressValue] = useState(0);
    const isAutoSsoLoginDisabled = useIsAutoSsoLoginDisabled();
    const ssoUserSettings = useModuleQuery(AuthenticationFlowContracts.authenticationFlowApi, 'getSsoUserSettings');
    ...
    const handleExistingUser = async () => {
        ...
        const hasRememberMeForSSO = props.lee.carbon.localAccounts?.find((account) => account.login === login && account.rememberMeType === 'sso');
        const isAutoSSORequested = (ssoUserSettings.status === DataStatus.Success &&
            ssoUserSettings.data.rememberMeForSSOPreference) ||
            localStorage.getItem(REMEMBER_ME_FOR_SSO_PREFERENCE) === 'true';
        const shouldRememberMeForSSO = !hasRememberMeForSSO &&
            isAutoSSORequested &&
        ...
        if (hasRememberMeForSSO) {
            routerGlobalSettings.store.dispatch(registerRedirectPath(AUTO_SSO_LOGIN_SUCCESS_URL_SEGMENT));
        }
    ...
    const confirmNewPassword = async ({ password, }: ConfirmNewPassword): Promise<void> => {
    ...
    const handleAcceptTermsSubmit = async ({ isEu, privacyPolicyAndToS, setIsLoading, subscribed, }: TermsSubmitOptions): Promise<void> => {
        if (isEu && !privacyPolicyAndToS) {
            const euTosError = new Error('termsOfService set to false on accept submit');
        ...
        const loginParams = {
            ...
            shouldRememberMeForSSO: localStorage.getItem(REMEMBER_ME_FOR_SSO_PREFERENCE) === 'true',
        };
        localStorage.removeItem(REMEMBER_ME_FOR_SSO_PREFERENCE);
    ...
    switch (ssoSteps) {
        case SsoSteps.ACCOUNT_CREATION:
            return (<Terms isEu={isEu} onSubmit={handleAcceptTermsSubmit} memberEmail={validatedEmail} error={error}/>);
        case SsoSteps.ASK_FOR_MP:
            return (<CreateMasterPassword onSubmit={confirmNewPassword} dispatchGlobal={lee.dispatchGlobal}/>);
        case SsoSteps.CHANGE_MP_PROGRESS:
            return (<Progress animation={loadingLottie} shouldLoopAnimation={true} progressValue={progressValue}/>);
