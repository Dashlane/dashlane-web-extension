...
import {
  OnboardingTask,
  PageView,
  UserCompleteOnboardingTaskEvent,
...
import { useIdentitiesCount } from "./hooks/use-identities-count";
import { useUserHasMobileDevice } from "./hooks/use-user-has-mobile-device";
const I18N_KEYS = {
  CHECK_ICON_TITLE_SVG: "web_onboarding_step_check_icon",
  RIGHT_FACING_ARROW_TITLE_SVG: "web_onboarding_step_right_facing_arrow",
  PASSWORD_COMPLETED_ICON_TITLE_SVG:
    "web_onboarding_step_password_completed_icon",
  ...
  AUTOFILL_COMPLETED_ICON_TITLE_SVG:
    "web_onboarding_step_autofill_completed_icon",
...
const ANIM_SIZE = 24;
const addMobileSpinner = (
  <Animation
    key={`${KEY_PREFIX}spinner`}
    height={ANIM_SIZE}
    width={ANIM_SIZE}
    animationParams={{
      renderer: "svg",
...
export const OnboardingSteps = ({ lee }: Props) => {
  const {
    ...
    tryAutofillOnWeb,
    saveCredentialOnWeb,
  } = lee.carbon.webOnboardingMode.completedSteps || {};
  const { flowAddMobileOnWeb } = lee.carbon.webOnboardingMode || {};
  const { translate } = useTranslate();
  const { routes } = useRouterGlobalSettingsContext();
  const [addPasswordStepCompleted, setAddPasswordStepCompleted] =
    useState(saveCredentialOnWeb);
  ...
  const isIdentityExisting =
    identitiesCount.status === DataStatus.Success && identitiesCount.count > 0;
  ...
  const checkIcon = (
    <CheckIcon
      title={translate(I18N_KEYS.CHECK_ICON_TITLE_SVG)}
      color={colors.white}
      viewBox="-8 -8 26 26"
    />
  );
  const orangeArrowIcon = (
    <ForwardIcon
      key={`${KEY_PREFIX}arrow`}
      title={translate(I18N_KEYS.RIGHT_FACING_ARROW_TITLE_SVG)}
      color={colors.orange00}
      size={24}
    />
  );
  const userHasMobileDeviceAuthorised = useUserHasMobileDevice();
  useEffect(() => {
  ...
  const markAddMobileComplete = () => {
    ...
    logEvent(
      new UserCompleteOnboardingTaskEvent({
  ...
  useEffect(() => {
    const shouldNotGetMobileDevices =
      addMobileOnWeb || showAddMobileError || showAddMobileSpinner;
    if (shouldNotGetMobileDevices) {
      return;
    }
    if (userHasMobileDeviceAuthorised.status !== DataStatus.Success) {
      return;
    }
    const TIMEOUT_DELAY = 2500;
    if (
      !flowAddMobileOnWeb &&
      !userHasMobileDeviceAuthorised.userHasMobileDevice
    ) {
      return;
    }
    if (flowAddMobileOnWeb) {
      scheduleAddMobileAnimations(
        userHasMobileDeviceAuthorised.userHasMobileDevice,
        TIMEOUT_DELAY
      );
      return;
    ...
    markAddMobileComplete();
  }, [flowAddMobileOnWeb, userHasMobileDeviceAuthorised]);
  useEffect(() => {
  ...
  const addPasswordIcon = (
    <PasswordsIcon
      title={
        addPasswordStepCompleted
          ? translate(I18N_KEYS.PASSWORD_COMPLETED_ICON_TITLE_SVG)
          : translate(I18N_KEYS.PASSWORD_ICON_TITLE_SVG)
      }
      color={addPasswordStepCompleted ? colors.dashGreen00 : colors.dashGreen04}
      size={40}
      viewBox="0 0 40 40"
    />
  );
  const addPasswordStepElement = addPasswordStepCompleted ? (
    <CompletedStep
      key={`${KEY_PREFIX}complete`}
      message={translate(I18N_KEYS.ADD_PASSWORD_TITLE)}
      leftIcon={addPasswordIcon}
      rightIcon={checkIcon}
    />
  ) : (
    <NonCompletedStep
      key={`${KEY_PREFIX}not-complete`}
      message={translate(I18N_KEYS.ADD_PASSWORD_TITLE)}
      onOnboardingStepClick={onAddPasswordStepClick}
      messageOnHover={translate(I18N_KEYS.ADD_PASSWORD_DESCRIPTION)}
      path={routes.userPasswordSites}
      leftIcon={addPasswordIcon}
      rightIcon={orangeArrowIcon}
    />
  );
    ...
    <MagicWandIcon
      title={
        tryAutofillOnWeb
          ...
          : translate(I18N_KEYS.AUTOFILL_ICON_TITLE_SVG)
      }
      color={tryAutofillOnWeb ? colors.dashGreen00 : colors.dashGreen04}
      size={40}
      viewBox="0 0 40 40"
    />
  );
  const tryAutofillStepElement = isAutofillStepCompleted ? (
    <CompletedStep
      key={`${KEY_PREFIX}complete`}
      message={translate(I18N_KEYS.TRY_AUTOFILL_TITLE)}
      leftIcon={tryAutofillIcon}
      rightIcon={checkIcon}
    />
  ) : (
    <NonCompletedStep
      key={`${KEY_PREFIX}not-complete`}
      message={translate(I18N_KEYS.TRY_AUTOFILL_TITLE)}
      onOnboardingStepClick={onTryAutofillStepClick}
      messageOnHover={translate(I18N_KEYS.TRY_AUTOFILL_DESCRIPTION)}
      path={routes.userTryAutofill}
      leftIcon={tryAutofillIcon}
      rightIcon={orangeArrowIcon}
    />
  );
    ...
    <MobileIcon
      title={
        addMobileOnWeb
          ...
          : translate(I18N_KEYS.MOBILE_ICON_TITLE_SVG)
      }
      color={addMobileOnWeb ? colors.dashGreen00 : colors.dashGreen04}
      size={38}
      viewBox="0 0 40 40"
    />
  );
  const addMobileRightIconElement = showAddMobileSpinner
    ? addMobileSpinner
    : orangeArrowIcon;
  const addMobileStepElement = addMobileOnWeb ? (
    <CompletedStep
      key={`${KEY_PREFIX}complete`}
      message={translate(I18N_KEYS.ADD_MOBILE_TITLE)}
      leftIcon={mobileIcon}
      rightIcon={checkIcon}
    />
  ) : (
    <NonCompletedStep
      key={`${KEY_PREFIX}not-complete`}
      message={translate(I18N_KEYS.ADD_MOBILE_TITLE)}
      onOnboardingStepClick={onAddMobileStepClick}
      messageOnHover={translate(I18N_KEYS.ADD_MOBILE_DESCRIPTION)}
      path={routes.userAddMobile}
      leftIcon={mobileIcon}
      rightIcon={
        <SwitchTransition>
          <CSSTransition
            key={
              showAddMobileSpinner
                ? `${KEY_PREFIX}spinner`
                : `${KEY_PREFIX}arrow`
            }
      ...
      messageOnError={
        showAddMobileError ? translate(I18N_KEYS.ADD_MOBILE_ERROR) : null
      }
    />
  );
  return (
    <div className={styles.onboardingStepsWrapper}>
      <CompletedStep
        message={translate(I18N_KEYS.GET_DASHLANE_TITLE)}
        leftIcon={dashlaneLogoIcon}
        rightIcon={checkIcon}
      />
