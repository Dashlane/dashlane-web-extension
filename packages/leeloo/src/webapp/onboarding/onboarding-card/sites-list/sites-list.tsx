...
import { SiteIcon } from 'webapp/onboarding/onboarding-card/sites-list/site-icon';
...
export const SitesList = ({ onSiteIconClick }: SitesListProps) => {
    ...
    const isB2BUser = spaces.status === DataStatus.Success && spaces.data.length > 0;
    const sites = isB2BUser ? b2bSites : b2cSites;
    return (<FlexContainer flexWrap="wrap" sx={{
            padding: '40px 0',
        }}>
      {sites.map((siteProps) => {
            return (<SiteIcon onSiteIconClick={onSiteIconClick} key={siteProps.label} {...siteProps}/>);
        })}
    </FlexContainer>);
};
