...
import { BackIcon, colors, MagicWandIcon, OpenWebsiteIcon, } from '@dashlane/ui-components';
import { openWebOnboardingTab, setOnboardingMode, } from 'webapp/onboarding/services';
...
const tryAutofillIcon = (<MagicWandIcon color={colors.dashGreen02} size={20} viewBox="0 0 40 40"/>);
const openWebsiteIcon = (<OpenWebsiteIcon color={colors.orange00} size={20} viewBox="0 2 20 20"/>);
export const StorePersonalInfo = () => {
    const { translate } = useTranslate();
    const { routes } = useRouterGlobalSettingsContext();
    const tryAutofillUrl = '*****';
    ...
    return (<div className={classnames(containerStyles.container, styles.container)}>
      <div className={containerStyles.routeContainer}>
        <Link to={routes.userOnboarding} className={containerStyles.arrowBtnContainer} onClick={onBackArrowIconClick}>
          {backArrowIcon}
        </Link>
      ...
      <div className={containerStyles.subContainer}>
        ...
        <p className={classnames(containerStyles.description, styles.description)}>
