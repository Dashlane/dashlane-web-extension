import { OnboardingType, WebOnboardingModeEvent, } from '@dashlane/communication';
import { tabsCreate } from '@dashlane/webextensions-apis';
import { carbonConnector } from 'libs/carbon/connector';
interface OnboardingOptions extends Pick<WebOnboardingModeEvent, 'leelooStep' | 'popoverStep' | 'completedSteps' | 'flowLoginCredentialOnWebSite'> {
    activeOnboardingType?: OnboardingType;
}
const makeOnboardingMode = (options?: OnboardingOptions): WebOnboardingModeEvent => {
    options = options || {};
...
export const getMobileDevices = async () => {
    return await carbonConnector.getDevicesList(null).then((result) => {
        const mobileDevicesList = result.devicesList.filter((item) => ['iphone', 'server_android', 'server_ipad', 'server_iphone'].includes(item.devicePlatform));
