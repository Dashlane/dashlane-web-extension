...
const GoPremiumButton = (props: { label: string; target: string }) => {
  const onClick = () => {
  ...
  return (
    <PrimaryButton
      classNames={[styles.goPremiumCta]}
      onClick={onClick}
      label={props.label}
    />
  );
};
const PanelInstructions = ({ target }: { target: string }) => {
  ...
  return (
    <div className={styles.panelText}>
      ...
      <GoPremiumButton
        label={translate(I18N_KEYS.UPSELL_CTA)}
        target={target}
      />
    </div>
  );
