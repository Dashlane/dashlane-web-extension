...
import { Heading, IndeterminateLoader, Infobox, jsx, Paragraph, } from '@dashlane/design-system';
...
export const PermissionStep = ({ goToStep, isLoading, isCollectionSharing = false, onSelectPermission, permission, onClick, }: PermissionStepProps) => {
    const { translate } = useTranslate();
    const handleChange = (event: React.FormEvent<HTMLInputElement>) => onSelectPermission(event.currentTarget.value as Permission);
    return (<>
      <Heading as="h1" sx={{ mb: '16px' }}>
        {translate(I18N_KEYS.SELECT)}
      </Heading>
      {isCollectionSharing && (<Infobox mood="brand" title={translate(I18N_KEYS.INFO_BOX)} sx={{ mb: '16px' }}/>)}
      <DialogBody>
        <div sx={{ height: 'auto' }}>
          <RadioButtonGroup groupName="permission" value={permission} onChange={handleChange}>
      ...
      <DialogFooter secondaryButtonOnClick={goToStep} secondaryButtonTitle={translate(I18N_KEYS.BACK_BUTTON)} secondaryButtonProps={{ disabled: isLoading, type: 'button' }} primaryButtonOnClick={onClick} primaryButtonTitle={isLoading ? (<IndeterminateLoader size={24} color="ds.text.inverse.catchy"/>) : (translate(I18N_KEYS.SHARE_BUTTON))} primaryButtonProps={{
