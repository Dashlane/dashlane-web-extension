...
export const useIsCollectionProtected = (
  collectionId: string
): [boolean | undefined, IsCollectionProtectedStatus] => {
  const [isProtected, setIsProtected] = useState<boolean | undefined>(
    undefined
  );
  const [status, setStatus] = useState<IsCollectionProtectedStatus>(
    IsCollectionProtectedStatus.Unknown
  );
  ...
  useEffect(() => {
    async function getCollectionItems(collectionView: ShareableCollection) {
      ...
      const collectionCredentialIds = collectionView.vaultItems
        ...
        .map(({ id }) => id);
      const hasProtectedItems = await checkForProtectedItems(
        collectionCredentialIds,
        []
      );
