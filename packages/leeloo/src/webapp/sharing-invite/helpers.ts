...
import { ItemsTabs, Sharing, SharingInviteStep, SharingInviteUser, } from './types';
...
export function getPrivateCollectionSharing(privateCollectionId: string): Sharing {
    return {
        ...getDefaultSharing(),
...
export function getProtectedPrivateCollectionSharing(privateCollectionId: string): Sharing {
    return {
        ...getDefaultSharing(),
        selectedPrivateCollections: [privateCollectionId],
...
export const sortGroups = sortBy((g: UserGroupDownload) => toLower(g.name || ''));
export const sortUsers = sortBy((u: SharingInviteUser) => toLower(u.id ?? u));
export const checkForProtectedItems = async (selectedCredentialList: string[], selectedNoteList: string[]) => {
    const mpSettingsResponse = await carbonConnector.arePasswordsProtected();
    if (mpSettingsResponse) {
        return true;
    }
    const credentials = await Promise.all(selectedCredentialList.map((credentialId) => carbonConnector.getCredential(credentialId)));
    const someCredentialsProtected = credentials.some((credential) => !!credential?.autoProtected);
    if (someCredentialsProtected) {
        return true;
    }
    const notes = await Promise.all(selectedNoteList.map((noteId) => carbonConnector.getNote(noteId)));
