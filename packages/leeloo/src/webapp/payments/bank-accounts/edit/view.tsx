...
import { BankAccount, vaultItemsCrudApi, VaultItemType, } from '@dashlane/vault-contracts';
import { Alert, AlertSeverity, AlertSize, colors, } from '@dashlane/ui-components';
...
import { useAlertDisplayLogic } from 'webapp/payments/bank-accounts/useAlertDisplayLogic';
...
export const BankAccountEditView = ({ item }: BankAccountEditViewProps) => {
    ...
    const { areProtectedItemsUnlocked, openProtectedItemsUnlocker, protectedItemsUnlockerShown, } = useProtectedItemsUnlocker();
    ...
    const handleEditForm = (newContent: BankAccount) => {
    ...
    const ignoreCloseOnEscape = protectedItemsUnlockerShown || displayConfirmDeleteDialog;
    return (<EditPanel onNavigateOut={showListView} ignoreCloseOnEscape={ignoreCloseOnEscape} formId={FORM_ID} onSubmit={handleSubmit} header={<PanelHeader icon={<BankAccountIcon />} iconBackgroundColor={colors.dashGreen06} title={item.accountName}/>} onClickDelete={openConfirmDeleteDialog} itemHasBeenEdited={hasDataBeenModified} isViewingExistingItem={true}>
      <BankAccountForm itemId={item.id} bankAccount={bankAccount} errors={errors} onCopy={showCopyAlert} signalEditedValues={handleEditForm} areProtectedItemsUnlocked={areProtectedItemsUnlocked} openProtectedItemsUnlocker={openProtectedItemsUnlocker} variant="edit"/>
      <ConfirmDeleteVaultItemDialog isVisible={displayConfirmDeleteDialog} itemId={item.id} closeConfirmDeleteDialog={closeConfirmDeleteDialog} onDeletionSuccess={showListView} translations={deleteTranslations} vaultItemType={VaultItemType.BankAccount}/>
      {copyAlert.isAlertVisible && (<div className={alertStyles.alertWrapper}>
          <Alert size={AlertSize.SMALL} severity={AlertSeverity.SUCCESS} closeIconName={translate(I18N_KEYS.DISMISS)} onClose={dismissAlert}>
