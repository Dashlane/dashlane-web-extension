...
import getBackgroundColor from "../../payment-card-icon/getBackgroundColorForPaymentCard";
import { useAlertDisplayLogic } from "../bank-accounts/useAlertDisplayLogic";
import alertStyles from "../bank-accounts/alert.css";
...
const PaymentCardEditPanelComponent = ({ item }: ComponentProps) => {
  ...
  const [errors, setErrors] =
    React.useState<Set<keyof PaymentCardFormFields>>();
  const [displayConfirmDeleteDialog, setDisplayConfirmDeleteDialog] =
    React.useState(false);
  const [paymentCardContent, setPaymentCardContent] =
    React.useState<PaymentCard>(item);
  React.useEffect(() => {
  ...
  const handleEditForm = (newContent: PaymentCard) => {
    setPaymentCardContent((currentContent) => {
    ...
    <EditPanel
      ...
      header={
        <PanelHeader
          icon={
            <PaymentCardIcon
              paymentCardColor={paymentCardContent.color}
              iconSize="large"
            />
          }
          iconBackgroundColor={getBackgroundColor(paymentCardContent.color)}
          title={itemTitle}
        />
      }
    >
      <PaymentCardForm
        itemId={item.id}
        errors={errors}
        paymentCardContent={paymentCardContent}
...
export const PaymentCardEditPanel = ({ match }: Props) => {
  ...
  return (
    <PaymentCardEditPanelComponent item={data.paymentCardsResult.items[0]} />
  );
};
