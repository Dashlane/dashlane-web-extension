...
export const BankAccountSearchItem = ({ bankAccount, onSelectBankAccount, getRoute, style, }: BankAccountSearchItemProps) => {
    ...
    return (<div style={style}>
      <div className={classNames(styles.item, editPanelIgnoreClickOutsideClassName)} onMouseLeave={() => {
            if (copyDropdownIsOpen) {
                setCopyDropdownIsOpen(false);
            }
        }}>
        <Link onClick={onSelectBankAccount} to={getRoute(id)} className={styles.link} onDragStart={preventDragAndDrop} onDrop={preventDragAndDrop}>
          <div className={styles.logoCell}>
            <Thumbnail size="small" icon={AllowedThumbnailIcons.Bank} backgroundColor={dashGreen00}/>
          </div>
        ...
        <div className={styles.actions}>
          <BankAccountActions bankAccount={bankAccount} dropdownIsOpen={copyDropdownIsOpen} setDropdownIsOpen={setCopyDropdownIsOpen} mode={BankAccountActionsMode.SEARCH}/>
        </div>
      </div>
    </div>);
};
