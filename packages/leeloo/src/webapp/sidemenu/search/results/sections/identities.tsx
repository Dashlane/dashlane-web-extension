...
import { getIdentityItemFullName, getViewedIdentitySecondLine, } from 'webapp/personal-info/services';
...
export const Identities = ({ query }: IdentitiesProps) => {
    ...
    const { loadMore, result } = useItemSearchData<Identity>(query, VaultItemType.Identity);
    if (!result?.matchCount) {
        return null;
    }
    const { items, matchCount } = result;
    SearchEventLogger.updateSearchSubTypes('identites', matchCount);
    return (<SearchResultsSection i18nKey={I18N_KEYS.IDENTITIES_HEADER} loadMore={loadMore} matchCount={matchCount} loadedCount={items.length}>
      {items.map((identity, index) => (<PersonalInfoSearchItem icon={<PersonalInfoIcon iconType={IconType.identity} iconSize={IconSize.smallIcon}/>} key={identity.id} text={getViewedIdentitySecondLine(identity.birthDate, identity.birthPlace, translate)} title={getIdentityItemFullName(identity)} detailRoute={routes.userPersonalInfoIdentity(identity.id)} onSelectPersonalInfo={() => {
                SearchEventLogger.logSearchEvent();
                logSelectPersonalInfo(identity.id, ItemType.Identity, index + 1, matchCount);
            }}/>))}
