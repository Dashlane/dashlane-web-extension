...
import { CredentialSearchItem } from 'webapp/sidemenu/search/items';
...
export const Credentials = ({ lee, query }: CredentialsProps) => {
    const { loadMore, result } = useItemSearchData<Credential>(query, VaultItemType.Credential);
    if (!result?.matchCount) {
        return null;
    }
    const { items, matchCount } = result;
    SearchEventLogger.updateSearchSubTypes('credentials', matchCount);
    return (<SearchResultsSection i18nKey={I18N_KEYS.PASSWORDS_HEADER} loadMore={loadMore} matchCount={matchCount} loadedCount={items.length}>
      {items.map((credential, index) => (<CredentialSearchItem lee={lee} credential={credential} key={credential.id} onSelectCredential={() => {
