...
import { IdVaultItemType } from "../../../../../ids/types";
...
const Ids: {
  ...
  { type: VaultItemType.DriversLicense, title: I18N_KEYS.DRIVER_LICENSES },
  { type: VaultItemType.FiscalId, title: I18N_KEYS.FISCAL_NUMBERS },
  { type: VaultItemType.IdCard, title: I18N_KEYS.ID_CARDS },
  { type: VaultItemType.Passport, title: I18N_KEYS.PASSPORTS },
  {
    type: VaultItemType.SocialSecurityId,
    title: I18N_KEYS.SOCIAL_SECURITY_IDS,
  },
];
export const IdDocuments = ({ query }: Props) => {
  return (
    <>
      {Ids.map((id) => (
        <IdDocument
          key={`search_result_id_${id.type}`}
          idVaultItemType={id.type}
          title={id.title}
          query={query}
        />
      ))}
