...
import { NoteSearchItem } from "../../items";
...
export const Notes = ({ query }: NotesProps) => {
  ...
  const { loadMore, result } = useItemSearchData<SecureNote>(
    query,
  ...
  return (
    <SearchResultsSection
      i18nKey={I18N_KEYS.SECURE_NOTES_HEADER}
      loadMore={loadMore}
      matchCount={matchCount}
      loadedCount={items.length}
    >
      {items.map((note: SecureNote, index: number) => (
        <NoteSearchItem
