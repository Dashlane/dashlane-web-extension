...
const PaginatedMenu = () => {
  ...
  const currentData = useMemo(
    () =>
      isPaginationNeeded
        ...
        : allCollections,
    [allCollections, currentPage, isPaginationNeeded]
  );
  ...
  return (
    <SidemenuSection title={<MenuTitle />}>
      {currentData.map((collection) => (
        <CollectionMenuItem
          key={`collections_sidemenu_${collection.id}`}
          collection={collection}
          isPersonalSpaceDisabled={isPersonalSpaceDisabledResult.isDisabled}
        />
      ))}

      {isPaginationNeeded && remaining > 0 && (
        <Button sx={{ ml: "8px" }} intensity="quiet" onClick={loadMore}>
      ...
      {isPaginationNeeded && remaining <= 0 && (
        <Button sx={{ ml: "8px" }} intensity="quiet" onClick={clear}>
...
export const CollectionsMenu = () => {
  ...
  const isCollectionsPageEnabled =
    featureFlipsStatus.data[
      FEATURE_FLIPS_WITHOUT_MODULE.CollectionsPageEnabled
    ];
  ...
  return (
    <SidemenuSection title={<MenuTitle />}>
      <MenuItem
        ...
        key="collections_sidemenu_all_collections"
        text="collections_overview_title"
        to={routes.userCollections}
      />
    </SidemenuSection>
  );
};
