...
import { ArrowDownIcon } from '@dashlane/ui-components';
...
import { SpaceEntry, SpaceIcon, } from 'webapp/components/space-and-sharing-icons/space-icon';
...
export class SpacesSwitch extends PureComponent<Props, ComponentState> {
    ...
    private selectSpaceEntry(spaceEntryKey: SpaceEntryKey, spaceId: string | null): void {
        this.setState({
            currentSpaceEntryKey: spaceEntryKey,
            isOpen: false,
        });
        logEvent(new UserSelectSpaceEvent({
    ...
    private getSpaceItem(spaceEntryKey: SpaceEntryKey, spaceEntry: SpaceEntry) {
        ...
        const onClick = () => isCurrentSpace
            ? this.toggle()
            : this.selectSpaceEntry(spaceEntryKey, spaceEntry.id);
        return (<button className={classnames(styles.row, {
                [styles.rowActive]: isOpen && isCurrentSpace,
                [styles.rowDefault]: !isOpen && !isCurrentSpace,
            })} key={`space-${spaceEntry.name}`} onClick={onClick} type="button">
        <div className={styles.rowLeft}>
          <SpaceIcon space={spaceEntry}/>
          <span className={styles.name}>{spaceEntry.name}</span>
        </div>
        {!isOpen ? (<ArrowDownIcon />) : isDefaultSpace ? (<ArrowDownIcon rotate={180}/>) : null}
      </button>);
    }
    public render() {
        ...
        {isOpen ? (<div className={styles.container} onClick={this.close}>
            {Object.entries(spaceEntries).map(([key, spaceEntry]) => this.getSpaceItem(key as SpaceEntryKey, spaceEntry))}
          </div>) : (this.getSpaceItem(currentSpaceEntryKey, currentSpaceEntry))}
      </OutsideClickHandler>);
    }
}
