...
export const MenuItemTooltip = ({
  ...
  showNotification,
  width,
}: PropsWithChildren<Props & TooltipProps>) => {
  ...
  return (
    <Tooltip
      ...
      passThrough={!showNotification || !(title ?? description)}
      arrowSize={8}
      placement={placement}
      trigger={trigger}
      content={
        <GridContainer sx={{ p: "8px" }} justifyItems="flex-start">
          {title ? (
            <Paragraph
              size="large"
              sx={{
          ...
          {description ? (
            <Paragraph
              size="x-small"
              sx={{ color: colors.dashGreen04, textAlign: "start" }}
            >
              {translateWithMarkup(description)}
            </Paragraph>
          ) : null}
          {(cancelLabel && handleCancel) || (confirmLabel && handleConfirm) ? (
            <GridChild sx={{ justifySelf: "end", mt: "16px" }}>
              {cancelLabel ? (
                <Button
                  size="small"
                  nature="secondary"
