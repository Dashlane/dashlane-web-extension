...
const SideMenuCollapsedProvider = ({ children }: Provider) => {
    ...
    const [isSideMenuCollapsed, setIsSideMenuCollapsed] = useState(isViewportTooSmallForSidemenu(previousViewportWidth.current));
    useEffect(() => {
        const windowResizeListener = () => {
            const currentViewportWidth = document.documentElement.clientWidth;
            const isWindowWidthDecreasing = currentViewportWidth < previousViewportWidth.current;
            previousViewportWidth.current = currentViewportWidth;
            if (isWindowWidthDecreasing &&
    ...
    return (<SideMenuCollapsedContext.Provider value={contextValue}>
      {children}
