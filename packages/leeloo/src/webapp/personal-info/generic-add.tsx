...
import { CreateVaultItemParam, vaultItemsCrudApi, VaultItemType, } from '@dashlane/vault-contracts';
import { Lee } from 'lee';
...
type RenderAddForm<F> = (lee: Lee, ref: Ref<F>, signalEditedValues: () => void, currentSpaceId: string | null) => JSX.Element;
interface GenericAddParams<F> {
    renderForm: RenderAddForm<F>;
    vaultItemType: PersonalInfoItemType;
    iconType: IconType;
}
export function getPersonalInfoAddPanel<F extends PersonalInfoForm>(params: GenericAddParams<F>) {
    ...
    const PersonalInfoAddPanel = ({ lee }: Props) => {
        ...
        const itemIcon = (<PersonalInfoIcon iconSize={IconSize.headerEditPanelIcon} iconType={iconType}/>);
        const itemTitle = getNewPersonalInfoName(translate, vaultItemType);
        return (<EditPanel isViewingExistingItem={false} itemHasBeenEdited={hasDataBeenModified} onNavigateOut={showListView} onSubmit={submit} formId="add_personalinfo_panel" header={<PanelHeader icon={itemIcon} iconBackgroundColor={colorsVar['--dash-green-00']} title={itemTitle}/>}>
