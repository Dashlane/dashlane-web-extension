...
import { Icon, jsx, mergeSx, Paragraph } from '@dashlane/design-system';
import { DataStatus, useModuleQuery } from '@dashlane/framework-react';
import { ItemType } from '@dashlane/hermes';
import { Address, Company, Email, Identity, Phone, vaultItemsCrudApi, VaultItemType, Website, } from '@dashlane/vault-contracts';
...
import GridView, { GridData } from 'webapp/grid-view';
import PersonalInfoIcon, { IconSize, IconType, } from 'webapp/personal-info-icon';
import { getIdentityItemFullName, getPhoneItemName, getViewedAddressSecondLine, getViewedIdentitySecondLine, } from 'webapp/personal-info/services';
...
const getIcon = (iconType: IconType): JSX.Element => (<PersonalInfoIcon iconType={iconType} iconSize={IconSize.largeIcon}/>);
...
const GridItemPersonalInfo = ({ id, icon, title, text, onClick, to, }: GridPersonalItemProps) => {
    return (<Link key={id} sx={{
      ...
      <div sx={{
        ...
        <Paragraph textStyle="ds.body.standard.regular" color="ds.text.neutral.catchy" sx={mergeSx([
            SX_STYLES.TEXT,
            {
                marginTop: '10px',
                alignItems: 'center',
        ...
        <Paragraph textStyle="ds.body.reduced.regular" color="ds.text.neutral.quiet" sx={mergeSx([
            SX_STYLES.TEXT,
            {
                marginTop: '10px',
            },
...
const renderAddress = (routes: NamedRoutes, address: Address, translate: TranslatorInterface) => (<GridItemPersonalInfo key={address.id} id={address.id} icon={getIcon(IconType.address)} text={getViewedAddressSecondLine(address.city, address.country, address.streetName, address.zipCode, translate)} title={address.itemName} onClick={() => {
        logSelectPersonalInfo(address.id, ItemType.Address);
    }} to={routes.userPersonalInfoAddress(address.id)}/>);
const renderCompany = (routes: NamedRoutes, company: Company) => (<GridItemPersonalInfo key={company.id} id={company.id} icon={getIcon(IconType.company)} text={company.jobTitle} title={company.companyName} onClick={() => {
        logSelectPersonalInfo(company.id, ItemType.Company);
    }} to={routes.userPersonalInfoCompany(company.id)}/>);
const renderEmail = (routes: NamedRoutes, email: Email) => (<GridItemPersonalInfo key={email.id} id={email.id} icon={getIcon(email.type === 'PRO' ? IconType.emailPro : IconType.email)} text={email.emailAddress} title={email.itemName} onClick={() => {
        logSelectPersonalInfo(email.id, ItemType.Email);
    }} to={routes.userPersonalInfoEmail(email.id)}/>);
const renderIdentity = (routes: NamedRoutes, identity: Identity, translate: TranslatorInterface) => (<GridItemPersonalInfo key={identity.id} id={identity.id} icon={getIcon(IconType.identity)} text={getViewedIdentitySecondLine(identity.birthDate, identity.birthPlace, translate)} title={getIdentityItemFullName(identity)} onClick={() => {
        logSelectPersonalInfo(identity.id, ItemType.Identity);
    }} to={routes.userPersonalInfoIdentity(identity.id)}/>);
const renderWebsite = (routes: NamedRoutes, website: Website) => (<GridItemPersonalInfo key={website.id} id={website.id} icon={getIcon(IconType.website)} text={website.URL} title={website.itemName} onClick={() => {
        logSelectPersonalInfo(website.id, ItemType.Website);
    }} to={routes.userPersonalInfoWebsite(website.id)}/>);
const renderPhone = (routes: NamedRoutes, phone: Phone, translate: TranslatorInterface) => (<GridItemPersonalInfo key={phone.id} id={phone.id} icon={getIcon(IconType.phone)} text={phone.phoneNumber} title={getPhoneItemName(phone, translate)} onClick={() => {
...
const getData = (categoryStates: CategoryStates, personalItems: PersonalData, routes: NamedRoutes, translate: TranslatorInterface): GridData<Address | Company | Email | Identity | Phone | Website>[] => {
    const { addresses, companies, emails, identities, websites, phones } = personalItems;
    return [
        {
            key: 'identities',
...
export const Content = ({ onToggleGrid, categoryStates, currentSpaceId, }: Props) => {
    ...
    if (status !== DataStatus.Success ||
        ...
        return (<EmptyView icon={<Icon name="UsersOutlined" color="ds.text.neutral.standard" sx={{ width: '64px', minWidth: '64px', height: '64px' }}/>} title={translate(I18N_KEYS.TITLE)}>
        <Paragraph color="ds.text.neutral.standard">
          {translate(I18N_KEYS.DESCRIPTION)}
        </Paragraph>
      </EmptyView>);
    }
    const gridData = getData(categoryStates, {
    ...
    return (<GridView data={gridData} onToggleGrid={onToggleGrid} sxProps={{
            width: '160px',
            padding: '32px',
            position: 'relative',
