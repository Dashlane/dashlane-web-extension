...
const NEW_PERSONAL_INFO_NAME_I18N_KEYS_DICTIONARY: Record<
  PersonalInfoItemType,
  string
> = {
  [VaultItemType.Address]:
    "webapp_personal_info_edition_header_address_new_item_title",
  [VaultItemType.Company]:
    "webapp_personal_info_edition_header_company_new_item_title",
  [VaultItemType.Email]:
    "webapp_personal_info_edition_header_email_new_item_title",
  [VaultItemType.Identity]:
    "webapp_personal_info_edition_header_identity_new_item_title",
  [VaultItemType.Phone]:
    "webapp_personal_info_edition_header_phone_new_item_title",
  [VaultItemType.Website]:
    "webapp_personal_info_edition_header_website_new_item_title",
};
export function getNewPersonalInfoName(
  translateFn: TranslateFunction,
  personalInfoItemType: PersonalInfoItemType
): string {
  return translateFn(
    NEW_PERSONAL_INFO_NAME_I18N_KEYS_DICTIONARY[personalInfoItemType]
  );
}
function getPhoneItemType(
  translateFn: TranslateFunction,
  phoneType: PhoneType
): string {
  switch (phoneType) {
...
export function getPhoneItemName(
  item: Phone,
  translateFn: TranslateFunction
): string {
...
export function getIdentityItemFullName(
  item: Pick<
    Identity,
    "firstName" | "middleName" | "lastName" | "lastName2" | "pseudo"
  >
): string {
...
export function getViewedIdentitySecondLine(
...
export function getViewedAddressSecondLine(
  city: string,
  country: Country,
  streetName: string,
  zipCode: string,
  ...
  return [
    ...
    zipCode,
    city,
    country ? translator(`country_name_${country.toUpperCase()}`) : "",
  ]
    .filter((addressPart) => !!addressPart)
    .join(", ");
}
export function getTranslatedAddressCountry(
  translateFn: TranslateFunction,
