...
import { Address, Company, Email, Identity, Phone, UpdateVaultItemCommandParam, vaultItemsCrudApi, VaultItemType, Website, } from '@dashlane/vault-contracts';
...
import { PersonalInfoForm, PersonalInfoItemType, } from 'webapp/personal-info/types';
...
type RenderEditForm<T, F> = (lee: Lee, item: T, ref: Ref<F>, signalEditedValues: () => void) => JSX.Element;
...
export function getPersonalInfoEditPanel<T extends Address | Company | Email | Identity | Phone | Website, F extends PersonalInfoForm>(params: GenericEditParams<T, F>) {
    const { getItemTypeDescription, getDeleteTitle, getTitle, iconType, vaultItemType, renderForm, } = params;
    const PersonalInfoEditPanel = ({ lee, item, match }: Props<T>) => {
        ...
        return (<EditPanel isViewingExistingItem={Boolean(match.params?.uuid)} itemHasBeenEdited={hasDataBeenModified} onNavigateOut={showListView} onSubmit={handleSubmit} onClickDelete={handleClickDelete} ignoreCloseOnEscape={displayConfirmDeleteDialog} formId="edit_personalinfo_panel" header={<PanelHeader icon={<PersonalInfoIcon iconSize={IconSize.headerEditPanelIcon} iconType={iconType}/>} iconBackgroundColor={colorsVar['--dash-green-00']} title={itemTitle} titleDescription={getItemTypeDescription(translate)}/>}>
        {renderForm(lee, item, formRef, handleEditedForm)}

        <ConfirmDeleteVaultItemDialog isVisible={displayConfirmDeleteDialog} itemId={item.id} closeConfirmDeleteDialog={closeConfirmDeleteDialog} onDeletionSuccess={showListView} translations={deleteTranslations} vaultItemType={vaultItemType}/>
      </EditPanel>);
    };
