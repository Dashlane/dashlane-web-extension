...
export const CollectionsSearch = () => {
    ...
    const placeholder = translate('collections_overview_search_input_placeholder');
    return (<TextInput ref={searchInputRef} placeholder={placeholder} aria-label={placeholder} prefixIcon={<Icon name="ActionSearchOutlined" size="medium"/>} onChange={onChangeHandler} actionButtons={[
            <SearchCloseIcon key="clear-collections-search" onClick={onSearchFieldClear} isHidden={!displayClearIcon}/>,
        ]} autoFocus intensity="quiet" sx={{
            maxWidth: '300px',
            margin: '16px 32px',
        }}/>);
};
