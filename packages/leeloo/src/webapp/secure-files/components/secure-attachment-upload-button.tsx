...
import { useSecureFileUpload } from 'webapp/secure-files/hooks/use-secure-file-upload';
import { validateSecureFile } from 'webapp/secure-files/services/validate-secure-file';
import { useIsSecureFileAttachmentUploadAvailable, useSecureFileAllowedExtensions, useSecureFileUploadAlerts, } from 'webapp/secure-files/hooks';
...
export const SecureAttachmentUploadButton = ({ onFileUploadStarted, onFileUploadDone, isQuotaReached, isShared, itemId, dataType, disabled = false, }: SecureAttachmentUploadButtonProps) => {
    const isSecureFileUploadAvailable = useIsSecureFileAttachmentUploadAvailable(dataType === 'KWIDCard') &&
        !isShared;
    const itemType = dataModelTypeToItemType[dataType] ??
    ...
    return isSecureFileUploadAvailable ? (<FlexContainer>
      <input type="file" id="secureFileInput" sx={{
            display: 'none',
        }} onChange={(event) => {
      ...
      <Button key="attach" mood="neutral" intensity="quiet" icon="AttachmentOutlined" layout="iconLeading" onClick={handleSelectFile} disabled={isQuotaReached || isUploading || disabled}>
