...
import { AddSecureFileResult, SecureFileResultErrorCode, } from '@dashlane/communication';
...
async function onFileLoad(fileReader: FileReader, file: File, itemType: ItemType, callback?: (result: AddSecureFileResult) => void) {
...
export function useSecureFileUpload(itemType: ItemType) {
    ...
    return {
        isUploading,
        upload: (file?: File | null, callback?: (result: AddSecureFileResult) => void) => {
            if (file) {
                if (file.size > MAX_FILE_SIZE) {
                    callback?.({
                        success: false,
                    ...
                    alert.showAlert(wrapWithLoadingIcon(translateWithMarkup({
                        key: I18N_KEYS.PROGRESS_CIPHERING,
                        params: {
                            fileName: displayName,
                        },
                    })), AlertSeverity.SUBTLE, false);
                    setIsUploading(true);
                    const fileReader = new FileReader();
                    const onceFinished: (result: AddSecureFileResult) => void = (result) => {
                        callback?.(result);
                        setIsUploading(false);
                    };
                    fileReader.onloadend = async () => onFileLoad(fileReader, file, itemType, onceFinished);
