...
import { useIsSecureNoteAttachmentEnabled } from './use-is-secure-note-attachment-enabled';
export function useIsSecureFileAttachmentUploadAvailable(fromIds: boolean) {
    ...
    const isFeatureEnabled = useIsSecureNoteAttachmentEnabled();
    const isCapabilityEnabled = useCapabilitiesEnabled(['secureFiles']);
    const isAvailable = isFeatureEnabled && isCapabilityEnabled;
