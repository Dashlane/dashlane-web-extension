...
const PaymentFailureChurningComponent = () => {
  ...
  const formattedEndDate = translate.shortDate(
    fromUnixTime(premiumStatus.endDate),
    LocaleFormat.LL
  );
  const goToCheckout = (): void => {
    ...
    const goPremiumUrl = routes.userGoPremium(subscriptionCode ?? "");
    openDashlaneUrl(goPremiumUrl, trackingParams);
  };
  return (
    <div className={styles.floatingNotificationsContainer}>
      <CSSTransition
        classNames={{
      ...
      >
        <InfoBox
          title={translate(I18N_KEYS.TITLE, { endDate: formattedEndDate })}
          size="descriptive"
          severity="alert"
          primary={goToCheckout}
          primaryLabel={translate(I18N_KEYS.CTA)}
        />
      </CSSTransition>
    </div>
  );
};
export const PaymentFailureChurning = React.memo(
