...
import { WebOnboardingLeelooStep, WebOnboardingPopoverStep, } from '@dashlane/communication';
...
export const OnboardingActionNotifications = ({ flowLoginCredentialOnWeb, leelooStep, popoverStep, }: Props) => {
    const { translate } = useTranslate();
    const isExtensionWebOnboardingEnabled = () => {
        return APP_PACKAGED_IN_EXTENSION;
    };
    const showExtensionWebCard = isExtensionWebOnboardingEnabled() &&
        flowLoginCredentialOnWeb &&
        leelooStep ===
            WebOnboardingLeelooStep.SHOW_LOGIN_USING_EXTENSION_NOTIFICATION &&
        popoverStep === null;
    const showBrowserPinningWebCard = showExtensionWebCard &&
        isChromiumExtension(window.location.href) &&
        !isInEdgeExtension(window.location.href);
    const showDefaultExtensionWebCard = showExtensionWebCard && !isChromiumExtension(window.location.href);
    useEffect(() => {
    ...
    return (<>
      <ActionNotification title={translate('webapp_onboarding_notification_chrome_pinning_title')} secondTitle={translate('webapp_onboarding_notification_chrome_pinning_subtitle')} imageSrc={extensionPinningImg} show={showBrowserPinningWebCard} cancelLabel={translate('webapp_onboarding_notification_chrome_pinning_cancel')} onCancel={onChromePinningDismiss}/>
      <ActionNotification title={translate('webapp_onboarding_notification_login_sites_title')} show={showDefaultExtensionWebCard}/>
    </>);
};
