...
export const LeakedMasterPasswordInfobox = () => {
    ...
    const onSettingsClick = () => redirectToUrl('*****');
    return isLeakedMasterPassword ? (<Infobox title={translate(I18N_KEYS.TITLE)} description={translate(I18N_KEYS.DESCRIPTION)} mood="danger" size="large" sx={{ marginBottom: '8px' }} actions={[
            <Button key="hc-link" mood="danger" intensity="quiet" onClick={onBlogClick} size="small">
          {translate(I18N_KEYS.BLOG_LINK)}
        </Button>,
            <Button key="settings" mood="danger" onClick={onSettingsClick} size="small">
