...
import { getBreachDate } from "../helpers/date";
import { ContentPasswordSection } from "./content/breach-details-password-content";
import { ContentOtherSection } from "./content/breach-details-other-content";
...
export const BreachDetailsDialog = ({
  breachId,
  onDismissClick,
}: BreachDetailsDialogProps) => {
  ...
  const { eventDate, leakedData, domains, impactedEmails, leakedPasswords } =
    breach;
  const [firstDomain] = domains;
  const isPasswordLeaked = leakedData.includes(BreachLeakedDataType.Password);
  const isCreditCardLeaked = leakedData.includes(
  ...
  return (
    <>
      ...
      <Dialog
        ...
        disableOutsideClickClose={!!passwordChangeCredentialId}
      >
        <DialogTitle>
          <div className={styles.dialogHeader}>
            <div className={styles.icon}>
              <CredentialThumbnail
                domain={firstDomain}
                title={firstDomain ?? DEFAULT_THUMBNAIL_TEXT}
        ...
        <DialogBody>
          ...
          {leakedPasswords.length === 0 ? (
            <ContentOtherSection
              ...
              isCreditCardLeaked={isCreditCardLeaked}
              leakedData={leakedData}
              domain={firstDomain}
            />
          ) : (
            <ContentPasswordSection
              passwords={leakedPasswords}
              leakedData={leakedData}
              domain={firstDomain}
            />
          )}
        </DialogBody>
        {compromisedCredentialsIds.length === 1 ? (
          <DialogFooter
            primaryButtonTitle={translate(I18N_KEYS.ACTION_DISMISS)}
            primaryButtonOnClick={onDismissClick}
            secondaryButtonTitle={translate(I18N_KEYS.ACTION_CHANGE_PASSWORD)}
          ...
          <DialogFooter
            primaryButtonTitle={translate(I18N_KEYS.ACTION_CLOSE)}
            primaryButtonOnClick={onDismissClick}
          />
        )}
      </Dialog>
