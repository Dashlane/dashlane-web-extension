...
import { AlertSeverity, UseAlert } from "@dashlane/ui-components";
...
const I18N_KEYS = {
  ...
  GO_PREMIUM_TOOLTIP:
    "webapp_darkweb_email_monitored_emails_add_email_go_premium_tooltip",
  ...
  REMOVE_EMAIL_SUCCESS:
    "webapp_darkweb_email_stop_monitoring_dialog_email_success_alert",
};
...
export const MonitoredEmails = ({
  ...
  const handleRemovalSuccess = useCallback(
    (email: string) => {
      removalOutcomeAlert.setMessage(
        translate(I18N_KEYS.REMOVE_EMAIL_SUCCESS, { email })
      );
    ...
    <>
      <Card sx={SX_STYLES.emailsCard}>
        <Flex sx={SX_STYLES.subtitle}>
          ...
          <Icon
            role="button"
            aria-label={translate(I18N_KEYS.MONITORED_EMAILS)}
            name="FeedbackInfoOutlined"
            size="xsmall"
            color="ds.text.neutral.quiet"
            tooltip={translate(getMonitoredTooltipTitleKey())}
            sx={{ marginLeft: "6px" }}
          />
        </Flex>
        <ul sx={{ justifyContent: "center" }}>
          {emails
            ? emails.map(({ email, state }) => (
                <MonitoredEmail
                  key={email}
                  email={email}
                  state={state}
                  canEmailBeRemoved={email !== b2bAssociatedEmail}
                  handleOnRemoveClick={openRemoveDialog}
                  handleOnClosePendingNotification={
                    handleOnClosePendingNotification
                  }
                  showPendingNotification={email === newlyAddedEmail}
                  showPendingTooltip={email !== newlyAddedEmail}
                />
              ))
            : null}
          {[...Array(availableEmailSpots)].map((val, index) => (
            <PlaceholderEmail key={index} />
          ))}
        </ul>

        <Flex sx={SX_STYLES.emailFooter}>
          <Paragraph
      ...
      {emailToRemove ? (
        <RemoveEmailDialog
          email={emailToRemove}
          handleOnCloseDialog={onCancelRemoveEmail}
          handleOnRemoveEmail={onRemoveEmail}
        />
      ) : null}
    </>
  );
