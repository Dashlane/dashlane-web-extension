...
import { Flex, Paragraph, Tooltip } from "@dashlane/design-system";
import { DataLeaksEmailStatus } from "@dashlane/password-security-contracts";
import useTranslate from "../../../../libs/i18n/useTranslate";
import { PropsWithChildren } from "react";
import {
  PendingEmailStatusWithTooltips,
  PendingEmailStatusWithTooltipsProps,
} from "./pending-email-status-with-tooltips";
const I18N_KEYS = {
  ...
  BUSINESS_EMAIL_INFO_ICON_TEXT:
    "webapp_darkweb_bussiness_email_monitoring_info",
};
...
const CantRemoveEmailIconWithTooltip = () => {
  ...
  return (
    <Tooltip
      content={translate(I18N_KEYS.BUSINESS_EMAIL_TOOLTIP)}
      location="bottom"
    >
      <div data-testid="autoEnroll-tooltip">
        <InfoCircleIcon size={16} data-testid="b2b-auto-enroll-email" />
      </div>
    </Tooltip>
  );
};
const MonitoredEmailStatusText = ({
  statusText,
  textColor,
  enabledRemoval,
}: {
...
const MonitoredEmailStatusWrapper = ({
  children,
  enabledRemoval,
}: PropsWithChildren<{
  enabledRemoval: boolean;
...
export const MonitoredEmailStatus = (props: MonitoredEmailStatusProps) => {
  ...
  return (
    <MonitoredEmailStatusWrapper enabledRemoval={canEmailBeRemoved}>
      {state === DataLeaksEmailStatus.PENDING ? (
        <PendingEmailStatusWithTooltips {...props}>
