...
import {
  NewPendingEmailTooltip,
  NewPendingEmailTooltipProps,
} from "./new-pending-email-tooltip";
import { PendingEmailTooltip } from "./pending-email-tooltip";
export type PendingEmailStatusWithTooltipsProps =
  NewPendingEmailTooltipProps & {
    showPendingTooltip: boolean;
    showPendingNotification: boolean;
    canEmailBeRemoved: boolean;
  };
export const PendingEmailStatusWithTooltips = (
  props: PropsWithChildren<PendingEmailStatusWithTooltipsProps>
) => {
  ...
  if (showPendingTooltip) {
    ...
    return (
      <NewPendingEmailTooltip {...props}>{children}</NewPendingEmailTooltip>
    );
  } else {
    return <>{children}</>;
  }
