...
import { DataLeaksEmail, DataLeaksEmailStatus, OptinDataLeaksResultErrorCode, } from '@dashlane/password-security-contracts';
import { Dialog, EmailField, Infobox } from '@dashlane/design-system';
import useTranslate from 'libs/i18n/useTranslate';
import { isValidEmail } from 'libs/validators';
import { OptInErrorCode } from 'webapp/dark-web-monitoring/hooks/useEmailDialog';
import styles from './styles.css';
const I18N_KEYS = {
    ERROR_ALREADY_MONITORING: 'webapp_darkweb_add_email_dialog_error_already_monitoring',
    ERROR_ALREADY_PENDING: 'webapp_darkweb_add_email_dialog_error_already_pending',
...
export const AddEmailDialog = ({ availableSpots, userLogin, existingEmails, error, resetError, handleOnAddEmail, handleOnCloseDialog, }: AddEmailDialogProps) => {
    const { translate } = useTranslate();
    const hasEmails = existingEmails && existingEmails.length > 0;
    const [email, setEmail] = React.useState<string | null>(hasEmails ? null : userLogin);
    const [validationError, setValidationError] = React.useState<ValidationError>(ValidationError.NONE);
    const onEmailInputChange = React.useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
        const newEmail = e.target.value;
        let newValidationError = ValidationError.NONE;
        if (existingEmails && newEmail && isValidEmail(newEmail)) {
            const existingEmail = existingEmails.find((item) => item.email === newEmail);
            if (existingEmail) {
    ...
    const isEmailDisabled = email === null ||
        !isValidEmail(email) ||
    ...
    const renderSubtitle = () => {
        const errorMessageKey = getErrorMessageKey(error) || getValidationErrorMessageKey();
        ...
        {!hasEmails && (<div className={styles.firstEmailInfo}>
            <Infobox title={translate(I18N_KEYS.FIRST_EMAIL_INFO)}/>
          </div>)}
        <form onSubmit={onSubmit}>
          <EmailField label={translate(I18N_KEYS.INPUT_PLACEHOLDER)} onChange={onEmailInputChange} onInput={onEmailInputInput} value={email ?? undefined} error={!!errorMessageKey} feedback={errorMessageKey
                ? {
    ...
    return (<Dialog isOpen onClose={handleOnCloseDialog} title={translate(emailsI18nKeys.TITLE)} closeActionLabel={translate('_common_dialog_dismiss_button')} actions={{
            primary: {
