...
const getDisplayFormat = (precision: DatePrecision, length: DateLength): LocaleFormat | undefined => {
    switch (precision) {
...
export const getBreachDate = (eventDate: string, dateLength: DateLength, translate: TranslatorInterface): BreachDate => {
    ...
    const shortDate = translate.shortDate(new Date(eventDate), getDisplayFormat(precision, dateLength));
    return {
        date: shortDate,
        precision,
    };
