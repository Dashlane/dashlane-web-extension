...
import { Badge, Button, Flex, Icon, Paragraph } from "@dashlane/design-system";
...
import { DatePrecision, getBreachDate } from "../../helpers/date";
import { getBreachTitle } from "../../helpers/breach";
...
const HoverActions = ({ onDelete, onView }: HoverActionsProps) => {
  const { translate } = useTranslate();
  return (
    <div sx={SX_STYLES.privateBreachActions}>
      <Button
        mood="neutral"
        intensity="quiet"
        layout="labelOnly"
        color="ds.container.expressive.neutral.quiet.idle"
        onClick={onView}
        data-testid="breaches-detail-button"
      >
      ...
      <Button
        ...
        intensity="supershy"
        layout="iconOnly"
        color="ds.container.expressive.danger.quiet.hover"
        icon="ActionCloseOutlined"
        tooltip={translate(I18N_KEYS.DELETE_ALERT)}
        aria-label={translate(I18N_KEYS.DELETE_ALERT)}
        onClick={onDelete}
        data-testid="breaches-delete-button"
      />
    </div>
  );
};
export const PrivateBreachRow = ({
  breachItemView,
  handleOnViewDetails,
}: PrivateBreachRowProps) => {
  const { translate } = useTranslate();
  const { dismissBreach } = useBreaches();
  const { domains, eventDate, impactedEmails, status } = breachItemView;
  const breachTitle = useMemo(
    () =>
      getBreachTitle(breachItemView) ??
      translate(I18N_KEYS.UNKNOWN_WEBSITE_TITLE),
    [breachItemView, translate]
  );
        ...
        <div sx={SX_STYLES.privateBreachInfo}>
          ...
          <div sx={SX_STYLES.privateBreachTextInfo}>
            ...
            >
              ...
              {breachItemView.status === BreachStatus.PENDING ? (
                <Badge
                  mood="danger"
                  intensity="quiet"
                  layout="labelOnly"
                  label={translate(I18N_KEYS.BREACH_NEW_BADGE)}
                />
              ) : null}
            </Flex>
            {impactedEmails.length > 0 ? (
              <span sx={SX_STYLES.privateBreachEmail}>
