...
import { PrivateBreachRow } from 'webapp/dark-web-monitoring/private-breaches/private-breach-row/private-breach-row';
import { useBreachDetailsDialog } from 'webapp/dark-web-monitoring/breach-details/use-breach-details-dialog';
import { EmptyBreaches } from 'webapp/dark-web-monitoring/private-breaches/empty-breaches/empty-breaches';
...
export type PrivateBreachesProps = PaginationProps<BreachItemView> & PrivateBreachesBaseProps;
export const PrivateBreachesComponent = ({ hasDataLeakCapability, hasNext, hasPrevious, isLoading, loadNext, loadPrevious, paginatedData, emails, availableEmailSpots, onOpenAddDialog, premiumUrl, }: PrivateBreachesProps) => {
    const { translate } = useTranslate();
    const { markBreachAsSeen } = useBreaches();
    const { openBreachDetailsDialog } = useBreachDetailsDialog();
    const breachesPages: BreachItemView[][] = useMemo(() => [...paginatedData.values()], [paginatedData]);
    const onOpen = useCallback((breach: BreachItemView) => {
    ...
    const renderBatch = useCallback((breaches: BreachItemView[]) => {
        return breaches.map((breach) => (<PrivateBreachRow key={breach.id} breachItemView={breach} handleOnViewDetails={onOpen}/>));
    }, [onOpen]);
    const renderEmptyLeaks = () => {
        return (<EmptyBreaches hasDataLeakCapability={hasDataLeakCapability} availableEmailSpots={availableEmailSpots} emails={emails} onOpenAddDialog={onOpenAddDialog} premiumUrl={premiumUrl}/>);
    };
    const renderLeaks = () => {
        const hasLeaks = breachesPages.some((page) => page.length > 0);
        return (<InfiniteScroll hasNext={hasNext} hasPrevious={hasPrevious} isLoading={isLoading} loadNext={loadNext} loadPrevious={loadPrevious}>
        <div sx={PrivateBreachesComponentStyles.listContainer}>
          {!hasLeaks ? renderEmptyLeaks() : breachesPages.map(renderBatch)}
        </div>
      </InfiniteScroll>);
    };
    return (<section sx={PrivateBreachesComponentStyles.monitoredLeaks}>
