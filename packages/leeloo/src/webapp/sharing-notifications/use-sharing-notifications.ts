...
export interface UseSharingNotifications {
  ...
  updateNotificationStatus: (
    ...
    matchingPrevStatus?: NotificationStatus
  ) => void;
...
export const useSharingNotifications = (): UseSharingNotifications => {
  ...
  const updateNotifications = useCallback(
    (latestNotifications: SharingNotification[]) => {
      setNotifications((prevNotifications) => [
        ...prevNotifications,
        ...latestNotifications.filter(
          ({ id }) =>
            !prevNotifications.some((notification) => notification.id === id)
        ),
      ]);
    },
    []
    ...
    (
      ...
      setNotifications((prevNotifications) =>
        prevNotifications.map((prevNotification) =>
          notificationIds.includes(prevNotification.id) &&
  ...
  const removeSharingNotifications = useCallback(
    (notificationIds: string[]) => {
      setCachedItemGroupIds((prevCache) =>
        prevCache.filter((id) => !notificationIds.includes(id))
      );
      setCachedUserGroupIds((prevCache) =>
        prevCache.filter((id) => !notificationIds.includes(id))
      );
      setCachedCollectionIds((prevCache) =>
        prevCache.filter((id) => !notificationIds.includes(id))
      );
      setNotifications((prevNotifications) =>
  ...
  useEffect(() => {
    if (sharingInvitesQuery.status === DataStatus.Success) {
      const pendingItemGroups = sharingInvitesQuery.data.pendingSharedItems;
      const latestItemGroupIds = pendingItemGroups.map(
        (sharingGroup) => sharingGroup.sharedItemId
      );
      if (!isEqual(cachedItemGroupIds, latestItemGroupIds)) {
        setCachedItemGroupIds(latestItemGroupIds);
        updateNotifications(
          pendingItemGroups.map((itemGroup) => ({
            itemGroup,
            status: NotificationStatus.Pending,
            id: itemGroup.sharedItemId,
            type: "item" as const,
          }))
        );
        const refusedNotifications = notifications.filter(
          ({ id, status }) =>
            cachedItemGroupIds.includes(id) &&
        ...
        const revokedNotifications = notifications.filter(
          ({ id, status }) =>
            cachedItemGroupIds.includes(id) &&
            !latestItemGroupIds.includes(id) &&
      ...
      if (!isEqual(cachedUserGroupIds, latestUserGroupIds)) {
        ...
        const refusedNotifications = notifications.filter(
          ({ id, status }) =>
            cachedUserGroupIds.includes(id) &&
      ...
      const latestCollectionIds = pendingCollections.map(
        (sharingCollection) => sharingCollection.id
      );
      if (!isEqual(cachedCollectionIds, latestCollectionIds)) {
        ...
        const refusedNotifications = notifications.filter(
          ({ id, status }) =>
            cachedCollectionIds.includes(id) &&
