...
import {
  useTwoFactorAuthenticationDisableFlow,
...
import {
  logTwoFactorAuthenticationDisable6DigitsCodePageView,
  ...
  logTwoFactorAuthenticationDisableConfirmationPageView,
  logTwoFactorAuthenticationDisableErrorEvent,
} from "../logs/disable-flow-logs";
import { getTwoFactorAuthenticationCodeLogError } from "../errors/disable";
import { TwoFactorAuthenticationDisableConfirmationDialog } from "../components/two-factor-authentication-dialog/two-factor-authentication-disable-confirmation-dialog";
...
const I18N_KEYS = {
  TWO_FACTOR_AUTHENTICATION_DISABLE_SUCCESS_ALERT:
    "webapp_account_security_settings_two_factor_authentication_disable_success",
...
export const TwoFactorAuthenticationDisableContainer = ({
  onDialogClose,
}: Props) => {
  const { translate } = useTranslate();
  const flowData: TwoFactorAuthenticationFlowStageData | null =
    useTwoFactorAuthenticationDisableFlow();
  const twoFactorAuthenticationInfo = useTwoFactorAuthenticationInfo();
  const isTwoFactorAuthenticationEnforced =
  ...
  useEffect(() => {
    if (stage === TwoFactorAuthenticationDisableStages.SUCCESS) {
      ...
      successAlert.showAlert(
        translate(I18N_KEYS.TWO_FACTOR_AUTHENTICATION_DISABLE_SUCCESS_ALERT),
  ...
  if (stage && VIEW_MAPPER[stage]) {
    ...
    switch (stage) {
      ...
      case TwoFactorAuthenticationDisableStages.AUTHENTICATION_CODE:
        if (error) {
          const { logErrorName, errorMessage } =
            getTwoFactorAuthenticationCodeLogError(error.code);
      ...
      case TwoFactorAuthenticationDisableStages.CONFIRMATION:
        (
          stageComponentProps as ConfirmationStageProps
