...
import { ProtectedItemsUnlockerProvider } from 'webapp/unlock-items';
import { SkeletonVault } from 'webapp/skeleton-components/skeleton-vault';
...
export const TwoFactorAuthenticationEnforceView = () => {
    ...
    useEffect(() => {
        if (shouldEnforce2FA.status === DataStatus.Success &&
            !shouldEnforce2FA.data) {
            history.push(routes.userCredentials);
        }
    }, [shouldEnforce2FA.status]);
    return (<ProtectedItemsUnlockerProvider>
      <SkeletonVault />
