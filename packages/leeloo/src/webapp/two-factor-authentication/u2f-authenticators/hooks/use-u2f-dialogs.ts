...
import { U2FAuthenticatorRemoveContainer } from "../containers/u2f-authenticator-remove-container";
...
export function useU2FDialogs(props: Props = {}): Type {
  ...
  const openU2FRemoveAuthenticatorDialog = useCallback(
    (keyHandle) => {
      onDialogStateChanged?.(true);
      openDialog(
        React.createElement(U2FAuthenticatorRemoveContainer, {
          onDialogClose: () => {
            closeDialog();
