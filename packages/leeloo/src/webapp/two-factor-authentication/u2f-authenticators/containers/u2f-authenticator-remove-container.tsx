...
import { U2FAuthenticatorRemoveWarningDialogContent } from 'webapp/two-factor-authentication/u2f-authenticators/components/remove-u2f-authenticator-dialog/u2f-authenticator-remove-warning-dialog-content';
...
import { removeU2FAuthenticator } from 'webapp/two-factor-authentication/services';
import { AuthenticationCode as AuthenticationErrorCode, type RemoveU2FAuthenticatorError, } from '@dashlane/communication';
import { logDeleteU2FAuthenticatorCancel, logDeleteU2FAuthenticatorComplete, logDeleteU2FAuthenticatorError, logDeleteU2FAuthenticatorStart, } from 'webapp/two-factor-authentication/u2f-authenticators/logs/logs';
...
const I18N_KEYS = {
    ...
    BACKUP_CODE_NOT_VALID: 'webapp_account_security_settings_two_factor_authentication_backup_code_invalid_security_code',
    CODE_ALREADY_USED: 'webapp_login_form_password_fieldset_security_code_error_otp_already_used',
    TOO_MANY_ATTEMPTS: 'webapp_login_form_password_fieldset_security_code_error_token_too_many_attempts',
    GENERIC_ERROR: 'webapp_account_security_settings_two_factor_authentication_turn_off_authenticator_generic_error',
...
export const U2FAuthenticatorRemoveContainer = ({ onDialogClose, keyHandle, }: Props) => {
    const login = useUserLogin() ?? '';
    const { translate } = useTranslate();
    const successAlert = useAlert();
    const [stage, setStage] = useState<U2FRemoveAuthenticatorStages>(U2FRemoveAuthenticatorStages.WARNING);
    ...
    const submitFinalStage = async ({ authenticationCode, }: {
        authenticationCode: string;
    }) => {
        const response = await removeU2FAuthenticator(authenticationCode, keyHandle);
        if (!response.success && response.error) {
            ...
            if (stageErrors[errorCode]) {
                ...
                logDeleteU2FAuthenticatorError(U2FRemoveAuthenticatorStageErrors.GenericError.logErrorName);
            }
        }
        else {
            successAlert.showAlert(translate(I18N_KEYS.SUCCESS_TOAST), AlertSeverity.SUBTLE);
            onComplete();
        }
    };
    ...
    const toggleAuthenticationCodeMode = () => {
        const targetStage = stage === U2FRemoveAuthenticatorStages.AUTHENTICATION_CODE
            ? U2FRemoveAuthenticatorStages.BACKUP_CODE
            : U2FRemoveAuthenticatorStages.AUTHENTICATION_CODE;
    ...
    return (<Dialog isOpen={true} modalContentClassName={classnames(allIgnoreClickOutsideClassName)} disableOutsideClickClose disableScrolling disableUserInputTrap disableEscapeKeyClose closeIconName={translate(I18N_KEYS.CLOSE)} onClose={onCancel}>
      {MappedElement}
    </Dialog>);
};
