...
const BACKUP_PHONE_ERROR_MAPPER = {
  ...
  [RequestTOTPActivationError.PHONE_VALIDATION_FAILED]: {
    message:
      "webapp_account_security_settings_two_factor_authentication_turn_on_backup_phone_invalid_format",
...
export const TwoFactorAuthenticationBackupPhoneDialog = ({
  handleClickOnSubmit,
  handleClickOnBack,
  error,
  ...
  useEffect(() => {
    if (errorCode && !isSubmitting) {
      const mappedError =
        BACKUP_PHONE_ERROR_MAPPER[
          errorCode as keyof typeof BACKUP_PHONE_ERROR_MAPPER
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      <DialogBody>
        <BackupPhoneNumberForm
          countryCode={countryCode}
          phoneNumber={phoneNumber}
          errorMessage={
            localErrorMessage ? translate(localErrorMessage) : undefined
          }
      ...
      <DialogFooter
        primaryButtonTitle={translate(I18N_KEYS.PRIMARY_BUTTON)}
        primaryButtonProps={{
          disabled: disableSubmit,
          type: "button",
        }}
        primaryButtonOnClick={() => formik.handleSubmit()}
        secondaryButtonTitle={translate(I18N_KEYS.SECONDARY_BUTTON)}
