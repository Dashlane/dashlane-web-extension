...
const I18N_KEYS = {
    AUTHENTICATOR_TITLE: 'webapp_account_security_settings_two_factor_authentication_turn_off_authenticator_title',
    AUTHENTICATOR_CONTENT: 'webapp_account_security_settings_two_factor_authentication_turn_off_authenticator_content',
    AUTHENTICATOR_LINK: 'webapp_account_security_settings_two_factor_authentication_turn_off_authenticator_link',
...
interface Props {
    ...
    updateAuthenticatorCode: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void;
    code: string;
    errorMessage?: string;
...
export const AuthenticationCodeForm = ({ handleAuthenticatorModeClick, code, updateAuthenticatorCode, errorMessage, login, showSyncDevicesHelp = false, hideAdditionalActions = false, }: Props) => {
    ...
    return (<div sx={{
      ...
      <TokenInput autoFocus={true} maxLength={TOKEN_LENGTH} value={code} onChange={updateAuthenticatorCode} feedbackType={errorMessage ? 'error' : undefined} feedbackText={errorMessage ? translate(errorMessage) : undefined} data-testid="two-factor-authentication-code"/>
      ...
      {!hideAdditionalActions ? (<>
          <Paragraph sx={{
            ...
            <button type="button" onClick={handleAuthenticatorModeClick} sx={{
                fontSize: '15px',
                fontWeight: 500,
          ...
          <Paragraph sx={{ lineHeight: 'initial', padding: '4px 0px' }} color={colors.lightTheme.ds.text.inverse.quiet}>
