...
const TARGETED_DOMAIN = "__REDACTED__";
export const AntiPhishing = () => {
  ...
  const [hasToBeAddedToRedirectionList, setHasToBeAddedToRedirectionList] =
    useState(false);
  ...
  const { isAutoRedirected, addToAutoRedirectedDomain } =
    useAntiphishing(phishingDomain);
  const hasNonAsciiCharacters = useMemo(
    () => !DOMAIN_WITHOUT_SPECIAL_CHARACTERS_REGEX.test(phishingDomain ?? ""),
    [phishingDomain]
  );
  ...
  return (
    ...
    >
      ...
      >
        <Logo height={48} name="DashlaneLockup" />
        ...
        >
          <TextInput
            ...
            label={translate(I18N_KEYS.ANTI_PHISHING_PHISHING_DOMAIN)}
            sx={{
              "> div": {
          ...
          />
          <TextInput
            value={TARGETED_DOMAIN}
            label={translate(I18N_KEYS.ANTI_PHISHING_TARGETED_DOMAIN)}
            sx={{
              "> div": {
        ...
        {hasNonAsciiCharacters ? (
          <Infobox
            size="small"
            title={translate(I18N_KEYS.ANTI_PHISHING_PUNYCODE_INFO)}
          />
        ) : null}
        <Link
          sx={{
        ...
        <Checkbox
          ...
          checked={hasToBeAddedToRedirectionList}
          onClick={onCheckHandler}
          label={translate(I18N_KEYS.ANTI_PHISHING_REDIRECT_CHECKBOX)}
        />
