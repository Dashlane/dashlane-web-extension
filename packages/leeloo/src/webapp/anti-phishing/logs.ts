import {
  AnonymousAntiphishingRedirectEvent,
  BrowseComponent,
  DomainType,
  hashDomain,
  HelpCenterArticleCta,
  Page,
  UserAntiphishingRedirectEvent,
...
export const onPageViewLog = () =>
  logPageView(Page.AntiphishingRedirect, BrowseComponent.MainApp);
export const onRedirectionLog = (phishingDomain: string) => {
  ...
  Promise.all([
    ...
    hashDomain("__REDACTED__"),
  ]).then((hashes) => {
    logEvent(
      new AnonymousAntiphishingRedirectEvent({
        phishingDomain: {
          type: DomainType.Web,
          id: hashes[0],
        },
