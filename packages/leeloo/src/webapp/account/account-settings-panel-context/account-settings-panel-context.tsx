...
const AccountSettingsPanelProvider = ({ children, lee }: Provider) => {
    ...
    return (<AccountSettingsPanelContext.Provider value={contextValue}>
      {children}

      <TransitionGroup className={styles.sideMenuTransitionGroup}>
        {isAccountPanelOpen && (<CSSTransition classNames={accountPanelTransitionStyles} timeout={panelAnimationDurationMs}>
            {isAccountPanelOpen && (<Account key="for-CSSTransition-account-panel" onNavigateOut={userClosedAccountPanel} onClickFamilyDashboard={handleClickFamilyDashboard} lee={lee}/>)}
          </CSSTransition>)}
      </TransitionGroup>
    </AccountSettingsPanelContext.Provider>);
};
const useAccountSettingsPanelContext = () => useContext(AccountSettingsPanelContext);
