...
import { DeviceInfo } from "@dashlane/device-contracts";
...
export const Device = ({
  device,
  onDeactivationRequest,
  isDeactivationRequested,
  hideDeactivationButton,
  onDeactivationCancelled,
  onConfirmedDeactivationRequest,
  onSave,
  onChange,
}: Props) => {
  const timer = React.useRef<NodeJS.Timeout | null>(null);
  const [actionState, setActionState] = React.useState(
    ActionState.CanRemoveDevice
  );
        ...
        <div className={styles.iconContainer}>
          <div
            className={classnames(
              styles.platformIcon,
              styles["platformIcon_" + device.devicePlatform]
            )}
          />
        </div>
        <div className={styles.textContainer}>
          {!device.isCurrentDevice && !hideDeactivationButton ? (
            <DeviceDeactivateButton
              actionState={actionState}
              handleClick={() => onDeactivationRequest(device.deviceId)}
            />
          ) : null}
          <DeviceName
            canToggle={!isDeactivationRequested}
          ...
          {isDeactivationRequested ? (
            <DeactivationConfirm
              key="deactivation-confirm"
              onConfirm={() => onConfirmedDeactivationRequest(device.deviceId)}
              onCancel={onDeactivationCancelled}
            />
          ) : (
            <DeviceDetails
              creationDate={new Date(device.creationDate * 1000)}
              lastUpdateDate={new Date(device.lastUpdateDate * 1000)}
            />
          )}
        </div>
      </div>
