...
export const DeviceName = ({
  ...
  const [saveFeedbackState, setSaveFeedbackState] =
    React.useState<SaveFeedbackState>(SaveFeedbackState.Off);
  React.useEffect(() => {
    if (editMode) {
  ...
  const onSubmit = () => {
    ...
    setSaveFeedbackState(
      shouldSaveName ? SaveFeedbackState.InProgress : SaveFeedbackState.Off
    );
    if (shouldSaveName) {
    ...
    return (
      <Heading
        ...
        className={classnames({
          [styles.name]: true,
          [styles.hintEditField]:
            saveFeedbackState !== SaveFeedbackState.InProgress && canToggle,
        })}
      >
    ...
    <div className={styles.wrap}>
      {editMode ? (
        <input
          ...
          placeholder={translate(I18N_KEYS.SET_NAME_PLACEHOLDER)}
        />
      ) : (
        getReadOnlyField()
      )}
