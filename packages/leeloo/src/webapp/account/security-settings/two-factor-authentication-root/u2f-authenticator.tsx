...
import { useIsRemoveU2FAuthenticatorAvailable } from "../../../two-factor-authentication/u2f-authenticators/hooks/use-is-remove-u2f-authenticator-available";
const I18N_KEYS = {
...
export const U2FAuthenticator = ({
  name,
  creationDateUnix,
  keyHandle,
  onRemoveAuthenticator,
}: Props) => {
  const isRemoveU2FAuthenticatorAvailable =
    useIsRemoveU2FAuthenticatorAvailable();
  const { translate } = useTranslate();
  const timestamp = fromUnixTime(creationDateUnix);
  const handleOnRemoveButtonClick = () => {
    onRemoveAuthenticator(keyHandle);
  };
  const DropdownDeleteAction = (
    <DropdownElement onClick={handleOnRemoveButtonClick}>
  ...
  return (
    ...
    >
      ...
      {isRemoveU2FAuthenticatorAvailable ? (
        <div sx={{ flexGrow: 0 }}>
          <DropdownMenu
            offset={[0, 4]}
            placement="bottom-end"
            content={DropdownDeleteAction}
          >
            <IconButton
              type="button"
              size="small"
              aria-label={translate(I18N_KEYS.MENU_TITLE)}
              title={translate(I18N_KEYS.MENU_TITLE)}
