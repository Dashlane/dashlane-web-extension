...
import { Select } from '@dashlane/ui-components';
...
const I18N_KEYS = {
    ...
    HELP_TEXT_ENFORCED_KDF: 'webapp_account_security_settings_help_text_enforced_kdf_markup',
    HELP_TEXT_PASSWORD_LESS: 'webapp_account_security_settings_help_text_password_less',
    ...
    INPUT_PBKDF2_ADVANCED: 'webapp_account_security_settings_input_value_pbkdf2_advanced',
    INPUT_PBKDF2_STANDARD: 'webapp_account_security_settings_input_value_pbkdf2_standard',
...
export const CryptoSettingsSection = () => {
    ...
    const [currentDerivation, setCurrentDerivation] = useState<SupportedDerivationMethods | ''>('');
    const isFFActivated = useHasFeatureEnabled(FEATURE_FLIPS_WITHOUT_MODULE.ItadminSAEXCryptoMigrationPhase3V1);
    const isDerivationMethodSelectable = useCanUserChangeCrypto() && isFFActivated;
    ...
    const getDerivationLabel = (kdf: SupportedDerivationMethods | ''): string => cryptoOptions.find((option) => option.value === kdf)?.label ?? '';
    const onCryptoSelectChange = async (event: React.ChangeEvent<HTMLSelectElement>) => {
        setIsLoading(true);
        setHasError(false);
        const newDerivationMethod = event.target
            .value as SupportedDerivationMethods;
    ...
    return (<SettingsSection sectionTitle={translate(I18N_KEYS.SUB_TITLE)}>
      <Select value={currentDerivation} options={cryptoOptions} placeholder={isDerivationMethodSelectable
            ? translate(I18N_KEYS.INPUT_PLACEHOLDER)
            : getDerivationLabel(currentDerivation)} onChange={onCryptoSelectChange} disabled={!isDerivationMethodSelectable ||
            isLoading ||
            (isMPLessUserStatus === DataStatus.Success && isMPLessUser)} intent={hasError ? 'error' : undefined} feedbackText={hasError ? translate(I18N_KEYS.GENERIC_ERROR_MESSAGE) : undefined} aria-labelledby={CRYPTO_SELECT_LABEL}/>
      <Paragraph textStyle="ds.body.helper.regular" color="ds.text.neutral.quiet" sx={{ marginTop: '16px' }} id={CRYPTO_SELECT_LABEL}>
