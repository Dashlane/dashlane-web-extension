...
export const TwoFactorAuthenticationSettingsSection = ({ changeActivePanel, onDialogStateChanged, twoFactorAuthenticationInfo, }: Props) => {
    const I18N_KEYS = {
        UNKNOWN_ERROR: 'webapp_account_security_settings_two_factor_authentication_turn_on_generic_error',
        TWO_FACTOR_AUTHENTICATION_DISABLED: 'webapp_account_security_settings_two_factor_authentication_disabled',
        TWO_FACTOR_AUTHENTICATION_DISABLED_SSO: 'webapp_account_security_settings_two_factor_authentication_disabled_sso',
    };
    if (!twoFactorAuthenticationInfo ||
        ...
        return (<TwoFactorAuthenticationAvailableSection changeActivePanel={changeActivePanel} onDialogStateChanged={onDialogStateChanged} twoFactorAuthenticationInfo={twoFactorAuthenticationInfo}/>);
    }
    else {
        const isSSOUser = twoFactorAuthenticationInfo?.status ===
            TwoFactorAuthenticationInfoRequestStatus.READY &&
            twoFactorAuthenticationInfo?.type === TwoFactorAuthenticationType.SSO;
        const unavailableMessage = twoFactorAuthenticationInfo?.status ===
        ...
        return (<TwoFactorAuthenticationNotAvailableSection error={twoFactorAuthenticationInfo.status ===
                TwoFactorAuthenticationInfoRequestStatus.ERROR} message={unavailableMessage}/>);
    }
};
