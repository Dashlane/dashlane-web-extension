...
import { Flex, LinkButton, Paragraph, Toggle } from "@dashlane/design-system";
import { InfoBox, ThemeUIStyleObject } from "@dashlane/ui-components";
...
import { useIsWebAuthnAvailable } from "../../../../webauthn/hooks/use-is-web-authn-available";
...
const I18N_KEYS = {
  ALL_AUTHENTICATORS_TOGGLE_ARIA_LABEL:
    "webapp_account_security_settings_passwordless_all_authenticators_toggle_aria_label",
  ALL_AUTHENTICATORS_TITLE:
    "webapp_account_security_settings_passwordless_all_authenticators_biometrics_title",
  ALL_AUTHENTICATORS_DESC:
    "webapp_account_security_settings_passwordless_all_authenticators_desc_markup",
  VIEW_OPTIONS_LINK:
    "webapp_account_security_settings_passwordless_view_biometrics_options",
  NO_PLATFORM_TITLE:
...
const I18N_WEBAUTHN_NOT_AVAILABLE = {
  ...
  [WebauthnNotAvailableReasons.UNSUPPORTED_BROWSER]: {
    TITLE:
      "webapp_account_security_settings_passwordless_all_authenticators_title",
    MESSAGE:
      "webapp_account_security_settings_passwordless_disabled_feature_unsupported",
  ...
  [WebauthnNotAvailableReasons.ENABLED_2FA]: {
    TITLE:
      "webapp_account_security_settings_passwordless_all_authenticators_title",
    MESSAGE:
      "webapp_account_security_settings_passwordless_disabled_by_two_factor_authentication",
  },
};
const SX_STYLES: {
...
export const WebAuthnSettingsSection = ({
  ...
  const [userBrowserSupportsWebAuthnAuthentication] = useState(
    browserSupportsWebAuthnAuthentication()
  );
  ...
  const onWebAuthnChange = useCallback(
  ...
  const onClickViewOptions = useCallback(
  ...
  if (
    ...
    const i18nWebAuthn =
      I18N_WEBAUTHN_NOT_AVAILABLE[isWebAuthnAvailable.reason];
    return (
      <WebAuthnSettingsNotAvailableSection
        title={translate(i18nWebAuthn.TITLE)}
        message={translate(i18nWebAuthn.MESSAGE)}
      />
    );
  } else {
    return (
        ...
        action={
          <Toggle
            aria-label={translate(
              I18N_KEYS.ALL_AUTHENTICATORS_TOGGLE_ARIA_LABEL
            )}
            checked={webAuthnOptedIn}
            onChange={onWebAuthnChange}
            disabled={!userBrowserSupportsWebAuthnAuthentication}
          />
        }
      >
        <Flex
        ...
        >
          {!userBrowserSupportsWebAuthnAuthentication ? (
            <InfoBox
              severity="subtle"
              size="small"
              title={translate(I18N_KEYS.BROWSER_NOT_SUPPORTED)}
            />
          ) : null}
          {userBrowserSupportsWebAuthnAuthentication ? (
            <div sx={SX_STYLES.WEBAUTHN_SECTION_DESCRIPTION}>
              {translate.markup(description)}
            </div>
          ) : null}
        </Flex>
        {showViewOptions ? (
          <LinkButton href="#" onClick={onClickViewOptions}>
