...
import { accountRecoveryKeyApi, RecoveryMethodsInfoQueryResult, } from '@dashlane/account-recovery-contracts';
...
const getRecoveryMethodsCount = (recoveryMethods: RecoveryMethodsInfoQueryResult) => Object.values(recoveryMethods).filter(Boolean).length;
interface Props {
    changeActivePanel: (panel: ActivePanel) => void;
}
export const AccountRecoverySettingsSection = ({ changeActivePanel, }: Props) => {
    const recoveryMethodsInfo = useModuleQuery(accountRecoveryKeyApi, 'recoveryMethodsInfo');
    const { translate } = useTranslate();
    const handleOpenAccountRecoveryPanel = (event: React.MouseEvent<HTMLAnchorElement>) => {
        event.preventDefault();
        changeActivePanel(ActivePanel.AccountRecovery);
    };
    return (<SettingsSection sectionTitle={translate(I18N_KEYS.ACCOUNT_RECOVERY_TITLE)}>
      <div>
        {recoveryMethodsInfo.status === DataStatus.Success ? (<AccountRecoveryAvailableSection recoveryMethodsCount={getRecoveryMethodsCount(recoveryMethodsInfo.data)} openAccountRecoveryPanel={handleOpenAccountRecoveryPanel}/>) : (<AccountRecoveryLoadingSection />)}
      </div>
    </SettingsSection>);
};
