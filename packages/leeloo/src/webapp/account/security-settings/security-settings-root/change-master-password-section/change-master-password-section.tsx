...
import { useIsChangeMasterPasswordEnabled } from "./hooks/use-is-change-master-password-enabled";
...
const I18N_KEYS = {
  ...
  CHANGE_MP_DISABLED_1:
    "webapp_account_security_settings_changemp_section_button_disabled_1",
  CHANGE_MP_DISABLED_2:
    "webapp_account_security_settings_changemp_section_button_disabled_2",
  TIP_MP_COMPROMISED_INFOBOX_TITLE:
    "webapp_account_security_settings_changemp_section_tip_mpcompromised_infobox_title",
  TIP_MP_COMPROMISED_INFOBOX_DESCRIPTION:
    "webapp_account_security_settings_changemp_section_tip_mpcompromised_infobox_description",
  TIP_MP_WEAK_INFOBOX_TITLE:
    "webapp_account_security_settings_changemp_section_tip_mpweak_infobox_title",
  TIP_MP_WEAK_INFOBOX_DESCRIPTION:
    "webapp_account_security_settings_changemp_section_tip_mpweak_infobox_description",
...
export const ChangeMasterPasswordSection = ({
  changeActivePanel,
  twoFactorAuthenticationInfo,
}: Props) => {
  ...
  const ActionButton = ({ disabled }: { disabled: boolean }) => (
    <Button
      type="button"
      mood="brand"
      size="small"
  ...
  const ChangeMasterPasswordLoadingSection = () => (
    <GridChild
      justifySelf="flex-start"
      id="changeMasterPasswordLoading"
      sx={{ marginTop: "20px" }}
    ...
    >
      <LoadingIcon
        ...
        size={40}
        aria-describedby="changeMasterPasswordLoading"
      />
    </GridChild>
  );
  const ChangeMasterPasswordErrorSection = () => (
    <GridChild justifySelf="flex-start">
      <InfoBox
        severity="alert"
        size="small"
        title={translate(I18N_KEYS.CHANGE_MP_DISABLED_1)}
      />
    </GridChild>
  );
  ...
  const isChangeMPSectionReady =
    twoFactorAuthenticationInfo?.status ===
    TwoFactorAuthenticationInfoRequestStatus.READY;
  const isChangeMPSectionError =
    twoFactorAuthenticationInfo?.status ===
    TwoFactorAuthenticationInfoRequestStatus.ERROR;
  return (
    <>
      <SettingsSection sectionTitle={translate(I18N_KEYS.TITLE)}>
        {isChangeMPSectionReady ? (
          <ChangeMasterPasswordReadySection />
        ) : isChangeMPSectionError ? (
          <ChangeMasterPasswordErrorSection />
        ) : (
          <ChangeMasterPasswordLoadingSection />
        )}
      </SettingsSection>
      {showAutorizationDialog ? (
        <AuthorizationStepDialog
          handleOnCancel={() => setShowAuthorizationDialog(false)}
          handleOnSubmit={showChangeMPPanel}
          login={login}
        />
      ) : null}
    </>
