...
const I18N_KEYS = {
    ARK_ACTIVATION_DISPLAY_KEY_STEP_WRONG_KEY: 'webapp_account_recovery_key_third_step_wrong_key_error',
};
export function useAccountRecoveryKeyInputActivationFlow(recoveryKey: string) {
    ...
    const handleChangeInputValue = (eventOrValue: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement> | string) => {
        ...
        setInputError('');
        if (value.length === RECOVERY_KEY_LENGTH &&
            value !== recoveryKey) {
            setInputError(translate(I18N_KEYS.ARK_ACTIVATION_DISPLAY_KEY_STEP_WRONG_KEY));
