...
export const AccountRecoveryRoot = ({ onNavigateOut }: Props) => {
  ...
  const isAdminAssistedRecoveryAvailable =
    isAdminAssistedRecoveryAvailableResult.status === DataStatus.Success &&
    isAdminAssistedRecoveryAvailableResult.data;
  const [activePanel, setActivePanel] = useState(
    AccountRecoveryActivePanel.Root
  );
  ...
  return activePanel === AccountRecoveryActivePanel.AdminRecoveryHelpPanel ? (
    <AdminRecoveryHelpSection onNavigateOut={handleShowRootPanel} />
  ) : (
    <AccountSubPanel
      headingText={translate(I18N_KEYS.ACCOUNT_RECOVERY_PANEL_TITLE)}
      onNavigateOut={onNavigateOut}
    >
      <RecoveryKeySection />
      {isAdminAssistedRecoveryAvailable ? (
        <AdminAssistedRecoverySection
          showAdminRecoveryHelp={handleShowAdminRecoveryHelp}
        />
      ) : null}
