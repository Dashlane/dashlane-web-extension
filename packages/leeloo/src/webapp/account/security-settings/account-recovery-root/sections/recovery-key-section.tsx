...
export const I18N_KEYS = {
  ...
  RECOVERY_KEY_DESCRIPTION_PASSWORDLESS:
    "webapp_account_security_settings_account_recovery_section_recovery_key_description_password_less",
  GENERATE_NEW_KEY_BUTTON:
    "webapp_account_security_settings_account_recovery_section_recovery_key_generate",
  LEARN_MORE_LINK:
    "webapp_account_security_settings_account_recovery_section_help_link",
};
type CurrentAccountRecoveryKeyDialog =
...
export const RecoveryKeySection = () => {
  ...
  const { resetActivationFlow } = useModuleCommands(accountRecoveryKeyApi);
  ...
  const [currentDialog, setCurrentDialog] =
    useState<CurrentAccountRecoveryKeyDialog>(undefined);
  const onToggleSettingChangeModal = async () => {
    await resetActivationFlow();
    setCurrentDialog(
      isAccountRecoveryKeyEnabled
        ? "accountRecoveryKeyDeactivation"
        : "accountRecoveryKeyActivation"
    );
  };
  const onGenerateNewKeyClick = async () => {
    await resetActivationFlow();
  ...
  return (
    <>
      <SettingsSection
        sectionTitle={translate(I18N_KEYS.RECOVERY_KEY_TITLE)}
        action={
          <Toggle
            id="recovery-key-toggle"
            checked={isAccountRecoveryKeyEnabled}
            onChange={onToggleSettingChangeModal}
            disabled={isMPLessUser}
          />
        }
      >
        ...
        {isMPLessUserStatus !== DataStatus.Success ? null : isMPLessUser ? (
          <Link
            ...
            href={HELPCENTER_ACCOUNT_RECOVERY_URL}
      ...
      {currentDialog === "accountRecoveryKeyActivation" ? (
        <AccountRecoveryKeyActivationContainer
          onClose={() => setCurrentDialog(undefined)}
        />
      ) : null}
      {currentDialog === "accountRecoveryKeyDeactivation" ? (
        <AccountRecoveryKeyDeactivationContainer
          onClose={() => setCurrentDialog(undefined)}
        />
      ) : null}
    </>
