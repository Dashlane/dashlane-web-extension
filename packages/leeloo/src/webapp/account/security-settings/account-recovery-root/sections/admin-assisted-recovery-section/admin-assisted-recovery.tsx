...
const I18N_KEYS = {
    ACCOUNT_RECOVERY_TITLE: 'webapp_account_security_settings_account_recovery_setting_title',
    ACCOUNT_RECOVERY_DESC: 'webapp_account_security_settings_account_recovery_setting_desc',
    ACCOUNT_RECOVERY_TITLE_TOGGLE_ARIA_LABEL: 'webapp_account_security_settings_account_recovery_setting_toggle_aria_label',
    HELP_LINK: 'webapp_account_security_settings_account_recovery_setting_help_link',
    WEB_ONLY_BADGE: '_common_settings_web_only_feature_badge',
};
interface Props {
    showAdminRecoveryHelp: () => void;
}
export const AdminAssistedRecoverySection = ({ showAdminRecoveryHelp, }: Props) => {
    ...
    React.useEffect(() => {
        const isRecoveryOptIn = adminRecoveryOptInSetting.status === DataStatus.Success &&
    ...
    const onAdminRecoveryChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    return (<SettingsSection sectionTitle={translate(I18N_KEYS.ACCOUNT_RECOVERY_TITLE)} action={<Toggle disabled={loading} id="account-recovery-optin" checked={isAdminRecoveryOptIn} onChange={onAdminRecoveryChange} aria-label={translate(I18N_KEYS.ACCOUNT_RECOVERY_TITLE_TOGGLE_ARIA_LABEL)}/>}>
      <div className={styles.accountRecoverySection}>
        <Badge label={translate(I18N_KEYS.WEB_ONLY_BADGE)}/>
        <label htmlFor="account-recovery-optin" className={styles.accountRecoverySectionLabel}>
          {translate(I18N_KEYS.ACCOUNT_RECOVERY_DESC)}
        </label>

        <Link href="#" role="button" color={colors.midGreen00} className={styles.linkButton} onClick={handleOnClickHelp}>
