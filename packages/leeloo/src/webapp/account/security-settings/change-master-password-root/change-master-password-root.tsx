...
const handleChangeMasterPassword = async (currentPassword: string | null, newPassword: string | null): Promise<void> => {
...
export const ChangeMasterPasswordRoot = ({ onNavigateOut }: Props) => {
    ...
    const onConfirmChangeMasterPassword = (currentPassword: string, newPassword: string) => {
    ...
    return (<div>
      <TransitionGroup>
        {activePanel === ChangeMPActivePanel.Root ? (<CSSTransition classNames={rootPanelTransitionStyles} timeout={panelAnimationDurationMs}>
            <ChangeMasterPasswordForm key="for-TransitionGroup-security-settings-changemp-form" onNavigateOut={onNavigateOut} onShowPasswordTips={onShowPasswordTips} onConfirmChangeMasterPassword={onConfirmChangeMasterPassword}/>
          </CSSTransition>) : null}

        {activePanel === ChangeMPActivePanel.PasswordTipsPanel ? (<CSSTransition classNames={subPanelTransitionStyles} timeout={panelAnimationDurationMs}>
            <PasswordTips key="for-TransitionGroup-security-settings-changemp-subpanel-passwordtips" onNavigateOut={onSubPanelExitRequest}/>
          </CSSTransition>) : null}
      </TransitionGroup>
    </div>);
};
