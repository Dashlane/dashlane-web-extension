...
} from "../../../../two-factor-authentication/components/two-factor-authentication-dialog";
import { getTwoFactorAuthenticationCodeLogError } from "../../../../two-factor-authentication/errors/disable";
import { TwoFactorAuthenticationErrorWithMessage } from "../../../../two-factor-authentication/types";
...
export const AuthorizationStepDialog = ({
  handleOnCancel,
  handleOnSubmit,
  login,
}: Props) => {
  ...
  const [authenticationMode, setAuthenticationMode] =
    React.useState<AuthenticationMode>(AuthenticationMode.SIX_DIGIT_TOKEN);
  const [authenticationError, setAuthenticationError] = React.useState<
    TwoFactorAuthenticationErrorWithMessage | undefined
  >(undefined);
    ...
    try {
      ...
      if (result.success) {
        ...
        setAuthenticationError({
          ...
          message:
            getTwoFactorAuthenticationCodeLogError(result.error.code)
  ...
  const dialogContent =
    authenticationMode === AuthenticationMode.BACKUP_CODE ? (
      <TwoFactorAuthenticationBackupCodeDialog
        {...dialogProps}
        toggleAuthenticationCodeMode={handleToggleAuthenticationMode.bind(
          null,
          AuthenticationMode.SIX_DIGIT_TOKEN
        )}
      />
    ) : (
      <TwoFactorAuthenticationCodeDialog
        ...
        toggleAuthenticationCodeMode={handleToggleAuthenticationMode.bind(
          null,
          AuthenticationMode.BACKUP_CODE
        )}
      />
    );
  return (
    <Dialog
      ...
      closeIconName={translate(I18N_KEYS.CLOSE_DIALOG)}
