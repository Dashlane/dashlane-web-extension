...
import { InvoiceCustomData, InvoiceCustomize, } from './invoice-customize/invoice-customize';
const I18N_KEYS = {
...
export const Invoices = ({ onNavigateOut }: InvoicesProps) => {
    ...
    const downloadInvoice = async (invoiceCustomData?: InvoiceCustomData) => {
        if (selectedInvoice) {
            const filename = `dashlane_invoice_${translate.shortDate(fromUnixTime(selectedInvoice?.startDate), LocaleFormat.L)}`;
    ...
    if (customizeInvoiceMode) {
        return (<AccountSubPanel headingText={translate(I18N_KEYS.CUSTOMIZE_PANEL_HEADING)} onNavigateOut={hideCustomizeInvoicePanel}>
        <InvoiceCustomize onDownloadInvoice={(invoiceCustomData: InvoiceCustomData) => downloadInvoice(invoiceCustomData)} onCancel={hideCustomizeInvoicePanel}/>
      </AccountSubPanel>);
    }
    else {
        return (<AccountSubPanel headingText={translate(I18N_KEYS.PANEL_HEADING)} onNavigateOut={onNavigateOut}>
        <InvoiceList invoices={invoices} onDownloadInvoiceRequested={invoiceSelected} onCancelDownloadRequested={deselectInvoice} selectedInvoiceId={selectedInvoice ? selectedInvoice.invoiceId : null} onCustomizeInvoiceRequested={customizeInvoiceRequested} onDownloadInvoiceConfirmed={() => downloadInvoice()}/>
      </AccountSubPanel>);
    }
};
