...
const filterInvoices = (invoices: Invoice[]) =>
  invoices.filter((invoice) => eventTypeWhitelist.includes(invoice.eventType));
const sortInvoices = (invoices: Invoice[]) =>
  invoices.sort((invoice1: Invoice, invoice2: Invoice) => {
...
export const InvoiceList = ({
  ...
  if (!invoices || !invoices.length) {
    return (
      <div>
        <img
          alt={translate(I18N_KEYS.LIST_NO_RECEIPTS_IMAGE_ALT)}
          src={invoiceImage}
          className={styles.noReceiptsImage}
        />
  ...
  return (
    <ol className={styles.invoiceList}>
      {sortInvoices(filteredInvoices).map((invoice: Invoice, ix: number) => (
        <InvoiceItem
          key={invoice.eventType + invoice.startDate}
          invoice={invoice}
          onDownloadInvoiceRequested={onDownloadInvoiceRequested}
          onCancelDownloadRequested={onCancelDownloadRequested}
          isDownloadInvoiceRequested={
            invoice.invoiceId ? selectedInvoiceId === invoice.invoiceId : false
          }
