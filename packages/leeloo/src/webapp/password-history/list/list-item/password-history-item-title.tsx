...
const I18N_KEYS = {
  ...
  SUBTITLE_GENERATED_WITHOUT_URL:
    "webapp_password_history_item_no_url_subtitle",
...
export const PasswordHistoryItemTitle = (props: {
  ...
  const { item, onCreateNewCredential, isShared } = props;
  ...
  if (item.type === PasswordHistoryItemType.Credential) {
    ...
    const subtitle =
      item.secondaryInfo || translate(I18N_KEYS.SUBTITLE_NO_LOGIN);
    return (
      <div className={styles.primaryInfo} onClick={onClick}>
        <CredentialInfo
  ...
  if (new ParsedURL(item.primaryInfo).isUrlValid()) {
    const onClick = () =>
      onCreateNewCredential(item.password, item.primaryInfo);
