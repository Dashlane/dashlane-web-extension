...
import { CredentialPasswordHistoryItemView, PasswordHistoryFilterCriterium, PasswordHistoryFilterField, PasswordHistoryItemType, PasswordHistoryItemView, PasswordHistorySortField, SortDirection, } from '@dashlane/communication';
...
import { RowDataProps } from 'webapp/list-view/row';
...
export const formatPasswordHistoryItem = (item: PasswordHistoryItemView, onPasswordCopied: (copyHandlerParams: PasswordCopyHandlerParams) => void, isProtected: boolean, onCreateNewCredential: (generatedPassword: string, website?: string) => void, onOpenRestorePasswordDialog: (newSelectedItem: CredentialPasswordHistoryItemView) => void): RowDataProps => {
    return {
        renderers: {
            primaryInfo: () => (<PasswordHistoryItemTitle item={item} onCreateNewCredential={onCreateNewCredential}/>),
            password: () => (<PasswordHistoryItemPassword item={item} isProtected={isProtected}/>),
            timestamp: () => (<IntelligentTooltipOnOverflow>
        ...
        actions: (<PasswordHistoryRowActions item={item} onPasswordCopied={onPasswordCopied} onOpenRestorePasswordDialog={onOpenRestorePasswordDialog} isProtected={isProtected}/>),
    };
};
...
export const getFilterCriteria = (filter: PasswordHistoryFilter, searchValue: string, deepLinkCredentialId?: string): PasswordHistoryFilterCriterium[] | undefined => {
    ...
    if (filter !== PasswordHistoryFilter.All) {
        filterCriterium.push({
            ...
            value: filter === PasswordHistoryFilter.Saved
...
export const getSortCriteriaFromSortingOptions = (sortingOptions: SortingOptions): {
    ...
    return [
        {
            ...
            direction: sortingOptions.direction === 'ascending' ? 'ascend' : 'descend',
        },
    ];
};
export const isItemPasswordProtected = (item: PasswordHistoryItemView, mpSettingsResponse: CarbonEndpointResult<boolean>) => {
