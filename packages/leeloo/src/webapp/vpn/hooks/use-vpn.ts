...
import { activateVpnAccount } from "../services";
...
export function useVpn(): UseVpn {
  ...
  const openActivateVpnSuccessAlert = useCallback(
    (alertSuccessText: ReactNode) => {
      alert.showAlert(alertSuccessText, AlertSeverity.SUCCESS);
    },
    [alert]
  );
  const openActivateVpnErrorAlert = useCallback(
    (alertErrorText: ReactNode) => {
