import { VpnAccountStatus, VpnAccountStatusType, } from '@dashlane/communication';
import { useEffect, useState } from 'react';
import { VPN_ACTIVATING_ANIMATION_DURATION } from '../components/animation-constants';
...
export const useDebounceVpnActivationStatus = (vpnCredential?: VpnAccountStatus | null) => {
    const [debouncedStatus, setDebouncedStatus] = useState(VpnCredentialActivationStatus.INITIAL);
    const [timerId, setTimerId] = useState<number>();
    useEffect(() => {
        if (vpnCredential?.status === VpnAccountStatusType.Activated &&
    ...
    return {
        ...
        isActivating: () => debouncedStatus === VpnCredentialActivationStatus.ACTIVATING,
        isActivated: () => debouncedStatus === VpnCredentialActivationStatus.ACTIVATED,
        isIntermediate: () => debouncedStatus === VpnCredentialActivationStatus.INTERMEDIATE,
    };
};
