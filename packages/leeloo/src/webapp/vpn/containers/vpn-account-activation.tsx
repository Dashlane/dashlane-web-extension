...
import { HOTSPOTSHIELD_SUPPORT_URL } from "../helpers/links";
const I18N_KEYS = {
  ACTIVE: {
...
export const VpnAccountActivation = ({
  stepNumber,
  vpnCredential,
  ...
  useEffect(() => {
    ...
    if (vpnCredential.status === VpnAccountStatusType.Ready) {
      openActivateVpnSuccessAlert(
        translateWithMarkup(I18N_KEYS.ACTIVE.ALERT_SUCCESS)
      );
      onCompleteVpnAccountActivation();
    }
    if (vpnCredential.status === VpnAccountStatusType.Error) {
      openActivateVpnErrorAlert(
        translateWithMarkup(I18N_KEYS_ERRORS[vpnCredential.error])
      );
      onClearVpnAccountErrors();
    }
  }, [
  ...
  return (
    <CredentialForm
      ...
      generateCredential={onActivateVpnAccount}
    />
  );
};
