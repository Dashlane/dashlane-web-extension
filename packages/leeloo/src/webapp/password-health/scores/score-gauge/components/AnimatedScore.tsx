...
const FRAMES_INTERVAL_MILLISECONDS = Math.floor(
  MILLISECONDS_IN_A_SECOND / FRAMES_PER_SECONDS
);
const ANIMATION_DURATION_MILLISECONDS = 1000;
const FRAMES_IN_ANIMATION_DURATION =
  (FRAMES_PER_SECONDS / MILLISECONDS_IN_A_SECOND) *
  ANIMATION_DURATION_MILLISECONDS;
export const AnimatedScore = ({ score }: AnimatedScoreProps) => {
  const timeoutRef = React.useRef<number | null>(null);
  const [currentScore, setCurrentScore] = React.useState(0);
  const [currentScoreIncrementStep, setCurrentScoreIncrementStep] =
    React.useState(0);
