...
import { RiskInfo } from "../risk-info/risk-info";
import {
  DatePrecision,
  getBreachDate,
} from "../../../dark-web-monitoring/helpers/date";
...
const I18N_PASSWORD_HEALTH_INFO_LABEL_KEYS = {
  ...
  COMPROMISED_WITHOUT_DOMAIN:
    "webapp_security_dashboard_compromised_credential_without_domain_markup",
  COMPROMISED_WITHOUT_DOMAIN_MONTH:
    "webapp_security_dashboard_compromised_credential_without_domain_month_markup",
  COMPROMISED_WITHOUT_DOMAIN_YEAR:
    "webapp_security_dashboard_compromised_credential_without_domain_year_markup",
...
const getCompromisedRiskLabelFromDatePrecision = (
  ...
  switch (precision) {
    case DatePrecision.DAY:
      return translate.markup(
        I18N_PASSWORD_HEALTH_INFO_LABEL_KEYS.COMPROMISED_WITHOUT_DOMAIN,
        {
          date,
        }
      );
    case DatePrecision.MONTH:
      return translate.markup(
        I18N_PASSWORD_HEALTH_INFO_LABEL_KEYS.COMPROMISED_WITHOUT_DOMAIN_MONTH,
        {
          date,
        }
      );
    case DatePrecision.YEAR:
      return translate.markup(
        I18N_PASSWORD_HEALTH_INFO_LABEL_KEYS.COMPROMISED_WITHOUT_DOMAIN_YEAR,
        {
          date,
        }
      );
...
const getRiskLabel = (
  ...
  switch (corruptionData.riskType) {
    case RiskType.Compromised:
      return getCompromisedRiskLabel(
        corruptionData.date,
        corruptionData.domain,
        translate
      );
    case RiskType.Weak:
...
export interface PasswordHealthListItemProps {
  ...
  onRowClick: (credentialId: string, website: string) => void;
...
export const PasswordHealthListItem = ({
  credential,
  onRowClick,
  onChangeNowClick,
  onIncludeButtonClick,
  onExcludeButtonClick,
}: PasswordHealthListItemProps) => {
  const {
    id: credentialId,
    title,
    login,
  ...
  const openCredentialEditPanel = useCallback(() => {
    onRowClick(credentialId, domain);
    history.push(
      routes.userVaultItem(credentialId, VaultItemType.Credential, pathname)
    );
  }, [onRowClick, credentialId, domain, history, routes, pathname]);
  const limitedRights = false;
  const exclusionTooltip = useMemo(
    () => (
      <Tooltip
        location="left"
        content={translate(
      ...
      >
        <Button
          ...
          icon={
            excluded ? (
              <Icon name="ActionUndoOutlined" />
            ) : (
              <Icon name="ActionRevokeOutlined" />
            )
          }
          onClick={excluded ? onIncludeClick : onExcludeClick}
    ...
    [excluded, onExcludeClick, onIncludeClick, translate]
  );
  return (
    <GridContainer
      as="li"
      alignItems="center"
      gridTemplateAreas="'info feedback buttons'"
      gridTemplateColumns="2fr 2.5fr auto"
      gridTemplateRows="62px"
      onClick={openCredentialEditPanel}
      sx={{
        cursor: "pointer",
        "&:not(:last-child)": {
    ...
    >
      ...
      >
        {!excluded && corruptionData ? (
          <RiskInfo severity={corruptionData.severity}>
            <Paragraph
      ...
      <GridChild gridArea="buttons">
        <Flex justifyContent="flex-end" alignItems="center" gap="6px">
          {excluded ? (
            ...
            <>
              <Tooltip
                passThrough={!limitedRights}
                location="left"
                content={translate(
                  I18N_PASSWORD_HEALTH_ROW_KEYS.TOOLTIP_LIMITED_RIGHTS
                )}
              >
                <Button
                  mood="brand"
                  intensity="supershy"
                  layout="labelOnly"
                  className={styles.passwordChangerButton}
                  size="small"
