...
export function useStates(handleError: (error: Error) => void) {
  const [geoStates, setGeoStates] =
    React.useState<null | GeographicStateCollection>(null);
  React.useEffect(() => {
    ...
    retrieveStatesStaticDataCollection().catch((error: Error) =>
      handleError(new Error(`[useStates] ${error}`))
    );
  }, []);
  return geoStates;
}
...
export function useCountryStates(
  ...
  const stateOptions = React.useMemo(() => {
    ...
    const maybeCountry =
      countryInput === null ? undefined : Country[countryInput];
    if (!maybeCountry) {
      return undefined;
    }
