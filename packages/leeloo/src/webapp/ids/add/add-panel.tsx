...
import { IdFormFields, IdVaultItemType } from "../types";
import { IdForm } from "../form/form";
import { IdDocumentAddFooter } from "./footer";
...
export function AddPanel<FormFields extends IdFormFields>({
  children,
  initialValues,
  listRoute,
  reportError,
  ...
  React.useEffect(() => {
    const typeToPageView: Record<IdVaultItemType, PageView> = {
      ...
      [VaultItemType.SocialSecurityId]:
        PageView.ItemSocialSecurityStatementCreate,
    };
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      if (isSuccess(createResult)) {
        ...
        displayGenericError();
        reportError(
          new Error("[ids][add] Unable to add id"),
          createResult.error?.errorMessage
        );
      }
    } catch (error) {
      setSubmitting(false);
  ...
  const formRef = React.useRef(
    (formInstance: FormikProps<FormFields> | null) => {
  ...
  return (
    <>
      ...
      <IdDocumentAddFooter
        ...
        hasDataBeenModified={hasDataBeenModified}
