...
export function useTabs(itemId: string) {
    const isAttachmentAllowed = useHasFeatureEnabled(FEATURE_FLIPS_WITHOUT_MODULE.TechweekWebAttachmentsForIdsV1);
    const editPanelRef = useRef<HTMLDivElement>(null);
    const [activeTab, setActiveTab] = useState<IdGroupTab>(getActiveTabFromSessionStorage(itemId) ?? IdGroupTab.DETAILS);
    const updateActiveTab = useCallback((tab: IdGroupTab) => {
    ...
    const focusAttachment = useCallback(() => updateActiveTab(IdGroupTab.ATTACHMENTS), [updateActiveTab]);
    const { translate } = useTranslate();
    const tabs = useMemo(() => [
        {
            active: activeTab === IdGroupTab.DETAILS,
            label: translate('webapp_secure_notes_addition_tab_title_details'),
            onClick: () => updateActiveTab(IdGroupTab.DETAILS),
        },
        {
            active: activeTab === IdGroupTab.ATTACHMENTS,
            label: translate('webapp_secure_notes_edition_field_tab_title_attachments'),
            onClick: focusAttachment,
        },
    ], [activeTab, focusAttachment, translate, updateActiveTab]);
    useEffect(() => {
