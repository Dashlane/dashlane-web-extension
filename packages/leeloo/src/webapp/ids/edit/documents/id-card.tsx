...
export const IdCardEditPanel = ({ listRoute, id, setDialogActive, lee, hasUnsavedData, setHasUnsavedData, }: IdDocumentProps) => {
    return id ? (<WithData listRoute={listRoute} id={`{${id}}`} setDialogActive={setDialogActive} lee={lee} hasUnsavedData={hasUnsavedData} setHasUnsavedData={setHasUnsavedData} ID_TYPE={VaultItemType.IdCard} I18N_KEYS={I18N_KEYS}>
      {({ handleCopy, values }) => (<IdCardForm variant="edit" handleCopy={handleCopy} country={values.country}/>)}
    </WithData>) : null;
};
