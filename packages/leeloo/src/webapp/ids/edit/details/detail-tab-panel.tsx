...
import { IdDocumentEditFooter } from "../footer";
import { IdForm } from "../../form/form";
import { DetailTabPanelProps } from "./details-types";
...
export const DetailTabPanel = ({
  ...
  const handleSubmit = async (
    values: IdItem,
    { setSubmitting, setFieldError }: FormikHelpers<IdItem>
  ) => {
    const requiredProperty =
      type === VaultItemType.FiscalId ? "fiscalNumber" : "idNumber";
    ...
    try {
      ...
      if (isSuccess(updateResult)) {
        ...
        displayGenericError();
        reportError(
          new Error("[ids][edit] Unable to update id"),
  ...
  return (
    <>
      <div
        aria-labelledby="tab-ids-detail"
        id="content-ids-detail"
      ...
      <IdDocumentEditFooter
        formId={FORM_ID}
        handleCancel={handleCancel}
        handleDelete={openConfirmDeleteDialog}
        hasDataBeenModified={hasUnsavedData}
        isSubmitting={isSubmitting}
        focusAttachmentTab={hasTabs ? focusAttachment : undefined}
      />
    </>
  );
};
