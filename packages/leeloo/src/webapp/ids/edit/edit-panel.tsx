...
import { DetailTabPane } from './details/detail-tab-pane';
...
export const EditPanel = ({ getDescription, title, ...props }: EditPanelProps) => {
    const { translate } = useTranslate();
    const { item, initialValues, type, children, showConfirmDelete, closeConfirmDeleteDialog, deleteDialogTitle, deleteAlertTranslation, listRoute, } = props;
    const { hasTabs, activeTab, tabs, focusAttachment, cleanActiveTab } = useTabs(item.id);
    ...
    React.useEffect(() => {
        const typeToPageView: Record<IdVaultItemType, PageView> = {
            ...
            [VaultItemType.SocialSecurityId]: PageView.ItemSocialSecurityStatementDetails,
        };
    ...
    return (<>
      <Header country={currentCountry} description={getDescription(currentCountry)} title={title} type={type} tabs={hasTabs ? tabs : undefined}/>
      
      {displayDetails ? (<DetailTabPane {...props} setCurrentCountry={setCurrentCountry} hasTabs={hasTabs} focusAttachment={focusAttachment} cleanActiveTab={cleanActiveTab} showListView={showListView}>
          {children}
        </DetailTabPane>) : null}
      <ConfirmDeleteVaultItemDialog isVisible={showConfirmDelete} itemId={item.id} closeConfirmDeleteDialog={closeConfirmDeleteDialog} onDeletionSuccess={showListView} translations={deleteTranslations} vaultItemType={VaultItemType.PaymentCard}/>
    </>);
};
