...
export const initEnableWebAuthnAuthentication = async () => {
    ...
    if (!relyingPartyId) {
        throw new Error('WebAuthn Authentication Enable cannot be started as extension Id is missing');
    }
    const initEnable = await carbonConnector.initEnableWebAuthnAuthentication({
        relyingPartyId,
    });
    if (!initEnable.success) {
        throw new Error('WebAuthn Authentication Enable cannot be completed as init was not possible');
    }
    return initEnable;
};
