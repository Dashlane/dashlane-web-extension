import { InitOpenSessionWithWebAuthnAuthenticatorResult, InitOpenSessionWithWebAuthnAuthenticatorsInfo, } from '@dashlane/communication';
import { carbonConnector } from 'libs/carbon/connector';
import { startAssertion } from 'webapp/webauthn/helpers/credential';
export const openSessionWithWebAuthnAuthenticator = async (initOpenSessionResult: InitOpenSessionWithWebAuthnAuthenticatorResult | undefined, login: string, authAbortSignal?: AbortSignal) => {
    if (!initOpenSessionResult || !initOpenSessionResult.success) {
        throw new Error('WebAuthn authentication - Init Open Sesion Result missing');
    }
