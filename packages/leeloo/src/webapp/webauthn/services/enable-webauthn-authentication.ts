import { InitEnableWebAuthnAuthenticationResult, WebAuthnCallTypes, } from '@dashlane/communication';
...
export const enableWebAuthnAuthentication = async (initEnableResult: InitEnableWebAuthnAuthenticationResult | undefined) => {
    ...
    switch (webAuthnType) {
        case WebAuthnCallTypes.CREATE:
            enableResult = await enableWebAuthnAuthenticationWithAttestation(publicKeyOptions);
            break;
        case WebAuthnCallTypes.GET:
            enableResult = await enableWebAuthnAuthenticationWithAssertion(publicKeyOptions);
            break;
        default:
            enableResult = {
                success: false,
