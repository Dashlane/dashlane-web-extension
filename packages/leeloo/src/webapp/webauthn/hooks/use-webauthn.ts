...
interface Type {
  ...
  readonly openWebAuthnRemoveLastAuthenticatorDialog: (
    credentialId: string,
    isRoaming: boolean
  ) => void;
  readonly onRemoveWebAuthnAuthenticator: (
    credentialId: string,
    isRoaming: boolean
  ) => void;
}
export function useWebAuthn(props: Props = {}): Type {
  ...
  const openWebAuthnEnablerDialog = useCallback(() => {
    const onUnlockSuccess = () => {
      openDialog(
        React.createElement(WebAuthnEnabler, {
          onDialogClose: () => {
            closeDialog();
  ...
  const onRemoveWebAuthnAuthenticator = useCallback(
    async (credentialId: string): Promise<void> => {
      await removeWebAuthnAuthenticator(credentialId);
    },
    []
  );
  const openWebAuthnRemoveLastAuthenticatorDialog = useCallback(
    (credentialId: string) => {
      onDialogStateChanged?.(true);
      openDialog(
        React.createElement(WebAuthnDisableLastAuthenticator, {
          credentialId,
          onDialogClose: () => {
            closeDialog();
            onDialogStateChanged?.(false);
