...
import { enableWebAuthnAuthentication, initEnableWebAuthnAuthentication, } from 'webapp/webauthn/services';
import { useHasWebAuthnPlatformAuthenticator } from 'webapp/webauthn/hooks';
import { EnableWebAuthnDialog } from 'webapp/webauthn/components/dialogs/enable-webauthn-dialog';
import { ConfirmEnableWebAuthnDialog } from 'webapp/webauthn/components/dialogs/confirm-enable-webauthn-dialog';
...
export const WebAuthnEnabler = ({ onDialogClose }: Props) => {
    const [initEnableResult, setInitEnableResult] = useState<InitEnableWebAuthnAuthenticationResult | undefined>();
    ...
    const Dialog = () => {
        switch (showWebAuthnDialog) {
            case DialogShown.ENABLE:
                return (<EnableWebAuthnDialog userHasWebAuthnPlatformAuthenticator={userHasWebAuthnPlatformAuthenticator} onAddAuthenticator={onConfirmEnableWebAuthnDialog} onDismiss={onDismissDialog}/>);
            case DialogShown.CONFIRM_ENABLE:
                return (<ConfirmEnableWebAuthnDialog onDone={onDismissDialog} userHasWebAuthnPlatformAuthenticator={userHasWebAuthnPlatformAuthenticator}/>);
