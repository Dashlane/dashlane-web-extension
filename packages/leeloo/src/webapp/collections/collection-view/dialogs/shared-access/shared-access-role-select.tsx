...
import { PaywalledCollectionRole } from "../../../../sharing-invite/components/paywalled-collection-role";
...
const I18N_KEYS = {
  ...
  REVOKE_DESCRIPTION:
    "webapp_shared_access_item_select_dropdown_revoke_description",
  REVOKE_BUTTON: "webapp_sharing_collection_access_dialog_revoke_button",
  REVOKE_INVITE_BUTTON:
    "webapp_sharing_collection_access_dialog_revoke_invite_button",
};
...
export const SharedAccessRoleSelect = ({
  isUpdatePending,
  ...
  const handleSelectedRole = (newRole?: Permission) => {
    ...
    const existingRole = memberToConfirm.find(
  ...
  const selectedRoleOption = roleOptions.find((roleOption) => {
    const foundMemberToConfirm = memberToConfirm.find(
      (m) => m.id === member.id
    );
  ...
  return (
    <SelectDropdownMenu
      name="sharedAccessSelect"
      id="shared-access-select"
      ...
      onChange={(newRole: any) => handleSelectedRole(newRole?.value)}
      customStyles={{
        menuPortal: (base: any) => ({
          ...base,
          zIndex: 200,
