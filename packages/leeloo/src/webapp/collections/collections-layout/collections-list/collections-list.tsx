...
export const CollectionsList = ({
  ...
  useEffect(() => {
    ...
    setWidths({
      ...
      totalCollectionItemsWidth: collectionItemsWidthsRef.current.reduce(
  ...
  useEffect(() => {
    ...
    if (widths.ulRowWidth >= widths.totalCollectionItemsWidth) {
      ...
      for (
        let index = 0;
        index < collectionItemsWidthsRef.current.length;
        index++
      ) {
  ...
  const onOverflowButtonClick = () => {
    setAmountToDisplay(
      isOverflowButtonOpen ? amountThatFitInListRef.current : listItemsLength
    );
