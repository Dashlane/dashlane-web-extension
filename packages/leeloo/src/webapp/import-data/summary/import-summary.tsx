...
export const ImportSummary = () => {
    ...
    const { preview: { state: { format }, }, import: { state }, importSource, defaultSpace, importMethod, } = useImportPreviewContext();
    const isDirectImport = importMethod === ImportMethod.DIRECT;
    useEffect(() => {
        const pageViewEvent = format === ImportFormats.Csv
    ...
    useEffect(() => {
        ...
        const backupFileType = format === ImportFormats.Csv
            ? BackupFileType.Csv
            : BackupFileType.SecureVault;
        if (state.totalCredentials ||
            backupFileType === BackupFileType.SecureVault) {
            logImportSuccess(backupFileType, importSource, state.importedCredentials, state.totalCredentials, defaultSpace === '' || defaultSpace === null
                ? Space.Personal
                : Space.Professional, isDirectImport);
        }
    }, [format, importSource, defaultSpace, state]);
    return (<FlexContainer flexDirection="column" gap={6}>
      {state.status === ImportDataStep.SUCCESS ? (<>
          ...
          {state.duplicateCredentials ? (<Paragraph color="ds.text.neutral.quiet">
        ...
        </>) : state.status === ImportDataStep.ERROR_GENERIC ? (<Heading size="x-small" sx={{ marginBottom: '24px' }}>
      ...
      <Button mood="brand" intensity="catchy" onClick={() => history.push(globalRouterContext.routes.userCredentials)}>
