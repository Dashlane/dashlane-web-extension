...
import { Button, Icon, Infobox, jsx, Paragraph } from '@dashlane/design-system';
import { BackupFileType, HelpCenterArticleCta, ImportDataStep, PageView, } from '@dashlane/hermes';
import { Eyebrow, FlexContainer, Heading } from '@dashlane/ui-components';
...
export const LastPassLoginPending = ({ openLastPassLoginTab, handleBackButtonClick, error, }: LastPassLoginPendingProps) => {
    ...
    const handleTryAgainButtonClick = () => {
        logImportStart(BackupFileType.Csv, ImportSource.Lastpass, ImportDataStep.DecryptLastpassVault, true);
        history.go(0);
    };
    return (<FlexContainer flexDirection="column" gap={4}>
      ...
      <FlexContainer flexDirection="column" sx={{ maxWidth: '480px' }}>
        ...
        {error ? (<Infobox mood="danger" size="large" actions={[
                <Button key="import" intensity="quiet" mood="danger" size="small" onClick={() => history.push(`${routes.importData}/${ImportDataRoutes.ImportSource}`)}>
                {translate(I18N_KEYS.ERROR_IMPORT_CSV_BUTTON)}
              </Button>,
                <Button key="tryAgain" intensity="catchy" mood="danger" size="small" onClick={handleTryAgainButtonClick}>
      ...
      <FlexContainer flexDirection="row" gap={6}>
        <Button onClick={handleBackButtonClick} mood="neutral" intensity="quiet">
          {translate(I18N_KEYS.BACK_BUTTON)}
        </Button>
        <Button type="button" mood={'brand'} intensity={'catchy'} onClick={openLastPassLoginTab} layout={'iconTrailing'} icon={<Icon name="ActionOpenExternalLinkOutlined"/>}>
