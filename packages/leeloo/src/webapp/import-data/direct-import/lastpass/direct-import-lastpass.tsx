...
import { logDirectImportFailed } from "../../logs";
import { UnlockLastPassVault } from "./unlock-lastpass-vault";
const lastPassCookieName = "PHPSESSID";
const lastPassLoginUrl = "__REDACTED__";
const lastPassIterationsUrl = "__REDACTED__";
const lastPassDataUrl = "__REDACTED__";
const I18N_KEYS = {
...
export const DirectImportLastPass = () => {
  ...
  const fetchLastPassData = useCallback(() => {
    ...
    fetch(lastPassDataUrl, {
      ...
      .then((vaultXMLString) => {
        ...
        const vaultXMLDoc = parser.parseFromString(
          vaultXMLString,
          "application/xml"
        );
        const errorNode = vaultXMLDoc.querySelector("parsererror");
        if (errorNode) {
          logDirectImportFailed(
            BackupFileType.Csv,
            importSource,
            ImportDataStep.DecryptLastpassVault,
            ImportDataStatus.FailedToDecryptVault
          );
      ...
      .catch(() => {
        logDirectImportFailed(
          ...
          importSource,
          ImportDataStep.DecryptLastpassVault,
          ImportDataStatus.FailedToFetchVault
        );
        setError(translate(I18N_ERRORS.GENERIC));
      });
  ...
  const processLastPassData = async (e: SyntheticEvent) => {
    ...
    try {
      ...
      const lastPassCsvFile = await parseLastPassItemsIntoCsv(
        login,
        password,
        iterations,
        vaultXML
  ...
  return !dataLoaded ? (
    <LastPassLoginPending
      error={error}
      openLastPassLoginTab={openLastPassLoginTab}
      handleBackButtonClick={handleBackButtonClick}
    />
  ) : (
    <UnlockLastPassVault
      processLastPassData={processLastPassData}
      error={error}
      handleBackButtonClick={handleBackButtonClick}
