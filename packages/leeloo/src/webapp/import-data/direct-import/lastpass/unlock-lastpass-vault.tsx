...
import { Eyebrow, FlexContainer, Heading, Paragraph, PasswordInput, TextInput, } from '@dashlane/ui-components';
...
const I18N_KEYS = {
    ...
    LOGIN_PLACEHOLDER: 'webapp_account_import_direct_import_lastpass_login_placeholder',
    PASSWORD_PLACEHOLDER: 'webapp_account_import_direct_import_lastpass_password_placeholder',
    ...
    ERROR_IMPORT_CSV_BUTTON: 'webapp_account_import_direct_import_error_import_csv',
};
...
export const UnlockLastPassVault = ({ error, processLastPassData, login, password, handlePasswordInputChange, handleLoginInputChange, handleBackButtonClick, }: UnlockLastPassVaultProps) => {
    const history = useHistory();
    const { translate } = useTranslate();
    const { routes } = useRouterGlobalSettingsContext();
    const { importSource } = useImportPreviewContext();
    const handleTryAgainButtonClick = () => {
        logImportStart(BackupFileType.Csv, importSource, ImportDataStep.DecryptLastpassVault, true);
        history.go(0);
    };
    return (<FlexContainer flexDirection="column" gap={4}>
      ...
      <FlexContainer flexDirection="column" gap={4} as="form" onSubmit={processLastPassData}>
        <Paragraph>{translate(I18N_KEYS.IMPORT_TIP)}</Paragraph>
        <TextInput fullWidth value={login} label={translate(I18N_KEYS.EMAIL_LABEL)} placeholder={translate(I18N_KEYS.LOGIN_PLACEHOLDER)} onChange={handleLoginInputChange}/>
        <PasswordInput fullWidth value={password} label={translate(I18N_KEYS.PASSWORD_LABEL)} placeholder={translate(I18N_KEYS.PASSWORD_PLACEHOLDER)} onChange={handlePasswordInputChange} hidePasswordTooltipText={translate(I18N_KEYS.TOOLTIP_HIDE_LABEL)} showPasswordTooltipText={translate(I18N_KEYS.TOOLTIP_SHOW_LABEL)}/>
        {error ? (<Infobox mood="danger" size="large" actions={[
                <Button size="small" key="import" intensity="quiet" mood="danger" onClick={() => history.push(`${routes.importData}/${ImportDataRoutes.ImportSource}`)}>
                {translate(I18N_KEYS.ERROR_IMPORT_CSV_BUTTON)}
              </Button>,
                <Button size="small" key="try-again" intensity="catchy" mood="danger" onClick={handleTryAgainButtonClick}>
                {translate(I18N_KEYS.ERROR_TRY_AGAIN_BUTTON)}
              </Button>,
            ]} title={translate(I18N_KEYS.ERROR_INFO_TITLE)} description={translate(I18N_KEYS.ERROR_INFO_BODY)}/>) : null}
        <FlexContainer flexDirection="row" gap={4}>
          <Button onClick={handleBackButtonClick} mood="neutral" intensity="quiet">
