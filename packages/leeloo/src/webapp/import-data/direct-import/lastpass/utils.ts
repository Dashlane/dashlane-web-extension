...
const GROUP_IDENTIFIER_URL = "__REDACTED__";
const escapeDoubleQuote = (content: string): string => {
...
const decryptField = async (fieldString: string, key: ArrayBuffer) => {
  ...
  if (fieldString.startsWith("!")) {
    const [encodedIvWithIdentifier, encodedEncryptedData] =
      fieldString.split("|");
    ...
    const decryptedData = arrayBufferToText(
      decryptAesEcb256(key, encryptedData)
    );
    return decryptedData;
  }
};
export const parseLastPassItemsIntoCsv = async (
  userLogin: string,
  userPassword: string,
  iterations: number,
  vaultXML: Document
  ...
  const key = await deriveKeyPbkdf2(
    textEncoder.encode(userPassword),
    textEncoder.encode(userLogin.toLocaleLowerCase()),
    {
  ...
  for (const node of nodes) {
    if (node.tagName === "account") {
      const loginNode = node.getElementsByTagName("login")?.[0];
      const encryptedName = node.getAttribute("name") ?? "";
      const encryptedCollection = node.getAttribute("group") ?? "";
      const encryptedNotes = node.getAttribute("extra") ?? "";
      const fav = node.getAttribute("fav") ?? "0";
      ...
      const notes = escapeDoubleQuote(await decryptField(encryptedNotes, key));
      ...
      csvRows.push(
        `"${url}","${username}","${password}","${otp}","${notes}","${name}","${collection}","${fav}"`
      );
    }
  }
