...
export const useNavigateAwayListener = ({
  importStep,
  importSource,
  isDirectImport,
  itemsToImportCount,
}: UseNavigateListenerProps) => {
  const history = useHistory();
  const routerContext = useRouterGlobalSettingsContext();
  const importDataRoutes = Object.values(ImportDataRoutes).map(
    (route) => `${routerContext.routes.importData}/${route}`
  );
