...
import { ImportPersonalDataError, ImportPersonalDataRequest, ImportPersonalDataStartingSuccess, ImportPersonalDataState, ImportPersonalDataStateType, ImportSource, } from '@dashlane/communication';
...
export const toLocalState = (carbonStatus: ImportPersonalDataState): ImportDataState => {
...
export interface UseImportData {
    startImport: (command: ImportPersonalDataRequest, source?: ImportSource, fileType?: BackupFileType, importMethod?: ImportMethod) => Promise<ImportPersonalDataStartingSuccess | ImportPersonalDataError>;
    dismiss: () => void;
...
export const useImportData = (): UseImportData => {
    ...
    const startImport = useCallback(async (command: ImportPersonalDataRequest, source?: ImportSource, fileType?: BackupFileType, importMethod?: ImportMethod) => {
        ...
        if (!result.success) {
            ...
            if (fileType && source) {
                ...
                logImportFailure(fileType, source, command.content.length, isDirectImport);
            }
        }
        return result;
    }, []);
