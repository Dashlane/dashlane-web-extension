...
import { Eyebrow, FlexContainer, Heading, jsx } from '@dashlane/ui-components';
...
import { BackupFileType, ImportDataDropAction, ImportDataStatus, ImportDataStep, PageView, UserImportDataEvent, } from '@dashlane/hermes';
import useTranslate from 'libs/i18n/useTranslate';
import { ImportDragNDrop } from 'webapp/import-data/components/import-drag-n-drop';
...
export const ImportSelect = () => {
    ...
    const { preview: { processFile, state, resetState, previewFile }, importSource, setPreviewDataItems, setPreviewDataHeaders, } = useImportPreviewContext();
    useEffect(() => {
        resetState();
    }, []);
    useEffect(() => {
        const pageViewEvent = importSource === ImportSource.Dash
            ? PageView.ImportDash
            : PageView.ImportCsv;
    ...
    const importReadyMessage = translate(isCSV ? I18N_KEYS.CSV_READY : I18N_KEYS.DASH_READY);
            ...
            previewFile(importSource).then((previewData) => {
                if (previewData.success && previewData.data.items.length) {
                    ...
                    logEvent(new UserImportDataEvent({
                        ...
                        importDataStatus: previewData.success && !previewData.data.items.length
    ...
    return (<FlexContainer flexDirection="column">
      ...
      <Heading size="small" color="ds.text.neutral.catchy" sx={{ mb: '24px' }}>
        {isCSV
            ? translate(I18N_KEYS.HEADER_CSV)
            : translate(I18N_KEYS.HEADER_DASH)}
      </Heading>
      <ImportDragNDrop onFileDropped={handleFileDropped} undoFileDropped={resetState} setErrorMessage={setErrorMessage} error={errorMessage} accept={fileAccept} idleMessage={importIdleMessage} fileSelectedMessage={importReadyMessage} setFileInvalid={setIsFileInvalid} importSource={importSource}/>
      {isFileInvalid ? (<div sx={{ marginTop: '24px' }}>
          <Infobox mood="brand" title={<>
                {translate(I18N_KEYS.ERROR_HELP_CENTER_TITLE)}
                <br />
                {translate.markup(I18N_KEYS_ERROR_TIP[importSource], {
                    supportLink: IMPORT_GUIDE[importSource],
                }, {
                    linkTarget: '_blank',
                })}
              </>}/>
        </div>) : displayFormatInfo ? (<div sx={{ marginTop: '24px' }}>
          <Infobox mood="danger" title={translate.markup(I18N_KEYS_IMPORT_SOURCE[importSource], {
                supportLink: IMPORT_GUIDE[importSource],
            }, {
                linkTarget: '_blank',
            })}/>
        </div>) : null}
      <FlexContainer gap="16px" sx={{ mt: '16px' }}>
        <Button type="button" mood="neutral" intensity="quiet" onClick={goBack}>
          {translate(I18N_KEYS.BACK_BUTTON)}
        </Button>
        <Button mood="brand" intensity="catchy" type="button" onClick={handlePreviewFile} disabled={!state.file || !!errorMessage || isLoading}>
