import { Eyebrow, FlexContainer, Heading, jsx, Paragraph, } from '@dashlane/ui-components';
...
const I18N_KEYS = {
    ...
    FORCED_DOMAIN_INFOBOX: 'webapp_account_import_space_select_forced_domain_infobox'
};
export const ImportSpaceSelect = () => {
    ...
    const { importSource, preview: { state }, defaultSpace, setDefaultSpace, importMethod, } = useImportPreviewContext();
    ...
    return (<FlexContainer flexDirection="column">
      ...
      <SpaceSelect onChange={handleDefaultSpaceChange} hideLabel={true} wrapperSx={{ display: 'block' }}/>
      <FlexContainer gap="16px" sx={{ my: '24px' }}>
        <Button mood="neutral" intensity="quiet" type="button" onClick={goBack}>
          {translate(I18N_KEYS.BACK_BUTTON)}
        </Button>
        <Button mood="brand" intensity="catchy" type="button" onClick={handleNextButtonClick} disabled={isLoading || defaultSpace === null}>
          {translate(I18N_KEYS.NEXT_BUTTON)}
        </Button>
      </FlexContainer>
      {isForcedDomainsEnabled ? (<Infobox size="medium" mood="neutral" title="" description={<Paragraph size="small">
