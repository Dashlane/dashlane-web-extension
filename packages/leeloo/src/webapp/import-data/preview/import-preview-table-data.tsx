...
import { FlexContainer, SelectDropdownMenu } from '@dashlane/ui-components';
import { DataModelType, ParsedCSVData, PremiumStatusSpace, } from '@dashlane/communication';
...
import { CELL_HEIGHT, innerTableCellStyles, ROW_HEIGHT, tableCellStyles, tableRowStyles, } from './styles';
...
export interface ImportPreviewTableDataProps {
    ...
    tableData: Array<[
        string,
        ParsedCSVData['items'][number]
    ]>;
    tableHeaders: ParsedCSVData['headers'];
    disabledItemRows: string[];
    spaceList: PremiumStatusSpace[];
    handleItemTypeChange: (itemId: string, modelType: ImportDataModelType) => void;
...
const importVaultIcons: Record<ImportDataModelType, ReactNode> = {
    KWAuthentifiant: (<Icon name="ItemLoginFilled" size="small" color="ds.text.neutral.standard"/>),
    KWSecureNote: (<Icon name="ItemSecureNoteFilled" size="small" color="ds.text.neutral.standard"/>),
    KWBankStatement: (<Icon name="ItemBankAccountFilled" size="small" color="ds.text.neutral.standard"/>),
    KWPaymentMean_creditCard: (<Icon name="ItemPaymentFilled" size="small" color="ds.text.neutral.standard"/>),
    DoNotImport: null,
};
...
const formatDataCellContent = (header: ParsedCSVData['headers'][number], item: ParsedCSVData['items'][number], translate: TranslatorInterface) => {
    ...
    switch (header.matched) {
        case 'Password': {
            cellContent = item.baseDataModel[header.matched] ? (<InputWithActionButton passwordInputProps={{
                    readOnly: true,
...
export const ImportPreviewTableDataComponent = ({ wrapperRef, tableData, tableHeaders, disabledItemRows, spaceList, handleItemTypeChange, handleSpaceSelectChange, }: ImportPreviewTableDataProps) => {
    ...
    const VAULT_ITEM_TYPE_OPTIONS = useMemo(() => Object.keys(KW_TYPE_TO_I18N_KEY).reduce((prev, itemType) => ({
        ...prev,
        [itemType]: {
            value: itemType,
    ...
    const spaceSelectOptions = useMemo(() => [
        ...spaceList.reduce((options, space) => [
            ...options,
            {
                value: space.teamId,
                label: (<FlexContainer alignItems="center">
    ...
    const FillerRow = ({ position }: {
        position: 'start' | 'end';
    }) => {
        const height = position === 'start'
            ? displayStart * ROW_HEIGHT
            : (tableData.length - displayEnd) * ROW_HEIGHT;
        return (<tr key={`${position}-loader`} sx={{
                height: height,
            }}>
    ...
    const setDisplayPositions = useCallback((scroll: number) => {
        const scrollWithOffset = Math.floor(scroll - rowsToRender - wrapperHeight / 2);
        const displayStartPosition = Math.floor(Math.max(0, Math.floor(scrollWithOffset / ROW_HEIGHT)));
        const startWithRows = Math.max(displayStartPosition + rowsToRender, 1);
        const displayEndPosition = Math.floor(Math.min(startWithRows, tableData.length));
    ...
    return (<tbody>
      ...
      {tableData
            ...
            .filter(([, dataItem]) => dataItem.baseDataModel.kwType !== DataModelType.KWCollection)
            .map((dataItem) => {
            const [itemId, item] = dataItem;
            return (<tr sx={tableRowStyles} key={item.baseDataModel.Id}>
              <td sx={tableCellStyles}>
                <div sx={innerTableCellStyles}>
                  <SelectDropdownMenu menuPortalTarget={document.body} isSearchable={false} onChange={(e: SelectOptionType) => {
                    handleItemTypeChange(itemId, DataModelType[e.value]);
                }} defaultValue={VAULT_ITEM_TYPE_OPTIONS[item.baseDataModel.kwType ?? DataModelType.KWSecureNote]} options={Object.values(VAULT_ITEM_TYPE_OPTIONS)}/>
                </div>
              </td>
              {spaceList.length && shouldDisplaySpaceSelector ? (<td sx={tableCellStyles}>
                  <div sx={innerTableCellStyles} data-testid="space-select">
                    <SelectDropdownMenu name="spaceSelect" menuPortalTarget={document.body} isSearchable={false} options={spaceSelectOptions} defaultValue={item.baseDataModel['SpaceId'] !== undefined
                        ? spaceSelectOptions.find((option) => option.value === item.baseDataModel['SpaceId']) ?? spaceSelectOptions[0]
                        : undefined} isDisabled={disabledItemRows.includes(itemId)} onChange={(e: SelectOptionType) => handleSpaceSelectChange(itemId, e.value)}/>
                  </div>
                </td>) : null}
              
              {tableHeaders.map((header) => header.matched ? (<td sx={tableCellStyles} key={header.original}>
                    <div sx={innerTableCellStyles}>
