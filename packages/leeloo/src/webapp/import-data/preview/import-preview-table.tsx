...
export const I18N_HEADER_KEYS: Record<
  string | SupportedVaultItemKeys | "Other",
  string
> = {
...
export type ImportDataModelType =
  | Extract<
      DataModelType,
      | "KWAuthentifiant"
      | "KWSecureNote"
...
export const ImportPreviewTable = ({
  ...
  useEffect(() => {
    ...
    Object.keys(tableData).forEach((itemId) => {
      ...
      const hasSmartSpaceField =
        credentialUrl || credentialEmail || credentialLogin;
      ...
      if (hasSmartSpaceField) {
        ...
        for (const [spaceId, domains] of memoizedForcedSpaceDomainsMap) {
          if (
            domains.includes(credentialDomain) ||
    ...
    if (item) {
      if (modelType === DataModelType.KWSecureNote) {
        if (item.rawData["Content"]) {
          ...
          item.baseDataModel["Content"] = Object.entries(item.rawData).reduce(
            (prev, [key, value]) => {
  ...
  return (
    <div sx={{ display: "inline-grid", position: "relative", marginY: "24px" }}>
      <div
        ref={wrapperRef}
        sx={{
          overflow: "auto",
      ...
      >
        ...
        >
          <thead sx={{ position: "sticky", zIndex: 1 }}>
            <tr>
              <th sx={headerCellStyles}>
                ...
                <Tooltip
                  content={<div>{translate(I18N_KEYS.ITEM_TYPE_TOOLTIP)}</div>}
                  placement="top"
                >
                  <IconButton
                    sx={{
                    ...
                    aria-label={translate(I18N_HEADER_KEYS.kwType)}
                    icon={<InfoCircleIcon />}
                  />
                </Tooltip>
              </th>
                ...
                <th sx={headerCellStyles}>
                  ...
                  >
                    <IconButton
                      ...
                      aria-label={translate(I18N_HEADER_KEYS.SpaceId)}
                      icon={<InfoCircleIcon />}
                    />
                  </Tooltip>
                </th>
              ...
              {tableHeaders.map((header) =>
                header.matched ? (
                  <th sx={headerCellStyles} key={header.original}>
                    <Eyebrow size="medium">
          ...
          <ImportPreviewTableData
            wrapperRef={wrapperRef}
            tableData={Object.entries(tableData)}
            tableHeaders={tableHeaders}
            disabledItemRows={disabledItemRows}
            spaceList={spaceList}
            handleItemTypeChange={handleItemTypeChange}
            handleSpaceSelectChange={handleSpaceSelectChange}
          />
        </table>
      </div>
    </div>
