...
import { AlertSeverity, Eyebrow, FlexChild, FlexContainer, Heading, jsx, Link, Paragraph, } from '@dashlane/ui-components';
...
import { BackupFileType, HelpCenterArticleCta, ImportDataStep as HermesImportDataStep, ImportDataDropAction, ImportDataStatus, PageView, UserImportDataEvent, } from '@dashlane/hermes';
import { logEvent, logPageView } from 'libs/logs/logEvent';
import { logHelpCenterEvent, logImportFailure, logImportFlowTerminated, } from '../logs';
import { BaseDataModelObject, DataModelType, ImportSource, PremiumStatusSpace, } from '@dashlane/communication';
...
import { ImportLocationState, ImportMethod, ImportSourcesToLogSources, } from '../types';
...
export const ImportPreview = () => {
    const { preview: { state, setAttemptedCount }, import: { startImport, state: importDataState }, importSource, previewDataItems, setPreviewDataItems, previewDataHeaders, importMethod, } = useImportPreviewContext();
    ...
    const totalAndCurrentStep = hasMultipleSpaces || (isDirectImport && isPersonalSpaceEnabled) ? 3 : 2;
    ...
    const filteredImportCount = Object.values(previewDataItems).filter((item) => item.baseDataModel.kwType &&
        item.baseDataModel.kwType !== DataModelType.KWCollection).length;
    useEffect(() => {
    ...
    useEffect(() => {
        switch (importDataState.status) {
            case ImportDataStep.ERROR_GENERIC: {
                logImportFailure(BackupFileType.Csv, importSource, state.attemptedItemCount, isDirectImport);
                showErrorAlert();
                return;
            }
    ...
    const handleImportClick = async () => {
        ...
        try {
            if (state.format) {
                ...
                }, importSource, backupFileType, importMethod);
    ...
    return (<FlexContainer flexDirection="column" gap="10px">
      {importStarted || importDataState.status === ImportDataStep.PROCESSING ? (<ImportDataDialogLoading showCloseIcon={false} isOpen={true}/>) : null}
      <CatchUnsavedChanges titleText={translate(I18N_KEYS.UNSAVED_CHANGES_TITLE)} bodyText={translate(I18N_KEYS.UNSAVED_CHANGES_BODY)} onUnsavedChangesCaught={() => logPageView(PageView.ImportLeaveProcess)} onLeavePage={() => logImportFlowTerminated(importSource, Object.keys(previewDataItems).length, isDirectImport)} dirty={!!state.fileName &&
      ...
      {Object.keys(previewDataItems).length ? (<ImportPreviewTable setHasChanges={setHasChanges} tableData={previewDataItems} setTableData={setPreviewDataItems} tableHeaders={previewDataHeaders}/>) : null}

      <FlexContainer gap="16px">
        <Button onClick={handleBackButtonClick} type="button" mood="neutral" intensity="quiet">
          {translate(I18N_KEYS.BACK_BUTTON)}
        </Button>
        <Button mood="brand" intensity="catchy" disabled={!filteredImportCount} type="button" onClick={handleImportClick}>
        ...
        <FlexChild flex={1}/>
        <FlexChild as={Link} alignSelf="center" color="ds.text.brand.standard" href={IMPORT_GUIDE[importSource]} target="_blank" rel="noopener noreferrer" onClick={() => logHelpCenterEvent(HelpCenterArticleCta.TroubleshootImportingIssues)}>
