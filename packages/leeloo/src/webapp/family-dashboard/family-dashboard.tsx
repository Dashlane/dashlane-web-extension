...
import { AlertSeverity, Lockup, LockupColor, LockupSize, } from '@dashlane/ui-components';
...
const I18N_KEYS = {
    ...
    MEMBER_REMOVED_AND_LINK_RESET: 'webapp_family_dashboard_member_removed_link_reset_success_message',
...
export const FamilyDashboard = () => {
    ...
    const { familyDetails, familyDetailsError, removeMemberFromFamilyDetails } = useFamilyDetails();
    useEffect(() => {
        if (!familyDetails) {
            return;
        }
    ...
    const removeSuccessAlert = useAlert(translate(I18N_KEYS.MEMBER_REMOVED), AlertSeverity.SUCCESS);
    const removeErrorAlert = useAlert(translate(I18N_KEYS.MEMBER_REMOVED_ERROR), AlertSeverity.ERROR);
    const removeAndResetSuccessAlert = useAlert(translate(I18N_KEYS.MEMBER_REMOVED_AND_LINK_RESET), AlertSeverity.SUCCESS);
    const resetSuccessAlert = useAlert(translate(I18N_KEYS.LINK_RESET), AlertSeverity.SUCCESS);
    const resetErrorAlert = useAlert(translate(I18N_KEYS.LINK_RESET_ERROR), AlertSeverity.ERROR);
    ...
    const onRemoveMember = useCallback(async (userId: number, resetLink: boolean) => {
        try {
    ...
    const invitationLink = `*****${joinToken}`;
    ...
    return (<>
      <div className={styles.header}>
        ...
        <div>
          <FamilySettings onMyAccountClick={onMyAccountClick} onResetLinkClick={onResetLinkClick}/>
        </div>
      </div>
      <div className={styles.content}>
        <FamilyHeader invitationLink={invitationLink} spots={spots} statusCode={familyDetails.statusCode}/>
        {familyDetails.members ? (<MemberList admin={familyDetails.members.admin} regularMembers={familyDetails.members.regular} removeMember={onRemoveMember}/>) : null}
      </div>
      ...
      {showResetLinkDialog ? (<ResetLinkDialog handleResetLink={onResetLinkConfirm} handleOnCancel={onCancelResetLink}/>) : null}
    </>);
};
