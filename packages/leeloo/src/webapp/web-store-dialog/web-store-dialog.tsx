...
export const WebStoreDialog = () => {
  ...
  const hasWebStoreMessage = messages.find(
    (message) =>
      message.type === UserMessageTypes.WEB_STORE && !message.dismissedAt
  );
  const handleDismiss = () => {
  ...
  if (showDialog) {
    return (
      <div sx={{ position: "absolute", bottom: "40px", right: "40px" }}>
        {dialogState === WebDialogState.RateForm ? (
          <RateForm
            onClose={handleDismiss}
            formResponse={(response: RateFormResponse) => {
        ...
        {dialogState === WebDialogState.ThankYou ? (
          <ThankYou onClose={handleDismiss} />
        ) : null}
        {dialogState === WebDialogState.WebStore ? (
          <WebStore onClose={handleDismiss} />
        ) : null}
      </div>
    );
  }
