...
): UseUserMessageOutput => {
  const messages = useUserMessages();
  const hasMessage = !!messages.find(
    (message) => message.type === messageType && !message.dismissedAt
  );
  const hasDismissedMessage = !!messages.find(
    (message) => message.type === messageType && message.dismissedAt
  );
