...
export interface GroupListProps {
  userGroups: UserGroupView[] | SharingGroup[];
...
export const GroupList = ({
  ...
  const [displayShowMoreButton, setDisplayShowMoreButton] = React.useState(
    userGroups.length > INITIAL_ROW_COUNT
  );
  const [maxRows, setMaxRows] = React.useState(INITIAL_ROW_COUNT);
  const [groups, setGroups] = React.useState<UserGroupView[] | SharingGroup[]>(
    []
  );
  ...
  const onResort = React.useCallback(
  ...
  return (
    <>
      <GroupHeader
        updateGroupSortDirection={onResort}
        currentSortDirection={currentSortDirection}
      />
      <ul className={styles.groupList}>
        {groups.slice(0, maxRows).map((group) => (
          <GroupRow
            key={group.id}
            group={group}
