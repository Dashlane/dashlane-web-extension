...
import { PaywallProps } from "../paywall/generic-paywall";
import { DarkWebMonitoringPaywall } from "../paywall/dark-web-monitoring-paywall";
import { SecureNotesPaywall } from "../paywall/secure-notes-paywall";
import { PasswordLimitDialog } from "../../credentials/header/password-limit-dialog";
import {
  useVpnPageAccess,
  VPNPaywallType,
} from "../../vpn/hooks/use-vpn-page-access";
...
export interface PaywallManagerProps
  extends Pick<PaywallProps, "mode" | "closePaywall"> {
  paywall?: PaywallName;
  children?: React.ReactNode;
}
export const shouldShowPaywall = (
  requiredCapability: PaywalledCapability,
  ...
  if (paywall) {
    if (paywall === PaywallName.SecureNote) {
      ...
      if (
        ...
        return (
          <DarkWebMonitoringPaywall mode={mode} closePaywall={closePaywall} />
        );
      }
      ...
      if (vpnPaywallInfo.isVisible) {
        return (
          <VpnPaywall
            ...
            premiumStatus={premiumStatus.data}
            isB2CPaywall={vpnPaywallInfo.type === VPNPaywallType.B2C}
          />
        );
      }
