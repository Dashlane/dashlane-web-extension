...
export const checkIfLinkedWebsitesHaveBeenModified = (
  newLinkedWebsitesAddedByUserList: string[],
  savedLinkedWebsitesAddedByUserList: string[]
): boolean => {
  return [
    ...
    .some(
      (website) =>
        !(
          newLinkedWebsitesAddedByUserList.includes(website) &&
          savedLinkedWebsitesAddedByUserList.includes(website)
        )
    );
};
const getCredentialMatchQuery = (
  domain: string,
  email: string
...
export const checkForDuplicateWebsites = async (
  credential: Credential,
  linkedWebsitesAddedByUser: string[]
): Promise<LinkedWebsitesDuplicateDialogData[]> => {
  const addedDomains = linkedWebsitesAddedByUser
    .filter(
      (website: string) => website && !credential.linkedURLs.includes(website)
    )
    .map((website) => new ParsedURL(website).getRootDomain());
  const matchedCredentialsPromises = addedDomains.map(
    async (domain) =>
      await carbonConnector.getCredentialsByDomain(
        getCredentialMatchQuery(domain, credential.email)
      )
  );
  const matchedCredentialResults = (
    await Promise.all(matchedCredentialsPromises)
  ).map((cred, index) => ({
    ...cred,
    linkedWebsite: addedDomains[index],
  }));
  const duplicateCredentials = matchedCredentialResults.filter(
    (credentialMatch) => credentialMatch.matchingCount > 0
  );
  return duplicateCredentials.map(
    (credential): LinkedWebsitesDuplicateDialogData => {
      return {
...
const hashMultipleDomains = async (domains: string[]) => {
  return await Promise.all(
    domains.map((website) => hashDomain(new ParsedURL(website).getRootDomain()))
  );
};
export const sendLinkedWebsitesLogs = async (
  credential: Credential,
  linkedWebsitesAddedByUser: string[]
) => {
  const addedWebsites = linkedWebsitesAddedByUser.filter(
    (website: string) => website && !credential.linkedURLs.includes(website)
  );
  const deletedWebsites = credential.linkedURLs.filter(
    (website: string) => website && !linkedWebsitesAddedByUser.includes(website)
  );
...
export const goToWebsite = async (url: string, credentialId: string) => {
