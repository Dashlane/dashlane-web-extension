...
import { Button, Icon, WebsiteField } from "@dashlane/design-system";
import useTranslate from "../../../libs/i18n/useTranslate";
import { AddLinkedWebsiteButton } from "./add-linked-website-button";
import { goToWebsite } from "./helpers";
const I18N_KEYS = {
  NEW_WEBSITE_PLACEHOLDER:
    "webapp_credential_linked_websites_new_website_placeholder",
...
const LinkedWebsitesListComponent = ({
  linkedWebsitesList,
  credentialId,
  isListEditable = false,
  isEditing = false,
  showUrlProtocol = false,
  updateLinkedWebsitesList,
}: LinkedWebsitesListProps) => {
  const { translate } = useTranslate();
  const focusField = React.useRef<HTMLInputElement>(null);
  const [addWebsiteButtonDisabled, setAddWebsiteButtonDisabled] =
    React.useState(false);
  const [counter, setCounter] = React.useState(linkedWebsitesList.length);
  const [rows, setRows] = React.useState<Row[]>(
    linkedWebsitesList.map((website, index) => {
      return {
  ...
  return (
    <div>
      {isListEditable ? (
        <div>
          {rows.map(({ linkedWebsite, id }, index) => (
            <WebsiteField
              className="row"
              key={id}
              name={id.toString()}
              label="example.com"
              labelPersists={false}
              placeholder={translate(I18N_KEYS.NEW_WEBSITE_PLACEHOLDER)}
              ref={focusField}
              ...
              actions={[
                <Button
                  ...
                  tooltip={translate(I18N_KEYS.DELETE)}
                />,
              ]}
          ...
          <div sx={{ display: "flex", justifyContent: "flex-end" }}>
            <AddLinkedWebsiteButton
              disabled={addWebsiteButtonDisabled}
              handleOnClickAddLinkedWebsiteButton={onAddNewItem}
              label={translate(I18N_KEYS.ADD_LINKED_WEBSITE)}
            />
          </div>
        </div>
      ) : (
          ...
          {rows.map(({ linkedWebsite, id }, index) => (
            <WebsiteField
              ...
              value={
                showUrlProtocol
                  ? linkedWebsite
                  : getUrlWithoutProtocol(linkedWebsite)
              }
              showOpenWebsite={{
                label: translate(I18N_KEYS.GO_TO_WEBSITE),
                onClick: () => goToWebsite(linkedWebsite, credentialId),
