...
export const CredentialEditionWebsitesComponent = ({
  url,
  ...
  const dashlaneLinkedWebsitesLength =
    dashlaneDefinedLinkedWebsites && dashlaneDefinedLinkedWebsites.length > 1
      ? dashlaneDefinedLinkedWebsites.length
      : 0;
  const totalLinkedWebsitesLength =
    userLinkedWebsitesLength + dashlaneLinkedWebsitesLength;
  return (
    <WebsitesComponent
      url={url}
    ...
    >
      <div
        sx={{
          display: "flex",
          justifyContent:
            totalLinkedWebsitesLength > 0 ? "space-between" : "flex-end",
        }}
      >
        <LinkedWebsitesCount
          totalLinkedWebsitesLength={totalLinkedWebsitesLength}
        />
        {onClickAddNewWebsite && !limitedPermissions ? (
          <AddLinkedWebsiteButton
            handleOnClickAddLinkedWebsiteButton={onClickAddNewWebsite}
            disabled={isWebsiteFieldReadonly}
            label={
              totalLinkedWebsitesLength > 0
                ? translate(I18N_KEYS.MANAGE_LINKED_WEBSITE)
                : translate(I18N_KEYS.ADD_LINKED_WEBSITE)
            }
          />
        ) : null}
      </div>
