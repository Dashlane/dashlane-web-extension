...
import { PAYWALL_ACTION, PAYWALL_SUBTYPE, PAYWALL_TYPE, } from 'webapp/paywall/logs';
...
const UTM_SOURCE_CODE = 'button:buy_dashlane++origin_page:paywall/free_user_password_limit+origin_component:webapp';
...
export const PasswordLimitDialog = ({ isVisible, handleDismiss, }: PasswordLimitDialogProps) => {
    ...
    const goToCheckout = () => {
        const buyDashlaneLink = `${GET_PREMIUM_URL}?subCode=${accountInfo?.subscriptionCode ?? ''}&utm_source=${UTM_SOURCE_CODE}`;
    ...
    return (<Dialog isOpen={isVisible} onClose={handleOnRequestClose} title={translate(I18N_KEYS.TITLE)} closeActionLabel={translate(I18N_KEYS.CLOSE)} actions={{
            primary: {
