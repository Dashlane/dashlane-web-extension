...
const I18N_KEYS = {
  ...
  SUBDOMAIN_ONLY: {
    ...
    NEW_DETAILS:
      "webapp_credential_edition_field_options_added_by_user_only_details",
  },
  REQUIRE_MP: {
    TITLE:
      "webapp_credential_edition_field_options_always_require_master_password",
    DETAILS:
      "webapp_credential_edition_field_options_always_require_master_password_details",
    FEEDBACK:
      "webapp_credential_edition_field_options_always_require_master_password_disable_feedback_markup",
  },
};
export const CredentialPreferences = ({
  credentialPreferences,
  isNewCredential,
  url: credentialUrl,
  isPreferenceDisabled = false,
  update,
}: CredentialPreferencesProps) => {
  ...
  const isUrlValid = useMemo(() => {
    ...
    try {
      ...
      try {
        new URL("__REDACTED__" + credentialUrl);
        return true;
      } catch (f) {
        return false;
      }
    ...
    <div>
      {!isUrlValid ? (
        <Infobox
          mood="warning"
          size="small"
          title={translate(I18N_KEYS.NO_URL_WARNING)}
          sx={{ marginBottom: "8px" }}
        />
      ) : null}
      <CredentialPreferenceSetting
        name="useAutoLogin"
        description={translate(I18N_KEYS.AUTOLOG.DETAILS)}
        isMpProtected={false}
        title={translate(I18N_KEYS.AUTOLOG.TITLE)}
        setValue={setUseAutoLogin}
        value={credentialPreferences.useAutoLogin}
        disabled={!isUrlValid || isPreferenceDisabled}
      />
      <CredentialPreferenceSetting
        name="onlyAutofillExactDomain"
        isMpProtected={false}
        title={translate(I18N_KEYS.SUBDOMAIN_ONLY.TITLE)}
        description={translate(I18N_KEYS.SUBDOMAIN_ONLY.NEW_DETAILS)}
        setValue={setOnlyAutofillExactDomain}
        value={credentialPreferences.onlyAutofillExactDomain}
        disabled={!isUrlValid || isPreferenceDisabled}
      />
      {isRequireMPAvailable ? (
        protectPasswordsSetting.status === DataStatus.Success ? (
          <CredentialPreferenceSetting
            name="requireMasterPassword"
            isMpProtected={!isNewCredential}
            title={translate(I18N_KEYS.REQUIRE_MP.TITLE)}
            description={
              <div>
                {protectPasswordsSetting.data ? (
                  <div sx={{ marginBottom: "8px", color: colors.midGreen00 }}>
            ...
            setValue={setRequireMasterPassword}
            value={
              protectPasswordsSetting.data
                ? true
          ...
          <CredentialPreferenceSetting
            ...
            isMpProtected={!isNewCredential}
            title={translate(I18N_KEYS.REQUIRE_MP.TITLE)}
            description={translate(I18N_KEYS.REQUIRE_MP.DETAILS)}
            setValue={setRequireMasterPassword}
            value={credentialPreferences.requireMasterPassword}
            disabled={!isUrlValid || isPreferenceDisabled}
          />
        )
      ) : null}
