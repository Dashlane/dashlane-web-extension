...
import { logUserEventAskAuthentication } from 'webapp/unlock-items/logs';
...
export const CredentialPreferenceSetting = ({ name, title, description, setValue, isMpProtected = false, value, disabled = false, }: PropsWithChildren<Props>) => {
    ...
    return (<Checkbox label={title} description={description} checked={value} onChange={onChange} id={id} disabled={disabled}/>);
};
