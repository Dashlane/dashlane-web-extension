...
export const I18N_KEYS = {
  ...
  OPTIONS_USE_SIMILAR_CHARACTERS:
    "webapp_credential_edition_field_generator_similar_characters",
};
...
export const GeneratorOptions = ({
  ...
  const onDigitsChanged = useCallback(
  ...
  const onLettersChanged = useCallback(
  ...
  const onSymbolsChanged = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const onSimilarCharactersChanged = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    ...
    >
      <div>
        <Checkbox
          label={translate(I18N_KEYS.OPTIONS_USE_LETTERS)}
          onChange={onLettersChanged}
          checked={options.letters}
          disabled={options.letters && !options.digits}
        />
        <Checkbox
          label={translate(I18N_KEYS.OPTIONS_USE_DIGITS)}
      ...
      <div>
        <Checkbox
          label={translate(I18N_KEYS.OPTIONS_USE_SYMBOLS)}
          onChange={onSymbolsChanged}
          checked={options.symbols}
        />
        <Checkbox
          label={translate(I18N_KEYS.OPTIONS_USE_SIMILAR_CHARACTERS)}
