...
import { useIsCredentialExcluded } from 'webapp/password-health/hooks/use-is-credential-excluded';
import { useCredentialHealthData } from 'webapp/password-health/hooks/use-credentials-health-data';
...
export const CredentialHealthBox = ({ credentialId, isShared, hasLimitedPermission, }: CredentialHealthBoxProps) => {
    ...
    if (!healthData) {
        return (<div sx={{ marginBottom: '24px' }}>
    ...
    return (<div sx={{ position: 'relative' }}>
      ...
      {healthData.excluded ? (<ExcludedHealthBox onPrimaryClick={onIncludePassword}/>) : (<IncludedHealthBox onPrimaryClick={onChangePasswordClick} onSecondaryClick={onViewPasswordHealthClick} isLimitedSharingPassword={isShared && hasLimitedPermission} corruptionData={healthData.corruptionData}/>)}
    </div>);
};
