...
const I18N_KEYS = {
  ...
  COMPROMISED_DESCRIPTION:
    "webapp_credential_edition_health_box_compromised_description",
  ...
  REUSED_DESCRIPTION:
    "webapp_credential_edition_health_box_reused_description_markup",
...
const IncludedHealthBoxComponent = ({
  onPrimaryClick,
  onSecondaryClick,
  isLimitedSharingPassword,
  corruptionData,
}: IncludedHealthBoxProps) => {
  ...
  const mood =
    corruptionData.severity === CorruptionDataSeverity.STRONG
      ? "warning"
      : "brand";
  ...
  const renderChangePassword = () => {
    return (
      <Tooltip
        passThrough={!isLimitedSharingPassword}
        key="change_password"
        content={translate(I18N_KEYS.LIMITED_RIGHTS_RESTRICTIONS)}
        placement="left"
      ...
      >
        <Button
          ...
          disabled={isLimitedSharingPassword}
        >
    ...
    >
      ...
      >
        <Infobox
          ...
          mood={mood}
          size="large"
          actions={[renderViewPasswordHealth(), renderChangePassword()]}
        />
      </div>
    </CSSTransition>
  );
