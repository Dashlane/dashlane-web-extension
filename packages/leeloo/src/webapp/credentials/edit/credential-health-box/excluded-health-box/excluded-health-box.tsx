...
const I18N_KEYS = {
  ...
  EXCLUDED_DESCRIPTION:
    "webapp_credential_edition_health_box_excluded_description",
  INCLUDE: "webapp_credential_edition_health_box_include",
};
export interface ExcludedHealthBoxProps {
  onPrimaryClick: () => void;
}
const ExcludedHealthBoxComponent = ({
  onPrimaryClick,
}: ExcludedHealthBoxProps) => {
  ...
  return (
    ...
    >
      ...
      >
        <Infobox
          title={translate(I18N_KEYS.EXCLUDED_TITLE)}
          description={translate(I18N_KEYS.EXCLUDED_DESCRIPTION)}
          mood="warning"
          size="large"
          actions={[renderIncludePassword()]}
        />
      </div>
    </CSSTransition>
  );
