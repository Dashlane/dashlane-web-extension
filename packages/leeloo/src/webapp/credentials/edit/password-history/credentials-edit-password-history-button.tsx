...
const I18N_KEYS = {
  PASSWORD_HISTORY_BUTTON:
    "webapp_password_history_detailed_password_view_button",
};
interface Props {
  credentialId: string;
}
export const PasswordHistoryButton = ({ credentialId }: Props) => {
  ...
  const routeLink =
    routes.userPasswordHistoryFilteredByCredential(credentialId);
  ...
  return credHasPasswordHistory ? (
    <div>
      <Link to={routeLink} tabIndex={-1}>
        <Button
          mood="neutral"
          intensity="supershy"
          title={translate(I18N_KEYS.PASSWORD_HISTORY_BUTTON)}
          icon="HistoryBackupOutlined"
          layout="iconLeading"
        >
