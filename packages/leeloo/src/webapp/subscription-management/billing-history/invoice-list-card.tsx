...
import { Card, colors, FlexContainer, Heading, jsx, Link, Pagination, Paragraph, } from '@dashlane/ui-components';
...
import { useInvoiceList, useInvoicesCount, } from 'libs/carbon/hooks/useInvoiceList';
...
const I18N_KEYS = {
    ...
    NO_INVOICES_RESET: 'manage_subscription_billing_section_no_results_try_again',
};
const PAGE_SIZE = 10;
...
export const InvoiceListCard = () => {
    ...
    const visibleInvoices = useMemo(() => {
        ...
        return [...filteredInvoices].slice(pageStartIndex, pageStartIndex + PAGE_SIZE);
    }, [currentPage, filteredInvoices]);
    const resetText = translate(I18N_KEYS.NO_INVOICES_RESET);
    const [preResetLink, resetLink, postResetLink] = resetText.split('_');
    const onCustomizeInvoiceRequested = (invoice: Invoice) => {
        setIsEditModalOpen(true);
        setSelectedInvoice(invoice);
    };
    const downloadInvoice = async (invoice: Invoice, invoiceCustomData?: InvoiceCustomData) => {
        if (invoice) {
    ...
    return (<Card sx={{ marginBottom: '16px', padding: '32px' }}>
      <Heading>{translate(I18N_KEYS.CARD_TITLE)}</Heading>
      <InvoiceListFilters changePlanFilter={changePlanFilter} changeYearFilter={changeYearFilter} currentPlanFilter={currentPlanFilter} currentYearFilter={currentYearFilter}/>
      {filteredInvoices.length > 0 ? (<>
          <ListHeader containerClassName={styles.listHeader} header={getHeader()} onSort={sortInvoices} options={sortOptions}/>
          {visibleInvoices.map((invoice: Invoice) => (<InvoiceItem key={invoice.invoiceId} invoice={invoice} onCustomizeInvoiceRequested={onCustomizeInvoiceRequested} onDownloadInvoiceRequested={downloadInvoice}/>))}
          <Pagination sx={{ marginTop: '8px' }} totalPages={filteredInvoices.length > PAGE_SIZE
                ? Math.ceil(filteredInvoices.length / PAGE_SIZE)
                : 0} currentPage={currentPage} onPageChange={setCurrentPage}/>
        </>) : null}
      {filteredInvoices.length === 0 && invoiceCount > 0 ? (<FlexContainer alignItems="center" flexDirection="column" justifyContent="center" sx={{ height: '300px' }}>
          <Paragraph color={colors.grey00}>
            {translate(I18N_KEYS.NO_INVOICES_FOUND)}
          </Paragraph>
          <Paragraph color={colors.grey00}>
            {preResetLink}
            <Link color={colors.midGreen00} onClick={resetFilters}>
              {resetLink}
            </Link>
            {postResetLink}
          </Paragraph>
      ...
      <EditInvoiceDialog isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} onDownloadInvoice={downloadCustomInvoice}/>
    </Card>);
};
