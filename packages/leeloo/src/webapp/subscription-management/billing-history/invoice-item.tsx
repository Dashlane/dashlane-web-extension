...
const I18N_KEYS = {
    BTN_ADD_DETAILS: 'manage_subscription_billing_section_table_add_details_button',
...
export const InvoiceItem = ({ invoice, onDownloadInvoiceRequested, onCustomizeInvoiceRequested, }: InvoiceItemProps) => {
    ...
    const invoiceLabelText = `${translate(planNameString)} (${translate(periodicityString)})`;
    const rowData = [
        {
            key: 'planDuration',
            content: (<FlexContainer flexDirection="column" sx={{ padding: '12px 0' }}>
        ...
        {
            key: 'startDate',
            content: (<Paragraph textStyle="ds.body.reduced.regular" color="ds.text.neutral.quiet">
        ...
        {
            key: 'button',
            content: (<div sx={{ display: 'flex', gap: '10px' }}>
          <Button intensity="quiet" mood="neutral" size="small" onClick={() => onCustomizeInvoiceRequested(invoice)}>
            {translate(I18N_KEYS.BTN_ADD_DETAILS)}
          </Button>
          <Button layout="iconOnly" onClick={() => onDownloadInvoiceRequested(invoice)} aria-label={translate(I18N_KEYS.DOWNLOAD_INVOICE)} icon="DownloadOutlined" size="small"/>
        </div>),
            className: styles.invoiceListColumn,
        },
    ];
