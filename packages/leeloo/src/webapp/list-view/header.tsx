...
import webappVariables from "../variables.css";
...
export const Header = ({
  ...
  const sort = (key: string, logSubaction?: string, sortable?: boolean) => {
    ...
    if (logSubaction) {
      sortingOptions.logSubaction = `${logSubaction}${
        sortingOptions.direction === OrderDir.descending ? "Reversed" : ""
      }`;
    }
  ...
  const renderHeaderCell = (item: HeaderCell) => {
    ...
    return (
      <div
        ...
        onClick={() => sort(item.key, item.logSubaction, item.sortable)}
        sx={mergeSx([
          SX_STYLES.CELL,
          item.sxProps ?? {},
          item.sortable
