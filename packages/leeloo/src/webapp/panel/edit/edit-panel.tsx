...
import { AlertSize, FlexContainer } from '@dashlane/ui-components';
...
const actionsAnimationDurationMs = parseInt(cssVariables['--transition-duration-slow'], 10);
...
export interface Props {
    ...
    onNavigateOut: (action?: NavigateOutAction, e?: React.MouseEvent<HTMLElement>) => void;
    onSubmit?: (e: React.FormEvent<HTMLFormElement> | React.MouseEvent<HTMLElement>) => void;
...
export const EditPanel = ({ children, formId, isViewingExistingItem, itemHasBeenEdited, isCardValid = true, isEditableItem = true, onClickDelete, onNavigateOut, onSubmit, header, primaryActions, secondaryActions, submitPending, submitDisabled, ignoreCloseOnEscape, withoutCloseButton, withoutDeleteButton, withoutConfirmationDialog, isSomeDialogOpen = false, isUsingNewDesign = false, }: React.PropsWithChildren<Props>) => {
    const { translate } = useTranslate();
    const [displayConfirmDiscardDialog, setDisplayConfirmDiscardDialog] = React.useState(false);
    ...
    const showCancelAction = (): boolean => {
        return Boolean(isEditableItem && (!isViewingExistingItem || itemHasBeenEdited));
    };
    ...
    const handleNavigateOut = (event?: React.MouseEvent<HTMLElement>) => {
        if ((itemHasBeenEdited && displayConfirmDiscardDialog) ||
            isSomeDialogOpen) {
            return;
        }
        if (onNavigateOut) {
            const action = getNavigateOutSubaction();
            if ((event === undefined && action === 'cancel') ||
    ...
    const getCloseAction = (): React.ReactNode => {
        ...
        <Button data-testid="detail_panel_close_button" aria-label={showCancelAction()
                ? translate(I18N_KEYS.CANCEL)
                : translate(I18N_KEYS.CLOSE)} mood="neutral" intensity="quiet" size="medium" onClick={handleNavigateOut}>
          <CloseActionLabels labels={[translate(I18N_KEYS.CLOSE), translate(I18N_KEYS.CANCEL)]} visibleLabel={showCancelAction()
                ? translate(I18N_KEYS.CANCEL)
    ...
    const getPrimaryActions = (): React.ReactNode[] => {
        ...
        const saveAction = (<CSSTransition classNames={{
            ...
            }} timeout={actionsAnimationDurationMs} key="save">
        <FlexContainer>
          <Button type="submit" size="medium" sx={{ minWidth: '80px' }} disabled={submitPending || submitDisabled} isLoading={submitPending} form={formId}>
    ...
    const getSecondaryActions = (): React.ReactNode[] => {
        const deleteSecondaryAction: React.ReactNode[] = isViewingExistingItem && isEditableItem && !withoutDeleteButton
            ? [
                <Button key="delete" aria-label={translate(I18N_KEYS.DELETE)} size="medium" mood="danger" icon={<Icon name="ActionDeleteOutlined"/>} layout="iconOnly" intensity="supershy" onClick={onClickDelete ?? noop}/>,
            ]
            : [];
    ...
    return (<AlertProvider portalId="alert-popup-portal" alertSize={AlertSize.SMALL}>
      <Panel onNavigateOut={handleNavigateOut} ignoreClickOutsideClassName={editPanelIgnoreClickOutsideClassName} ignoreCloseOnEscape={ignoreCloseOnEscape}>
        {header}
        <form sx={{
            backgroundColor: isUsingNewDesign
                ? 'ds.background.alternate'
                : 'ds.container.agnostic.neutral.supershy',
        }} className={styles.form} autoComplete="off" noValidate={true} onSubmit={handleSubmit} id={formId}>
          {children}
          
          <div className={styles.alertPopup} id="alert-sidepanel-popup-portal"/>
        </form>
        ...
        {displayConfirmDiscardDialog ? (<ConfirmDiscardDialog onDismissClick={handleDismissDiscardDialog} onConfirmClick={handleConfirmDiscardDialog}/>) : null}
      </Panel>
    </AlertProvider>);
};
