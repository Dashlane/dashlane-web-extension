...
import { AlertSeverity, Checkbox, FlexContainer, InfoBox, Paragraph, PasswordInput, } from '@dashlane/ui-components';
import { Button, jsx } from '@dashlane/design-system';
import { UnlockProtectedItemsStatus } from '@dashlane/communication';
import { carbonConnector } from 'libs/carbon/connector';
import { logChangeMasterPasswordProtectSetting, logCredentialProtectionChange, } from 'webapp/unlock-items/logs';
import { useAlert } from 'libs/alert-notifications/use-alert';
import { ProtectedItemsUnlockedDialog } from './protected-items-unlocked';
import { LockedItemType, ProtectedItemsUnlockRequest, UnlockRequestCustomizableField, } from '../types';
...
const I18N_KEYS = {
    ...
    CHECKBOX_LABEL_GLOBAL_SETTING: 'webapp_lock_items_require_master_password_never_ask_checkbox_label',
    CHECKBOX_LABEL_FOR_CREDENTIAL: 'webapp_lock_items_require_master_password_never_ask_checkbox_label_for_credential',
    ...
    CHECKBOX_SUCCESS_CREDENTIAL: 'webapp_lock_items_require_master_password_never_ask_checkbox_success_alert_for_credential',
};
...
export const ProtectedItemsForm = ({ unlockRequest, setUnlockRequest, requirePasswordGlobalSetting = false, }: ProtectedItemsFormProps) => {
    ...
    const { verify, handleSuccess, onSuccessWithLogs } = useProtectedItemsUnlockSuccess({
        unlockRequest,
        setUnlockRequest,
    });
    ...
    if (verify) {
        return (<ProtectedItemsUnlockedDialog onDismiss={handleDismiss} onSuccess={handleSuccess}/>);
    }
    ...
    const handleSubmitPassword = async (values: {
        ...
        const commandResult = await carbonConnector.unlockProtectedItems(values.password);
            ...
            case UnlockProtectedItemsStatus.SUCCESS:
                if (values.neverAskCheckbox) {
                    if (requirePasswordGlobalSetting) {
                        ...
                        alert.showAlert(translate(I18N_KEYS.CHECKBOX_SUCCESS_GLOBAL), AlertSeverity.SUCCESS);
                    }
                    else if (unlockRequest.itemType === LockedItemType.Password) {
                        const credential = await carbonConnector.getCredential(unlockRequest.credentialId);
                        await carbonConnector.disableCredentialProtection({
                            credentialId: unlockRequest.credentialId,
                        });
                        logCredentialProtectionChange(false, credential.id, credential.url, credential.spaceId ?? undefined);
                        alert.showAlert(translate(I18N_KEYS.CHECKBOX_SUCCESS_CREDENTIAL), AlertSeverity.SUCCESS);
                    }
                }
                onSuccessWithLogs();
                break;
    ...
    return (<div>
      <Formik initialValues={initialValues} onSubmit={handleSubmitPassword}>
        {({ values, handleChange, isSubmitting, }: FormikProps<typeof initialValues>) => (<Form>
            {subtitle ? (<Paragraph sx={{ marginBottom: '27px' }}>{subtitle}</Paragraph>) : null}

            <PasswordInput name="password" value={values.password} autoFocus placeholder={getTranslatedValue(Placeholder) as string} onChange={(e) => {
                handleChange(e);
                setNoErrorState();
            }} showPasswordTooltipText={translate(I18N_KEYS.SHOW_LABEL)} hidePasswordTooltipText={translate(I18N_KEYS.HIDE_LABEL)} feedbackType={error ? 'error' : undefined} feedbackText={error ? translate(I18N_KEYS.WRONG_PASSWORD) : undefined}/>
            {unlockRequest.itemType === LockedItemType.Password &&
                unlockRequest.showNeverAskOption ? (<>
                <Checkbox name="neverAskCheckbox" data-testid="neverAskCheckbox" checked={values.neverAskCheckbox} onChange={handleChange} sx={{ margin: '12px 0px' }} label={requirePasswordGlobalSetting
                    ? translate(I18N_KEYS.CHECKBOX_LABEL_GLOBAL_SETTING)
                    : translate(I18N_KEYS.CHECKBOX_LABEL_FOR_CREDENTIAL)}/>
                {requirePasswordGlobalSetting && (<InfoBox severity="warning" size="small" title={translate(I18N_KEYS.CHECKBOX_WARNING)}/>)}
              </>) : null}
            <FlexContainer gap="16px" justifyContent="flex-end" sx={{
                maxWidth: '100%',
                marginTop: '40px',
            }}>
              ...
              <Button id="protected-items-unlocker-confirm-button" type="submit" disabled={isSubmitting || !values.password}>
