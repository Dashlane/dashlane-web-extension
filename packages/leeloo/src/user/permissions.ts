...
export function makePermissionChecker(
  ...
  const loggedIn = Boolean(
    globalState.carbon.loginStatus && globalState.carbon.loginStatus.loggedIn
  );
  const hasFullAccess = state.tacAccessPermissions.has("FULL");
  const hasBillingAccess = state.tacAccessPermissions.has("BILLING");
  const hasGroupAccess = Array.from(state.tacAccessPermissions).some((p) =>
    groupPermissions.includes(p)
  );
