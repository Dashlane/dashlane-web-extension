...
export interface UseDeleteOrResetAccount {
  ...
  completeFlow: (
    token: string,
    isDelete: boolean
  ) => Promise<CompleteFlowResultErrorCode | null>;
}
export const useDeleteOrResetAccount = (): UseDeleteOrResetAccount => {
  const accountVerificationResult = useModuleQuery(
    deleteOrResetAccountApi,
    "userAuthenticationMethod"
  );
  const isFlowCompletedResult = useModuleQuery(
    deleteOrResetAccountApi,
    "isFlowCompleted"
  );
  const { initiateFlow, completeFlow } = useModuleCommands(
    deleteOrResetAccountApi
  );
  return {
    userAuthenticationMethod:
      accountVerificationResult.status === DataStatus.Success
        ? accountVerificationResult.data
        : null,
    isFlowCompleted:
      isFlowCompletedResult.status === DataStatus.Success &&
