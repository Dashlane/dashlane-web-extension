...
import {
  ...
  LocalChannel,
  PortNames,
  RuntimeOnConnectMultiple,
  RuntimeOnConnectSingle,
} from "@dashlane/legacy-communication";
import { Connectors } from "./connectors.types";
import {
  CARBON_GRAPHENE_TIMEOUT,
  makeAutoFillEngineConnectors as makeAutoFillEngineCarbonConnectors,
  makeApiConnector as makeCarbonApiConnector,
  makeDebugConnector as makeCarbonDebugConnector,
  makeInfrastructureConnectors as makeCarbonInfrastructureConnectors,
  makeLegacyConnectors as makeCarbonLegacyConnectors,
  makeExtensionConnector,
  makeLegacyCarbonWebappConnector,
  makeLegacyMaverickCarbonConnector,
  makeModulesConnector,
  makeTiresiasBackgroundConnectors,
} from "./carbon/carbon-connectors";
import { logger } from "../logs/app-logger";
const COMM_TIMEOUT = __REDACTED__ ? 5000 : 3600000;
const { PopupCarbon, PopupCarbonLoader, OptionsCarbon, WebappCarbon } =
  PortNames;
export function makeConnectors(): Connectors {
  logger.debug(`Initializing communication connectors ${COMM_TIMEOUT}`);
  const runtimeOnConnectPopupLoaderChannel = new RuntimeOnConnectSingle(
    PopupCarbonLoader,
    COMM_TIMEOUT
  );
  runtimeOnConnectPopupLoaderChannel.listen();
  const runtimeOnConnectPopupChannel = new RuntimeOnConnectSingle(
    PopupCarbon,
    COMM_TIMEOUT
  );
  runtimeOnConnectPopupChannel.listen();
  const runtimeOnConnectOptionsChannel = new RuntimeOnConnectSingle(
    OptionsCarbon,
    COMM_TIMEOUT
  );
  runtimeOnConnectOptionsChannel.listen();
  const runtimeOnConnectWebappChannel = new RuntimeOnConnectMultiple(
    WebappCarbon,
    COMM_TIMEOUT
  );
  ...
  const { carbonToExtensionLegacyConnector, extensionToCarbonLegacyConnector } =
    makeCarbonLegacyConnectors([
      runtimeOnConnectPopupLoaderChannel,
      runtimeOnConnectPopupChannel,
    ]);
  const carbonToGrapheneLeelooChannel = new LocalChannel(
    CARBON_GRAPHENE_TIMEOUT
  );
  const grapheneToCarbonLeelooChannel = new LocalChannel(
    CARBON_GRAPHENE_TIMEOUT
  );
  connectLocalChannels(
    carbonToGrapheneLeelooChannel,
    grapheneToCarbonLeelooChannel
  );
  const carbonLegacyWebappConnector = makeLegacyCarbonWebappConnector([
    runtimeOnConnectWebappChannel,
    carbonToGrapheneLeelooChannel,
  ]);
  const carbonLegacyWebappConnectorForGraphene =
    makeLegacyCarbonWebappConnector(
      [grapheneToCarbonLeelooChannel],
      LEGACY_LEELOO_SLOTS_TO_IGNORE
    );
  ...
  const { autofillEngineCarbonConnector, autofillEngineLegacyCarbonConnector } =
    makeAutoFillEngineCarbonConnectors(carbonToAutofillEngineLocalChannel);
  const tiresiasBackgroundToCarbonConnector = makeTiresiasBackgroundConnectors(
    carbonToTiresiasBackground
  );
  const applicationModulesToCarbonApiConnector = makeModulesConnector(
    carbonToModulesLocalChannel
  );
  const extensionToCarbonApiConnector = makeExtensionConnector(
    carbonApiToExtensionLocalChannel
  );
  return {
