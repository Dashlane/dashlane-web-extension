...
export class EnforcePinChecker implements PostLoginRequirementsChecker {
    ...
    constructor(private readonly carbon: CarbonLegacyClient, private readonly pinCode: PinCodeClient, private readonly exceptionLogger: ExceptionLogger) { }
    check(login?: string): Observable<NotAllowedReason[]> {
        ...
        }).pipe(map(({ accountAuthenticationType, pinCodeStatus }) => {
            if (!isSuccess(accountAuthenticationType) ||
                !isSuccess(pinCodeStatus)) {
                console.log('One of these queries has failed: [accountAuthenticationType, pinCodeStatus]');
                this.exceptionLogger.captureException({
                    message: 'One of these queries has failed: [accountAuthenticationType, pinCodeStatus]',
                }, { useCaseName: 'isAllowedQuery' });
                return [];
            }
            const shouldEnforcePinCode = ENFORCE_PIN_FOR[accountAuthenticationType.data] &&
