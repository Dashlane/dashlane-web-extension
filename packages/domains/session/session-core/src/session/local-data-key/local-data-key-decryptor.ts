...
import { textToArrayBuffer, utf8ChunkEncode, } from '@dashlane/framework-encoding';
...
export class LocalDataKeyDecryptor {
    public constructor(private carbonLegacyClient: CarbonLegacyClient, private flexibleDecryptor: FlexibleDecryptor) { }
        ...
        return await firstValueFrom(carbonStateList({
            ...
            success: ([localKey, masterPassword, serverKey]) => {
                if (typeof localKey === 'string') {
                    return this.flexibleDecryptor.decrypt(textToArrayBuffer(localKey), transportableData);
                }
                    ...
                    return this.flexibleDecryptor.decrypt(textToArrayBuffer(keyUtf8), transportableData);
                }
                else {
