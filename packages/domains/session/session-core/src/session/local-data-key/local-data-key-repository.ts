...
import { arrayBufferToBase64, base64ToArrayBuffer, textToArrayBuffer, } from '@dashlane/framework-encoding';
...
export class LocalDataKeyRepository {
    constructor(private carbon: CarbonLegacyClient, private store: SessionsStateStore, private settings: SessionKeyCryptoSettingsRepository) { }
    getLocalKey(login: string): Observable<ArrayBuffer | null> {
        ...
        return this.store.state$.pipe(mergeMap(async (state) => {
            if (login in state) {
    ...
    public async generate(): Promise<ArrayBuffer | null> {
        ...
        const carbonResult = await firstValueFrom(carbonStateList({
