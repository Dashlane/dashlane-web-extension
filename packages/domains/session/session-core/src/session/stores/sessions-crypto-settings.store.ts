...
export const isSessionsCryptoSettingsState = (
  x: unknown
): x is SessionsCryptoSettingsState => {
  ...
  return Object.entries(x).every(([, entry]) => {
    return (
      typeof entry === "object" && "derivation" in entry && "salt" in entry
    );
  });
};
export class SessionsCryptoSettingsStore extends defineStore<
  SessionsCryptoSettingsState,
  SessionsCryptoSettingsState
>({
