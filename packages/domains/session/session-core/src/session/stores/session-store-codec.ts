...
@Injectable()
export class SessionStoreCodec
  implements Codec<PersistedSessionsState, SessionsState>
{
  constructor(
  ...
  decode(
    toDecode: PersistedSessionsState
  ): SessionsState | Promise<SessionsState> {
  ...
  async encode(toEncode: SessionsState): Promise<PersistedSessionsState> {
    ...
    for (const [k, v] of Object.entries(toEncode)) {
      ...
      r[k] = {
        encryptedLocalKey: arrayBufferToBase64(
          await this.encryptor.encrypt(
            base64ToArrayBuffer(v.localKey),
            v.sessionKey,
            settingsQuery.data
          )
        ),
