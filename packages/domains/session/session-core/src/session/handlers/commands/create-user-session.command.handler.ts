  ...
  implements ICommandHandler<CreateUserSessionCommand>
{
  ...
  async execute({
    ...
    const encryptedLk = localKey
      ...
      : null;
    return SessionStoreMutex.runExclusive(async () => {
      ...
      switch (noExportSessionKey.type) {
        ...
        case "sso":
          throw new Error(
            `Creating a SSO session in session module is not supported`
          );
        default:
          return assertUnreachable(noExportSessionKey);
      }
    });
  }
}
