...
import { MultiAccountNotYetSupported, OpenUserSessionCommand, SessionAlreadyOpened, SessionNotCreated, } from '@dashlane/session-contracts';
...
export class OpenUserSessionCommandHandler implements ICommandHandler<OpenUserSessionCommand> {
    constructor(private carbon: CarbonLegacyClient, private sessionStore: SessionsStateStore, private sessionKeyDecryptor: SessionKeyDecryptor, private importer: SessionKeyImporter) { }
    async execute({ body: { email, sessionKey, rememberPassword }, }: OpenUserSessionCommand) {
        const nonExportedSessionKey = sessionKey.type === 'exported'
            ...
            await sessionStore.set({
                ...
                [email]: {
                    ...
                    localKey: sessionToOpen.encryptedLocalKey
                        ? arrayBufferToBase64(await this.sessionKeyDecryptor.decrypt(base64ToArrayBuffer(sessionToOpen.encryptedLocalKey), nonExportedSessionKey))
                        : null,
                },
            });
            let successInCarbon = false;
            try {
                const openSessionInCarbon = await carbon.commands.openSessionWithMasterPassword({
                    login: email,
