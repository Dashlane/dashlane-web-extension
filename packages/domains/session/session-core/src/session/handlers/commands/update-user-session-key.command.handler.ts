...
import { UnableToUpdateSessionKeyForNonExistingAccount, UnableToUpdateSessionSessionNotOpened, UpdateUserSessionKeyCommand, UpdateUserSessionKeyErrors, } from '@dashlane/session-contracts';
import { SessionKeyImporter } from '../../services/session-key-importer';
@CommandHandler(UpdateUserSessionKeyCommand)
export class UpdateUserSessionKeyCommandHandler implements ICommandHandler<UpdateUserSessionKeyCommand> {
    constructor(private readonly store: SessionsStateStore, private readonly importer: SessionKeyImporter) { }
    async execute({ body: { email, sessionKey }, }: UpdateUserSessionKeyCommand): Promise<Result<CommandSuccess, UpdateUserSessionKeyErrors>> {
