...
export class SelectedOpenedSessionQueryHandler
  implements IQueryHandler<SelectedOpenedSessionQuery>
{
  ...
  execute(): Observable<Result<string | undefined>> {
    return this.sessionsStateStore.state$.pipe(
      map((sessionsState: SessionsState) => {
