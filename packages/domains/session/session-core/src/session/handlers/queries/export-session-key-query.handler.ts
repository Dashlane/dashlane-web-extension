...
export class ExportSessionKeyQueryHandler
  implements IQueryHandler<ExportSessionKeyQuery>
{
  constructor(
    private readonly store: SessionsStateStore,
    private readonly exporter: SessionKeyExporter
  ) {}
  execute(): Observable<Result<ExportedSessionKey>> {
    return this.store.state$.pipe(
      concatMap(async (sessionsState) => {
        let currentLogin = undefined;
        for (const login in sessionsState) {
          if (sessionsState[login].status === SessionStates.Open) {
            currentLogin = login;
          }
        }
        if (!currentLogin) {
