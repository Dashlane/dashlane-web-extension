...
export class CloseSessionOnIdleCronHandler implements ICronTaskHandler {
  constructor(
    ...
    private readonly conf: SessionConfig
  ) {}
  public async run() {
  ...
  public async isRunnable() {
    const getPremiumStatusQuery = await firstValueFrom(
      this.carbonLegacyClient.queries.getPremiumStatus()
    );
    if (
      !isSuccess(getPremiumStatusQuery) ||
      !getPremiumStatusQuery.data.spaces
    ) {
      return false;
    }
    const activeSpace = getPremiumStatusQuery.data.spaces.find(
