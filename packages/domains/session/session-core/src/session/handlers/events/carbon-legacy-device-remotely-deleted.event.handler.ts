...
export class CarbonLegacyDeviceRemotelyDeletedEventHandler implements IEventHandler<CarbonLegacyDeviceRemotelyDeleted> {
    constructor(private store: SessionsStateStore) { }
    async handle({ body: { user }, }: CarbonLegacyDeviceRemotelyDeleted): Promise<void> {
