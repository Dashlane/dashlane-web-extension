...
export class CarbonLegacySessionEventHandler
  implements IEventHandler<CarbonLegacyEvent>
{
  constructor(
    private carbon: CarbonLegacyClient,
    private sessionsStateStore: SessionsStateStore,
    private sessionsCryptoSettingsStore: SessionsCryptoSettingsStore,
    private eventEmitter: SessionEventEmitter,
    private randomValuesGetter: RandomValuesGetter,
    private localKeyRepo: LocalDataKeyRepository,
  ...
  async getUserCryptoSettings(): Promise<UserCryptoSettings> {
    ...
    const { cryptoUserPayload, cryptoFixedSalt: carbonFixedSalt } =
      userCryptoSettings.data;
    return {
  ...
  async getSessionKey(needsSsoMigration: boolean): Promise<SessionKey> {
    ...
    if (!isSuccess(sessionKey)) {
      throw new Error(
        "Fail to fetch master password and server key from carbon"
      );
    }
  ...
  async handleLogin(login: string, needsSsoMigration: boolean) {
    const userSessionKey = await SessionStoreMutex.runExclusive(async () => {
      const { cryptoUserPayload, cryptoFixedSalt } =
        await this.getUserCryptoSettings();
      ...
      const salt = cryptoFixedSalt
        ? cryptoFixedSalt
        : arrayBufferToBase64(
            this.randomValuesGetter.get(derivation.saltLength)
          );
      const sessionsCryptoSettingsStore =
        await this.sessionsCryptoSettingsStore.getState();
    ...
    await this.eventEmitter.sendEvent("sessionOpened", {
  ...
  async handleLogout() {
    ...
    await SessionStoreMutex.runExclusive(async () => {
      ...
      for (const [login, existing] of Object.entries(oldSssionsState)) {
        ...
        const settingsQuery = await firstValueFrom(
          this.settings.getConfig(login)
        );
        ...
        const encryptedLocalKey = existing.localKey
          ? arrayBufferToBase64(
              await this.encryptor.encrypt(
                base64ToArrayBuffer(existing.localKey),
                existing.sessionKey,
                settings
              )
            )
  ...
  async handle(event: CarbonLegacyEvent) {
    if (event.body.eventName === "carbonLoginStatusChanged") {
