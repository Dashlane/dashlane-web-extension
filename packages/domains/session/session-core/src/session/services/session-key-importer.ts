...
export class SessionKeyImporter {
  constructor(
    private readonly cryptoManager: ExportedSessionKeyCryptoManager,
  ...
  async import(exportedSessionKey: ExportedSessionKey): Promise<SessionKey> {
    const key = await firstValueFrom(
      this.cryptoManager.getOrCreateEncryptionKey()
    );
    const content = await this.flexibleDecryptor.decrypt(
      key,
      base64ToArrayBuffer(exportedSessionKey.content)
    );
