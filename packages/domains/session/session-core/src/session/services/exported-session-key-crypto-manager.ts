...
export class ExportedSessionKeyCryptoManager {
    constructor(private readonly store: ExportSessionKeyCryptoStore, private readonly keyGenerator: KeyGeneratorAes256) { }
    ...
    getOrCreateEncryptionKey(): Observable<ArrayBuffer> {
        return this.store.state$.pipe(concatMap(async (state) => {
