...
export const compareMachines = (
  prevState: TrustedDeviceFlowMachineState,
  currState: TrustedDeviceFlowMachineState
): boolean => {
...
export const getDelayBeforeRequestExpiry = (requestTimestamp: number) =>
  1000 * requestTimestamp + REQUEST_TIMEOUT_DELAY - Date.now();
...
export function secureDeviceName(deviceName: string): string {
  ...
  return deviceName.replace(/__REDACTED__/, " ");
}
